// cavestack community-pulse edge function
...
Deno.serve(async () => {
  ...
  try {
    ...
    const { data: crashes, error: crashErr } = await supabase
      .from("crash_clusters")
      .select("error_class, cavestack_version, total_occurrences, identified_users")
      .limit(5);
    ...
    for (const row of versionRows ?? []) {
      if (row.cavestack_version) {
        versionCounts[row.cavestack_version] = (versionCounts[row.cavestack_version] ?? 0) + 1;
      }
    }
    ...
    // Fields emitted by cavestack-telemetry-log --event-type attack_attempt:
    ...
    // cavestack user who polls /community-pulse. With it, the dashboard shows
