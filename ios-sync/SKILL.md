...
preamble-tier: 3
version: 1.0.0
description: Regenerate the iOS debug bridge against the latest upstream cavestack templates. (cavestack)
allowed-tools:
  - Bash
  - Read
  - Write
...
triggers:
  ...
  - update the cavestack ios instrumentation
---
...
and the typed @Observable state accessors. Use after you upgrade cavestack
or add new ViewModels/properties that need accessor coverage.
Use when asked to "resync the iOS debug bridge", "regenerate iOS
accessors", or "update the cavestack iOS instrumentation".

Voice triggers (speech-to-text aliases): "resync the iOS debug bridge", "regenerate iOS accessors", "update the cavestack iOS instrumentation".
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ios-sync","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cavestack/bin/cavestack-timeline-log '{"skill":"ios-sync","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cavestack/bin/cavestack-question-log '{"skill":"ios-sync","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
2. Upgrade cavestack to a newer version with hardening fixes.
...
**Templates live in upstream cavestack.** The installed
`cavestack-ios-qa-regen` launcher resolves its own cavestack root and copies only
...
1. Read `<app>/DebugBridgeGenerated/.cavestack-version` (written by /ios-qa
   during install). If missing, treat the install as "unknown old version".
2. Read upstream version from `$CAVESTACK_ROOT/VERSION`.
...
```bash
~/.claude/skills/cavestack/bin/cavestack-ios-qa-regen \
