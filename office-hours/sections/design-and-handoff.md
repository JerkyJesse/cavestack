...
visible, committable, team-shareable. The `~/.cavestack` copy is still written
...
1. **Scan at sink first.** The repo copy leaves the private store, so scan the
   ...
   `~/.claude/skills/cavestack/bin/cavestack-redact --from-file <tmp>`; exit 3
   (HIGH) blocks the repo copy (keep the ~/.cavestack copy, tell the user why);
   exit 2 (MEDIUM) confirms per finding before writing.
2. **Fallback is never blocking.** Read-only checkout, non-git directory, a
   failed write, or an unconfirmed MEDIUM → keep the `~/.cavestack` copy and say
...
**"Design doc saved to: {repo path if written, else ~/.cavestack path}{when both: ' (cross-session copy in ~/.cavestack)'}. Other skills (/plan-ceo-review, /plan-eng-review) will find it automatically."**
...
open "$CAVESTACK_STATE_ROOT/builder-journey.md"
```
...
```bash
~/.claude/skills/cavestack/bin/cavestack-config get founder_resources 2>/dev/null || echo "true"
```
...
`cavestack-config set founder_resources true`.)
...
`~/.claude/skills/cavestack/bin/cavestack-config set founder_resources false`, then
VERIFY the write (`cavestack-config get founder_resources` must read back
...
```bash
eval "$(~/.claude/skills/cavestack/bin/cavestack-slug 2>/dev/null || true)"
~/.claude/skills/cavestack/bin/cavestack-developer-profile --log-session '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/cavestack/bin/cavestack-telemetry-log --event-type handoff --skill office-hours --outcome accepted --session-id "$_SESSION_ID" 2>/dev/null || true
```
On D, log declined and stop:
```bash
~/.claude/skills/cavestack/bin/cavestack-telemetry-log --event-type handoff --skill office-hours --outcome declined --session-id "$_SESSION_ID" 2>/dev/null || true
```
