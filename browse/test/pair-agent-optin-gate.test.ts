...
const savedEnv = { CAVESTACK_HOME: process.env.CAVESTACK_HOME, CAVESTACK_PAIR_AGENT: process.env.CAVESTACK_PAIR_AGENT };
const tmpHomes: string[] = [];

function tmpHomeWith(config: Record<string, string> | null): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-pair-'));
  tmpHomes.push(dir);
  if (config !== null) {
    // Canonical store: flat YAML lines, the shape bin/cavestack-config writes.
  ...
  delete process.env.CAVESTACK_PAIR_AGENT;
  return dir;
}

afterEach(() => {
  for (const k of ['CAVESTACK_HOME', 'CAVESTACK_PAIR_AGENT'] as const) {
...
describe('isPairAgentEnabled — fail-closed default', () => {
  ...
  test('env override wins: CAVESTACK_PAIR_AGENT=on forces ON even with config off', () => {
    tmpHomeWith({ pair_agent: 'off' });
    process.env.CAVESTACK_PAIR_AGENT = 'on';
    expect(isPairAgentEnabled()).toBe(true);
  });

  test('env override wins: CAVESTACK_PAIR_AGENT=off forces OFF even with config on', () => {
    tmpHomeWith({ pair_agent: 'on' });
    process.env.CAVESTACK_PAIR_AGENT = 'off';
    expect(isPairAgentEnabled()).toBe(false);
  });
});
...
describe('gate wiring — every tunnel activation point consults the guard', () => {
  ...
  test('CLI consent-off branch names the real remedy, never ngrok reinstall', () => {
    ...
    expect(branch).toContain('cavestack-config set pair_agent on');
  ...
  test('/tunnel/start refuses with the enable hint when disabled', () => {
    ...
    expect(block).toContain('cavestack-config set pair_agent on');
  });
