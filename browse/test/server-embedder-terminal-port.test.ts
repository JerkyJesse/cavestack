...
// getting those files clobbered by cavestack's shutdown(). The flag (default true)
...
// False = embedder owns them, cavestack stays hands-off.
//
// Pre-v1.44 used `pkill -f terminal-agent\.ts` which matched sibling cavestack
...
describe('buildFetchHandler ownsTerminalAgent gate', () => {
  ...
  // not a per-test temp dir. If a real cavestack daemon is running on this host,
