...
function loadBackground() {
  ...
  const run = new Function('chrome', 'importScripts', 'cavestackSenderAuth', 'fetch', BG_SRC);
...
describe('background.js ↔ sender-auth.js wiring', () => {
  ...
  test('background.js consults cavestackSenderAuth.denialFor in the message listener', () => {
    expect(BG_SRC).toContain('cavestackSenderAuth.denialFor(msg.type, sender, chrome.runtime.id)');
  });
