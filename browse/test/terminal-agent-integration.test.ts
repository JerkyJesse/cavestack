...
beforeAll(() => {
  stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-term-'));
  ...
  test('rejects extension-Origin upgrades without a granted cookie', async () => {
    const resp = await fetch(`http://127.0.0.1:${agentPort}/ws`, {
      headers: {
        ...
        'Cookie': 'cavestack_pty=never-granted',
      },
    });
...
describe('terminal-agent: PTY round-trip via real WebSocket (Cookie auth)', () => {
  ...
  test('Sec-WebSocket-Protocol auth path: browser-style upgrade with token in protocol', async () => {
    ...
    //
    // The browser sends `cavestack-pty.<token>` and the agent must:
    //   1) strip the cavestack-pty. prefix
    ...
    const resp = await fetch(`http://127.0.0.1:${agentPort}/ws`, {
      headers: {
        ...
        'Sec-WebSocket-Protocol': `cavestack-pty.${token}`,
        'Origin': 'chrome-extension://test-extension-id',
      },
    });
    ...
    expect(resp.headers.get('sec-websocket-protocol')).toBe(`cavestack-pty.${token}`);
  });
  ...
  test('upgrade response contains exactly ONE Sec-WebSocket-Protocol header', async () => {
    ...
    expect(protoLines).toEqual([`Sec-WebSocket-Protocol: cavestack-pty.${token}`]);
  });
  ...
  test('Sec-WebSocket-Protocol auth: rejects unknown token even with valid Origin', async () => {
    const resp = await fetch(`http://127.0.0.1:${agentPort}/ws`, {
      headers: {
        ...
        'Sec-WebSocket-Version': '13',
        'Sec-WebSocket-Key': 'dGhlIHNhbXBsZSBub25jZQ==',
        'Sec-WebSocket-Protocol': 'cavestack-pty.never-granted-token',
        'Origin': 'chrome-extension://test-extension-id',
      },
    });
    expect(resp.status).toBe(401);
