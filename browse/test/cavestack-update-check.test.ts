/**
 * Tests for bin/cavestack-update-check bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * CAVESTACK_DIR / CAVESTACK_STATE_DIR / CAVESTACK_REMOTE_URL env overrides
 * for full isolation.
 */
...
function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  // cavestack-config (which this script shells out to for update_check) resolves
  // state as CAVESTACK_STATE_ROOT > CAVESTACK_HOME > CAVESTACK_STATE_DIR > ~/.cavestack.
  // Strip the higher-precedence vars so harness-env leftovers can never
  // outrank the per-test CAVESTACK_STATE_DIR isolation.
  const env: Record<string, string | undefined> = {
    ...process.env,
    CAVESTACK_DIR: cavestackDir,
    CAVESTACK_STATE_DIR: stateDir,
    CAVESTACK_REMOTE_URL: `file://${join(cavestackDir, 'REMOTE_VERSION')}`,
  };
  delete env.CAVESTACK_STATE_ROOT;
  delete env.CAVESTACK_HOME;
...
beforeEach(() => {
  cavestackDir = mkdtempSync(join(tmpdir(), 'cavestack-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'cavestack-state-test-'));
  ...
  // v1.63+: the script sources bin/cavestack-egress-lib.sh unconditionally
  // (receipted fetch helpers). A real install always has it beside
  // cavestack-config; without this link every test failed at the source line —
  // masked until the suite-truncation fix because the runner died first.
  symlinkSync(
    join(import.meta.dir, '..', '..', 'bin', 'cavestack-egress-lib.sh'),
    join(binDir, 'cavestack-egress-lib.sh'),
  );
});

afterEach(() => {
  rmSync(cavestackDir, { recursive: true, force: true });
...
describe('cavestack-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(cavestackDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(cavestackDir, 'VERSION'), '0.4.0\n');
  ...
  test('just-upgraded marker does not mask newer remote version', () => {
    writeFileSync(join(cavestackDir, 'VERSION'), '0.4.0\n');
    writeFileSync(join(stateDir, 'just-upgraded-from'), '0.3.3\n');
    writeFileSync(join(cavestackDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('just-upgraded with no further updates writes UP_TO_DATE cache', () => {
    writeFileSync(join(cavestackDir, 'VERSION'), '0.4.0\n');
    writeFileSync(join(stateDir, 'just-upgraded-from'), '0.3.3\n');
    writeFileSync(join(cavestackDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(cavestackDir, 'VERSION'), '0.3.3\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(cavestackDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(cavestackDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(cavestackDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(cavestackDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits silently when remote URL is unreachable', () => {
    ...
    const { exitCode, stdout } = run({
      CAVESTACK_REMOTE_URL: 'file:///nonexistent/path/VERSION',
    });
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
    const cache = readFileSync(join(stateDir, 'last-update-check'), 'utf-8');
  ...
  test('creates state dir if it does not exist', () => {
    ...
    const { exitCode } = run({ CAVESTACK_STATE_DIR: newStateDir });
    expect(exitCode).toBe(0);
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(cavestackDir, 'VERSION'), version + '\n');
  ...
  test('remote older than local (stale CDN) → silent, cache UP_TO_DATE', () => {
    writeFileSync(join(cavestackDir, 'VERSION'), '1.34.0.0\n');
    writeFileSync(join(cavestackDir, 'REMOTE_VERSION'), '1.33.2.0\n');
  ...
  test('multi-segment sort: 1.9.0.0 < 1.10.0.0', () => {
    writeFileSync(join(cavestackDir, 'VERSION'), '1.9.0.0\n');
    writeFileSync(join(cavestackDir, 'REMOTE_VERSION'), '1.10.0.0\n');
  ...
  test('multi-segment reverse sort: 1.10.0.0 > 1.9.0.0 → no rewind', () => {
    writeFileSync(join(cavestackDir, 'VERSION'), '1.10.0.0\n');
    writeFileSync(join(cavestackDir, 'REMOTE_VERSION'), '1.9.0.0\n');
