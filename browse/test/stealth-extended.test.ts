 ...
 * 2. CAVESTACK_STEALTH=extended adds EXTENDED_STEALTH_SCRIPT on top of Layer C.
...
beforeEach(() => {
  originalEnv = process.env.CAVESTACK_STEALTH;
});

afterEach(() => {
  if (originalEnv === undefined) delete process.env.CAVESTACK_STEALTH;
  else process.env.CAVESTACK_STEALTH = originalEnv;
});

describe('extended stealth — opt-in mode flag', () => {
  test('default mode is OFF (consistency-first contract)', () => {
    delete process.env.CAVESTACK_STEALTH;
    expect(isExtendedStealthEnabled()).toBe(false);
  });

  test('CAVESTACK_STEALTH=extended enables extended mode', () => {
    process.env.CAVESTACK_STEALTH = 'extended';
    expect(isExtendedStealthEnabled()).toBe(true);
  });

  test('CAVESTACK_STEALTH=1 also enables (env-style boolean)', () => {
    process.env.CAVESTACK_STEALTH = '1';
    expect(isExtendedStealthEnabled()).toBe(true);
  });

  test('CAVESTACK_STEALTH=anything-else does NOT enable', () => {
    process.env.CAVESTACK_STEALTH = 'verbose';
    expect(isExtendedStealthEnabled()).toBe(false);
  });
});
