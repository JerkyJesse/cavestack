...
describe('buildStealthScript — T3 Layer C', () => {
  ...
  test('script does not include "CaveStackBrowser" branding string', () => {
    const s = buildStealthScript(hw);
    // D6: dropped from UA, must not leak in via stealth payload either.
    expect(s).not.toContain('CaveStackBrowser');
  });
});

describe('buildCaveStackLaunchArgs — Pack 1 cmdline-switch construction', () => {
  // Helper: clear all CAVESTACK_* env, run test body, restore env.
  function withEnv(env: Record<string, string | undefined>, body: () => void) {
    const saved: Record<string, string | undefined> = {};
    for (const k of Object.keys(process.env)) {
      if (k.startsWith('CAVESTACK_')) {
    ...
    try {
      ...
      for (const k of Object.keys(process.env)) {
        if (k.startsWith('CAVESTACK_')) delete process.env[k];
      }
  ...
  test('empty env produces no switches (suppress-stack is opt-in)', () => {
    withEnv({}, () => {
      ...
      // emitted when CAVESTACK_CDP_STEALTH is on/1/true. Empty env → [].
      expect(buildCaveStackLaunchArgs()).toEqual([]);
    });
  });

  test('all env values populated (incl. CDP stealth opt-in) → all 7 switches emitted', () => {
    withEnv({
      CAVESTACK_GPU_VENDOR: 'Apple Inc.',
      CAVESTACK_GPU_RENDERER: 'ANGLE (Apple, ANGLE Metal Renderer: Apple M4 Max, Unspecified Version)',
      CAVESTACK_PLATFORM: 'MacARM',
      CAVESTACK_GPU_CHIPSET: 'Apple M4 Max',
      CAVESTACK_HW_CONCURRENCY: '16',
      CAVESTACK_DEVICE_MEMORY: '8',
      CAVESTACK_CDP_STEALTH: 'on',
    }, () => {
      const args = buildCaveStackLaunchArgs();
      expect(args).toContain('--cavestack-gpu-vendor=Apple Inc.');
      expect(args).toContain('--cavestack-gpu-renderer=ANGLE (Apple, ANGLE Metal Renderer: Apple M4 Max, Unspecified Version)');
      expect(args).toContain('--cavestack-ua-platform=macOS');
      expect(args).toContain('--cavestack-ua-model=Apple M4 Max');
      expect(args).toContain('--cavestack-hw-concurrency=16');
      expect(args).toContain('--cavestack-device-memory=8');
      expect(args).toContain('--cavestack-suppress-prepare-stack-trace');
      expect(args.length).toBe(7);
    });
  });

  test('platform mapping: MacARM and MacIntel both → macOS', () => {
    withEnv({ CAVESTACK_PLATFORM: 'MacARM' }, () => {
      expect(buildCaveStackLaunchArgs()).toContain('--cavestack-ua-platform=macOS');
    });
    withEnv({ CAVESTACK_PLATFORM: 'MacIntel' }, () => {
      expect(buildCaveStackLaunchArgs()).toContain('--cavestack-ua-platform=macOS');
    });
  });

  test('platform mapping: Win32 → Windows, Linux x86_64 → Linux', () => {
    withEnv({ CAVESTACK_PLATFORM: 'Win32' }, () => {
      expect(buildCaveStackLaunchArgs()).toContain('--cavestack-ua-platform=Windows');
    });
    withEnv({ CAVESTACK_PLATFORM: 'Linux x86_64' }, () => {
      expect(buildCaveStackLaunchArgs()).toContain('--cavestack-ua-platform=Linux');
    });
  });

  test('partial env: only set switches that have values', () => {
    withEnv({ CAVESTACK_HW_CONCURRENCY: '12' }, () => {
      const args = buildCaveStackLaunchArgs();
      // hw only — suppress-stack is opt-in and CAVESTACK_CDP_STEALTH is unset.
      expect(args).toContain('--cavestack-hw-concurrency=12');
      expect(args).not.toContain('--cavestack-suppress-prepare-stack-trace');
      expect(args.length).toBe(1);
    });
  });

  test('prepare-stack-trace suppression is opt-in via CAVESTACK_CDP_STEALTH', () => {
    // on/1/true enable it; off and unset omit it, so stock Playwright
    // Chromium (no CAVESTACK_CDP_STEALTH) never receives the unknown switch.
    for (const v of ['on', '1', 'true']) {
      withEnv({ CAVESTACK_CDP_STEALTH: v }, () => {
        expect(buildCaveStackLaunchArgs()).toContain('--cavestack-suppress-prepare-stack-trace');
      });
    }
    withEnv({ CAVESTACK_CDP_STEALTH: 'off' }, () => {
      expect(buildCaveStackLaunchArgs()).not.toContain('--cavestack-suppress-prepare-stack-trace');
    });
    withEnv({}, () => {
      expect(buildCaveStackLaunchArgs()).not.toContain('--cavestack-suppress-prepare-stack-trace');
    });
  ...
  test('unrecognized platform falls through without --cavestack-ua-platform', () => {
    withEnv({ CAVESTACK_PLATFORM: 'OS/2' }, () => {
      const args = buildCaveStackLaunchArgs();
      expect(args.some(a => a.startsWith('--cavestack-ua-platform='))).toBe(false);
    });
  });

  test('GPU vendor with spaces survives intact (no quote/escape regression)', () => {
    withEnv({ CAVESTACK_GPU_VENDOR: 'NVIDIA Corporation' }, () => {
      const args = buildCaveStackLaunchArgs();
      expect(args).toContain('--cavestack-gpu-vendor=NVIDIA Corporation');
    });
  });
});
...
describe('readHostProfile — clamp/fallback', () => {
  ...
  afterEach(() => {
    if (savedHw === undefined) delete process.env.CAVESTACK_HW_CONCURRENCY;
    else process.env.CAVESTACK_HW_CONCURRENCY = savedHw;
    if (savedMem === undefined) delete process.env.CAVESTACK_DEVICE_MEMORY;
    else process.env.CAVESTACK_DEVICE_MEMORY = savedMem;
  });
  function withHw(hw: string | undefined, mem: string | undefined): ReturnType<typeof readHostProfile> {
    savedHw = process.env.CAVESTACK_HW_CONCURRENCY;
    savedMem = process.env.CAVESTACK_DEVICE_MEMORY;
    if (hw === undefined) delete process.env.CAVESTACK_HW_CONCURRENCY; else process.env.CAVESTACK_HW_CONCURRENCY = hw;
    if (mem === undefined) delete process.env.CAVESTACK_DEVICE_MEMORY; else process.env.CAVESTACK_DEVICE_MEMORY = mem;
