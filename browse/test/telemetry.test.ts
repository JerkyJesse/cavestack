...
const TMP_HOME = path.join(os.tmpdir(), `cavestack-telemetry-test-${process.pid}-${Date.now()}`);
const TELEMETRY_FILE = path.join(TMP_HOME, 'analytics', 'browse-telemetry.jsonl');

// Use CAVESTACK_HOME env to redirect telemetry writes (read each call,
...
describe('telemetry: signals fire to ~/.cavestack/analytics/browse-telemetry.jsonl', () => {
  ...
  it('CAVESTACK_TELEMETRY_OFF=1 silences all events', async () => {
