/**
 * Tests for bin/cavestack-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * CAVESTACK_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'cavestack-config');

let stateDir: string;

function run(args: string[] = [], extraEnv: Record<string, string> = {}) {
  // The script resolves its state dir as CAVESTACK_STATE_ROOT > CAVESTACK_HOME >
  // CAVESTACK_STATE_DIR > $HOME/.cavestack. Strip the higher-precedence vars so a
  // stray value in the harness env (another test file's leftovers, operator
  // shell) can never outrank the per-test CAVESTACK_STATE_DIR isolation.
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'cavestack-config-test-'));
});
...
describe('cavestack-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { CAVESTACK_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
  ...
  test('first set writes annotated header with docs', () => {
    ...
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    expect(content).toContain('# cavestack configuration');
  ...
  test('header written only once, not duplicated on second set', () => {
    ...
    run(['set', 'baz', 'qux']);
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    const headerCount = (content.match(/# cavestack configuration/g) || []).length;
  ...
  test('existing config file is not overwritten with header', () => {
    ...
    expect(content).not.toContain('# cavestack configuration');
  });
