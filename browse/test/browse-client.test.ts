...
describe('browse-client', () => {
  ...
  beforeEach(async () => {
    ...
    for (const k of ['CAVESTACK_PORT', 'CAVESTACK_SKILL_TOKEN', 'BROWSE_STATE_FILE', 'BROWSE_TAB']) {
  ...
  describe('resolveBrowseAuth', () => {
    it('uses CAVESTACK_PORT + CAVESTACK_SKILL_TOKEN env when present', () => {
      process.env.CAVESTACK_PORT = String(server.port);
      process.env.CAVESTACK_SKILL_TOKEN = 'scoped-token';
    ...
    it('rejects CAVESTACK_PORT env values with trailing characters', () => {
      process.env.CAVESTACK_PORT = `${server.port}abc`;
      process.env.CAVESTACK_SKILL_TOKEN = 'scoped-token';
