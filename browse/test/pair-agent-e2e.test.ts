...
import { CAVESTACK_EXTENSION_ID } from '../src/server';
...
describe('pair-agent flow end-to-end (HTTP only, no ngrok)', () => {
  ...
  test('GET /health returns daemon status and NEVER includes a token (even for chrome-extension origins)', async () => {
    const resp = await fetch(`${daemon.baseUrl}/health`, {
      headers: { Origin: `chrome-extension://${CAVESTACK_EXTENSION_ID}` },
    });
  ...
  test('POST /sse-session with root Bearer returns a Set-Cookie for cavestack_sse', async () => {
    ...
    expect(setCookie!).toContain('cavestack_sse=');
  ...
  test('/welcome regex gate: safe slug resolves; dangerous slug does not path-traverse', async () => {
    // The regex gate lives in server.ts — we can't easily flip CAVESTACK_SLUG
