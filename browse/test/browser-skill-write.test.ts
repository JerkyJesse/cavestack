 ...
 * never touch the user's real ~/.cavestack/ tree. The contract under test:
 *
 *   stageSkill    → writes files into ~/.cavestack/.tmp/skillify-<spawnId>/<name>/
...
beforeEach(() => {
  ...
  // Synthetic tmp root keeps tests off the real ~/.cavestack/.tmp/.
  stagingTmpRoot = path.join(tmpRoot, 'home', '.cavestack', '.tmp');
});
