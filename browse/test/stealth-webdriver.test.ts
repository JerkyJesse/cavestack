...
describe('applyStealth — per-install hardware from env', () => {
  ...
  beforeAll(async () => {
    ...
    process.env.CAVESTACK_HW_CONCURRENCY = '12';
    process.env.CAVESTACK_DEVICE_MEMORY = '4';
...
describe('applyStealth — extended mode layered on Layer C (CAVESTACK_STEALTH=extended)', () => {
  let ctx: BrowserContext;
  let savedStealth: string | undefined;

  beforeAll(async () => {
    savedStealth = process.env.CAVESTACK_STEALTH;
    process.env.CAVESTACK_STEALTH = 'extended';
    ctx = await browser.newContext();
  ...
  afterAll(async () => {
    await ctx.close();
    if (savedStealth === undefined) delete process.env.CAVESTACK_STEALTH;
    else process.env.CAVESTACK_STEALTH = savedStealth;
  });
