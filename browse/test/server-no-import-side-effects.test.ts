 ...
 * were registered. The subprocess uses HOME=tmp + CAVESTACK_HOME=tmp so any
...
describe('server.ts module import has no auto-start side effects', () => {
  test('importing server.ts does not bind Bun.serve, register signal handlers, or write state', async () => {
    ...
    const tmpCavestack = path.join(tmpHome, '.cavestack');
...
// Check that the cavestack home directory wasn't populated as a side effect.
...
let cavestackPopulated = false;
try {
  const entries = fs.readdirSync(${JSON.stringify(tmpCavestack)}).filter(e => e !== '.gitignore');
  cavestackPopulated = entries.length > 0;
} catch {
...
`;
    ...
    const proc = Bun.spawn(['bun', '-e', childScript], {
      env: {
        ...
        // happens) won't crawl the host's real .cavestack/.
        BROWSE_STATE_FILE: path.join(tmpCavestack, 'browse.json'),
      },
      stdout: 'pipe',
      stderr: 'pipe',
    });
    ...
    // cavestack home should remain empty — initRegistry/initAuditLog/etc. side
    ...
    expect(snapshot.cavestackPopulated).toBe(false);
