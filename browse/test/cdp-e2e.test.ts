...
const TMP_HOME = path.join(os.tmpdir(), `cavestack-cdp-e2e-${process.pid}-${Date.now()}`);
...
// exact leak once pointed a sibling test's CAVESTACK_HOME at our temp dir,
// which then got baked into artifacts that outlived it (dangling symlinks
// into a deleted render dir). Save + restore in afterAll.
const ORIGINAL_CAVESTACK_HOME = process.env.CAVESTACK_HOME;
process.env.CAVESTACK_HOME = TMP_HOME;
process.env.CAVESTACK_TELEMETRY_OFF = '1'; // don't pollute analytics during tests
...
afterAll(async () => {
  if (ORIGINAL_CAVESTACK_HOME === undefined) delete process.env.CAVESTACK_HOME;
  else process.env.CAVESTACK_HOME = ORIGINAL_CAVESTACK_HOME;
