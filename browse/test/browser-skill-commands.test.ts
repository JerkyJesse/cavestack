...
beforeEach(() => {
  ...
  tiers = {
    project: path.join(tmpRoot, 'project', '.cavestack', 'browser-skills'),
    global: path.join(tmpRoot, 'home', '.cavestack', 'browser-skills'),
    bundled: path.join(tmpRoot, 'cavestack-install', 'browser-skills'),
  };
...
describe('buildSpawnEnv', () => {
  ...
  beforeEach(() => {
    ...
    process.env.CAVESTACK_TOKEN = 'root-token';
  ...
  it('untrusted: drops $HOME and secrets', () => {
    ...
    expect(env.CAVESTACK_TOKEN).toBeUndefined();
  });
  ...
  it('untrusted: injects CAVESTACK_PORT + CAVESTACK_SKILL_TOKEN', () => {
    const env = buildSpawnEnv({ trusted: false, port: 1234, skillToken: 'tok-xyz' });
    expect(env.CAVESTACK_PORT).toBe('1234');
    expect(env.CAVESTACK_SKILL_TOKEN).toBe('tok-xyz');
  });
  ...
  it('trusted: still strips CAVESTACK_TOKEN (defense in depth)', () => {
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'tok' });
    expect(env.CAVESTACK_TOKEN).toBeUndefined();
  });
  ...
  it('CAVESTACK_PORT/CAVESTACK_SKILL_TOKEN can never be overridden by parent env', () => {
    process.env.CAVESTACK_PORT = '99999'; // attacker-set
    process.env.CAVESTACK_SKILL_TOKEN = 'attacker-tok';
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'real-tok' });
    expect(env.CAVESTACK_PORT).toBe('1234');
    expect(env.CAVESTACK_SKILL_TOKEN).toBe('real-tok');
  });
});
...
describe.skipIf(SKIP_SPAWN)('spawnSkill: lifecycle', () => {
  ...
  it('untrusted spawn: CAVESTACK_SKILL_TOKEN visible, root env scrubbed', async () => {
    const dir = makeSkillDir(tiers.bundled, 'env-probe',
      'name: env-probe\nhost: x.com',  // trusted defaults to false
      `console.log(JSON.stringify({
        port: process.env.CAVESTACK_PORT,
        token: process.env.CAVESTACK_SKILL_TOKEN,
        home: process.env.HOME ?? null,
        gh: process.env.GITHUB_TOKEN ?? null,
        cavestack: process.env.CAVESTACK_TOKEN ?? null,
      }));`,
    );
    const origEnv = { ...process.env };
    process.env.GITHUB_TOKEN = 'gh-secret';
    process.env.CAVESTACK_TOKEN = 'root';
    try {
      ...
      expect(parsed.cavestack).toBeNull();
    } finally {
