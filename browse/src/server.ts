/**
 * cavestack browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.cavestack/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.cavestack/browse-{console,network,dialog}.log
...
export interface ServerConfig {
  ...
  startTime: number;
  /**
   * Overlay hook. Runs AFTER cavestack resolves auth and BEFORE route dispatch.
   * Invalid tokens are auto-rejected at the cavestack layer (401 returned
   * before hook fires), so the hook only ever sees valid TokenInfo or null
   * (no token presented). Returning a Response short-circuits cavestack
   * dispatch; returning null falls through.
   */
  beforeRoute?: (req: Request, surface: Surface, auth: TokenInfo | null) => Promise<Response | null>;
  /**
   * Whether cavestack owns the lifecycle of the terminal-agent process and its
   ...
   *      matched sibling cavestack sessions on the same host — see
   ...
   *
   * This is correct for cavestack's CLI path, which spawns `terminal-agent.ts` as
...
/**
 * Build a ServerConfig-shaped object from process.env. Used by cavestack's
...
/**
 * The cavestack sidebar extension's pinned Chrome extension ID. Derived from
 ...
 */
export const CAVESTACK_EXTENSION_ID = 'dgbkdbjebeiblbajiilljmhjdpmiglep';
...
/**
 * Read ngrok authtoken from env var, ~/.cavestack/ngrok.env, or ngrok's native
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const ngrokEnvPath = path.join(home, '.cavestack', 'ngrok.env');
...
function generateHelpText(): string {
  ...
  const lines = ['cavestack browse — headless browser for AI agents', '', 'Commands:'];
...
// ─── Buffer flush (console / network / dialog logs) ─────────────
...
// so this branch is the normal path for /open-cavestack-browser.
...
function parentWatchdogTick(parentPid: number = BROWSE_PARENT_PID): void {
  try {
    ...
    //    /open-cavestack-browser sessions.
...
// ─── Sidebar Agent (CaveStack keep — gstack 1.68 ripped chat; we did not) ─
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.cavestack', 'sidebar-sessions');
...
function getChatBuffer(tabId?: number): ChatEntry[] {
  const id = tabId ?? activeBrowserManager?.getActiveTabId?.() ?? 0;
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', 'skills', 'cavestack', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.claude', 'skills', 'cavestack', 'browse', 'dist', 'browse'),
  ];
...
function findClaudeBin(): string | null {
  ...
  try {
    const proc = Bun.spawnSync(['which', 'claude'], { stdout: 'pipe', stderr: 'pipe', timeout: 2000, windowsHide: true });
    if (proc.exitCode === 0) {
...
function addChatEntry(entry: Omit<ChatEntry, 'id'>, tabId?: number): ChatEntry {
  const targetTab = tabId ?? agentTabId ?? activeBrowserManager?.getActiveTabId?.() ?? 0;
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.cavestack', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null, forTabId?: number | null): void {
  ...
  agentTabId = forTabId ?? activeBrowserManager?.getActiveTabId?.() ?? null;
  ...
  try {
    fs.mkdirSync(cavestackDir, { recursive: true, mode: 0o700 });
...
function killAgent(targetTabId?: number | null): void {
  ...
  const cancelDir = path.join(process.env.HOME || '/tmp', '.cavestack');
...
// fighting with cavestack's. CLI path is unchanged.
if (import.meta.main) {
...
export function buildFetchHandler(cfg: ServerConfig): ServerHandle {
  ...
  // to cavestack-owns. Matches the "default-true preserves CLI bit-for-bit"
  ...
  const AGENT_WATCHDOG_TICK_MS = parseInt(
    process.env.CAVESTACK_AGENT_WATCHDOG_TICK_MS || '60000',
    10,
  );
  ...
  async function shutdown(exitCode: number = 0) {
    ...
    if (ownsTerminalAgent) {
      ...
      // terminal-agent\.ts` regex teardown which matched sibling cavestack
  ...
  // block cavestack shutdown — defensive symmetry with the safeUnlinkQuiet /
  ...
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      // Welcome page — served when CaveStack Browser launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Gate CAVESTACK_SLUG on a strict regex BEFORE interpolating it into
          // the filesystem path. Without this, a slug like "../../etc/passwd"
          // would resolve to ~/.cavestack/projects/../../etc/passwd/... — path
          ...
          const rawSlug = process.env.CAVESTACK_SLUG || 'unknown';
          const slug = /^[a-z0-9_-]+$/.test(rawSlug) ? rawSlug : 'unknown';
          const homeDir = process.env.HOME || process.env.USERPROFILE || '/tmp';
          const projectWelcome = `${homeDir}/.cavestack/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
          if (fs.existsSync(projectWelcome)) return projectWelcome;
          // Fallback: built-in welcome page from cavestack install.  Reject
          // SKILL_ROOT values containing '..' for the same defense-in-depth
          // reason as the CAVESTACK_SLUG regex above.  Not exploitable today
          // (env set at install time), but the gate is one check.
          const rawSkillRoot = process.env.CAVESTACK_SKILL_ROOT || `${homeDir}/.claude/skills/cavestack`;
        ...
        return new Response(
          `<!DOCTYPE html><html><head><title>CaveStack Browser</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>CaveStack Browser ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      // exactly `chrome-extension://<CAVESTACK_EXTENSION_ID>`, where the ID
      ...
      if (url.pathname === '/extension-token' && req.method === 'POST') {
        ...
        const originOk =
          req.headers.get('origin') === `chrome-extension://${CAVESTACK_EXTENSION_ID}`;
      ...
      // extension's cavestackInjectToTerminal callers. The extension routes
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        if (!isPairAgentEnabled()) {
          ...
          return new Response(JSON.stringify({
            ...
            hint: 'enable once with: cavestack-config set pair_agent on — or run /pair-agent, which asks for consent and sets it',
          }), { status: 403, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        // 1) Resolve ngrok authtoken from env / .cavestack / native config
...
export async function start() {
  ...
  if (process.env.BROWSE_TUNNEL === '1' && !isPairAgentEnabled()) {
    console.error('[browse] BROWSE_TUNNEL=1 ignored: pair-agent is off. Enable once with: cavestack-config set pair_agent on');
  } else if (process.env.BROWSE_TUNNEL === '1') {
    const authtoken = resolveNgrokAuthtoken();
    if (!authtoken) {
      console.error('[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.cavestack/ngrok.env');
    } else {
