 ...
 * Marks hidden elements with data-cavestack-hidden attribute.
  ...
  return page.evaluate((ariaPatterns: string[]) => {
    ...
    for (const el of elements) {
      if (el instanceof HTMLElement) {
        ...
        if (isHidden) {
          el.setAttribute('data-cavestack-hidden', 'true');
...
export async function getCleanTextWithStripping(page: Page | Frame): Promise<string> {
  const raw = await page.evaluate(() => {
    ...
    clone.querySelectorAll('[data-cavestack-hidden]').forEach(el => el.remove());
...
/**
 * Clean up data-cavestack-hidden attributes from the page.
 * Should be called after extraction is complete.
 */
export async function cleanupHiddenMarkers(page: Page | Frame): Promise<void> {
  await page.evaluate(() => {
    document.querySelectorAll('[data-cavestack-hidden]').forEach(el => {
      el.removeAttribute('data-cavestack-hidden');
    });
  });
}
