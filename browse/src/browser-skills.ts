 ...
 *   project:  <project>/.cavestack/browser-skills/<name>/
 *   global:   ~/.cavestack/browser-skills/<name>/
 *   bundled:  <cavestack-install>/browser-skills/<name>/   (read-only, ships with cavestack)
 *
...
export function defaultTierPaths(opts: { projectRoot?: string; home?: string; bundledRoot?: string } = {}): TierPaths {
  ...
  return {
    project: projectRoot ? path.join(projectRoot, '.cavestack', 'browser-skills') : null,
    global: path.join(home, '.cavestack', 'browser-skills'),
...
function detectBundledRoot(): string {
  // The browse binary lives at <cavestack-install>/browse/dist/browse.
  // The bundled browser-skills/ dir is a sibling of browse/ (i.e. <cavestack-install>/browser-skills/).
  ...
  // browse/src/browser-skills.ts → ../../  (the cavestack root).
 ...
 *
 * Cannot tombstone bundled skills — they ship with cavestack and are read-only.
 * To remove a bundled skill, override it with a global/project entry, or
 * remove the file from the cavestack source tree.
 */
