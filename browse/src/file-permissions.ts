/**
 * Cross-platform file permission restriction for sensitive cavestack state.
 *
 * Why this exists
 * ----------------
 * POSIX mode bits (`0o600` for files, `0o700` for dirs) are how cavestack marks
 ...
 * install, sensitive cavestack state files inherit whatever ACL the parent
...
function warnIcaclsFailure(fsPath: string, err: unknown): void {
  ...
  console.warn(
    `[cavestack] Failed to restrict Windows ACL on ${fsPath}: ${msg}\n` +
