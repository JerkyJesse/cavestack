...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // redundant safety, kept so `.cavestack/` still reads as ignored in git status.
  try {
  ...
  // First, check if git already ignores .cavestack/ (via global excludes, .git/info/exclude, or parent .gitignore)
  if (isIgnoredByGit(config.projectDir, '.cavestack/')) return;
...
/**
 * Resolve the cavestack home directory.
 *
 * Honors the existing convention used by telemetry.ts and domain-skills.ts:
 *   1. CAVESTACK_HOME env (explicit override)
 *   2. $HOME/.cavestack (default)
 */
export function resolveCavestackHome(): string {
  return process.env.CAVESTACK_HOME || path.join(os.homedir(), '.cavestack');
}

/**
 * Read one key from the flat-YAML config store at <cavestack home>/config.yaml
 * (the shape bin/cavestack-config writes: `key: value` lines). Tolerates
 ...
 */
export function readCavestackConfigYamlKey(key: string): string | null {
  const escaped = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  try {
    const yaml = fs.readFileSync(path.join(resolveCavestackHome(), 'config.yaml'), 'utf-8');
    // Last match wins: bin/cavestack-config's `get` reads duplicates with
 ...
 * stays OFF unless the user explicitly ran `cavestack-config set pair_agent on`
 ...
 *
 * Env override `CAVESTACK_PAIR_AGENT=on|off` wins (used by tests and as an
 * emergency knob), mirroring the telemetry env-hint convention.
 */
export function isPairAgentEnabled(): boolean {
  const env = process.env.CAVESTACK_PAIR_AGENT;
  if (env === 'on') return true;
  if (env === 'off') return false;
  // Canonical store: ~/.cavestack/config.yaml (flat `key: value` lines, written
  // by bin/cavestack-config — which is what the /pair-agent consent step runs).
  ...
  const yamlValue = readCavestackConfigYamlKey('pair_agent');
  if (yamlValue === 'on') return true;
  if (yamlValue === 'off') return false;
  try {
    const raw = fs.readFileSync(path.join(resolveCavestackHome(), 'config.json'), 'utf-8');
