 ...
 *
 * The session cookie (cavestack_picker) is isolated from the scoped token system.
...
function getSessionFromCookie(req: Request): string | null {
  ...
  const match = cookie.match(/cavestack_picker=([^;]+)/);
...
export async function handleCookiePickerRoute(
  ...
  try {
    ...
    if (pathname === '/cookie-picker' && req.method === 'GET') {
      ...
      if (code) {
        ...
        return new Response(null, {
          ...
          headers: {
            ...
            'Set-Cookie': `cavestack_picker=${session}; HttpOnly; SameSite=Strict; Path=/cookie-picker; Max-Age=3600`,
      ...
      return new Response('Access denied. Open the cookie picker from cavestack.', {
        status: 403,
        headers: { 'Content-Type': 'text/plain' },
      });
    }
