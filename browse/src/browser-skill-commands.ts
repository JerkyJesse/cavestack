 ...
 * and passes it via CAVESTACK_SKILL_TOKEN. The skill never sees the daemon root
 ...
 * CAVESTACK_PORT, so a synchronous wait would deadlock it.
 */
async function runToFiles(cmd: string[], opts: RunToFilesOptions): Promise<RunToFilesResult> {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-skill-'));
 ...
 *    CAVESTACK_PORT and CAVESTACK_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get CAVESTACK_PORT +
 * CAVESTACK_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'CAVESTACK_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip CAVESTACK_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.CAVESTACK_PORT = String(opts.port);
  out.CAVESTACK_SKILL_TOKEN = opts.skillToken;
