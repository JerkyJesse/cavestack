 ...
 *
 * /skillify stages a candidate skill into ~/.cavestack/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.cavestack/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(os.homedir(), '.cavestack', '.tmp');
