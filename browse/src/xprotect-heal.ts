 ...
 *      a CAVESTACK_CHROMIUM_PATH bundle belongs to the wrapper/embedder and is
 *      never touched (same scope contract as probePoisonedChromiumBundle).
 *   3. Force-reinstall Chromium FROM THE CAVESTACK INSTALL ROOT (ENG-OV3: the
...
/**
 * Find the cavestack install root whose node_modules pins the SAME
 ...
 */
export function findCavestackInstallRoot(
  ...
  const roots = candidates ?? [
    ...
    // path '.claude/skills/cavestack' under the daemon's cwd — often an
    ...
    path.join(os.homedir(), '.claude', 'skills', 'cavestack'),
  ];
 ...
 * on a CAVESTACK_CHROMIUM_PATH bundle — that belongs to the wrapper/embedder.
...
/**
 * Run `bunx playwright install --force chromium` from the cavestack install
...
export async function maybeHealXProtectKill(
  ...
  if (opts.usesCustomExecutable) {
    // A CAVESTACK_CHROMIUM_PATH bundle belongs to the wrapper/embedder — never
  ...
  const root = (deps.installRoot ?? findCavestackInstallRoot)(revision);
  if (!root) {
...
export function buildXProtectGuidance(originalMessage: string): string {
  return (
    ...
    'Fix manually: run `bunx playwright install chromium` from your cavestack install ' +
    '(the directory whose node_modules pins playwright — ~/.claude/skills/cavestack for ' +
