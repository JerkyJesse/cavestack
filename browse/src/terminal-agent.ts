/**
 * Terminal Agent — PTY-backed Claude Code terminal for the cavestack browser
...
const STATE_FILE = process.env.BROWSE_STATE_FILE || path.join(process.env.HOME || '/tmp', '.cavestack', 'browse.json');
...
const OWNER_WATCHDOG_MS = parseInt(
  process.env.CAVESTACK_TERMINAL_OWNER_WATCHDOG_MS || '15000',
  10,
);
...
const KEEPALIVE_INTERVAL_MS = parseInt(
  process.env.CAVESTACK_PTY_KEEPALIVE_INTERVAL_MS || '25000',
  10,
);
...
const RING_BUFFER_MAX_BYTES = parseInt(
  process.env.CAVESTACK_PTY_RING_BUFFER_BYTES || `${1024 * 1024}`,
  10,
);
...
const DETACH_WINDOW_MS = parseInt(
  process.env.CAVESTACK_PTY_DETACH_WINDOW_MS || '60000',
  10,
);
 ...
 *      (updated continuously by the cavestack browser extension).
...
function buildTabAwarenessHint(stateDir: string): string {
  ...
  return [
    'You are running inside the cavestack browser sidebar with live access to the user\'s browser tabs.',
    '',
...
function spawnClaude(cols: number, rows: number, onData: (chunk: Buffer) => void) {
  ...
  // headed-mode browser; BROWSE_NO_AUTOSTART prevents claude's cavestack
...
function buildServer(port: number) {
  return Bun.serve({
    ...
    fetch(req, server) {
      ...
      //         - Cookie cavestack_pty (works for non-browser callers and
      ...
      if (url.pathname === '/ws') {
        ...
        // with a `cavestack-pty.` prefix (which we strip). Browsers send a
        ...
        for (const raw of protoHeader.split(',').map(s => s.trim()).filter(Boolean)) {
          const candidate = raw.startsWith('cavestack-pty.') ? raw.slice('cavestack-pty.'.length) : raw;
        ...
        // Fallback: Cookie cavestack_pty (legacy / non-browser callers).
...
async function main() {
  ...
  // sibling cavestack sessions. Callers (cli.ts spawn site, server.ts
