 ...
 *
 * Scope:
 *   - per-project: ~/.cavestack/projects/<slug>/learnings.jsonl
 *   - global:      ~/.cavestack/global-domain-skills.jsonl
 *
...
function globalFile(): string {
  return path.join(cavestackHome(), 'global-domain-skills.jsonl');
}

function projectFile(slug: string): string {
  return path.join(cavestackHome(), 'projects', slug, 'learnings.jsonl');
}
