 ...
 * CAVESTACK_STEALTH=extended, off by default) DOES fake plugins — that mode
 ...
 *      detectors flag.
 *   4. Report per-install hardware values via CAVESTACK_HW_CONCURRENCY /
 *      CAVESTACK_DEVICE_MEMORY env vars (set by gbd at startup via
 ...
 * on macOS), passed through the CAVESTACK_* env vars. Each field falls
...
// the UA-CH cmdline switch in buildCaveStackLaunchArgs (which reads CAVESTACK_PLATFORM
...
export function readHostProfile(): HostProfile {
  ...
  const concurrency = Number(env.CAVESTACK_HW_CONCURRENCY);
  const memory = Number(env.CAVESTACK_DEVICE_MEMORY);
  return {
...
export function buildStealthScript(hw: HostProfile): string {
  ...
  // ──── Per-install hardware values from CAVESTACK_* env (T2) ────
...
function extendedModeEnabled(): boolean {
  const v = process.env.CAVESTACK_STEALTH;
 ...
 *   3. EXTENDED_STEALTH_SCRIPT — only when CAVESTACK_STEALTH=extended (off by
 ...
 * disagree with the env-driven GPU spoof in buildCaveStackLaunchArgs on non-Apple
...
/**
 * Build the `--cavestack-*=` cmdline switches that the Pack 1 Chromium
 * patches read (webgl-vendor-spoof, ua-client-hints-stealth, worker-
 * navigator-stealth). Values come from the CAVESTACK_* env vars that
 ...
 *
 * TRUSTED-SOURCE ONLY. These CAVESTACK_* values are populated by gbd from
 ...
 *   CAVESTACK_GPU_VENDOR        → --cavestack-gpu-vendor        → webgl-vendor-spoof.patch
 *   CAVESTACK_GPU_RENDERER      → --cavestack-gpu-renderer      → webgl-vendor-spoof.patch
 *   CAVESTACK_PLATFORM          → --cavestack-ua-platform       → ua-client-hints-stealth.patch
 *                              (maps MacARM/MacIntel → "macOS")
 *   CAVESTACK_GPU_CHIPSET       → --cavestack-ua-model          → ua-client-hints-stealth.patch
 *   CAVESTACK_HW_CONCURRENCY    → --cavestack-hw-concurrency    → worker-navigator-stealth.patch
 *   CAVESTACK_DEVICE_MEMORY     → --cavestack-device-memory     → worker-navigator-stealth.patch
 */
export function buildCaveStackLaunchArgs(): string[] {
  const env = (globalThis as any).process?.env ?? {};
  const args: string[] = [];

  const vendor = env.CAVESTACK_GPU_VENDOR;
  if (vendor) args.push(`--cavestack-gpu-vendor=${vendor}`);

  const renderer = env.CAVESTACK_GPU_RENDERER;
  if (renderer) args.push(`--cavestack-gpu-renderer=${renderer}`);
  ...
  const platform = env.CAVESTACK_PLATFORM;
  if (platform === 'MacARM' || platform === 'MacIntel') {
    args.push('--cavestack-ua-platform=macOS');
  } else if (platform === 'Win32') {
    args.push('--cavestack-ua-platform=Windows');
  } else if (platform && platform.startsWith('Linux')) {
    args.push('--cavestack-ua-platform=Linux');
  }

  const chipset = env.CAVESTACK_GPU_CHIPSET;
  if (chipset) args.push(`--cavestack-ua-model=${chipset}`);

  const hw = env.CAVESTACK_HW_CONCURRENCY;
  if (hw) args.push(`--cavestack-hw-concurrency=${hw}`);

  const memory = env.CAVESTACK_DEVICE_MEMORY;
  if (memory) args.push(`--cavestack-device-memory=${memory}`);
  ...
  //
  // OPT-IN (off by default): only emitted when CAVESTACK_CDP_STEALTH is
  // on/1/true. This switch is read by a C++ patch that only exists in
  // gbrowser builds; gbd opts in by exporting CAVESTACK_CDP_STEALTH=on. Stock
  ...
  // unless CAVESTACK_CDP_STEALTH=off, which contradicted this very comment.)
  const cdpStealth = env.CAVESTACK_CDP_STEALTH;
  if (cdpStealth === 'on' || cdpStealth === '1' || cdpStealth === 'true') {
    args.push('--cavestack-suppress-prepare-stack-trace');
  }
