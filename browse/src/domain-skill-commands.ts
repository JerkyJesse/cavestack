 ...
 *     suggested-action. Matches the cavestack house style.
 *
...
function formatSavedOk(row: DomainSkillRow, slug: string): string {
  return [
    ...
    `Version: ${row.version}`,
    `Stored at: ~/.cavestack/projects/${slug}/learnings.jsonl`,
    '',
...
async function handleEdit(args: string[]): Promise<string> {
  ...
  const tmpFile = path.join(os.tmpdir(), `cavestack-domain-skill-${process.pid}-${Date.now()}.md`);
...
async function handlePromoteToGlobal(args: string[]): Promise<string> {
  ...
  return [
    ...
    `Stored at: ~/.cavestack/global-domain-skills.jsonl`,
    '',
