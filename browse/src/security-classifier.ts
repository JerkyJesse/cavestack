 ...
 * layout at ~/.cavestack/models/testsavant-small/ on first use.
 *
 ...
 */
const MODELS_DIR = path.join(os.homedir(), '.cavestack', 'models');
...
export function loadTestsavant(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.CAVESTACK_SECURITY_OFF === '1') {
    testsavantState = 'failed';
    testsavantLoadError = 'CAVESTACK_SECURITY_OFF=1 — ML classifier kill switch engaged';
