 ...
 *
 * Piggybacks on ~/.cavestack/analytics/skill-usage.jsonl pattern (existing
 * cavestack telemetry). Hostname + aggregate counters only; no body content,
 * no agent text, no command args. Respects the user's telemetry tier
 * setting (off | anonymous | community) via cavestack-config.
 *
...
import { readCavestackConfigYamlKey } from './config';
...
function analyticsDir(): string {
  return path.join(cavestackHome(), 'analytics');
}
 ...
 * ~/.cavestack/config.yaml, and only a granted tier enables emission. Tiers,
 * checked in order:
 *
 *   1. Env hint CAVESTACK_TELEMETRY_OFF=1 (set by preambles and test
 ...
 *      means consent was never granted — matching bin/cavestack-config's
 *      DEFAULTS table, which reports 'off' for an unset telemetry key.
 *      Anything else would be a split-brain where `cavestack-config get
 *      telemetry` tells the user 'off' while a direct-$B daemon emits.
 *      One escape hatch: CAVESTACK_TELEMETRY_OFF=0 is a harness-side consent
 *      assertion that flips this DEFAULT only (test harnesses exercising the
 *      write path against a scratch CAVESTACK_HOME) — it never overrides an
...
export function isTelemetryDisabled(): boolean {
  ...
  if (process.env.CAVESTACK_TELEMETRY_OFF === '1') {
    telemetryDisabled = true;
    return true;
  }
  // Persistent tier: an explicit user-written value always wins next.
  const tier = readCavestackConfigYamlKey('telemetry');
  if (tier === 'off') {
    telemetryDisabled = true;
    return true;
  }
  ...
  telemetryDisabled = process.env.CAVESTACK_TELEMETRY_OFF !== '0';
