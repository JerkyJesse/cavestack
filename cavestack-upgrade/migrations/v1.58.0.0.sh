...
#
# Affected: users who run cavestack inside Conductor and don't already have the
...
#
# Idempotent: cavestack-settings-hook dedupes by (event, matcher, source), and a
...
SETTINGS_HOOK="${SCRIPT_DIR}/bin/cavestack-settings-hook"
PREF_HOOK="${SCRIPT_DIR}/hosts/claude/hooks/question-preference-hook"
CONFIG_BIN="${SCRIPT_DIR}/bin/cavestack-config"
