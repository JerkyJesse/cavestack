...
# (~/.claude/skills/cavestack), rewriting ~16 TRACKED SKILL.md files. The
# checkout stayed permanently dirty, and every /cavestack-upgrade `git stash`
...
# version. v1.67 renders to an untracked out-dir (~/.cavestack/render/claude)
...
#
# Affected: global-git installs that ever ran ./setup or `cavestack-config
# gbrain-refresh` with gbrain detected, before v1.67.0.0.
#
# Idempotent: a clean checkout is a no-op. Non-fatal throughout.
set -u

INSTALL_DIR="${CAVESTACK_INSTALL_DIR:-$HOME/.claude/skills/cavestack}"

# Only operate on a real (non-symlink) git checkout that looks like cavestack.
...
if [ "$_RESTORED" -gt 0 ]; then
  ...
  echo "  Brain-aware blocks now render to ~/.cavestack/render/claude — the checkout stays clean from here on."
fi
if [ "$_LEFT" -gt 0 ]; then
