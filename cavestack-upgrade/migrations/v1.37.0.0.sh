...
#       ~/.claude/skills/cavestack/bin/cavestack-config set local_code_index_offered true
#
# When silent: anything else (Path 1/2/3 users, anyone already on PGLite,
# anyone who opted out, anyone without remote-http MCP).
#
# Idempotency: writes a touchfile at ~/.cavestack/.migrations/v1.37.0.0.done
...
MIGRATIONS_DIR="$CAVESTACK_HOME/.migrations"
DONE_TOUCH="$MIGRATIONS_DIR/v1.37.0.0.done"
CONFIG_BIN="$HOME/.claude/skills/cavestack/bin/cavestack-config"
...
if is_remote_http_mcp && is_local_engine_missing; then
  ...
  ┌──────────────────────────────────────────────────────────────────┐
  │  cavestack v1.37.0.0 — split-engine gbrain                          │
  │                                                                  │
  ...
  │  permanently:                                                    │
  │    cavestack-config set local_code_index_offered true               │
  └──────────────────────────────────────────────────────────────────┘
