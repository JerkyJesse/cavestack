#!/usr/bin/env bash
# Migration: v1.27.0.0 — rename cavestack-brain-* → cavestack-artifacts-*
#
# Phase C of the v1.27.0.0 plan. Hard-rename, no compat shim. Steps:
#   1. gh_repo_renamed       — gh/glab repo rename cavestack-brain-$USER →
#                              cavestack-artifacts-$USER (skipped on user opt-out)
#   2. remote_txt_renamed    — mv ~/.cavestack-brain-remote.txt → artifacts-remote.txt
#   3. config_key_renamed    — rewrite gbrain_sync_mode → artifacts_sync_mode
#                              in ~/.cavestack/config.yaml
...
#
# Interruption-safe via journal at ~/.cavestack/.migrations/v1.27.0.0.journal:
# each step writes its name on success; re-entry resumes from the next un-done
# step. Done touchfile at ~/.cavestack/.migrations/v1.27.0.0.done.
#
...
# Configuration
# ---------------------------------------------------------------------------
CAVESTACK_HOME="${HOME}/.cavestack"
SKILLS_DIR="${HOME}/.claude/skills"
BIN_DIR="${SKILLS_DIR}/cavestack/bin"
CONFIG_BIN="${BIN_DIR}/cavestack-config"
URL_BIN="${BIN_DIR}/cavestack-artifacts-url"

MIGRATION_DIR="${CAVESTACK_HOME}/.migrations"
...
RERUN_CMD="CAVESTACK_MIGRATE_ASSUME_YES=1 bash ${SELF_PATH}"

USER_NAME="${USER:-$(whoami 2>/dev/null || echo unknown)}"
OLD_REPO_NAME="cavestack-brain-${USER_NAME}"
NEW_REPO_NAME="cavestack-artifacts-${USER_NAME}"
OLD_REMOTE_TXT="${HOME}/.cavestack-brain-remote.txt"
NEW_REMOTE_TXT="${HOME}/.cavestack-artifacts-remote.txt"
...
#   - presence of ~/.cavestack-brain-remote.txt (legacy file)
#   - presence of ~/.cavestack/.git (brain-init ever ran)
HAS_LEGACY_STATE=0
[ -f "$OLD_REMOTE_TXT" ] && HAS_LEGACY_STATE=1
[ -d "$CAVESTACK_HOME/.git" ] && HAS_LEGACY_STATE=1

# If nothing to migrate, finalize silently.
if [ "$HAS_LEGACY_STATE" = "0" ]; then
  echo "  [v1.27.0.0] no legacy cavestack-brain state detected — nothing to migrate." >&2
  touch "$DONE"
...
if [ ! -f "$JOURNAL" ]; then
  ...
  [v1.27.0.0] cavestack-brain has been renamed to cavestack-artifacts.
  ...
  This migration will:
    ...
    2. mv ~/.cavestack-brain-remote.txt → ~/.cavestack-artifacts-remote.txt
    3. Rename gbrain_sync_mode → artifacts_sync_mode in ~/.cavestack/config.yaml
...
EOF
  if [ -t 0 ]; then
    ...
    # proceeds unattended.
    if [ "${CAVESTACK_MIGRATE_ASSUME_YES:-0}" = "1" ]; then
      echo "  (non-interactive: proceeding — CAVESTACK_MIGRATE_ASSUME_YES=1)" >&2
    else
...
# ---------------------------------------------------------------------------
# Step 2: rename ~/.cavestack-brain-remote.txt → ~/.cavestack-artifacts-remote.txt
# ---------------------------------------------------------------------------
if ! journal_done "remote_txt_renamed"; then
  echo "  [v1.27.0.0] step 2: rename ~/.cavestack-brain-remote.txt → ~/.cavestack-artifacts-remote.txt" >&2
  if [ -f "$OLD_REMOTE_TXT" ] && [ ! -f "$NEW_REMOTE_TXT" ]; then
    # Update the URL inside if the rename happened on the host: replace
    # cavestack-brain-$USER with cavestack-artifacts-$USER in the URL.
...
if ! journal_done "config_key_renamed"; then
  ...
  CFG="$CAVESTACK_HOME/config.yaml"
  if [ -f "$CFG" ]; then
...
if ! journal_done "claude_md_block_rewritten"; then
  ...
  # Look in cwd's CLAUDE.md (where /setup-gbrain wrote it) and ~/.cavestack/CLAUDE.md
  ...
  for CMD in "$PWD/CLAUDE.md" "$CAVESTACK_HOME/CLAUDE.md"; do
    [ -f "$CMD" ] || continue
...
EOF
  ...
  elif command -v gbrain >/dev/null 2>&1 && [ -d "$CAVESTACK_HOME/.git" ]; then
    ...
    WORKTREE="${CAVESTACK_BRAIN_WORKTREE:-$HOME/.cavestack-brain-worktree}"
    ...
    echo "    gbrain CLI not available or no ~/.cavestack/.git — skipping" >&2
    mark_done "sources_swapped"
  fi
fi
