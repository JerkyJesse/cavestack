...
# Info.plist ("Google Chrome for Testing" → "CaveStack Browser") and overwrote
...
#
# Idempotent: detection is content-based (plist contains "CaveStack Browser");
...
if [ -d "${PW_CACHE}" ]; then
  ...
  while IFS= read -r plist; do
    if grep -q "CaveStack Browser" "${plist}" 2>/dev/null; then
...
if [ "${REMOVED}" = "1" ]; then
  ...
  # headed use the same bundle). Run from the cavestack install root so bunx
