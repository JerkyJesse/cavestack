...
# so the migration runner retries on the next /cavestack-upgrade.

set -u

CAVESTACK_HOME="${HOME}/.cavestack"
ALLOWLIST="${CAVESTACK_HOME}/.brain-allowlist"
PRIVACY="${CAVESTACK_HOME}/.brain-privacy-map.json"
GITATTRS="${CAVESTACK_HOME}/.gitattributes"
...
if [ -f "${PRIVACY}" ]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    if ! jq -e . "${PRIVACY}" >/dev/null 2>&1; then
      echo "  [v1.40.0.0] WARN: ${PRIVACY} is not valid JSON; skipping privacy-map repair. Fix manually or run cavestack-artifacts-init." >&2
      incomplete=1
    else
      for PATTERN in "${NEW_PATTERNS[@]}"; do
    ...
    echo "  [v1.40.0.0] WARN: jq not found; skipping privacy-map repair. Install jq and re-run cavestack-upgrade, or run cavestack-artifacts-init manually." >&2
    incomplete=1
  fi
fi
...
if [ "${incomplete}" = "0" ]; then
  ...
  # unnecessary. A fresh-init user's bin/cavestack-artifacts-init now writes the
  ...
  echo "  [v1.40.0.0] INFO: marker not written; cavestack-upgrade will retry once prerequisites are met." >&2
fi
