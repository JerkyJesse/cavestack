...
```bash
~/.claude/skills/cavestack/bin/cavestack-evidence run --label tests -- 'bin/test-lane 2>&1' &
~/.claude/skills/cavestack/bin/cavestack-evidence run --label vitest -- 'npm run test 2>&1' &
wait
```
...
`~/.claude/skills/cavestack/bin/cavestack-detach` instead — it survives in its own
...
```bash
~/.claude/skills/cavestack/bin/cavestack-detach --label ship-evals --lock cavestack-evals --timeout 5400 -- <project eval command>
```
