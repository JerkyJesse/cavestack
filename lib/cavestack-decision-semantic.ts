/**
 * cavestack-decision-semantic — OPTIONAL gbrain enhancement for decision resurfacing.
 *
 * This is the ONLY decision module that touches gbrain. The reliable core
 * (lib/cavestack-decision.ts) has zero gbrain imports and works with gbrain OFF; this
 * module is loaded lazily by `cavestack-decision-search` only on `--semantic`, and every
 ...
 *  - The curated-memory source is the one whose local_path is the cavestack brain
 *    worktree (`~/.cavestack-brain-worktree`), id `default` by convention — NOT a
 *    `cavestack-brain-<user>` id. Scoping search to it keeps code/doc corpora out.
 */
...
const BRAIN_WORKTREE_SUFFIX = ".cavestack-brain-worktree";
...
/**
 * Resolve the curated-memory source id (the cavestack brain worktree). Returns null
