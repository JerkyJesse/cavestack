 ...
 * Conductor's CAVESTACK_-prefixed forms are present. Promote the CAVESTACK_ form to
...
/**
 * Pure form: returns a copy of `base` with each CAVESTACK_-prefixed key promoted
...
export function promotedEnv(base: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of PROMOTED_KEYS) {
    if (!out[key] && out[`CAVESTACK_${key}`]) {
      out[key] = out[`CAVESTACK_${key}`];
    }
  }
  return out;
}
