 ...
 * cavestack-initiated off-machine send (`~/.cavestack/security/egress.jsonl`, 0600).
 *
...
export interface WriteReceiptOptions {
  /** cavestack home; resolved from env when omitted */
  home?: string;
  /** env for home resolution (tests) */
  env?: Env;
  /** which cavestack component is sending */
  sink: string;
...
/**
 * Same resolution order as the rest of cavestack (shell sinks, selection code):
 * CAVESTACK_HOME, legacy CAVESTACK_STATE_DIR, then $HOME/.cavestack.
 */
export function resolveEgressHome(env: Env = process.env): string {
  const configured = env.CAVESTACK_HOME || env.CAVESTACK_STATE_DIR;
  if (configured) return path.resolve(configured);
  return path.join(env.HOME || os.homedir(), '.cavestack');
}
 ...
 * cavestack ATTEMPTS to send off-machine), how to inspect it, and that
 * trimming arrives with rotation.
 */
export function ledgerSizeWarning(ledger: string, size: number): string {
  const mb = (size / (1024 * 1024)).toFixed(1);
  return (
    `cavestack: egress ledger is large (${mb}MB): ${ledger}. ` +
    `This file records what cavestack ATTEMPTS to send off-machine (content-free receipts, for auditing). ` +
    `Inspect it with 'cavestack-egress list'. Trimming arrives with ledger rotation (TODO); until then it only grows.`
  );
}
