 ...
 * trust store (bin/cavestack-gbrain-repo-policy, a bash CLI that owns URL
 ...
 * veto; bin/cavestack-gbrain-sync.ts code-import gate) each spawnSync'd the script
...
/** Absolute path of the policy store for this env (CAVESTACK_HOME-aware). */
export function repoPolicyStorePath(env: NodeJS.ProcessEnv = process.env): string {
  const home = env.CAVESTACK_HOME || join(env.HOME || homedir(), ".cavestack");
...
const POLICY_SCRIPT = join(import.meta.dir, "..", "bin", "cavestack-gbrain-repo-policy");

/**
 * Trust tier for a remote URL, via `cavestack-gbrain-repo-policy get <url>`.
 *
...
export function repoPolicyTier(url: string | null, env: NodeJS.ProcessEnv = process.env): RepoPolicyResult {
  ...
  const res = spawnSync(cmd, args, {
    ...
    // process.env mutations (e.g. tests redirecting CAVESTACK_HOME).
...
/**
 * Bulk trust-tier lookup via `cavestack-gbrain-repo-policy get --batch` — ONE
