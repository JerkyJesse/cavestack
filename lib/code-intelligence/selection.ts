 ...
 * per-repo indexing consent. Stored at `$CAVESTACK_HOME/code-intelligence.json`
 * (default `~/.cavestack/`), the same home the rest of cavestack uses.
 *
 * Portions copyright (c) 2026 Sina Matian, time-attack/cavestack (CaveStack 2), MIT.
 *
...
/**
 * The per-remote trust store (cavestack-gbrain-repo-policy) is the SINGLE
 ...
 * (the exact semantics runCodeImport in bin/cavestack-gbrain-sync.ts enforces —
