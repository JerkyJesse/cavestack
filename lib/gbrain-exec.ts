 ...
 *
 * Every `gbrain ...` spawn from `bin/cavestack-gbrain-sync.ts` and
 * `bin/cavestack-memory-ingest.ts` MUST go through `spawnGbrain` (or
 ...
 *      → /tmp/x/.gbrain/config.json). Every cavestack-side read goes through
 *      `gbrainConfigDir()` below so cavestack and gbrain agree on which
 *      config file matters.
 *
 * **Escape hatch:** `CAVESTACK_RESPECT_ENV_DATABASE_URL=1` returns the
 ...
 * Unset → ~/.gbrain. Every cavestack-side gbrain-config read MUST resolve
 * through this helper, or cavestack classifies engine status from a file
 * gbrain never reads.
 */
 ...
 * unchanged when:
 *   - `CAVESTACK_RESPECT_ENV_DATABASE_URL=1` (intentional opt-out),
...
export function buildGbrainEnv(opts: BuildGbrainEnvOptions = {}): NodeJS.ProcessEnv {
  ...
  if (baseEnv.CAVESTACK_RESPECT_ENV_DATABASE_URL === "1") return out;
 ...
 * `gbrain.cmd`/`.ps1` shim) or a shebang script like the bash `cavestack-brain-sync`
...
/**
 * How to invoke a **bash shebang script** (`cavestack-brain-sync`) on this platform.
 *
 ...
 * `~/.cavestack` and were never committed, while `/sync-gbrain` printed one red
 ...
 * repo. `CAVESTACK_BASH` overrides everything for unusual installs.
 *
...
export function bashScriptInvocation(
  ...
  const override = env.CAVESTACK_BASH?.trim();
 ...
 * listeners (e.g., `gbrain import` in `cavestack-memory-ingest.ts`). Always
