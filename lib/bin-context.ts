/**
 * bin-context — tiny shared helpers for non-interactive cavestack bins that need the
 * project slug, current branch, and argv flags. Extracted from the decision bins
 * (cavestack-decision-log / cavestack-decision-search) so the slug/branch/flag plumbing
...
/** Keep the slug inside the [a-zA-Z0-9._-] alphabet cavestack-slug promises (`tr -cd`). */
 ...
 * `C:\Users\j\foo` → `/c/Users/j/foo`. cavestack-slug keys its cache on THAT form
 * (`tr '/' '_'`), so a native lookup must reproduce it exactly or it misses the very
 * entry cavestack-slug wrote and silently re-derives instead of staying consistent.
...
// Marker tiers mirror bin/cavestack-slug's `_outermost_project_root` exactly.
...
/**
 * Native port of bin/cavestack-slug's `_outermost_project_root`: walk UP
...
/**
 * Native port of bin/cavestack-slug's `_outermost_remote_repo` (step 1a): walk UP
...
/**
 * Native port of bin/cavestack-slug's resolution order, used when that script cannot be
 ...
 * test/bin-context-windows-slug.test.ts against test/cavestack-slug-cwd-walk-up.test.ts
 * and test/cavestack-slug-parity.test.ts):
 *   0. $CAVESTACK_PROJECT_SLUG env override — wins over everything, never cached.
 ...
 *      like `url = ..` must never escape ~/.cavestack/projects/<slug>.
 *   4. Project root's basename; else basename(cwd) for plain non-project folders.
 */
export function slugFromEnvironment(cavestackHome?: string, cwd: string = process.cwd()): string {
  const home = cavestackHome || process.env.CAVESTACK_HOME || join(homedir(), ".cavestack");
  ...
  const envSlug = sanitizeSlug((process.env.CAVESTACK_PROJECT_SLUG || "").trim());
  if (envSlug) return envSlug;

  // 1. outermost project root along the cwd ancestor chain (may be "").
  const projectRoot = outermostProjectRoot(cwd);

  // Lazy, memoized remote discovery (mirrors cavestack-slug's _resolve_remote):
  ...
  // 3. canonical remote-derived slug from the outermost remote-bearing repo.
  //    Parse mirrors bin/cavestack-slug step 2 exactly (byte-parity with
  ...
  if (!slug) {
    ...
    if (url) {
      ...
      // Dot-only / degenerate guard (mirrors bin/cavestack-slug): a hostile
      // origin like `url = ..` yields "." or ".." here, which would file
      // state OUTSIDE ~/.cavestack/projects/. Reject and let the basename
  ...
  // 5. cache it, as cavestack-slug does — atomic, self-healing (only rewrites when
 ...
 *  PATHEXT match for an explicit path), so cavestack-slug spawns ENOENT there. */
export const NEEDS_NATIVE_SLUG_ON_WINDOWS = process.platform === "win32";

/**
 * Resolve the project slug via the `cavestack-slug` helper (parses `SLUG=...`).
 *
 ...
 * ~/.cavestack/projects/unknown/ — one bucket shared by every project on the machine —
