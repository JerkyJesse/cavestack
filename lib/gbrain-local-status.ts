 ...
 *
 * Shared between bin/cavestack-gbrain-detect (preamble probe on every skill start)
 * and bin/cavestack-gbrain-sync.ts (orchestrator SKIP-when-not-ok semantics).
 ...
 *   - Cache: 60s TTL at ~/.cavestack/.gbrain-local-status-cache.json, keyed on
 ...
 * Timeout → probe exceeded CAVESTACK_GBRAIN_PROBE_TIMEOUT_MS (default 15s) with no
...
interface CacheEntry {
  // Local-cache schema version, controlled by cavestack. Not to be confused
  // with `gbrain doctor --json` output schema_version (gbrain v0.25+ emits
  // schema_version: 2). Doctor-output parsing lives in
  // lib/cavestack-memory-helpers.ts:freshDetectEngineTier and accepts both
...
/**
 * Effective probe timeout. `CAVESTACK_GBRAIN_PROBE_TIMEOUT_MS` overrides the
...
export function probeTimeoutMs(env?: NodeJS.ProcessEnv): number {
  const raw = (env ?? process.env).CAVESTACK_GBRAIN_PROBE_TIMEOUT_MS;
...
/** Cache path computed fresh on each call so tests can mutate CAVESTACK_HOME per case. */
export function cacheFilePath(): string {
  return join(
    process.env.CAVESTACK_HOME || join(userHome(), ".cavestack"),
...
export function hasRemoteOnlyGbrainMcp(
  ...
  // Same classification rules as cavestack-gbrain-detect's detectMcpMode tier 3,
