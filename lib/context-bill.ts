/**
 * cavestack context-bill — token bill-of-materials for an installed cavestack skills tree.
 *
 ...
 * envelope subtracted. Regenerate with `cavestack-context-bill <tree> --exact
...
function totalMd(dir: string, tokensOf: TokensOf): { bytes: number; tokens: number } {
  ...
  for (const p of walkMd(dir)) {
    // A skill dir that CONTAINS other skill dirs (the cavestack root skill wraps
 ...
 * installed ~/.claude/skills tree contains flat skill dirs PLUS a full cavestack
 * repo checkout (`cavestack/`, with .git). Its nested SKILL.md files are the
...
const USAGE =
  "Usage:\n" +
  "  cavestack-context-bill [TREE] [--json] [--skill <name>]\n" +
  "  cavestack-context-bill --diff <treeA> <treeB> [--json]\n" +
  "  cavestack-context-bill [TREE] --budget <budget.json> [--json]\n" +
  "\n" +
  ...
  "                       cavestack-egress); if it cannot be written, the run falls\n" +
