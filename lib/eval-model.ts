 ...
 *
 *   CAVESTACK_EVAL_MODEL_<KIND>  (e.g. CAVESTACK_EVAL_MODEL_WARMUP) — per-kind
 *   CAVESTACK_EVAL_MODEL                                        — global
...
export function resolveEvalModel(
  ...
  const perKind = env[`CAVESTACK_EVAL_MODEL_${kind.toUpperCase()}`];
  if (perKind) return perKind;
  if (env.CAVESTACK_EVAL_MODEL) return env.CAVESTACK_EVAL_MODEL;
