 ...
 *
 * Mirrors the bash logic in bin/cavestack-gbrain-source-wireup:204-310 but in a form
 * importable by other TS callers (currently bin/cavestack-gbrain-sync.ts; future
...
import { withErrorContext } from "./cavestack-memory-helpers";
 ...
 * forensic logging to ~/.cavestack/.gbrain-errors.jsonl.
 */
 ...
 * cavestack-side gbrain call). `env` is the caller's base env (tests inject a
 * shim on PATH).
 */
