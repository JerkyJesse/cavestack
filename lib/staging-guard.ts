/**
 * staging-guard — fail-closed ownership proof for cavestack ingest staging dirs.
 *
 * Fixes #1802. The /sync-gbrain memory stage stages prepared pages to a
 * throwaway dir under ~/.cavestack and `rm -rf`s it when done. The resume path
 ...
 * proved it owned. This module is the single definition of "a path cavestack is
 ...
 *   2. Structural    — canonical path is a DIRECT child of $CAVESTACK_HOME named
 ...
 *                      git working tree.
 *   4. Minted by us  — a `.cavestack-staging` marker file (written by
 ...
 * mitigation at cavestack's own rm -rf boundary; see the companion gbrain issue.
 */
...
export const STAGING_MARKER = ".cavestack-staging";
...
/**
 * Prove (fail-closed) that `dir` is a cavestack-owned ingest staging directory
 ...
 * @param cavestackHome  Resolved $CAVESTACK_HOME (injected for testability).
 */
export function checkOwnedStagingDir(dir: string, cavestackHome: string): StagingVerdict {
  ...
  try {
    ...
    home = realpathSync(cavestackHome);
  } catch {
  ...
  if (dirname(canon) !== home) {
    return { ok: false, reason: `not a direct child of CAVESTACK_HOME (${home})` };
  }
...
export function isOwnedStagingDir(dir: string, cavestackHome: string): boolean {
  return checkOwnedStagingDir(dir, cavestackHome).ok;
}
