/**
 * cavestack-memory-helpers — shared helpers for the V1 memory ingest + retrieval pipeline.
 *
 * Imported by:
 *   - bin/cavestack-memory-ingest.ts (Lane A)
 *   - bin/cavestack-gbrain-sync.ts   (Lane B)
 *   - bin/cavestack-brain-context-load.ts (Lane C)
 ...
 *   ED1 — engine-tier cache lives in ~/.cavestack/.gbrain-engine-cache.json (60s TTL)
 *
 ...
 *
 * Examples:
 *   https://github.com/JerkyJesse/cavestack.git → github.com/JerkyJesse/cavestack
 *   git@github.com:garrytan/cavestack.git     → github.com/JerkyJesse/cavestack
...
function gitleaksAvailable(): boolean {
  ...
  try {
    ...
    process.stderr.write(
      "[cavestack-memory-helpers] gitleaks not in PATH; secret scanning disabled. " +
 ...
 * Replaces the inadequate regex scanner in bin/cavestack-brain-sync (which only
...
function cavestackHome(): string {
  return process.env.CAVESTACK_HOME || join(homedir(), ".cavestack");
}

function engineCachePath(): string {
  return join(cavestackHome(), ".gbrain-engine-cache.json");
}

function errorLogPath(): string {
  return join(cavestackHome(), ".gbrain-errors.jsonl");
}

/**
 * Detect which gbrain engine is active (PGLite vs Supabase) and cache the
 * answer for 60s in ~/.cavestack/.gbrain-engine-cache.json. Caching avoids
...
export function detectEngineTier(): EngineDetect {
  ...
  try {
    ...
    writeFileSync(
      engineCachePath(),
      JSON.stringify({ ...fresh, last_writer: "cavestack-memory-helpers.detectEngineTier" }, null, 2),
      "utf-8"
    );
  } catch {
...
// Best-effort JSONL append to ~/.cavestack/.gbrain-errors.jsonl. Never throws.
 ...
 * runtime read path used by cavestack-brain-context-load; it tolerates extra
...
const ERROR_LOG_PATH = join(cavestackHome(), ".gbrain-errors.jsonl");

/**
 * Wrap an op with structured error logging. Logs success/failure + duration
 * to ~/.cavestack/.gbrain-errors.jsonl for forensic debugging. Replaces ad-hoc
