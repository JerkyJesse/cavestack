 ...
 * bin/cavestack-gbrain-supabase-provision (thin bun-shebang entry).
 *
 ...
 * mirroring `_receipted_curl closed` in bin/cavestack-egress-lib.sh.
 *
 * Exit codes:
 *   0 — success
...
const PROG = 'cavestack-gbrain-supabase-provision';
 ...
 * is not reproduced.
 */
export const HELP_TEXT = `cavestack-gbrain-supabase-provision — Supabase Management API wrapper for
 ...
 * and refusal message as _receipted_curl closed in cavestack-egress-lib.sh).
 */
  ...
  for (;;) {
    ...
    try {
      const receipt = writeReceipt({
        ...
        consent: 'user ran cavestack-gbrain-supabase-provision',
      });
      receiptId = receipt.id;
    } catch (error) {
      // Refused — the send never happens. Same problem/cause/fix contract as
      // _cavestack_egress_refusal in cavestack-egress-lib.sh, then exit 8.
      const home = resolveEgressHome(ctx.env);
      const cause = `EGRESS_RECEIPT_FAILED: ${(error as Error)?.message ?? error}`.replace(/\n/g, ' ');
      ctx.stderr(
        `cavestack: supabase-provision NOT sent — the egress receipt could not be written (${cause}). ` +
          `Fix: chmod -R u+w ${path.join(home, 'security')} (or check CAVESTACK_HOME). ` +
          `What this is: cavestack records everything it ATTEMPTS to send off-machine; see cavestack-egress.\n`,
      );
...
async function cmdPoolerUrl(ctx: Ctx, args: string[]): Promise<void> {
  ...
  // Override with CAVESTACK_SUPABASE_TRUST_API_PORT=1 if a future API version
  // starts returning a working transaction port and this rewrite is wrong.
  if ((ctx.env.CAVESTACK_SUPABASE_TRUST_API_PORT ?? '0') !== '1' && poolMode === 'transaction' && dbPort === '6543') {
    ctx.stderr(
      'pooler-url: API returned transaction pooler (port 6543); shared pooler for new projects listens on session port 5432 — rewriting (set CAVESTACK_SUPABASE_TRUST_API_PORT=1 to disable)\n',
    );
    dbPort = '5432';
    poolMode = 'session';
  }
...
export async function runProvision(argv: string[], options: ProvisionOptions = {}): Promise<number> {
  ...
  try {
    switch (cmd) {
      ...
      case undefined:
      case '':
        die(ctx, 'usage: cavestack-gbrain-supabase-provision {list-orgs|create|wait|pooler-url|list-orphans|delete-project|--help}');
        break;
      default:
