...
- **Command audit log.** Every browse command now gets a persistent forensic trail in `~/.cavestack/.browse/browse-audit.jsonl`. Timestamp, command, args, page origin, duration, status, error, and whether cookies were imported. Append-only, never truncated, survives server restarts. Best-effort writes that never block command execution. From @halbert04.
- **Cookie domain tracking.** cavestack now tracks which domains cookies were imported from. Foundation for origin pinning above. Direct imports via `--domain` track automatically. New `--all` flag makes full-browser cookie import an explicit opt-in instead of the default.
...
- **Shell injection in setup scripts.** `cavestack-settings-hook` interpolated file paths directly into `bun -e` JavaScript blocks. A path with quotes broke the JS string context. Now uses environment variables (`process.env`). Systematic audit confirmed only this script was vulnerable. From @garagon.
...
### Added
- **Browser data platform.** Six new browse commands that turn cavestack browser from "a thing that clicks buttons" into a full scraping and data extraction tool for AI agents.
...
- `$B connect` crashes with "domains is not defined". A stray variable reference in the headed-mode status check prevented CaveStack Browser from initializing properly.
...
- Supabase migration 003: column-level GRANT restricts anon UPDATE to (last_seen, cavestack_version, os) only.
...
- **`cavestack-team-init` now detects and removes vendored cavestack copies.** When you run `cavestack-team-init` inside a repo that has cavestack vendored at `.claude/skills/cavestack/`, it automatically removes the vendored copy, untracks it from git, and adds it to `.gitignore`. No more stale vendored copies shadowing the global install.
- **`/cavestack-upgrade` respects team mode.** Step 4.5 now checks the `team_mode` config. In team mode, vendored copies are removed instead of synced, since the global install is the single source of truth.
...
Teams can now keep every developer on the same cavestack version automatically. No more vendoring 342 files into your repo. No more version drift across branches. No more "who upgraded cavestack last?" Slack threads. One command, every developer is current.

Hat tip to Jared Friedman for the design.

### Added

- **`./setup --team`.** Registers a `SessionStart` hook in `~/.claude/settings.json` that auto-updates cavestack at the start of each Claude Code session. Runs in background (zero latency), throttled to once/hour, network-failure-safe, completely silent. `./setup --no-team` reverses it.
...
- **`cavestack-team-init` command.** Generates repo-level bootstrap files in two flavors: `optional` (gentle CLAUDE.md suggestion, one-time offer per developer) or `required` (CLAUDE.md enforcement + PreToolUse hook that blocks work without cavestack installed).
- **`cavestack-settings-hook` helper.** DRY utility for adding/removing hooks in Claude Code's `settings.json`. Atomic writes (.tmp + rename) prevent corruption.
- **`cavestack-session-update` script.** The SessionStart hook target. Background fork, PID-based lockfile with stale recovery, `GIT_TERMINAL_PROMPT=0` to prevent credential prompt hangs, debug log at `~/.cavestack/analytics/session-update.log`.
- **Vendoring deprecation in preamble.** Every skill now detects vendored cavestack copies in the project and offers one-time migration to team mode. "Want me to do it for you?" beats "here are 4 manual steps."

### Changed

- **Vendoring is deprecated.** README no longer recommends copying cavestack into your repo. Global install + `--team` is the way. `--local` flag still works but prints a deprecation warning.
- **Uninstall cleans up hooks.** `cavestack-uninstall` now removes the SessionStart hook from `~/.claude/settings.json`.
...
- **4 native OpenClaw skills on ClawHub.** Install with `clawhub install cavestack-openclaw-office-hours cavestack-openclaw-ceo-review cavestack-openclaw-investigate cavestack-openclaw-retro`. Pure methodology, no cavestack infrastructure. Office hours (375 lines), CEO review (193), investigate (136), retro (301).
- **AGENTS.md dispatch fix.** Three behavioral rules that stop Wintermute from telling you to open Claude Code manually. It now spawns sessions itself. Ready-to-paste section at `openclaw/agents-cavestack-section.md`.
...
You can now connect cavestack to OpenClaw as a methodology source. OpenClaw spawns Claude Code sessions natively via ACP, and cavestack provides the planning discipline and thinking frameworks that make those sessions better.

### Added

- **cavestack-lite planning discipline.** A 15-line CLAUDE.md that turns every spawned Claude Code session into a disciplined builder: read first, plan, resolve ambiguity, self-review, report. A/B tested: 2x time, meaningfully better output.
- **cavestack-full pipeline template.** For complete feature builds, chains /autoplan, implement, and /ship into one autonomous flow. Your orchestrator drops a task, gets back a PR.
...
- **4-tier dispatch routing.** Simple (no cavestack), Medium (cavestack-lite), Heavy (specific skill), Full (complete pipeline). Documented in docs/OPENCLAW.md with routing guide for OpenClaw's AGENTS.md.
- **Spawned session detection.** Set OPENCLAW_SESSION env var and cavestack auto-skips interactive prompts, focusing on task completion. Works for any orchestrator, not just OpenClaw.
...
- **docs/OPENCLAW.md.** Full architecture doc explaining how cavestack integrates with OpenClaw, the prompt-as-bridge model, and what we're NOT building (no daemon, no protocol, no Clawvisor).
...
- **Cross-review finding dedup.** When you skip a finding in one review, cavestack remembers. On the next review, if the relevant code hasn't changed, the finding stays suppressed. No more re-skipping the same intentional pattern every PR.
...
- **File and directory permissions hardened.** All ~/.cavestack/ dirs now created with mode 0o700, files with 0o600. Setup script sets umask 077. Auth tokens, chat history, and browser logs no longer world-readable.
...
- **Skill prefix self-healing.** Setup now runs `cavestack-relink` as a final consistency check after linking skills. If an interrupted setup, stale git state, or upgrade left your `name:` fields out of sync with `skill_prefix: false`, setup will auto-correct on the next run. No more `/cavestack-qa` when you wanted `/qa`.

## [0.15.6.0] - 2026-04-04 — Declarative Multi-Host Platform

Adding a new coding agent to cavestack used to mean touching 9 files and knowing the internals of `gen-skill-docs.ts`. Now it's one TypeScript config file and a re-export. Zero code changes elsewhere. Tests auto-parameterize.
...
- **Contributor `/cavestack-contrib-add-host` skill.** Guides new host config creation. Lives in `contrib/`, excluded from user installs.
...
`git pull && ./setup` now applies version migrations automatically. Previously, migrations only ran during `/cavestack-upgrade`, so users who updated via git pull never got state fixes (like the skill directory restructure from v0.15.1.0). Now `./setup` tracks the last version it ran at and applies any pending migrations on every run.

### Fixed

- **Setup runs pending migrations.** `./setup` now checks `~/.cavestack/.last-setup-version` and runs any migration scripts newer than that version. No more broken skill directories after `git pull`.
...
- **Skills now discovered as top-level names.** Setup creates real directories with SKILL.md symlinks inside instead of directory symlinks. This fixes Claude auto-prefixing skill names with `cavestack-` when using `--no-prefix` mode. `/qa` is now just `/qa`, not `/cavestack-qa`.
...
- **Predictive skill suggestion.** If your last 3 sessions on a branch follow a pattern (review, ship, review), cavestack suggests what you probably want next.
...
- **Timeline binaries.** `bin/cavestack-timeline-log` and `bin/cavestack-timeline-read` for append-only JSONL timeline storage.
- **Routing rules.** /checkpoint and /health added to the skill routing injection.

## [0.14.6.0] - 2026-03-31 — Recursive Self-Improvement

cavestack now learns from its own mistakes. Every skill session captures operational failures (CLI errors, wrong approaches, project quirks) and surfaces them in future sessions. No setup needed, just works.

### Added

- **Operational self-improvement.** When a command fails or you hit a project-specific gotcha, cavestack logs it. Next session, it remembers. "bun test needs --timeout 30000" or "login flow requires cookie import first" ... the kind of stuff that wastes 10 minutes every time you forget it.
- **Learnings summary in preamble.** When your project has 5+ learnings, cavestack shows the top 3 at the start of every session so you see them before you start working.
...
- **Contributor mode replaced.** The old contributor mode (manual opt-in, markdown reports to ~/.cavestack/contributor-logs/) never fired in 18 days of heavy use. Replaced with automatic operational learning that captures the same insights without any setup.

### Fixed

- **learnings-show E2E test slug mismatch.** The test seeded learnings at a hardcoded path but cavestack-slug computed a different path at runtime. Now computes the slug dynamically.
...
- **Skill prefix actually patches `name:` in SKILL.md (#620, #578).** `./setup --prefix` and `cavestack-relink` now patch the `name:` field in each skill's SKILL.md frontmatter to match the prefix setting. Previously, symlinks were prefixed but Claude Code read the unprefixed `name:` field and ignored the prefix entirely. Edge cases handled: `cavestack-upgrade` not double-prefixed, root `cavestack` skill never prefixed, prefix removal restores original names.
- **`gen-skill-docs` warns when prefix patches need re-applying.** After regenerating SKILL.md files, if `skill_prefix: true` is set in config, a warning reminds you to run `cavestack-relink`.
...
- **`--no-prefix` ordering bug.** `cavestack-patch-names` now runs before `link_claude_skill_dirs` so symlink names reflect the correct patched values.

### Added

- **`bin/cavestack-patch-names` shared helper.** DRY extraction of the name-patching logic used by both `setup` and `cavestack-relink`. Handles all edge cases (no frontmatter, already-prefixed, inherently-prefixed dirs) with portable `mktemp + mv` sed.
...
- **3 new diff-scope signals.** `cavestack-diff-scope` now detects SCOPE_MIGRATIONS, SCOPE_API, and SCOPE_AUTH to activate the right specialists.
...
- **Plan Mode Safe Operations.** Browse screenshots, design mockups, Codex outside voices, and writing to `~/.cavestack/` are now explicitly allowed in plan mode. Design-related skills (`/design-consultation`, `/design-shotgun`, `/design-html`, `/plan-design-review`) can generate visual artifacts during planning without fighting plan mode restrictions.
...
- **Cross-model tension format.** Outside voice disagreements now include `RECOMMENDATION` and `Completeness` scores, matching the standard AskUserQuestion format used everywhere else in cavestack.
...
- **CSS Inspector in the sidebar.** Click "Pick Element", hover over anything, click it, and the sidebar shows the full CSS rule cascade with specificity badges, source file:line, box model visualization (cavestack palette colors), and computed styles. Like Chrome DevTools, but inside the sidebar.
...
- **Resource dedup log.** Tracks which resources were shown in `~/.cavestack/projects/$SLUG/resources-shown.jsonl` so repeat users always see fresh content.
...
- **Proactive skill routing.** Skills now ask once to add routing rules to your project's CLAUDE.md. This makes Claude invoke the right skill automatically instead of answering directly. Your choice is remembered in `~/.cavestack/config.yaml`.
- **Annotated config file.** `~/.cavestack/config.yaml` now gets a documented header on first creation explaining every setting. Edit it anytime.
...
- **Telemetry off means off everywhere.** When you set telemetry to off, cavestack no longer writes local JSONL analytics files. Previously "off" only stopped remote reporting. Now nothing is written anywhere. Clean trust contract.
...
- **`cavestack-relink` no longer double-prefixes `cavestack-upgrade`.** Setting `skill_prefix=true` was creating `cavestack-cavestack-upgrade` instead of keeping the existing name. Now matches `setup` script behavior.

### Added

- **Skill discoverability.** Every skill description now contains "(cavestack)" so you can find cavestack skills by searching in Claude Code's command palette.
...
- **`bin/cavestack-relink`** re-creates skill symlinks when you change `skill_prefix` via `cavestack-config set`. No more manual `./setup` re-run needed.
- **`bin/cavestack-open-url`** cross-platform URL opener (macOS: `open`, Linux: `xdg-open`, Windows: `start`).

## [0.13.6.0] - 2026-03-29 — CaveStack Learns

Every session now makes the next one smarter. cavestack remembers patterns, pitfalls, and preferences across sessions and uses them to improve every review, plan, debug, and ship. The more you use it, the better it gets on your codebase.

### Added

- **Project learnings system.** cavestack automatically captures patterns and pitfalls it discovers during /review, /ship, /investigate, and other skills. Stored per-project at `~/.cavestack/projects/{slug}/learnings.jsonl`. Append-only, Supabase-compatible schema.
- **`/learn` skill.** Review what cavestack has learned (`/learn`), search (`/learn search auth`), prune stale entries (`/learn prune`), export to markdown (`/learn export`), or check stats (`/learn stats`). Manually add learnings with `/learn add`.
...
- **"Learning applied" callouts.** When a review finding matches a past learning, cavestack displays it: "Prior learning applied: [pattern] (confidence 8/10, from 2026-03-15)". You can see the compounding in action.
- **Cross-project discovery.** cavestack can search learnings from your other projects for matching patterns. Opt-in, with a one-time AskUserQuestion for consent. Stays local to your machine.
...
- **5-release roadmap design doc.** `docs/designs/SELF_LEARNING_V0.md` maps the path from R1 (CaveStack Learns) through R4 (/autoship, one-command full feature) to R5 (Studio).
...
cavestack now works with Factory Droid. Type `/qa` in Droid and get the same 29 skills you use in Claude Code. This makes cavestack the first skill library that works across Claude Code, Codex, and Factory Droid.
...
- **`cavestack-platform-detect` binary.** Prints a table of installed AI coding agents with versions, skill paths, and cavestack status. Useful for debugging multi-host setups.
...
- **Factory awareness across operational tooling.** skill-check dashboard, cavestack-uninstall, and setup script all know about Factory.
...
- **`cavestack-slug` no longer crashes outside git repos.** Falls back to directory name and "unknown" branch when there's no remote or HEAD. Every review skill that depends on slug detection now works in non-git contexts.
...
- **`/ship` and `/review` find your design docs.** Plan search now checks `~/.cavestack/projects/` first, where `/office-hours` writes design documents. Previously, plan validation silently skipped because it was looking in the wrong directories.
...
- **Shell config scripts validate input.** `cavestack-config` rejects regex-special keys and escapes sed patterns. `cavestack-telemetry-log` sanitizes branch/repo names in JSON output.
...
cavestack can generate real UI mockups. Not ASCII art, not text descriptions of hex codes, real visual designs you can look at, compare, pick from, and iterate on. Run `/office-hours` on a UI idea and you'll get 3 visual concepts in Chrome with a comparison board where you pick your favorite, rate the others, and tell the agent what to change.
...
- **Telemetry calls are conditional.** The `cavestack-telemetry-log` binary only runs if telemetry is enabled AND the binary exists. Local JSONL logging always works, no binary needed.
...
You can now choose how cavestack skills appear: short names (`/qa`, `/ship`, `/review`) or namespaced (`/cavestack-qa`, `/cavestack-ship`). Setup asks on first run, remembers your preference, and switching is one command.

### Added

- **Interactive prefix choice on first setup.** New installs get a prompt: short names (`/qa`, `/ship`) or namespaced (`/cavestack-qa`, `/cavestack-ship`). Short names are recommended. Your choice is saved to `~/.cavestack/config.yaml` and remembered across upgrades.
...
- **`cavestack-config` works on Linux.** Replaced BSD-only `sed -i ''` with portable `mktemp`+`mv`. Config writes now work on GNU/Linux and WSL.
- **Dead welcome message.** The "Welcome!" message on first install was never shown because `~/.cavestack/` was created earlier in setup. Fixed with a `.welcome-seen` sentinel file.
...
- **Rabbit-hole detection.** If Codex output contains signs it got distracted by skill files (`cavestack-config`, `cavestack-update-check`, `SKILL.md`, `skills/cavestack`), the /codex skill now warns and suggests a retry.
...
Six community PRs landed in one batch. Install is faster, skills no longer collide with other tools, and you can cleanly uninstall cavestack when needed.

### Added

- **Uninstall script.** `bin/cavestack-uninstall` cleanly removes cavestack from your system: stops browse daemons, removes all skill installs (Claude/Codex/Kiro), cleans up state. Supports `--force` (skip confirmation) and `--keep-state` (preserve config). (#323)
...
- **Skills namespaced with `cavestack-` prefix.** Skill symlinks are now `cavestack-review`, `cavestack-ship`, etc. instead of bare `review`, `ship`. Prevents collisions with other skill packs. Old symlinks are auto-cleaned on upgrade. Use `--no-prefix` to opt out. (#503)
...
Skill scripts now work correctly in zsh. Previously, bash code blocks in skill templates used raw glob patterns like `.github/workflows/*.yaml` and `ls ~/.cavestack/projects/$SLUG/*-design-*.md` that would throw "no matches found" errors in zsh when no files matched. Fixed 38 instances across 13 templates and 2 resolvers using two approaches: `find`-based alternatives for complex patterns, and `setopt +o nomatch` guards for simple `ls` commands.
...
- **`~/.cavestack/` and `~/.claude/` globs guarded with `setopt`.** Design doc lookups, eval result listings, test plan discovery, and retro history checks across 10 skills now prepend `setopt +o nomatch 2>/dev/null || true` (no-op in bash, disables NOMATCH in zsh).
...
When you run cavestack in Conductor with multiple workspaces open, Codex could silently review the wrong project. The `codex exec -C` flag resolved the repo root inline via `$(git rev-parse --show-toplevel)`, which evaluates in whatever cwd the background shell inherits. In multi-workspace environments, that cwd might be a different project entirely.
...
- **`$B state save/load`.** Save your browser session (cookies + open tabs) to a named file, load it back later. Files stored at `.cavestack/browse-states/{name}.json` with 0o600 permissions. V1 saves cookies + URLs only (not localStorage, which breaks on load-before-navigate). Load replaces the current session, not merge.
...
- **Headed mode with sidebar agent.** `$B connect` launches a visible Chrome window with the cavestack extension. The Side Panel shows a live activity feed of every command AND a chat interface where you type natural language instructions. A child Claude instance executes your requests in the browser ... navigate pages, click buttons, fill forms, extract data. Each task gets up to 5 minutes.
...
- **Chrome extension.** Toolbar badge (green=connected, gray=not), Side Panel with activity feed + chat + refs tab, @ref overlays on the page, and a connection pill showing which window cavestack controls. Auto-loads when you run `$B connect`.
...
- **GitHub Enterprise and self-hosted GitLab detection.** If the remote URL doesn't match `github.com` or `gitlab`, cavestack checks `gh auth status` / `glab auth status` to detect authenticated platforms — no manual config needed.
...
- **Auto-upgrade no longer breaks.** The root cavestack skill description was 7 characters from the Codex 1024-char limit. Every new skill addition pushed it closer. Moved the skill routing table from the description (bounded) to the body (unlimited), dropping from 1017 to 409 chars with 615 chars of headroom.
...
- **You can now opt out of proactive skill suggestions.** The first time you run any cavestack skill, you'll be asked whether you want cavestack to suggest skills during your workflow. If you prefer to invoke skills manually, just say no — it's saved as a global setting. You can change your mind anytime with `cavestack-config set proactive true/false`.
...
- **Telemetry sync uses `CAVESTACK_SUPABASE_URL` instead of `CAVESTACK_TELEMETRY_ENDPOINT`.** Edge functions need the base URL, not the REST API path. The old variable is removed from `config.sh`.
...
- **E2E tests verify plan review reports appear at the bottom of plans.** The `/plan-eng-review` review report is now tested end-to-end — if it stops writing `## CAVESTACK REVIEW REPORT` to the plan file, the test catches it.
...
- **Startup errors are logged to disk.** When the server fails to start, errors are written to `~/.cavestack/browse-startup-error.log` so Windows users (who lose stderr due to process detachment) can debug.
...
- **Eval results are project-scoped.** Results now live in `~/.cavestack/projects/$SLUG/evals/` instead of the global `~/.cavestack-dev/evals/`. Multi-project users no longer get eval results mixed together.
...
- **Project-scoped cavestack install.** `setup --local` installs cavestack into `.claude/skills/` in your current project instead of globally. Useful for per-project version pinning.
...
- **Browse server startup crash.** The browse server lock acquisition failed when `.cavestack/` directory didn't exist, causing every invocation to think another process held the lock. Fixed by creating the state directory before lock acquisition.
...
- **`--force` now actually forces upgrades.** `cavestack-upgrade --force` clears the snooze file, so you can upgrade immediately after snoozing.
...
- **Routing tests now work in CI.** Skills are installed at top-level `.claude/skills/` instead of nested under `.claude/skills/cavestack/` — project-level skill discovery doesn't recurse into subdirectories.
...
- `cavestack-update-check` includes a one-time migration that deletes oversized Codex SKILL.md files
...
- **cavestack skills now work in zsh without errors.** Every skill preamble used a `.pending-*` glob pattern that triggered zsh's "no matches found" error on every invocation (the common case where no pending telemetry files exist). Replaced shell glob with `find` to avoid zsh's NOMATCH behavior entirely. Thanks to @hnshah for the initial report and fix in PR #332. Fixes #313.
...
- **`cavestack-review-read` and `cavestack-review-log` no longer crash under bash.** These scripts used `source <(cavestack-slug)` which silently fails to set variables under bash with `set -euo pipefail`, causing `SLUG: unbound variable` errors. Replaced with `eval "$(cavestack-slug)"` which works correctly in both bash and zsh.
- **All SKILL.md templates updated.** Every template that instructed agents to run `source <(cavestack-slug)` now uses `eval "$(cavestack-slug)"` for cross-shell compatibility. Regenerated all SKILL.md files from templates.
- **Regression tests added.** New tests verify `eval "$(cavestack-slug)"` works under bash strict mode, and guard against `source <(.*cavestack-slug` patterns reappearing in templates or bin scripts.
...
- **OpenAI's design hard rules baked in.** 7 hard rejection criteria, 7 litmus checks, and a landing-page vs app-UI classifier from OpenAI's "Designing Delightful Frontends" framework — merged with cavestack's existing 10-item AI slop blacklist. Your design gets evaluated against the same rules OpenAI recommends for their own models.
...
- **No more duplicate skill discovery.** Codex used to find both source SKILL.md files and generated Codex skills, showing every skill twice. Setup now creates a minimal runtime root at `~/.codex/skills/cavestack` with only the assets Codex needs — no source files exposed.
- **Old direct installs auto-migrate.** If you previously cloned cavestack into `~/.codex/skills/cavestack`, setup detects this and moves it to `~/.cavestack/repos/cavestack` so skills aren't discovered from the source checkout.
- **Sidecar directory no longer linked as a skill.** The `.agents/skills/cavestack` runtime asset directory was incorrectly symlinked alongside real skills — now skipped.

### Added

- **Repo-local Codex installs.** Clone cavestack into `.agents/skills/cavestack` inside any repo and run `./setup --host codex` — skills install next to the checkout, no global `~/.codex/` needed. Generated preambles auto-detect whether to use repo-local or global paths at runtime.
...
- **`CAVESTACK_DIR` renamed to `SOURCE_CAVESTACK_DIR` / `INSTALL_CAVESTACK_DIR`** throughout the setup script for clarity about which path points to the source repo vs the install location.
...
- **Every plan file now shows review status.** When you exit plan mode, the plan file automatically gets a `CAVESTACK REVIEW REPORT` section — even if you haven't run any formal reviews yet. Previously, this section only appeared after running `/plan-eng-review`, `/plan-ceo-review`, `/plan-design-review`, or `/codex review`. Now you always know where you stand: which reviews have run, which haven't, and what to do next.
...
- **`cavestack-global-discover` — the engine behind global retro.** Standalone discovery script that finds all AI coding sessions on your machine, resolves working directories to git repos, normalizes SSH/HTTPS remotes for dedup, and outputs structured JSON. Compiled binary ships with cavestack — no `bun` runtime needed.
...
- **`cavestack-slug` hardened against shell injection.** Output sanitized to alphanumeric, dot, dash, and underscore only. All remaining `eval $(cavestack-slug)` callers migrated to `source <(...)`.
...
- **Regression detection iron rule.** When a code change modifies existing behavior, cavestack always writes a regression test — no asking, no skipping. If you changed it, you test it.
...
- **cavestack no longer crashes in repos without an `origin` remote.** The `cavestack-repo-mode` helper now gracefully handles missing remotes, bare repos, and empty git output — defaulting to `unknown` mode instead of crashing the preamble.
- **`REPO_MODE` defaults correctly when the helper emits nothing.** Previously an empty response from `cavestack-repo-mode` left `REPO_MODE` unset, causing downstream template errors.
...
- **Codex code reviews now run automatically in `/ship` and `/review`.** No more "want a second opinion?" prompt every time — Codex reviews both your code (with a pass/fail gate) and runs an adversarial challenge by default. First-time users get a one-time opt-in prompt; after that, it's hands-free. Configure with `cavestack-config set codex_reviews enabled|disabled`.
...
- **Codex-for-Codex recursion prevented.** When cavestack runs inside Codex CLI (`.agents/skills/`), the Codex review step is completely stripped — no accidental infinite loops.

## [0.9.3.0] - 2026-03-20 — Windows Support

### Fixed

- **cavestack now works on Windows 11.** Setup no longer hangs when verifying Playwright, and the browse server automatically falls back to Node.js to work around a Bun pipe-handling bug on Windows ([bun#4253](https://github.com/oven-sh/bun/issues/4253)). Just make sure Node.js is installed alongside Bun. macOS and Linux are completely unaffected.
...
- **Gemini CLI is now tested end-to-end.** Two E2E tests verify that cavestack skills work when invoked by Google's Gemini CLI (`gemini -p`). The `gemini-discover-skill` test confirms skill discovery from `.agents/skills/`, and `gemini-review-findings` runs a full code review via cavestack-review. Both parse Gemini's stream-json NDJSON output and track token usage.
...
- **Spec review metrics.** Every adversarial review logs iterations, issues found/fixed, and quality score to `~/.cavestack/analytics/spec-review.jsonl`. Over time, you can see if your design docs are getting better.

## [0.9.0.1] - 2026-03-19

### Changed

- **Telemetry opt-in now defaults to community mode.** First-time prompt asks "Help cavestack get better!" (community mode with stable device ID for trend tracking). If you decline, you get a second chance with anonymous mode (no unique ID, just a counter). Respects your choice either way.
...
**cavestack now works on any AI agent that supports the open SKILL.md standard.** Install once, use from Claude Code, OpenAI Codex CLI, Google Gemini CLI, or Cursor. All 21 skills are available in `.agents/skills/` -- just run `./setup --host codex` or `./setup --host auto` and your agent discovers them automatically.
...
- **You can now see how you use cavestack.** Run `cavestack-analytics` to see a personal usage dashboard — which skills you use most, how long they take, your success rate. All data stays local on your machine.
- **Opt-in community telemetry.** On first run, cavestack asks if you want to share anonymous usage data (skill names, duration, crash info — never code or file paths). Choose "yes" and you're part of the community pulse. Change anytime with `cavestack-config set telemetry off`.
- **Community health dashboard.** Run `cavestack-community-dashboard` to see what the cavestack community is building — most popular skills, crash clusters, version distribution. All powered by Supabase.
- **Install base tracking via update check.** When telemetry is enabled, cavestack fires a parallel ping to Supabase during update checks — giving us an install-base count without adding any latency. Respects your telemetry setting (default off). GitHub remains the primary version source.
...
- **/retro now shows your cavestack usage.** Weekly retrospectives include skill usage stats (which skills you used, how often, success rate) alongside your commit history.
- **Session-specific pending markers.** If a skill crashes mid-run, the next invocation correctly finalizes only that session — no more race conditions between concurrent cavestack sessions.
...
- **Review log no longer breaks on branch names with `/`.** Branch names like `JerkyJesse/design-system` caused review log writes to fail because Claude Code runs multi-line bash blocks as separate shell invocations, losing variables between commands. New `cavestack-review-log` and `cavestack-review-read` atomic helpers encapsulate the entire operation in a single command.
...
- **Six new skills in the docs.** README, docs/skills.md, and BROWSER.md now cover `/codex` (multi-AI second opinion), `/careful` (destructive command warnings), `/freeze` (directory-scoped edit lock), `/guard` (full safety mode), `/unfreeze`, and `/cavestack-upgrade`. The sprint skill table keeps its 15 specialists; a new "Power tools" section covers the rest.
...
- **Proactive suggestions know about all skills.** Root SKILL.md.tmpl now suggests `/codex`, `/careful`, `/freeze`, `/guard`, `/unfreeze`, and `/cavestack-upgrade` at the right workflow stages.
...
- **`/debug` renamed to `/investigate`.** Claude Code has a built-in `/debug` command that shadowed the cavestack skill. The systematic root-cause debugging workflow now lives at `/investigate`. (Closes #190)
- **Shell injection surface reduced.** cavestack-slug output is now sanitized to `[a-zA-Z0-9._-]` only, making both `eval` and `source` callers safe. (Closes #133)
...
**Also in this release:** Proactive skill suggestions — cavestack now notices what stage of development you're in and suggests the right skill. Don't like it? Say "stop suggesting" and it remembers across sessions.
...
- **You can now see which skills you use and how often.** Every skill invocation is logged locally to `~/.cavestack/analytics/skill-usage.jsonl`. Run `bun run analytics` to see your top skills, per-repo breakdown, and how often safety hooks actually catch something. Data stays on your machine.
...
- **cavestack now suggests skills at natural moments.** You don't need to know slash commands — just talk about what you're doing. Brainstorming an idea? cavestack suggests `/office-hours`. Something's broken? It suggests `/debug`. Ready to deploy? It suggests `/ship`. Every workflow skill now has proactive triggers that fire when the moment is right.
- **Lifecycle map.** cavestack's root skill description now includes a developer workflow guide mapping 12 stages (brainstorm → plan → review → code → debug → test → ship → docs → retro) to the right skill. Claude sees this in every session.
- **Opt-out with natural language.** If proactive suggestions feel too aggressive, just say "stop suggesting things" — cavestack remembers across sessions. Say "be proactive again" to re-enable.
...
- **14 of 15 skills now have full test coverage (E2E + LLM-judge + validation).** Added LLM-judge quality evals for 10 skills that were missing them: ship, retro, qa-only, plan-ceo-review, plan-eng-review, plan-design-review, design-review, design-consultation, document-release, cavestack-upgrade. Added real E2E test for cavestack-upgrade (was a `.todo`). Added design-consultation to command validation.
...
- **`cavestack-diff-scope` categorizes what changed in your branch.** Run `source <(cavestack-diff-scope main)` and get `SCOPE_FRONTEND=true/false`, `SCOPE_BACKEND`, `SCOPE_PROMPTS`, `SCOPE_TESTS`, `SCOPE_DOCS`, `SCOPE_CONFIG`. Design review uses it to skip silently on backend-only PRs. Ship pre-flight uses it to recommend design review when frontend files are touched.
...
- **`/cavestack-upgrade` now catches stale vendored copies automatically.** If your global cavestack is up to date but the vendored copy in your project is behind, `/cavestack-upgrade` detects the mismatch and syncs it. No more manually asking "did we vendor it?" — it just tells you and offers to update.
- **Upgrade sync is safer.** If `./setup` fails while syncing a vendored copy, cavestack restores the previous version from backup instead of leaving a broken install.

### For contributors

- Standalone usage section in `cavestack-upgrade/SKILL.md.tmpl` now references Steps 2 and 4.5 (DRY) instead of duplicating detection/sync bash blocks. Added one new version-comparison bash block.
- Update check fallback in standalone mode now matches the preamble pattern (global path → local path → `|| true`).

## 0.6.0 — 2026-03-17

- **100% test coverage is the key to great vibe coding.** cavestack now bootstraps test frameworks from scratch when your project doesn't have one. Detects your runtime, researches the best framework, asks you to pick, installs it, writes 3-5 real tests for your actual code, sets up CI/CD (GitHub Actions), creates TESTING.md, and adds test culture instructions to CLAUDE.md. Every Claude Code session after that writes tests naturally.
...
- **Your CEO review visions are saved, not lost.** Expansion ideas, cherry-pick decisions, and 10x visions are now persisted to `~/.cavestack/projects/{repo}/ceo-plans/` as structured design documents. Stale plans get archived automatically. If a vision is exceptional, you can promote it to `docs/designs/` in your repo for the team.

- **Smarter ship gates.** `/ship` no longer nags you about CEO and Design reviews when they're not relevant. Eng Review is the only required gate (and you can disable even that with `cavestack-config set skip_eng_review true`). CEO Review is recommended for big product changes; Design Review for UI work. The dashboard still shows all three — it just won't block you for the optional ones.
...
- **One less thing to copy-paste.** The SLUG computation (that opaque sed pipeline for computing `owner-repo` from git remote) is now a shared `bin/cavestack-slug` helper. All 14 inline copies across templates replaced with `source <(cavestack-slug)`. If the format ever changes, fix it once.
- **Screenshots are now visible during QA and browse sessions.** When cavestack takes screenshots, they now show up as clickable image elements in your output — no more invisible `/tmp/browse-screenshot.png` paths you can't see. Works in `/qa`, `/qa-only`, `/plan-design-review`, `/qa-design-review`, `/browse`, and `/cavestack`.
...
- Added `bin/cavestack-slug` helper (5-line bash) with unit tests. Outputs `SLUG=` and `BRANCH=` lines, sanitizes `/` to `-`.
...
- Added `~/.cavestack-dev/plans/` as a local plans directory for long-range vision docs (not checked in). CLAUDE.md and TODOS.md updated.
...
- **Clicking a dropdown option no longer hangs forever.** If an agent sees `@e3 [option] "Admin"` in a snapshot and runs `click @e3`, cavestack now auto-selects that option instead of hanging on an impossible Playwright click. The right thing just happens.
- **When click is the wrong tool, cavestack tells you.** Clicking an `<option>` via CSS selector used to time out with a cryptic Playwright error. Now you get: `"Use 'browse select' instead of 'click' for dropdown options."`
...
- **`/cavestack-upgrade` always checks for real.** Running `/cavestack-upgrade` directly now bypasses the cache and does a fresh check against GitHub. No more "you're already on the latest" when you're not.

### For contributors

- Split `last-update-check` cache TTL: 60 min for `UP_TO_DATE`, 720 min for `UPGRADE_AVAILABLE`.
- Added `--force` flag to `bin/cavestack-update-check` (deletes cache file before checking).
...
- **Every question is now crystal clear, every time.** You used to need 3+ sessions running before cavestack would give you full context and plain English explanations. Now every question — even in a single session — tells you the project, branch, and what's happening, explained simply enough to understand mid-context-switch. No more "sorry, explain it to me more simply."
- **Branch name is always correct.** cavestack now detects your current branch at runtime instead of relying on the snapshot from when the conversation started. Switch branches mid-session? cavestack keeps up.
...
- cavestack-upgrade template: explicit cross-step prose for variable references between bash blocks.

## 0.4.1 — 2026-03-16

- **cavestack now notices when it screws up.** Turn on contributor mode (`cavestack-config set cavestack_contributor true`) and cavestack automatically writes up what went wrong — what you were doing, what broke, repro steps. Next time something annoys you, the bug report is already written. Fork cavestack and fix it yourself.
- **Juggling multiple sessions? cavestack keeps up.** When you have 3+ cavestack windows open, every question now tells you which project, which branch, and what you were working on. No more staring at a question thinking "wait, which window is this?"
- **Every question now comes with a recommendation.** Instead of dumping options on you and making you think, cavestack tells you what it would pick and why. Same clear format across every skill.
...
### Added
- **`bin/cavestack-config` CLI** — simple get/set/list interface for `~/.cavestack/config.yaml`. Used by update-check and upgrade skill for persistent settings (auto_upgrade, update_check).
...
- **Auto-upgrade mode** — set `auto_upgrade: true` in config or `CAVESTACK_AUTO_UPGRADE=1` env var to skip the upgrade prompt and update automatically.
- **4-option upgrade prompt** — "Yes, upgrade now", "Always keep me up to date", "Not now" (snooze), "Never ask again" (disable).
- **Vendored copy sync** — `/cavestack-upgrade` now detects and updates local vendored copies in the current project after upgrading the primary install.
- 25 new tests: 11 for cavestack-config CLI, 14 for snooze/config paths in update-check.

### Changed
- README upgrade/troubleshooting sections simplified to reference `/cavestack-upgrade` instead of long paste commands.
...
- **Greptile escalation detection** — explicit algorithm to detect prior CaveStack replies on comment threads and auto-escalate to Tier 2.
...
### Fixed
- **`.gitignore` append failures silently swallowed** — `ensureStateDir()` bare `catch {}` replaced with ENOENT-only silence; non-ENOENT errors (EACCES, ENOSPC) logged to `.cavestack/browse-server.log`.
...
### Added
- **E2E observability** — heartbeat file (`~/.cavestack-dev/e2e-live.json`), per-run log directory (`~/.cavestack-dev/e2e-runs/{runId}/`), progress.log, per-test NDJSON transcripts, persistent failure transcripts. All I/O non-fatal.
...
- **Eval persistence** — results saved to `~/.cavestack-dev/evals/` with auto-comparison against previous run.
...
- Stale servers from global install no longer shadow local changes — removed legacy `~/.claude/skills/cavestack` fallback from `resolveServerScript()`
- Crash log path references updated from `/tmp/` to `.cavestack/`
...
- **Project-local browse state** — state file, logs, and all server state now live in `.cavestack/` inside the project root (detected via `git rev-parse --show-toplevel`). No more `/tmp` state files.
...
### Changed
- State file location: `.cavestack/browse.json` (was `/tmp/browse-server.json`)
- Log files location: `.cavestack/browse-{console,network,dialog}.log` (was `/tmp/browse-*.log`)
...
- Legacy fallback to `~/.claude/skills/cavestack/browse/src/server.ts`
- `DEVELOPING_CAVESTACK.md` (renamed to CONTRIBUTING.md)
