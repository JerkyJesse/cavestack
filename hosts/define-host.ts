 ...
 * paths derived from the host name (`.{name}/skills/cavestack`), allowlist
...
/** Claude-only skills. External hosts skip these. */
export const CAVEMAN_SKILLS: string[] = [
  'caveman',
  'caveman-commit',
  'caveman-help',
  'caveman-review',
 ...
 * (`~/.claude/skills/cavestack` → `~/{globalRoot}`, `.claude/skills/cavestack` →
 * localSkillRoot, `.claude/skills` → `{hostSubdir}/skills`). Hosts whose
 * rewrites aren't mechanically derivable (codex, factory use $CAVESTACK_ROOT and
...
export function defineHost<const N extends string>(overrides: HostOverrides<N>): HostConfig & { name: N } {
  const {
    ...
    cliAliases = [],
    defaultModel = 'claude',
    globalRoot = `.${name}/skills/cavestack`,
    localSkillRoot = `.${name}/skills/cavestack`,
    hostSubdir = `.${name}`,
    usesEnvVars = true,  // false only for Claude (literal ~ paths, no $CAVESTACK_ROOT)
    frontmatter = {
      mode: 'allowlist',
    ...
    generation = {
      ...
      skipSkills: ['codex', ...CAVEMAN_SKILLS],
    },
    pathRewrites,
    extraPathRewrites,
    toolRewrites,
  ...
  const resolvedPathRewrites: PathRewrite[] = pathRewrites ?? [
    { from: '~/.claude/skills/cavestack', to: `~/${globalRoot}` },
    { from: '.claude/skills/cavestack', to: localSkillRoot },
