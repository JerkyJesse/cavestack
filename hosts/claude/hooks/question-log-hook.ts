 ...
 * tool_input/tool_response, and writes them via cavestack-question-log so the
 ...
 *           "hooks": [
 *             { "type": "command",
 *               "command": "$CLAUDE_PROJECT_DIR/.claude/skills/cavestack/hosts/claude/hooks/question-log-hook",
 *               "timeout": 5 }
 *           ]
 *         }
 *       ]
 ...
 *     Errors land in ~/.cavestack/hook-errors.log for postmortem.
 *   - Spawns cavestack-question-log as a subprocess; that bin handles
 *     validation, dedup (source+tool_use_id), async derive.
 *   - Marker-first question_id (`<cavestack-qid:foo-bar>`), hash fallback
...
const MARKER_RE = /<cavestack-qid:([a-z0-9-]{1,64})>/i;
const RECOMMENDED_LABEL_RE = /\(recommended\)\s*$/i;

function logHookError(msg: string): void {
  try {
    const stateRoot =
      process.env.CAVESTACK_STATE_ROOT ||
      process.env.CAVESTACK_HOME ||
      path.join(os.homedir(), '.cavestack');
 ...
 * <cavestack-qid:...> wrapper) when present, else a hash-based hook- id.
...
function detectSkill(cwd: string | undefined): string {
  ...
  // skill marker (<cavestack-skill:NAME>) embedded in question text per
...
function spawnLog(payload: Record<string, unknown>, cwd?: string): void {
  const res = runBin('cavestack-question-log', [JSON.stringify(payload)], {
    encoding: 'utf-8',
    stdio: ['ignore', 'pipe', 'pipe'],
    timeout: 3000,
    // Run from the originating tool call's cwd so cavestack-slug resolves to
    // the project the user is actually in, not the hook script's location.
    cwd: cwd && fs.existsSync(cwd) ? cwd : undefined,
  });
  if (res.status !== 0) {
    logHookError(`cavestack-question-log exited ${res.status}: ${res.stderr || res.stdout}`);
  }
}
