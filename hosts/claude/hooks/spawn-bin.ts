/**
 * Windows-safe resolution + spawn for cavestack's bash bins. Two Windows-only
 ...
 * 2. `bin/cavestack-*` are extensionless bash scripts. Windows has no shebang
...
function bashExe(): string {
  return process.env.CAVESTACK_BASH || (fs.existsSync(GIT_BASH) ? GIT_BASH : 'bash');
}

/** cavestack install root. This file lives at hosts/claude/hooks/. */
