 ...
 *   1. Extract question_id via marker (<cavestack-qid:foo-bar>). If no marker,
 ...
 *
 * Always exits 0. Hook errors land in ~/.cavestack/hook-errors.log.
 ...
 * invalidates by being missing — cavestack-distill-apply doesn't touch the
...
function slugFromCwd(cwd: string | undefined): string {
  // Mirror cavestack-slug's basename fallback. The full slug resolver shells out
...
function logAutoDecided(
  ...
  try {
    ...
    runBin('cavestack-question-log', [JSON.stringify(payload)], {
      encoding: 'utf-8',
      stdio: ['ignore', 'pipe', 'pipe'],
      timeout: 3000,
      // cwd of the originating tool call so cavestack-slug resolves to the
...
async function main(): Promise<void> {
  ...
  if (isConductor()) {
    const conductorReason =
      ...
      'typed confirmation and do NOT proceed on a vague reply. Capture the decision with cavestack-question-log ' +
