...
### TODOS.md updates
Present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `~/.claude/skills/cavestack/review/TODOS-format.md`.
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+cavestack: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
~/.claude/skills/cavestack/bin/cavestack-decision-log '{"decision":"CEO review (MODE): SCOPE_SUMMARY","rationale":"VERDICT","scope":"branch","source":"skill","confidence":8}' 2>/dev/null || true
```

The second command records the accepted scope as a durable cross-session decision so the next session sees what was settled (and why) without re-litigating it. It writes to `~/.cavestack/` (same pattern as review-log), is non-interactive, and is best-effort (`|| true` — never blocks the review). Substitute `SCOPE_SUMMARY` (e.g. "accepted 4 of 6 proposals" for expansion, or "held scope" / "cut 3 items" for HOLD/REDUCTION) and `VERDICT` (the one-line verdict from the summary).
...
eval "$(~/.claude/skills/cavestack/bin/cavestack-slug 2>/dev/null)" 2>/dev/null || true
  ~/.claude/skills/cavestack/bin/cavestack-brain-cache invalidate product --project "$SLUG" 2>/dev/null || true
  ~/.claude/skills/cavestack/bin/cavestack-brain-cache invalidate goals --project "$SLUG" 2>/dev/null || true
  ~/.claude/skills/cavestack/bin/cavestack-brain-cache invalidate competitive-intel --project "$SLUG" 2>/dev/null || true
```
