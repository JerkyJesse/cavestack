#!/usr/bin/env bun
/**
 * cavestack-memory-ingest — V1 memory ingest helper.
 *
 * Walks coding-agent transcript sources + ~/.cavestack/ curated artifacts and writes
 ...
 *
 * Usage:
 *   cavestack-memory-ingest --probe                 # count what would ingest, no writes
 *   cavestack-memory-ingest --incremental [--quiet] # default; mtime fast-path; cheap
 *   cavestack-memory-ingest --bulk [--all-history]  # first-run; full walk
 *   cavestack-memory-ingest --bulk --benchmark      # time the bulk pass + report
 *   cavestack-memory-ingest --include-unattributed  # also ingest sessions with no git remote
 *
 * Sources walked:
 ...
 *   ~/.cavestack/projects/<slug>/learnings.jsonl       — typed: learning
 *   ~/.cavestack/projects/<slug>/timeline.jsonl        — typed: timeline
 *   ~/.cavestack/projects/<slug>/ceo-plans/*.md        — typed: ceo-plan
 *   ~/.cavestack/projects/<slug>/*-design-*.md         — typed: design-doc
 *   ~/.cavestack/analytics/eureka.jsonl                — typed: eureka
 *   ~/.cavestack/builder-profile.jsonl                 — typed: builder-profile-entry
 *
 * State: ~/.cavestack/.transcript-ingest-state.json (LOCAL per ED1, never synced).
 * Secret scanning: gitleaks via lib/cavestack-memory-helpers#secretScanFile (D19).
...
import {
  ...
  detectEngineTier,
  withErrorContext,
} from "../lib/cavestack-memory-helpers";
...
interface CliArgs {
   ...
   * default — the cross-machine boundary (cavestack-brain-sync, git push)
...
interface BulkResult {
   ...
   * ~/.cavestack/gbrain-repo-policy.json is `read-only` (search allowed, page
...
const HOME = homedir();
const CAVESTACK_HOME = process.env.CAVESTACK_HOME || join(HOME, ".cavestack");
const STATE_PATH = join(CAVESTACK_HOME, ".transcript-ingest-state.json");
...
function printUsage(): void {
  console.error(`Usage: cavestack-memory-ingest [--probe|--incremental|--bulk] [options]
...
Options:
  ...
  --scan-secrets       Opt-in per-file gitleaks scan during prepare. Off by
                       default; cavestack-brain-sync already gates the git-push
...
function parseArgs(): CliArgs {
  ...
  let noWrite = process.env.CAVESTACK_MEMORY_INGEST_NO_WRITE === "1";
  let scanSecrets = process.env.CAVESTACK_MEMORY_INGEST_SCAN_SECRETS === "1";
...
function loadState(): IngestState {
  if (!existsSync(STATE_PATH)) {
    return {
      schema_version: 1,
      last_writer: "cavestack-memory-ingest",
      sessions: {},
    };
  }
  try {
    ...
    if (parsed.schema_version !== 1) {
      ...
      return { schema_version: 1, last_writer: "cavestack-memory-ingest", sessions: {} };
    }
    return parsed;
  } catch (err) {
...
function saveState(state: IngestState): void {
  ...
  // in cavestack-gbrain-sync.ts:saveSyncState.
  try {
...
function* walkCavestackArtifacts(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  const projectsRoot = join(CAVESTACK_HOME, "projects");

  // Eureka log: ~/.cavestack/analytics/eureka.jsonl
  const eurekaLog = join(CAVESTACK_HOME, "analytics", "eureka.jsonl");
  if (existsSync(eurekaLog) && ctx.args.sources.has("eureka")) {
    yield { path: eurekaLog, type: "eureka" };
  }

  // Builder profile: ~/.cavestack/builder-profile.jsonl
  const builderProfile = join(CAVESTACK_HOME, "builder-profile.jsonl");
...
function* walkAllSources(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  ...
  yield* walkCavestackArtifacts(ctx);
}
...
function buildArtifactPage(path: string, type: MemoryType): PageRecord {
  ...
  // Extract repo slug from path: ~/.cavestack/projects/<slug>/...
  let slug_repo = "_unattributed";
  const m = path.match(/\/\.cavestack\/projects\/([^/]+)\//);
  if (m) slug_repo = m[1];
 ...
 *
 *   The actual cross-machine exfiltration boundary is `cavestack-brain-sync`,
 *   which runs a regex-based secret scanner on the staged diff before
 *   `git commit` (see bin/cavestack-brain-sync:78-110: AWS keys, GitHub
...
function preparePages(
  ...
  for (const { path, type } of walkAllSources(ctx)) {
    ...
    // because cavestack-brain-sync already gates the cross-machine boundary
  ...
  // code-import gate honors (bin/cavestack-gbrain-sync.ts). One batch spawn for
  ...
  if (policyStoreExists) {
    ...
    if (remotes.length > 0) {
      ...
      // cavestack-gbrain-sync's code-import gate: never bypass a set policy.
      ...
      if (broken) {
        ...
        policyError =
          ...
          `bypassing a possibly-set deny policy. Inspect with: cavestack-gbrain-repo-policy list; ` +
...
/**
 * Make a per-run staging directory at ~/.cavestack/.staging-ingest-<pid>-<ts>/
...
function makeStagingDir(): string {
  const dir = join(CAVESTACK_HOME, `.staging-ingest-${process.pid}-${Date.now()}`);
 ...
 *
 * Instead of staging to ~/.cavestack/.staging-ingest-<pid>-<ts>/ and cleaning up
 * after `gbrain import`, remote-http users get a stable path that survives.
 * cavestack-brain-sync's allowlist pushes ~/.cavestack/transcripts/** to the
 * artifacts repo; the brain admin's pull job indexes them into the remote
 * brain. Local PGLite (if present) stays code-only.
 *
 * Path: ~/.cavestack/transcripts/<run-id>/  (run-id pid+ts so concurrent passes
 ...
 *
 * Reads ~/.claude.json directly (same fallback chain as cavestack-gbrain-detect
...
function cleanupStagingDir(dir: string): void {
  ...
  const verdict = checkOwnedStagingDir(dir, CAVESTACK_HOME);
  if (!verdict.ok) {
    console.error(
  ...
  const forward = (signal: NodeJS.Signals) => () => {
    ...
    if (_activeStagingDir) {
      if (stagingDirIsCheckpointed(_activeStagingDir)) {
        ...
        // (in cavestack-gbrain-sync.ts) will see the checkpoint + dir and
 ...
 * configurable via CAVESTACK_INGEST_TIMEOUT_MS (default 30 min, 1min–24h) so large
...
export function resolveImportTimeoutMs(
  raw: string | undefined = process.env.CAVESTACK_INGEST_TIMEOUT_MS,
): number {
  if (raw === undefined || raw === "") return DEFAULT_IMPORT_TIMEOUT_MS;
  const n = Number.parseInt(raw, 10);
  if (!Number.isFinite(n) || Number.isNaN(n) || n < 60_000 || n > 86_400_000) {
    console.error(
      `[memory-ingest] CAVESTACK_INGEST_TIMEOUT_MS="${raw}" invalid (need 60000–86400000ms); using ${DEFAULT_IMPORT_TIMEOUT_MS}ms`,
    );
  ...
  if (failedOnUnknownIncludeGitignored(first.status, first.stderr)) {
    ...
    console.error(
      "[memory-ingest] installed gbrain does not support --include-gitignored — " +
        ...
        "(cavestack-gbrain-install) so staged pages inside gitignored dirs are visible.",
    );
...
function runGbrainImportOnce(
  ...
  return new Promise((resolve) => {
    ...
    // parent cavestack-gbrain-sync seeding the bun grandchild's env).
    // --include-gitignored is load-bearing, not a convenience. Pages are
    // staged into ~/.cavestack/.staging-ingest-<pid>-<ts>/, and ~/.cavestack is a
    ...
    // through a symlink (macOS /var -> /private/var, symlinked $CAVESTACK_HOME)
...
async function ingestPass(args: CliArgs): Promise<BulkResult> {
  ...
  if (args.noWrite) {
    ...
    state.last_writer = "cavestack-memory-ingest";
    saveState(state);
    return {
      written,
      skipped_secret: prep.skippedSecret,
  ...
  // PERSISTENT dir under ~/.cavestack/transcripts/ and SKIP `gbrain import`
  // entirely. cavestack-brain-sync push will pick the dir up via its allowlist
  ...
  // CAVESTACK_INGEST_RESUME_DIR (because gbrain's import-checkpoint.json points
  ...
  const resumeDir = process.env.CAVESTACK_INGEST_RESUME_DIR;
  // #1802 second entry point: this binary is runnable directly, so it must not
  // trust CAVESTACK_INGEST_RESUME_DIR just because it exists — a stale/poisoned env
  ...
  const resuming = !remoteHttpMode
    ...
    && checkOwnedStagingDir(resumeDir, CAVESTACK_HOME).ok;
  if (!remoteHttpMode && resumeDir && resumeDir.length > 0 && !resuming) {
    console.error(
      `[memory-ingest] ignoring CAVESTACK_INGEST_RESUME_DIR="${resumeDir}" — not a proven staging dir (#1802); staging fresh.`,
    );
  }
  ...
  try {
    ...
    // staged markdown lives under ~/.cavestack/transcripts/<run-id>/ and the
    // next cavestack-brain-sync push will move it to the artifacts repo. From
    ...
    if (remoteHttpMode) {
      ...
      state.last_writer = "cavestack-memory-ingest (remote-http mode)";
      saveState(state);
      if (!args.quiet) {
        console.error(
    ...
    // happens in cavestack-brain-sync, which writes its own receipt at the push.
    try {
      writeReceipt({
        sink: "memory-ingest",
      ...
      if (importResult.timedOut) {
        ...
        const msg = checkpointed
          ? `gbrain import timed out after ${mins}min; checkpoint preserved — re-run ` +
            `/sync-gbrain to resume (raise CAVESTACK_INGEST_TIMEOUT_MS for big brains)`
          : `gbrain import timed out after ${mins}min before writing a checkpoint; ` +
            `re-run /sync-gbrain to restage (raise CAVESTACK_INGEST_TIMEOUT_MS for big brains)`;
    ...
    // `cavestack-artifacts-init` writes `.gitignore = "*"` into $CAVESTACK_HOME.
    // makeStagingDir() stages under $CAVESTACK_HOME, so on any machine that has
    ...
    // dir (makePersistentTranscriptDir, under ~/.cavestack/transcripts/) that
    // cavestack-brain-sync push must pick up — it is NOT a `.staging-ingest-*` dir
...
function printProbeReport(r: ProbeReport, json: boolean): void {
  ...
  if (r.skipped_policy_deny > 0) {
    console.log(`Skipped (policy deny):  ${r.skipped_policy_deny}  (remote tier is deny; change with: cavestack-gbrain-repo-policy set <remote> read-write)`);
  }
...
function printBulkResult(r: BulkResult, args: CliArgs): void {
  ...
  if (r.skipped_policy_deny > 0) {
    console.log(`  skipped (policy deny):      ${r.skipped_policy_deny}  (change with: cavestack-gbrain-repo-policy set <remote> read-write)`);
  }
...
// The orchestrator runs it as `bun cavestack-memory-ingest.ts ...`, where
// import.meta.main is true, so the CLI path is unaffected.
if (import.meta.main) {
  main().catch((err) => {
    console.error(`cavestack-memory-ingest fatal: ${err instanceof Error ? err.message : String(err)}`);
    process.exit(1);
  });
}
