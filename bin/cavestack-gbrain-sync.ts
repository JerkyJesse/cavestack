#!/usr/bin/env bun
/**
 * cavestack-gbrain-sync — V1 unified sync verb.
 *
 ...
 *   2. Transcripts + curated memory → cavestack-memory-ingest (typed put_page)
 *   3. Curated artifacts to git    → cavestack-brain-sync (existing pipeline)
 *
 * Modes:
 ...
 *   - Lock file at ~/.cavestack/.sync-gbrain.lock (PID + start ts).
 ...
 * than building a cavestack-side daemon.
 */
...
import { detectEngineTier, withErrorContext, canonicalizeRemote } from "../lib/cavestack-memory-helpers";
...
const STATE_PATH = join(CAVESTACK_HOME, ".gbrain-sync-state.json");
const LOCK_PATH = join(CAVESTACK_HOME, ".sync-gbrain.lock");
 ...
 * CAVESTACK_HOME at runtime — same pattern as cacheFilePath() in
 * lib/gbrain-local-status.ts. Avoids the ESM static-import hoist trap where a
 * module-load-time const captures the real ~/.cavestack before a test can redirect.
 */
export function dreamMarkerPath(): string {
  return join(process.env.CAVESTACK_HOME || join(homedir(), ".cavestack"), ".dream-in-progress");
}

// Default 35-minute timeout for code-walk + memory-ingest stages. Override via
// CAVESTACK_SYNC_CODE_TIMEOUT_MS / CAVESTACK_SYNC_MEMORY_TIMEOUT_MS. Bounds-checked
 ...
 */
export function decideResume(cavestackHome: string = CAVESTACK_HOME): ResumeVerdict {
  const cp = readGbrainCheckpoint();
  if (!cp || !cp.dir) return { kind: "no-checkpoint" };
  const stagingDir = cp.dir;
  // #1802: only resume into a path we can PROVE is a cavestack-minted staging dir.
  ...
  const verdict = checkOwnedStagingDir(stagingDir, cavestackHome);
  if (!verdict.ok) {
...
function printUsage(): void {
  console.error(`Usage: cavestack-gbrain-sync [--incremental|--full|--dry-run] [options]
...
Options:
  ...
  --no-memory          Skip the cavestack-memory-ingest stage (transcripts + artifacts).
  --no-brain-sync      Skip the cavestack-brain-sync git pipeline stage.
  ...
  --dream              Force the source-scoped dream cycle that builds this
                       ...
                       timeout 45min, override CAVESTACK_SYNC_DREAM_TIMEOUT_MS.
 ...
 *
 * Pattern: `cavestack-code-<slug>-<hostpathhash8>` where slug comes from origin
 ...
 *
 * `CAVESTACK_HOSTNAME` env override is honored for deterministic tests; in
...
function deriveCodeSourceId(repoPath: string): string {
  const host = process.env.CAVESTACK_HOSTNAME || hostname();
  ...
  if (remote) {
    ...
    const fullId = constrainSourceId("cavestack-code", `${slugSource}-${hostPathHash}`);
    ...
    return constrainSourceId("cavestack-code", `${repoOnly}-${hostPathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("cavestack-code", `${base}-${hostPathHash}`);
}
 ...
 *
 * Earlier /sync-gbrain versions registered `cavestack-code-<slug>` (no pathhash
...
function deriveLegacyCodeSourceId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("cavestack-code", slugSource);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("cavestack-code", base);
}
 ...
 * repo path: `cavestack-code-<slug>-<sha1(path).slice(0,8)>`. After #1468 the
...
export function derivePathOnlyHashLegacyId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("cavestack-code", `${slugSource}-${pathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("cavestack-code", `${base}-${pathHash}`);
}
...
function acquireLock(): boolean {
  mkdirSync(CAVESTACK_HOME, { recursive: true });
...
/**
 * Acquire the dream marker (`~/.cavestack/.dream-in-progress`). Returns false when
...
function skipStageForLocalStatus(
  ...
  const reasons: Record<Exclude<LocalEngineStatus, "ok">, string> = {
    ...
    "timeout":
      "engine probe timed out; raise CAVESTACK_GBRAIN_PROBE_TIMEOUT_MS if your pooler is slow",
    "thin-client":
      "thin client (remote-HTTP MCP brain, no local engine by design, #2051); " +
...
function warnProbeTimeout(stage: "code" | "memory" | "dream"): void {
  process.stderr.write(
    `[cavestack-gbrain-sync] ${stage}: engine probe timed out — proceeding anyway; ` +
      `raise CAVESTACK_GBRAIN_PROBE_TIMEOUT_MS if your pooler is slow\n`,
  );
}


/**
 * Per-repo trust tier from ~/.cavestack/gbrain-repo-policy.json, read through
 * the bin/cavestack-gbrain-repo-policy CLI (which owns URL normalization and
...
export function repoPolicyTier(url: string | null): "read-write" | "read-only" | "deny" | "unset" | "error" {
  ...
  if (res.error === "spawn-failed") {
    process.stderr.write(
      "[cavestack-gbrain-sync] the repo-policy helper could not be spawned (bash missing from PATH?) — " +
...
async function runCodeImport(args: CliArgs): Promise<StageResult> {
  ...
  if (tier === "read-only") {
    ...
    return {
      ...
      ok: true,
      duration_ms: Date.now() - t0,
      summary: `skipped — repo policy is read-only for ${policyUrl} (code ingest writes pages). Change with: cavestack-gbrain-repo-policy set ${policyUrl} read-write`,
      detail: { source_id: sourceId, source_path: root, status: "skipped-policy-read-only" },
    };
  }
  if (tier === "deny" || tier === "error") {
    const why = tier === "deny"
      ? `repo policy is deny for ${policyUrl} — no gbrain ingest for this repo. Change with: cavestack-gbrain-repo-policy set ${policyUrl} read-write`
      : "repo policy store exists but could not be read (cavestack-gbrain-repo-policy get failed) — refusing ingest rather than bypassing a set policy";
    return {
      name: "code",
      ran: true,
      ok: false,
  ...
  // Earlier /sync-gbrain versions registered `cavestack-code-<slug>` (no path
  ...
  const codeTimeoutMs = resolveStageTimeoutMs(
    process.env.CAVESTACK_SYNC_CODE_TIMEOUT_MS,
    "CAVESTACK_SYNC_CODE_TIMEOUT_MS",
  );
  ...
  // cavestack's own .gitignore. Without the consumer-side entry, the pin gets
...
function runMemoryIngest(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "memory", ran: false, ok: true, duration_ms: 0, summary: "would: cavestack-memory-ingest --probe" };
  }

  // Split-engine pre-flight (per plan D12). cavestack-memory-ingest shells out
  ...
  if (resume.kind === "resume") {
    ...
    childEnv.CAVESTACK_INGEST_RESUME_DIR = resume.stagingDir;
  ...
  const ingestPath = join(import.meta.dir, "cavestack-memory-ingest.ts");
  ...
  // .env.local footgun affects cavestack-memory-ingest.ts too, not just the
  ...
  const memoryTimeoutMs = resolveStageTimeoutMs(
    process.env.CAVESTACK_SYNC_MEMORY_TIMEOUT_MS,
    "CAVESTACK_SYNC_MEMORY_TIMEOUT_MS",
  );
...
function runBrainSyncPush(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "would: cavestack-brain-sync --discover-new --once" };
  }

  const brainSyncPath = join(import.meta.dir, "cavestack-brain-sync");
  if (!existsSync(brainSyncPath)) {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "skipped (cavestack-brain-sync not installed)" };
  }

  // cavestack-brain-sync is a bash shebang script, so it needs an INTERPRETER, not
  ...
  if (!discover || !once) {
    return {
      ...
      ok: true,
      duration_ms: Date.now() - t0,
      summary: "skipped (no bash found; set CAVESTACK_BASH to your Git bash.exe)",
    };
  }
  ...
  return {
    ...
    ok: result.status === 0,
    duration_ms: Date.now() - t0,
    summary: result.status === 0 ? "curated artifacts pushed" : `cavestack-brain-sync exited ${result.status}`,
  };
}
...
export async function runDream(args: CliArgs): Promise<StageResult> {
  ...
  try {
    const dreamTimeoutMs = resolveStageTimeoutMs(
      process.env.CAVESTACK_SYNC_DREAM_TIMEOUT_MS,
      "CAVESTACK_SYNC_DREAM_TIMEOUT_MS",
...
function loadSyncState(): SyncState {
  if (!existsSync(STATE_PATH)) {
    return { schema_version: 1, last_writer: "cavestack-gbrain-sync" };
  }
  try {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!args.noCode) {
      stages.push(await withErrorContext("sync:code", () => runCodeImport(args), "cavestack-gbrain-sync"));
    }
    if (!args.noMemory) {
      stages.push(await withErrorContext("sync:memory", () => runMemoryIngest(args), "cavestack-gbrain-sync"));
    }
    if (!args.noBrainSync) {
      stages.push(await withErrorContext("sync:brain-sync", () => runBrainSyncPush(args), "cavestack-gbrain-sync"));
    }
  ...
  if (!args.quiet || args.mode === "dry-run") {
    ...
    console.log(`\ncavestack-gbrain-sync (${args.mode}):`);
...
if (import.meta.main) {
  main().catch((err) => {
    console.error(`cavestack-gbrain-sync fatal: ${err instanceof Error ? err.message : String(err)}`);
    releaseLock();
    process.exit(1);
  });
}
