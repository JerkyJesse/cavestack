# cavestack-egress-lib.sh — shared egress-receipt helpers for bash sinks.
#
# This file is NOT executable; source it:
#
#   . "$(dirname "$0")/cavestack-egress-lib.sh"
#
...
#     the receipt id is exported as _CAVESTACK_EGRESS_LAST_RECEIPT so callers
...
#
# NO EXIT traps in this file, ever: callers (cavestack-telemetry-sync) own
...
case "${BASH_SOURCE[0]}" in
  */*) _cavestack_egress_lib_dir="$(cd "${BASH_SOURCE[0]%/*}" && pwd)" ;;
  *) _cavestack_egress_lib_dir="$(pwd)" ;;
esac

_cavestack_egress_home() {
  if [ -n "${CAVESTACK_HOME:-}" ]; then
    printf '%s' "$CAVESTACK_HOME"
  elif [ -n "${CAVESTACK_STATE_DIR:-}" ]; then
    printf '%s' "$CAVESTACK_STATE_DIR"
  else
    printf '%s' "$HOME/.cavestack"
  fi
}

# Problem + cause + fix, in plain language (DX contract for every
# fail-closed refusal). $1 = sink, $2 = cause text from the receipt bridge.
_cavestack_egress_refusal() {
  local home
  home="$(_cavestack_egress_home)"
  echo "cavestack: $1 NOT sent — the egress receipt could not be written (${2:-unknown cause}). Fix: chmod -R u+w $home/security (or check CAVESTACK_HOME). What this is: cavestack records everything it ATTEMPTS to send off-machine; see cavestack-egress." >&2
}
...
_cavestack_egress_run() {
  local policy="$1" sink="$2" host="$3" class="$4" consent="$5" payload="$6"
  shift 6
  local bin="$_cavestack_egress_lib_dir/cavestack-egress-receipt"

  local payload_flag=(--no-payload)
  [ "$payload" != "--no-payload" ] && payload_flag=(--payload-file "$payload")

  local receipt_id="" receipt_err="" err_file=""
  err_file="$(mkdir -p "${TMPDIR:-${TMP:-$HOME/.cavestack/tmp}}" && mktemp "${TMPDIR:-${TMP:-$HOME/.cavestack/tmp}}/cavestack-egress-err-XXXXXX" 2>/dev/null)" || err_file=""
  if [ -n "$err_file" ]; then
  ...
  _CAVESTACK_EGRESS_LAST_RECEIPT="$receipt_id"

  if [ -z "$receipt_id" ]; then
    if [ "$policy" = "closed" ]; then
      _cavestack_egress_refusal "$sink" "$receipt_err"
      [ "$payload" != "--no-payload" ] && rm -f "$payload"
      return 3
    fi
    echo "cavestack: egress receipt could not be written for $sink (${receipt_err:-unknown cause}) — sending anyway (fail-open). cavestack normally records everything it ATTEMPTS to send off-machine; see cavestack-egress." >&2
  fi
...
_receipted_git() {
  ...
  shift 5
  _cavestack_egress_run "$policy" "$sink" "$host" "$class" "$consent" --no-payload "$@"
}
