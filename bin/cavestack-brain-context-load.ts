#!/usr/bin/env bun
/**
 * cavestack-brain-context-load — V1 retrieval surface (Lane C).
 *
 * Called from the cavestack preamble at every skill start. Reads the active skill's
 ...
 *
 * Usage:
 *   cavestack-brain-context-load --skill office-hours --repo garrytan-cavestack
 *   cavestack-brain-context-load --skill-file ./SKILL.md --repo X --user Y
 *   cavestack-brain-context-load --window 14d --explain
 *   cavestack-brain-context-load --quiet
 */
...
import { parseSkillManifest, type GbrainManifest, type GbrainManifestQuery, withErrorContext } from "../lib/cavestack-memory-helpers";
...
const MCP_TIMEOUT_MS = Math.max(1, parseInt(process.env.CAVESTACK_BRAIN_TIMEOUT_MS || "", 10) || 500);
const PAGE_SIZE_CAP = 10 * 1024; // 10KB per query result before truncation

// ── CLI ────────────────────────────────────────────────────────────────────

function printUsage(): void {
  console.error(`Usage: cavestack-brain-context-load [options]
...
function resolveSkillFile(args: CliArgs): string | null {
  ...
  // Look in common cavestack skill locations
  const candidates = [
    join(HOME, ".claude", "skills", args.skill, "SKILL.md"),
    join(HOME, ".claude", "skills", "cavestack", args.skill, "SKILL.md"),
...
main().catch((err) => {
  console.error(`cavestack-brain-context-load fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
