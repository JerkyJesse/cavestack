...
preamble-tier: 2
version: 1.0.0
description: Codify the most recent successful /scrape flow into a permanent browser-skill on disk. (cavestack)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"skillify","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cavestack/bin/cavestack-timeline-log '{"skill":"skillify","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cavestack/bin/cavestack-question-log '{"skill":"skillify","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
A) Keep "<proposed-name>" at global tier — ~/.cavestack/browser-skills/<proposed-name>/  (recommended)
B) Keep "<proposed-name>" but at project tier — <project>/.cavestack/browser-skills/<proposed-name>/
...
The canonical SDK lives at `<cavestack-install>/browse/src/browse-client.ts`.
The bundled-skill loader walks the install tree to find it; mirror that.

Resolve the cavestack install dir. Two reliable signals (in order):

1. The bundled `hackernews-frontpage` skill — look at its tier path from
   `$B skill list` (the `bundled` row). The skill dir is
   `<cavestack-install>/browser-skills/hackernews-frontpage/`, so the install
   dir is two `dirname` calls above its `_lib/browse-client.ts`.
2. The active cavestack skills install at `~/.claude/skills/cavestack/`. Read
...
function resolveSdkPath(): string {
  const candidates = [
    path.join(os.homedir(), '.claude', 'skills', 'cavestack', 'browse', 'src', 'browse-client.ts'),
...
```ts
import { stageSkill } from '<cavestack-install>/browse/src/browser-skill-write';
...
2. If still failing after two retries, OR the failure is an
   ...
   ```ts
   import { discardStaged } from '<cavestack-install>/browse/src/browser-skill-write';
...
moves the staged folder into ~/.cavestack/browser-skills/ where /scrape
...
```ts
import { commitSkill } from '<cavestack-install>/browse/src/browser-skill-write';
...
- **Bun runtime required.** The codified skill runs as a Bun process
  ...
  For now: the skill works on any machine that has cavestack installed,
...
```bash
~/.claude/skills/cavestack/bin/cavestack-learnings-log '{"skill":"skillify","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
