...
preamble-tier: 3
version: 0.1.0
description: Turn vague intent into a precise, executable spec in five phases. (cavestack)
allowed-tools:
  - Bash
  - Read
  - Grep
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"spec","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cavestack/bin/cavestack-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cavestack/bin/cavestack-question-log '{"skill":"spec","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
gh issue list --search "<keywords>" --state open --limit 10 --json number,title,url 2>/dev/null \
  | jq -r '.[] | "#\(.number) \(.title)"' \
  | ~/.claude/skills/cavestack/bin/cavestack-issue-guard --stdin --source issue-dedupe 2>/dev/null || true
```
...
bun ~/.claude/skills/cavestack/lib/redact-audit-log.ts \
...
# Resolve visibility once; cache + reuse. Order: local config (~/.cavestack, never
...
REDACT_BODY_EOF
REDACT_JSON=$(~/.claude/skills/cavestack/bin/cavestack-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json)
REDACT_CODE=$?
```
...
```bash
TMPERR_GATE=$(mkdir -p "${TMPDIR:-${TMP:-$HOME/.cavestack/tmp}}" && mktemp "${TMPDIR:-${TMP:-$HOME/.cavestack/tmp}}/spec-gate-XXXXXXXX")
...
reuse it; write the exact bytes to `$REDACT_FILE`; `~/.claude/skills/cavestack/bin/cavestack-redact --from-file "$REDACT_FILE"
...
~/.claude/skills/cavestack/bin/cavestack-decision-log '{"decision":"Spec filed #ISSUE_NUMBER: TITLE","rationale":"APPROACH","scope":"issue","issue":"ISSUE_NUMBER","source":"skill","confidence":7}' 2>/dev/null || true
```
...
Resolve the archive path via the existing `cavestack-paths` helper (handles
`CAVESTACK_HOME`, `CLAUDE_PLUGIN_DATA`, Windows fallback):

```bash
eval "$(~/.claude/skills/cavestack/bin/cavestack-paths)"
eval "$(~/.claude/skills/cavestack/bin/cavestack-slug)"
ARCHIVE_DIR="$CAVESTACK_STATE_ROOT/projects/$SLUG/specs"
mkdir -p "$ARCHIVE_DIR"
...
spec_plan_mode: ${CAVESTACK_PLAN_MODE:-unset}
