...
Write to `~/.cavestack/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
(`cavestack-config set codex_reviews disabled`).
...
_CODEX_CFG=$(~/.claude/skills/cavestack/bin/cavestack-config get codex_reviews 2>/dev/null || echo enabled)
source ~/.claude/skills/cavestack/bin/cavestack-codex-probe 2>/dev/null || true
...
- **`disabled`** — the user turned Codex reviews off (`codex_reviews=disabled`). Skip this section entirely; do NOT fall back to a Claude subagent — disabled means no extra review step. Print: "Codex review skipped (codex_reviews disabled). Re-enable: `cavestack-config set codex_reviews enabled`."
...
stays discoverable: "Running the outside voice automatically (standard step). Disable: `cavestack-config set codex_reviews disabled`."
...
```bash
TMPERR_PV=$(mkdir -p "${TMPDIR:-${TMP:-$HOME/.cavestack/tmp}}" && mktemp "${TMPDIR:-${TMP:-$HOME/.cavestack/tmp}}/codex-planreview-XXXXXXXX")
...
### TODOS.md updates
After all review sections are complete, present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `~/.claude/skills/cavestack/review/TODOS-format.md`.
...
TASKS_DIR="${HOME}/.cavestack/projects/${SLUG:-unknown}"
mkdir -p "$TASKS_DIR"
...
~/.claude/skills/cavestack/bin/cavestack-decision-log '{"decision":"Eng review (MODE): ARCH_SUMMARY","rationale":"KEY_DECISION","scope":"branch","source":"skill","confidence":8}' 2>/dev/null || true
```

The second command records the architecture verdict as a durable cross-session decision (so a future session inherits the chosen approach and what was hardened, not just the count). Same `~/.cavestack/` write pattern as review-log, non-interactive, best-effort (`|| true`). Substitute `ARCH_SUMMARY` (e.g. "N findings, all folded" or "M unresolved") and `KEY_DECISION` (the load-bearing architecture call from the report, one line — omit if the review found nothing durable).
...
```bash
~/.claude/skills/cavestack/bin/cavestack-learnings-log '{"skill":"plan-eng-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
1. Brain trust policy for the active endpoint is `personal` (check via
   `~/.claude/skills/cavestack/bin/cavestack-config get brain_trust_policy@<endpoint-hash>`).
...
a cavestack:takes fence block (documented but uglier path).
...
```bash
eval "$(~/.claude/skills/cavestack/bin/cavestack-slug 2>/dev/null)" 2>/dev/null || true
(~/.claude/skills/cavestack/bin/cavestack-brain-cache refresh --project "$SLUG" 2>/dev/null &) || true
```
