...
// Command handlers take (command, args, session: TabSession, bm). Resolve the
// active TabSession from the manager — passing the manager where a session is
// expected breaks as soon as a handler uses a session method (clearLoadedHtml).
...
beforeAll(async () => {
  ...
  // - Injects __CAVESTACK_SERVER_URL into the HTML
    ...
    fetch(req) {
      ...
      if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/index.html')) {
        const injected = currentHtml.replace(
          '</head>',
          `<script>window.__CAVESTACK_SERVER_URL = '${url.origin}';</script>\n</head>`
        );
...
describeBrowser('Submit: browser click → feedback.json on disk', () => {
  test('clicking Submit writes feedback.json that the agent can poll for', async () => {
    ...
    // Navigate to the board (served with __CAVESTACK_SERVER_URL injected)
    await writeCmd('goto', [baseUrl]);

    // Verify __CAVESTACK_SERVER_URL was injected
    const hasServerUrl = await readCmd('js', [
      '!!window.__CAVESTACK_SERVER_URL'
    ]);
