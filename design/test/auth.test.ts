 ...
 *   - prefer ~/.cavestack/openai.json over OPENAI_API_KEY
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cavestack-design-auth-"));
...
describe("resolveApiKeyInfo", () => {
  test("uses ~/.cavestack/openai.json before OPENAI_API_KEY", () => {
    ...
    expect(describeApiKeySource(resolution!)).toBe("~/.cavestack/openai.json");
...
describe("saveApiKey", () => {
  test("stores the key file owner-only, even under a permissive umask", () => {
    ...
    const keyPath = path.join(tmpHome, ".cavestack", "openai.json");
