 ...
 * file (.cavestack/design.json). Replaces the per-invocation server in
...
<meta charset="utf-8"><title>cavestack design boards</title><style>
...
function handleBoardGet(board: Board): Response {
  ...
  // No __CAVESTACK_SERVER_URL injection — board JS uses relative URLs that
...
function boardExpiredHtml(id: string): string {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>Board expired — cavestack</title>
