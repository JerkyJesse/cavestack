...
export const CMDLINE_MARKER = "cavestack-design-daemon";
...
export function resolveStateFilePath(): string {
  ...
  try {
    ...
    if (root) return path.join(root, ".cavestack", "design.json");
  } catch {
    // not in a git repo — fall through
  }
  return path.join(process.cwd(), ".cavestack", "design.json");
}
...
export function resolveDaemonLogPath(): string {
  return path.join(os.homedir(), ".cavestack", "design-daemon.log");
}

export function resolveStartupLogPath(): string {
  return path.join(os.homedir(), ".cavestack", "design-daemon-startup.log");
}

/**
 * Read the cavestack version both client and daemon should agree on. Looks
