 ...
 *
 * Resolution order:
 * 1. ~/.cavestack/openai.json → { "api_key": "sk-..." }
...
function configPath(): string {
  return path.join(process.env.HOME || "~", ".cavestack", "openai.json");
}
...
export function resolveApiKeyInfo(): ApiKeyResolution | null {
  ...
  if (process.env.OPENAI_API_KEY) {
    ...
    const warning = envFile
      ? `Warning: OPENAI_API_KEY matches ${envFile} in the current directory. Design generation may bill that project's OpenAI account. Run $D setup to store a cavestack-specific key in ~/.cavestack/openai.json.`
      : undefined;
...
export function describeApiKeySource(resolution: ApiKeyResolution): string {
  if (resolution.source === "config") return "~/.cavestack/openai.json";
...
/**
 * Save an API key to ~/.cavestack/openai.json with 0600 permissions.
 */
...
export function requireApiKey(): string {
  ...
  if (!resolution) {
    ...
    console.error("  or save to ~/.cavestack/openai.json: { \"api_key\": \"sk-...\" }");
