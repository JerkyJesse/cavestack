...
```bash
jq -r '"--- comment id \(.id) (\(.path // "top-level")) ---\n\(.body)"' /tmp/greptile_line.json | ~/.claude/skills/cavestack/bin/cavestack-issue-guard --stdin --source greptile-line 2>/dev/null || true
jq -r '"--- comment id \(.id) (top-level) ---\n\(.body)"' /tmp/greptile_top.json | ~/.claude/skills/cavestack/bin/cavestack-issue-guard --stdin --source greptile-top 2>/dev/null || true
```
...
Example entries:
```
2026-03-13 | JerkyJesse/myapp | fp | app/services/auth_service.rb | race-condition
2026-03-13 | JerkyJesse/myapp | fix | app/models/user.rb | null-check
2026-03-13 | JerkyJesse/myapp | already-fixed | lib/payments.rb | error-handling
```
