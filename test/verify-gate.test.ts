/**
 * cavestack-verify-gate — Stop-hook enforcement tier.
 *
 * Pins the behaviours the gate exists for:
 *   trust    — a declared command NEVER runs until the user records it via
 *              `cavestack-verify-gate --trust` (per-repo command trust store).
...
const GATE = path.join(ROOT, 'bin', 'cavestack-verify-gate');

let dir: string;
let cavestackHome: string;

beforeEach(() => {
  dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-verify-gate-'));
  cavestackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-verify-gate-home-'));
});

afterEach(() => {
  fs.rmSync(dir, { recursive: true, force: true });
  fs.rmSync(cavestackHome, { recursive: true, force: true });
});

/** Declare a verification command in the project's CLAUDE.md (comment form). */
function declareCheck(command: string): void {
  fs.writeFileSync(path.join(dir, 'CLAUDE.md'), `# Fixture\n\n<!-- cavestack:verify: ${command} -->\n`);
}
...
function gateEnv(projectDirEnv: boolean): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...process.env, CAVESTACK_HOME: cavestackHome };
...
describe('cavestack-verify-gate trust store', () => {
  ...
  test('non-comment declaration form (cavestack:verify: cmd without <!-- -->) is honored', () => {
    fs.writeFileSync(path.join(dir, 'CLAUDE.md'), '# Fixture\n\ncavestack:verify: ./check.sh\n');
    check(0, 'all good');
  ...
  test('the trust store file is created 0600 under CAVESTACK_HOME', () => {
    declareCheck('./check.sh');
    check(0, 'all good');
    expect(trust().code).toBe(0);

    const store = path.join(cavestackHome, 'verify-gate-trust');
...
describe('cavestack-verify-gate', () => {
  ...
  test('fails open when CLAUDE.md declares no check', () => {
    ...
    expect(r.code).toBe(0);
    expect(r.stdout).toContain("declares no 'cavestack:verify:' command");
  });
  ...
  test('never invents a command: an empty declaration fails open', () => {
    fs.writeFileSync(path.join(dir, 'CLAUDE.md'), '<!-- cavestack:verify: -->\n');
...
describe('cavestack-verify-gate re-entry enforcement (no one-shot bypass)', () => {
...
describe('cavestack-verify-gate trust-grant audit trail', () => {
  test('every --trust grant appends a JSON audit line (right sha256, 0600, verbatim cmd)', () => {
    ...
    const log = path.join(cavestackHome, 'security', 'verify-gate-trust-grants.jsonl');
...
describe('opt-in contract (adapted from the fork: NOT registered by default)', () => {
  ...
  test('./setup does NOT register the gate — a Stop hook running the verify command after every turn is opt-in', () => {
    ...
    for (const line of mentions) {
      const t = line.trim();
      const allowed = t.startsWith('#') || t.includes('CAVESTACK_SWEEP_EXCLUDE_SOURCES="verify-gate"');
  ...
  test('the bin documents its own registration and removal commands', () => {
    ...
    expect(gate).toContain('cavestack:verify:');
  });
});
