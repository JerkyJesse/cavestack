/**
 * cavestack-egress CLI — list | verify | grants smoke tests. Free tier.
 *
 * Spawns the real bin against a temp CAVESTACK_HOME: list filters, verify
...
const BIN = path.join(ROOT, 'bin', 'cavestack-egress');

let home: string;

beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-egress-cli-'));
});
...
describe('cavestack-egress list', () => {
...
describe('cavestack-egress verify', () => {
  ...
  test('prints the sizeWarning when the ledger exceeds the threshold', () => {
    ...
    expect(parsed.sizeWarning).toContain('cavestack-egress list');
    const human = run(['verify']);
    expect(human.stdout).toContain('egress ledger is large');
  });
});

describe('cavestack-egress grants', () => {
  ...
  test('--json flips granted=true when telemetry and sync mode are enabled', () => {
    const config = spawnSync(path.join(ROOT, 'bin', 'cavestack-config'), ['set', 'telemetry', 'community'], {
      encoding: 'utf-8',
      env: { ...process.env, CAVESTACK_HOME: home },
    });
    expect(config.status).toBe(0);
    spawnSync(path.join(ROOT, 'bin', 'cavestack-config'), ['set', 'artifacts_sync_mode', 'full'], {
      encoding: 'utf-8',
      env: { ...process.env, CAVESTACK_HOME: home },
    });
...
describe('cavestack-egress usage', () => {
