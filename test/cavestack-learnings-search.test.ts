...
const BIN = path.join(ROOT, 'bin', 'cavestack-learnings-search');

const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-search-test-'));
const tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-search-cwd-'));
// cavestack-slug derives slug from git remote (none here) → falls back to basename of cwd.
...
describe('cavestack-learnings-search token-OR query semantics', () => {
...
describe('cavestack-learnings-search cross-project trust gating', () => {
