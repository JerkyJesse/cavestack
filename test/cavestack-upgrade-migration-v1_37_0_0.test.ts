/**
 * Unit tests for cavestack-upgrade/migrations/v1.37.0.0.sh — split-engine notice.
 *
...
function makeEnv(opts: {
  ...
  const claudeSkillsBin = join(home, ".claude", "skills", "cavestack", "bin");
  const claudeJson = join(home, ".claude.json");
  const gbrainConfig = join(gbrainDir, "config.json");
  const configBin = join(claudeSkillsBin, "cavestack-config");
  ...
  // Fake cavestack-config: returns "true" iff opted-out (matches the real bin's
...
`,
  ...
  return {
    ...
    home,
    cavestackHome,
    doneTouch: join(cavestackHome, ".migrations", "v1.37.0.0.done"),
    claudeJson,
    gbrainConfig,
    configBin,
...
function runMigration(env: MigEnv): { stdout: string; stderr: string; exitCode: number } {
  const result = spawnSync("bash", [MIGRATION], {
    ...
    env: {
      ...
      // The script looks for cavestack-config at $HOME/.claude/skills/cavestack/bin
...
describe("cavestack-upgrade/migrations/v1.37.0.0.sh", () => {
