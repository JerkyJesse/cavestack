...
describe('setup: Conductor worktree guard', () => {
  test('setup contains the real-dir guard before the symlink-or-copy into ~/.claude/skills/', () => {
    ...
    const lnIdx = content.indexOf('_link_or_copy "$SOURCE_CAVESTACK_DIR" "$CLAUDE_CAVESTACK_LINK"');
  ...
  test('guard resolves the existing real dir with `pwd -P` and compares against source', () => {
    const content = fs.readFileSync(SETUP_SCRIPT, 'utf-8');
    expect(content).toContain('[ -d "$CLAUDE_CAVESTACK_LINK" ] && [ ! -L "$CLAUDE_CAVESTACK_LINK" ]');
    expect(content).toContain('cd "$CLAUDE_CAVESTACK_LINK" 2>/dev/null && pwd -P');
    expect(content).toContain('"$_EXISTING_REAL" != "$SOURCE_CAVESTACK_DIR"');
  });
  ...
  test('skip branch prints "registration skipped" + remediation hint', () => {
    ...
    expect(content).toContain('rm -rf $CLAUDE_CAVESTACK_LINK');
  });
  ...
  test('BSD ln -snf into an existing real dir creates a child symlink (bug reproduces)', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-setup-guard-'));
    try {
  ...
  test('guard logic refuses to ln when dest is a real dir pointing elsewhere', () => {
    ...
    try {
      ...
      const script = `
        set -e
        SOURCE_CAVESTACK_DIR='${source}'
        CLAUDE_CAVESTACK_LINK='${dest}'
        _SKIP_CLAUDE_REGISTER=0
        if [ -d "$CLAUDE_CAVESTACK_LINK" ] && [ ! -L "$CLAUDE_CAVESTACK_LINK" ]; then
          _EXISTING_REAL=$(cd "$CLAUDE_CAVESTACK_LINK" 2>/dev/null && pwd -P || echo "")
          if [ -n "$_EXISTING_REAL" ] && [ "$_EXISTING_REAL" != "$SOURCE_CAVESTACK_DIR" ]; then
  ...
  test('guard allows ln when dest is a real dir already pointing to source (self-rerun)', () => {
    ...
    try {
      const source = path.join(tmp, 'source-worktree');
      fs.mkdirSync(source);
      // Mirror setup's SOURCE_CAVESTACK_DIR resolution (`pwd -P`) so the comparison
      ...
      const script = `
        set -e
        SOURCE_CAVESTACK_DIR='${resolvedSource}'
        CLAUDE_CAVESTACK_LINK='${dest}'
        _SKIP_CLAUDE_REGISTER=0
        if [ -d "$CLAUDE_CAVESTACK_LINK" ] && [ ! -L "$CLAUDE_CAVESTACK_LINK" ]; then
          _EXISTING_REAL=$(cd "$CLAUDE_CAVESTACK_LINK" 2>/dev/null && pwd -P || echo "")
