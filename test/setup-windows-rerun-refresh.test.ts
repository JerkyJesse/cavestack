 ...
 * re-run: `./setup --host codex` reported "cavestack ready" but never refreshed
...
describe('setup: Windows re-run refresh — static guard sites (#2444)', () => {
  ...
  // skill-dir installer must gate the replacement on provable cavestack
...
describe('setup: Windows refresh ownership gate — behavior fixture (#2142)', () => {
  test("IS_WINDOWS=1: a user's own real dir on a cavestack* name survives; a bannered install refreshes", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-owned-'));
    try {
      const fake = path.join(tmp, 'cavestack');
      ...
      for (const name of ['cavestack-demo', 'cavestack-notes']) {
        const d = path.join(fake, '.agents', 'skills', name);
        fs.mkdirSync(d, { recursive: true });
        fs.writeFileSync(path.join(d, 'SKILL.md'), `${banner}upstream-v2\n`);
      }
      fs.mkdirSync(skills, { recursive: true });
      // cavestack-demo: a prior cavestack install (bannered) — must refresh.
      fs.mkdirSync(path.join(skills, 'cavestack-demo'), { recursive: true });
      fs.writeFileSync(path.join(skills, 'cavestack-demo', 'SKILL.md'), `${banner}installed-v1\n`);
      // cavestack-notes: the USER'S own hand-written skill — must survive.
      fs.mkdirSync(path.join(skills, 'cavestack-notes'), { recursive: true });
      fs.writeFileSync(path.join(skills, 'cavestack-notes', 'SKILL.md'), '# my own notes\n');

      const r = runInstaller(
        '1',
        ['_owned_for_windows_refresh', 'link_codex_skill_dirs'],
        `link_codex_skill_dirs "${tmp}/cavestack" "${skills}"`,
      );
      expect(r.status).toBe(0);
      expect(fs.readFileSync(path.join(skills, 'cavestack-demo', 'SKILL.md'), 'utf-8')).toContain('upstream-v2');
      expect(fs.readFileSync(path.join(skills, 'cavestack-notes', 'SKILL.md'), 'utf-8')).toBe('# my own notes\n');
      expect(r.stderr).toContain('left in place');
      expect(r.stderr).toContain('cavestack-notes');
    } finally {
  ...
  test('IS_WINDOWS=1: create_agents_sidecar refuses a user-owned root and writes nothing into it', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-owned-sidecar-'));
    try {
      const fake = path.join(tmp, 'cavestack');
      fs.mkdirSync(path.join(fake, 'bin'), { recursive: true });
      fs.writeFileSync(path.join(fake, 'bin', 'tool.sh'), 'v1\n');
      // The user's own skill squats on .agents/skills/cavestack.
      const root = path.join(fake, '.agents', 'skills', 'cavestack');
      fs.mkdirSync(root, { recursive: true });
      fs.writeFileSync(path.join(root, 'SKILL.md'), '# hand-written\n');

      const vars = `SOURCE_CAVESTACK_DIR="${fake}"`;
      const r = runInstaller(
        '1',
...
describe('setup: Windows re-run refresh — behavior fixture (#2444)', () => {
  test('IS_WINDOWS=1: link_codex_skill_dirs refreshes an already-installed skill', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-rerun-'));
    try {
      const fake = path.join(tmp, 'cavestack');
      const skills = path.join(tmp, 'skills');
      const demo = path.join(fake, '.agents', 'skills', 'cavestack-demo');
      ...
      expect(r.status).toBe(0);
      const installed = path.join(skills, 'cavestack-demo', 'SKILL.md');
      expect(fs.readFileSync(installed, 'utf-8')).toBe(`${banner}v1-original\n`);
      expect(fs.lstatSync(path.join(skills, 'cavestack-demo')).isSymbolicLink()).toBe(false);
  ...
  test('IS_WINDOWS=1: create_agents_sidecar refreshes copied runtime assets', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-rerun-sidecar-'));
    try {
      const fake = path.join(tmp, 'cavestack');
      fs.mkdirSync(path.join(fake, 'bin'), { recursive: true });
      fs.writeFileSync(path.join(fake, 'bin', 'tool.sh'), 'v1\n');
      ...
      expect(r.status).toBe(0);
      const sidecarBin = path.join(fake, '.agents', 'skills', 'cavestack', 'bin', 'tool.sh');
      const sidecarEthos = path.join(fake, '.agents', 'skills', 'cavestack', 'ETHOS.md');
  ...
  test('IS_WINDOWS=1: nested gitignored build output does NOT survive the runtime-asset copy (P5)', () => {
    ...
    // survive.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-rerun-prune-'));
    try {
  ...
  test('IS_WINDOWS=0: the Unix symlink path never prunes through into the source', () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-rerun-prune-unix-'));
    try {
      const src = path.join(tmp, 'skill-src');
      const dst = path.join(tmp, 'skill-dst');
      fs.mkdirSync(path.join(src, 'scripts', 'gen-tool', '.build'), { recursive: true });
  ...
  test('IS_WINDOWS=1: the cavestack sidecar dir is still skipped by the skill loop', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-rerun-skip-'));
    try {
      const fake = path.join(tmp, 'cavestack');
      const skills = path.join(tmp, 'skills');
      const sidecar = path.join(fake, '.agents', 'skills', 'cavestack');
      ...
      expect(r.status).toBe(0);
      expect(fs.existsSync(path.join(skills, 'cavestack'))).toBe(false);
    } finally {
      fs.rmSync(tmp, { recursive: true, force: true });
    }
  });
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    test('IS_WINDOWS=0: installs a symlink and re-runs still refresh through it', () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-rerun-unix-'));
      try {
        const fake = path.join(tmp, 'cavestack');
        const skills = path.join(tmp, 'skills');
        const demo = path.join(fake, '.agents', 'skills', 'cavestack-demo');
        ...
        expect(r.status).toBe(0);
        const target = path.join(skills, 'cavestack-demo');
