/**
 * cavestack-distill-apply — Layer 8 proposal application (plan-tune cathedral T11).
 *
 * Verifies the three apply paths:
 *   - memory-nugget → appended to ~/.cavestack/free-text-memory.json (local
 *     source-of-truth; gbrain is mirror when configured).
 *   - preference   → routed through cavestack-question-preference with
...
const BIN = path.join(ROOT, 'bin', 'cavestack-distill-apply');
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-apply-'));
...
describe('memory-nugget apply', () => {
  test('appends to ~/.cavestack/free-text-memory.json with full metadata', () => {
    writeProposals([
      {
        kind: 'memory-nugget',
        confidence: 0.9,
...
describe('preference apply', () => {
  test('routes through cavestack-question-preference with source=plan-tune', () => {
    writeProposals([
      {
        kind: 'preference',
        confidence: 0.9,
