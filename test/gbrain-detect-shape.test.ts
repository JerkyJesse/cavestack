/**
 * Shape regression test for bin/cavestack-gbrain-detect.
 *
 ...
 * callers across most cavestack skill preambles shell out to this script and
...
const DETECT_BIN = join(import.meta.dir, "..", "bin", "cavestack-gbrain-detect");
...
interface DetectShape {
  ...
  cavestack_brain_sync_mode: string;
  cavestack_brain_git: boolean;
  cavestack_artifacts_remote: string;
  gbrain_local_status: string;
}

describe("bin/cavestack-gbrain-detect — shape regression", () => {
  it("emits valid JSON", () => {
  ...
  it("contains all 9 pre-existing keys + the new gbrain_local_status key", () => {
    ...
    try {
      ...
      expect(parsed).toHaveProperty("cavestack_brain_sync_mode");
      expect(parsed).toHaveProperty("cavestack_brain_git");
      expect(parsed).toHaveProperty("cavestack_artifacts_remote");
  ...
  it("preserves field types from the bash version", () => {
    ...
    try {
      ...
      expect(typeof parsed.cavestack_brain_git).toBe("boolean");
      ...
      expect(typeof parsed.cavestack_brain_sync_mode).toBe("string");
      expect(typeof parsed.cavestack_artifacts_remote).toBe("string");
  ...
  it("cavestack_brain_sync_mode is one of the three documented values", () => {
    const tmp = mkdtempSync(join(tmpdir(), "detect-shape-"));
    try {
      const out = runDetect({
        HOME: tmp,
      ...
      const parsed = JSON.parse(out) as DetectShape;
      expect(["off", "artifacts-only", "full"]).toContain(parsed.cavestack_brain_sync_mode);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  });
...
describe("bin/cavestack-gbrain-detect --is-ok — live gate", () => {
...
`;
  ...
  it("exit code agrees with the JSON gbrain_local_status (no skew)", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "detect-isok-"));
    try {
      const env = { HOME: tmp, PATH: "/usr/bin:/bin", CAVESTACK_HOME: tmp, CAVESTACK_DETECT_NO_CACHE: "1" };
