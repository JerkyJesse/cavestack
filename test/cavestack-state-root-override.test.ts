/**
 * CAVESTACK_STATE_ROOT override — verifies the 3 plan-tune bins honor
 * CAVESTACK_STATE_ROOT as a higher-priority override over CAVESTACK_HOME.
 *
 * Surfaced by plan-tune cathedral D16 (Codex outside voice): tests can't
 * isolate from real ~/.cavestack today because the bins ignore STATE_ROOT.
 * Without this override, the cathedral's E2E + integration tests would
 * silently pollute the user's real profile.
 *
 * Contract:
 *   - CAVESTACK_STATE_ROOT set → bins write under STATE_ROOT (HOME ignored).
 *   - Only CAVESTACK_HOME set → bins write under HOME (existing behavior).
 *   - Neither set → falls back to $HOME/.cavestack (existing behavior).
...
const BIN_LOG = path.join(ROOT, 'bin', 'cavestack-question-log');
const BIN_PREF = path.join(ROOT, 'bin', 'cavestack-question-preference');
const BIN_DEV = path.join(ROOT, 'bin', 'cavestack-developer-profile');

let stateRoot: string;
let homeRoot: string;

beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-state-'));
  homeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-home-'));
});
...
function runBin(
  ...
  if (env.CAVESTACK_STATE_ROOT === undefined) delete cleaned.CAVESTACK_STATE_ROOT;
  if (env.CAVESTACK_HOME === undefined) delete cleaned.CAVESTACK_HOME;
...
describe('cavestack-question-log honors CAVESTACK_STATE_ROOT', () => {
  test('STATE_ROOT set, HOME unset → writes under STATE_ROOT', () => {
    const r = runBin(BIN_LOG, [JSON.stringify(SAMPLE_LOG)], {
      CAVESTACK_STATE_ROOT: stateRoot,
      CAVESTACK_HOME: undefined,
    });
  ...
  test('only HOME set → preserves existing behavior (writes under HOME)', () => {
    const r = runBin(BIN_LOG, [JSON.stringify(SAMPLE_LOG)], {
      CAVESTACK_STATE_ROOT: undefined,
      CAVESTACK_HOME: homeRoot,
    });
    expect(r.status).toBe(0);
...
describe('cavestack-question-preference honors CAVESTACK_STATE_ROOT', () => {
  test('STATE_ROOT set → preferences file lives under STATE_ROOT', () => {
    const write = runBin(
      ...
      { CAVESTACK_STATE_ROOT: stateRoot, CAVESTACK_HOME: undefined },
    );
...
describe('cavestack-developer-profile honors CAVESTACK_STATE_ROOT', () => {
