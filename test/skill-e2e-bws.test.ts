...
Report whether it worked.`,
  ...
  testConcurrentIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/cavestack/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with cavestack installed globally, the fallback path
    // ~/.claude/skills/cavestack/browse/dist/browse exists, so we get READY.
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testConcurrentIfSelected('operational-learning', async () => {
    ...
    const cavestackHome = path.join(opDir, '.cavestack-home');
    ...
    // Copy bin scripts + the lib module they import. cavestack-learnings-log
    ...
    for (const script of ['cavestack-learnings-log', 'cavestack-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }
    // cavestack-learnings-log imports $SCRIPT_DIR/../lib/jsonl-store.ts (shared
    ...
    // cavestack-learnings-log will create the project dir automatically via cavestack-slug
...
\`\`\`bash
CAVESTACK_HOME="${cavestackHome}" ${binDir}/cavestack-learnings-log '{"skill":"qa","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
Log the operational learning now. Then say what you logged.`,
    ...
    const projectsDir = path.join(cavestackHome, 'projects');
  ...
  testConcurrentIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a cavestack skill. The session preamble detected _SESSIONS=4 (the user has 4 cavestack windows open).
