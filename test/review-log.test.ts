...
function run(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/cavestack-review-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-revlog-'));
  // cavestack-review-log uses cavestack-slug which needs a git repo — create the projects dir
...
describe('cavestack-review-log', () => {
  ...
  test('append still succeeds outside a git repo (binding fields omitted)', () => {
    const nonGit = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-nongit-'));
    try {
      const execOpts: ExecSyncOptionsWithStringEncoding = {
        cwd: nonGit,
        env: { ...process.env, CAVESTACK_HOME: tmpDir },
      ...
      execSync(`${BIN}/cavestack-review-log '{"skill":"review","status":"clean"}'`, execOpts);
...
describe('cavestack-wtree', () => {
  function withScratchRepo(fn: (repoDir: string, wtree: () => string) => void) {
    const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-wtree-'));
    try {
      ...
      git('commit -q -m init');
      const wtree = () => execSync(`${BIN}/cavestack-wtree`, { cwd: repoDir, encoding: 'utf-8', timeout: 10000 }).trim();
      fn(repoDir, wtree);
    } finally {
  ...
  test('exits non-zero outside a git repo', () => {
    const nonGit = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-wtree-nongit-'));
    try {
      expect(() => execSync(`${BIN}/cavestack-wtree`, { cwd: nonGit, timeout: 10000, stdio: 'pipe' })).toThrow();
    } finally {
      fs.rmSync(nonGit, { recursive: true, force: true });
    }
  });
...
describe('cavestack-review-read', () => {
  test('emits ---WTREE---, ---TREE--- and ---DIRTY--- sections', () => {
    const out = execSync(`${BIN}/cavestack-review-read`, {
      cwd: ROOT,
      env: { ...process.env, CAVESTACK_HOME: tmpDir },
      encoding: 'utf-8',
      timeout: 10000,
