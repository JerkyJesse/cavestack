/**
 * cavestack-distill-free-text — Layer 8 dream cycle (plan-tune cathedral T10).
 *
...
const BIN = path.join(ROOT, 'bin', 'cavestack-distill-free-text');
const QLOG_BIN = path.join(ROOT, 'bin', 'cavestack-question-log');
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-dist-'));
