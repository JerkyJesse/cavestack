 ...
 *           root router's `_cavestack-command` alias frontmatter as one separate
 ...
 *   result  61 authored skills + root router = 4,835 bytes
 *           = 1,209 token-equivalents (measured 2026-08-24, gstack 1.68.3
 *           port plus CaveStack caveman/help/checkpoint extras)
 * Ceiling is 1,250 token-equivalents (5,000 bytes). Dominant skill still
 * design-consultation at the per-skill cap.
 */
const CATALOG_BUDGET_TOKEN_EQUIVALENTS = 1_250;
...
function catalogEntries(): CatalogEntry[] {
  ...
  // `_cavestack-command` alias — not an authored skill, but its frontmatter
