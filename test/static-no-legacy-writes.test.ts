 ...
 * append directly to ~/.cavestack/builder-profile.jsonl. All session writes
 * must go through `cavestack-developer-profile --log-session`. The legacy
 * file is now read-only — populated only by the pre-existing migration
 * and reconcile paths in bin/cavestack-developer-profile.
 *
...
const ALLOWED_FILES = new Set<string>([
  ...
  'bin/cavestack-developer-profile',
  // The legacy-shim binary that delegates reads.
  'bin/cavestack-builder-profile',
...
describe('#1671 invariant: no production code writes to builder-profile.jsonl', () => {
    ...
    if (offending.length > 0) {
      ...
      throw new Error(
        `Found production writes to builder-profile.jsonl outside the allowlist.\n` +
          ...
          `Use \`cavestack-developer-profile --log-session\` instead.\n${msg}`,
      );
    }
  ...
  test('office-hours/SKILL.md uses --log-session, not raw echo append', () => {
    ...
    expect(skill).toContain('cavestack-developer-profile --log-session');
  ...
  test('office-hours/SKILL.md.tmpl uses --log-session, not raw echo append', () => {
    ...
    expect(tmpl).toContain('cavestack-developer-profile --log-session');
