 ...
 *   name ("seville-v3" vs "cavestack") and aggregate order was filesystem-
...
describe('toYamlInlineScalar', () => {
  ...
  test("scalar containing '...' (YAML document-end marker) is quoted and round-trips", () => {
    const out = toYamlInlineScalar('Truncated lead ends with... more (cavestack)');
...
describe('splitCatalogDescription', () => {
  test('extracts lead sentence + routing prose from simple multi-line description', () => {
    ...
    expect(parts.hasCavestackTag).toBe(true);
    ...
    // (cavestack) tag stripped from routingProse
    expect(parts.routingProse).not.toContain('(cavestack)');
  });
  ...
  test('REGRESSION: "TODOS.md backlog" style sentence keeps full lead + routing', () => {
    const desc =
      ...
      'Proactively suggest after a plan is approved. (cavestack)';
    const parts = splitCatalogDescription(desc);
  ...
  test('>200 char first sentence WITH embedded periods still truncates with ellipsis and keeps routing', () => {
    ...
    const desc = firstSentence + ' Use when asked to resync the bridge. (cavestack)';
    const parts = splitCatalogDescription(desc);
  ...
  test('idempotent: calling on already-trimmed output returns the same parts', () => {
    const desc = 'Already trimmed. (cavestack)';
    ...
    expect(parts2.hasCavestackTag).toBe(true);
    expect(parts2.routingProse).toBe('');
  });
});

describe('buildTrimmedDescription', () => {
  test('appends (cavestack) when hasCavestackTag is true', () => {
    const out = buildTrimmedDescription({
      lead: 'Some lead.',
      routingProse: 'routing',
      voiceLine: null,
      hasCavestackTag: true,
    });
    expect(out).toBe('Some lead. (cavestack)');
  });

  test('omits (cavestack) when hasCavestackTag is false', () => {
    const out = buildTrimmedDescription({
      lead: 'No tag.',
      routingProse: '',
      voiceLine: null,
      hasCavestackTag: false,
    });
  ...
  test('trims whitespace from lead', () => {
    ...
    expect(out).toBe('Lead with whitespace. (cavestack)');
  });
});
...
description: |
  ...
  Proactively suggest when the user mentions examples. (cavestack)
preamble-tier: 2
---
...
`;
  ...
  test('rewrites multi-line description into one-line + body section', () => {
    ...
    // Frontmatter description is now ONE line ending with (cavestack). #1778: a
  ...
  test('returns null for already-short descriptions (no-op)', () => {
    const shortSkill = minimalSkill.replace(
      ...
      'description: Already short. (cavestack)\n',
    );
  ...
  test('keeps the newline between description and next YAML field (no field collision)', () => {
    ...
    // `description: ... (cavestack)preamble-tier:` with no newline.
    const result = applyCatalogTrim(minimalSkill, 'example');
    expect(result).not.toBeNull();
    expect(result!.content).not.toMatch(/\(cavestack\)preamble-tier/);
    expect(result!.content).not.toMatch(/\(cavestack\)allowed-tools/);
    // #1778: optional closing quote when the description was YAML-quoted.
    expect(result!.content).toMatch(/\(cavestack\)"?\n[a-z-]+:/);
  });
...
describe('proactive-suggestions.json stays retired', () => {
  test('the generator no longer emits scripts/proactive-suggestions.json', () => {
    ...
    // tests (sorted keys, root keyed as "cavestack", no timestamp fields) that
    // lived here before.
    const fs = require('fs');
