/**
 * Tests for the dream (call-graph build) stage of bin/cavestack-gbrain-sync.ts.
 *
...
describe("dream marker — concurrency guard", () => {
  const saved = process.env.CAVESTACK_HOME;
  let tmp: string;

  afterEach(() => {
    if (tmp) rmSync(tmp, { recursive: true, force: true });
    if (saved === undefined) delete process.env.CAVESTACK_HOME;
    else process.env.CAVESTACK_HOME = saved;
  });

  function redirectHome(): void {
    tmp = mkdtempSync(join(tmpdir(), "gbrain-dream-marker-"));
    process.env.CAVESTACK_HOME = tmp;
  }
