...
describeSol('GPT-5.6 Sol full-artifact scope termination', () => {
  beforeAll(() => {
    ...
    const priorAgentsBackup = fs.existsSync(agentsDir)
      ? fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-agents-backup-'))
      : '';
    ...
    const generatedDir = path.join(agentsDir, 'skills', 'cavestack-investigate');
    ...
    skillDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-sol-skill-'));
    ...
    scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-sol-scope-'));
    run('git', ['init', '-b', 'main']);
...
`);
  ...
  testIfSelected('codex-sol-scope-termination', async () => {
    const result = await runCodexSkill({
      skillDir,
      skillName: 'cavestack-investigate',
      cwd: scratch,
      ...
      prompt: `Use the cavestack-investigate skill to diagnose and fix why parseLimit('0') returns 10.
