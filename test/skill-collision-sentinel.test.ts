 ...
 * for /rewind, silently shadowing the cavestack /checkpoint skill. Users
 ...
 *
 * This file is the "never again" test. It enumerates every cavestack skill name
 * from every SKILL.md.tmpl file in the repo and cross-checks against a
 * per-host list of known built-in slash commands. If any cavestack skill name
 ...
 * below. If a cavestack skill needs to coexist with a built-in anyway (e.g.,
...
const KNOWN_BUILTINS: Record<string, string[]> = {
  ...
  // invoke codex from cavestack, we don't install skills INTO codex the same
...
interface CavestackSkill {
  name: string;
  templatePath: string;
}

function enumerateCavestackSkills(): CavestackSkill[] {
  const skills: CavestackSkill[] = [];
...
describe('skill-collision-sentinel', () => {
  const skills = enumerateCavestackSkills();
  ...
  test('no duplicate skill names within cavestack', () => {
  ...
  // Hard check: no cavestack skill name collides with a known host built-in
    ...
    test(`no skill name collides with a ${host} built-in (or has written justification)`, () => {
      ...
      if (collisions.length > 0) {
        const msg = collisions.map(c =>
          ...
          `    Fix: rename the cavestack skill (precedent: /checkpoint → /context-save+/context-restore),\n` +
  ...
  test('KNOWN_COLLISIONS_TOLERATED entries are all still active collisions', () => {
    ...
    for (const name of Object.keys(KNOWN_COLLISIONS_TOLERATED)) {
      if (!skillNames.has(name)) {
        stale.push(`  "${name}" is in KNOWN_COLLISIONS_TOLERATED but no cavestack skill has that name — remove the exception`);
