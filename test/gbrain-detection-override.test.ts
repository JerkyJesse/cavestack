 ...
 * with --respect-detection, it reads ~/.cavestack/gbrain-detection.json and
 ...
 *
 * Tests drive gen-skill-docs as a subprocess against a temp CAVESTACK_HOME
...
/**
 * Run gen-skill-docs with --respect-detection and an isolated CAVESTACK_HOME.
...
function regenAndSnapshot(opts: {
  ...
  try {
    execFileSync('bun', args, {
      cwd: REPO_ROOT,
      env: { ...process.env, CAVESTACK_HOME: opts.tmpHome },
...
describe('gbrain detection override → gen-skill-docs', () => {
  ...
  test('with status "timeout" (slow-but-healthy, #1964), brain blocks render like "ok"', () => {
    ...
    try {
      ...
      // treatment as "ok" (matches cavestack-gbrain-detect --is-ok).
      expect(content).toContain('## Save Results to Brain');
      expect(content).toContain('gbrain put "office-hours/');
    } finally {
      cleanup();
