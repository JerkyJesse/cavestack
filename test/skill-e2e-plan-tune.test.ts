...
describeIfSelected('PlanTune E2E', ['plan-tune-inspect'], () => {
  ...
  beforeAll(() => {
    ...
    // cavestack-developer-profile --derive imports from scripts/ — copy those too.
    ...
    const cfgDir = path.join(cavestackHome);
...
IMPORTANT:
- Use CAVESTACK_HOME="${cavestackHome}" as an environment variable for all bin calls.
- Replace any ~/.claude/skills/cavestack/bin/ references with ./bin/ (relative path).
- Replace any ~/.claude/skills/cavestack/scripts/ references with ./scripts/.
