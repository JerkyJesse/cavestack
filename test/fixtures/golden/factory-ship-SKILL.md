...
`.factory/skills/cavestack/`. Vendoring is deprecated. We will not keep vendored copies
...
> A) Investigate and fix now (human: ~2-4h / CC: ~15min)
...
>
> RECOMMENDATION: Choose B — assign it to whoever broke it so the right person fixes it.
> A) Investigate and fix now anyway
> B) Blame + assign GitHub issue to the author
> C) Add as P0 TODO
> D) Skip — ship anyway
...
if [ "$_CROSS_PROJ" = "true" ]; then
  $CAVESTACK_BIN/cavestack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
...
1. Read `.factory/skills/cavestack/review/checklist.md`. Unreadable = **STOP**.
...
Follow `.factory/skills/cavestack/review/greptile-triage.md` — fetch, filter, classify, **escalation detection**.
...
OLD_CFG=$($CAVESTACK_ROOT/bin/cavestack-config get codex_reviews 2>/dev/null || true)
...
Format ref: `.factory/skills/cavestack/review/TODOS-format.md`.
