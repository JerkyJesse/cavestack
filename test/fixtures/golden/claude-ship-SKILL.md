...
preamble-tier: 4
version: 1.0.0
description: "Ship workflow: detect + merge base branch, run tests, review diff, bump VERSION, update CHANGELOG, commit, push, create PR. (cavestack)"
allowed-tools:
  - Bash
  - Read
  - Write
...
~/.claude/skills/cavestack/bin/cavestack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cavestack/bin/cavestack-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**STOP and Read `~/.claude/skills/cavestack/ship/sections/apple-release.md` FIRST**
...
For Design Review: run `source <(~/.claude/skills/cavestack/bin/cavestack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
> **STOP.** Before running the test suites and (if prompt files changed) the eval suites (Steps 4-6), Read `~/.claude/skills/cavestack/ship/sections/tests.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before auditing test coverage of the diff (Step 7), Read `~/.claude/skills/cavestack/ship/sections/test-coverage.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before auditing plan completion, verification, and scope drift (Step 8), Read `~/.claude/skills/cavestack/ship/sections/plan-completion.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before the pre-landing review and specialist dispatch (Step 9), Read `~/.claude/skills/cavestack/ship/sections/review-army.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before addressing Greptile review comments when a PR exists (Step 10), Read `~/.claude/skills/cavestack/ship/sections/greptile.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before the adversarial review and learnings capture (Step 11), Read `~/.claude/skills/cavestack/ship/sections/adversarial.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
1. **Classify state** — pure reader, never writes:
   ```bash
   bun run ~/.claude/skills/cavestack/bin/cavestack-version-bump classify --base <base>
   ```
...
3. **Queue-aware pick** (workspace-aware ship):
   ```bash
   QUEUE_JSON=$(bun run ~/.claude/skills/cavestack/bin/cavestack-next-version --base <base> --bump "$BUMP_LEVEL" --current-version "$BASE_VERSION" 2>/dev/null || echo '{"offline":true}')
...
4. **Write the bump** (FRESH, or an approved rebump):
   ```bash
   bun run ~/.claude/skills/cavestack/bin/cavestack-version-bump write --version "$NEW_VERSION"
   ```
...
5. **Record the release decision** (durable cross-session memory). The bump level is a real decision the next session should not re-derive blind:
   ```bash
   ~/.claude/skills/cavestack/bin/cavestack-decision-log '{"decision":"Ship NEW_VERSION (BUMP_LEVEL)","rationale":"WHY","scope":"repo","source":"skill","confidence":9}' 2>/dev/null || true
   ```
...
> **STOP.** Before writing the CHANGELOG entry (Step 13), Read `~/.claude/skills/cavestack/ship/sections/changelog.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
~/.claude/skills/cavestack/bin/cavestack-evidence check --label tests --expect-cmd '<exact tests-lane command from Step 5>' --label vitest --expect-cmd '<exact vitest-lane command from Step 5>' --max-age 24 --allow-paths CHANGELOG.md,VERSION,package.json
```
...
- **Any STALE/MISSING (exit non-zero):** run live, wrapped, so the fresh run is
  recorded: `~/.claude/skills/cavestack/bin/cavestack-evidence run --label <lane> -- '<command>'`.
...
```bash
_REDACT_PREPUSH=$(~/.claude/skills/cavestack/bin/cavestack-config get redact_prepush_hook 2>/dev/null || echo "false")
...
1. **`REDACT_PREPUSH: true` and `HOOK_INSTALLED: no` and `HOOKS_IN_GIT_DIR: yes`** —
   ...
   ```bash
   ~/.claude/skills/cavestack/bin/cavestack-redact install-prepush-hook
   ```
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   ...
   If A: run `~/.claude/skills/cavestack/bin/cavestack-config set redact_prepush_hook true`
   then `~/.claude/skills/cavestack/bin/cavestack-redact install-prepush-hook`.
   If B: run `~/.claude/skills/cavestack/bin/cavestack-config set redact_prepush_hook false`.
...
```bash
~/.claude/skills/cavestack/bin/cavestack-review-log '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"'"$(git rev-parse --abbrev-ref HEAD)"'"}'
```
...
_QT=$(~/.claude/skills/cavestack/bin/cavestack-config get question_tuning 2>/dev/null || echo "false")
