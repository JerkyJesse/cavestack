 ...
 * gbrain see when spawned from cavestack." The bug it prevents: gbrain's
...
describe("buildGbrainEnv", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "cavestack-build-env-"));
  ...
  it("leaves DATABASE_URL untouched when CAVESTACK_RESPECT_ENV_DATABASE_URL=1", () => {
    writeFileSync(join(gbrainHome, "config.json"), JSON.stringify({ database_url: "postgresql://gbrain/db" }));
    const baseEnv = {
      HOME: home,
      DATABASE_URL: "postgresql://intentional/app-db",
      CAVESTACK_RESPECT_ENV_DATABASE_URL: "1",
    };
    const result = buildGbrainEnv({ baseEnv });
