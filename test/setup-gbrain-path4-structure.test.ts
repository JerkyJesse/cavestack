...
//     by unit tests for cavestack-gbrain-mcp-verify and cavestack-artifacts-init.
//   - The structural prose is the source of regressions for AUQ pacing
//     (the failure mode the cavestack repo has tracked since v1.26.x:
...
describe('setup-gbrain Path 4 (Remote MCP) — structural contract', () => {
  ...
  test('Step 4c invokes cavestack-gbrain-mcp-verify and STOPs on failure', () => {
    expect(tmpl).toContain('cavestack-gbrain-mcp-verify');
  ...
  test('Step 7 calls cavestack-artifacts-init with --url-form-supported flag', () => {
    expect(tmpl).toMatch(/cavestack-artifacts-init.*--url-form-supported/);
  });
