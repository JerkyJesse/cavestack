/**
 * Subprocess tests for bin/cavestack-decision-log + bin/cavestack-decision-search.
 * Mirrors the learnings-bins test pattern (run the bin with CAVESTACK_HOME=tmp).
 */
...
const LOG = path.join(ROOT, "bin", "cavestack-decision-log");
const SEARCH = path.join(ROOT, "bin", "cavestack-decision-search");

let tmpDir: string;

function opts(): ExecSyncOptionsWithStringEncoding {
  return { cwd: ROOT, env: { ...process.env, CAVESTACK_HOME: tmpDir }, encoding: "utf-8", timeout: 20000 };
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cavestack-decision-"));
  fs.mkdirSync(path.join(tmpDir, "projects"), { recursive: true });
});
afterEach(() => fs.rmSync(tmpDir, { recursive: true, force: true }));

describe("cavestack-decision-log", () => {
...
describe("cavestack-decision-search", () => {
  ...
  test("redact + compact expunges everywhere", () => {
    ...
    const archive = path.join(tmpDir, "projects", "garrytan-cavestack", "decisions.archive.jsonl");
...
describe("cavestack-decision-search --semantic (optional gbrain enhancement)", () => {
  ...
  function searchWithPath(args: string, pathPrefix?: string): string {
    const env = { ...process.env, CAVESTACK_HOME: tmpDir } as NodeJS.ProcessEnv;
  ...
  test("--semantic --query appends a related-memory block when gbrain returns hits", () => {
    ...
    const dir = shimDir(
      `#!/usr/bin/env bash
if [ "$1" = "sources" ]; then echo '{"sources":[{"id":"default","local_path":"/u/.cavestack-brain-worktree"}]}'; exit 0; fi
...
describe("cavestack-decision-search --recent / --scope / datamark", () => {
