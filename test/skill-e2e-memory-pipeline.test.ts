 ...
 *      ~/.cavestack/ artifacts (eureka, learning, ceo-plan, design-doc, retro,
 *      builder-profile)
 *   2. Run cavestack-memory-ingest --probe → verify stage counts match disk
 *      (post-attribution headline + unattributed skip line, #2394)
 *   3. Run cavestack-memory-ingest --bulk → verify state file gets written +
 *      session_id dedup works on re-run (idempotency)
 *   4. Run cavestack-gbrain-sync --dry-run → verify all 3 stages preview
 *   5. Run cavestack-brain-context-load against a real V1 skill manifest
 ...
 * may or may not be available in CI. We assert on side effects cavestack
...
const INGEST = join(REPO_ROOT, "bin", "cavestack-memory-ingest.ts");
const SYNC = join(REPO_ROOT, "bin", "cavestack-gbrain-sync.ts");
const CONTEXT = join(REPO_ROOT, "bin", "cavestack-brain-context-load.ts");

function makeFixtureHome(): string {
  return mkdtempSync(join(tmpdir(), "cavestack-e2e-pipeline-"));
}

function setupFixture(home: string): { cavestackHome: string; counts: Record<string, number> } {
  ...
  mkdirSync(join(cavestackHome, "projects", "test-repo", "ceo-plans"), { recursive: true });
  mkdirSync(join(cavestackHome, "projects", "test-repo", "retros"), { recursive: true });
  ...
  // cavestack artifacts
  writeFileSync(join(cavestackHome, "analytics", "eureka.jsonl"), '{"insight":"boil the lake"}\n', "utf-8");
  writeFileSync(join(cavestackHome, "builder-profile.jsonl"), '{"date":"2026-05-01","mode":"startup"}\n', "utf-8");
  writeFileSync(join(cavestackHome, "projects", "test-repo", "learnings.jsonl"), '{"key":"a","insight":"b","confidence":8}\n', "utf-8");
  writeFileSync(join(cavestackHome, "projects", "test-repo", "timeline.jsonl"), '{"skill":"office-hours","event":"completed"}\n', "utf-8");
  writeFileSync(join(cavestackHome, "projects", "test-repo", "ceo-plans", "2026-05-01-test.md"), "# CEO Plan: Test\n\nbody\n", "utf-8");
  writeFileSync(join(cavestackHome, "projects", "test-repo", "garrytan-main-design-20260501-090000.md"), "# Design: Test\n", "utf-8");
  writeFileSync(join(cavestackHome, "projects", "test-repo", "retros", "2026-05-01-week.md"), "# Retro\n", "utf-8");
...
describe("V1 memory ingest pipeline E2E", () => {
  it("--probe accounts for all 9 fixture files: 7 attributable + 2 unattributed transcripts skipped (#2394)", () => {
    ...
    const { cavestackHome, counts } = setupFixture(home);
    const env = { HOME: home, CAVESTACK_HOME: cavestackHome, CAVESTACK_MEMORY_INGEST_NO_WRITE: "1" };
    ...
    // both; the cavestack artifacts are store-local and always attributable.
  ...
  it("--incremental writes a state file with schema_version: 1 + last_writer", () => {
    const home = makeFixtureHome();
    const { cavestackHome } = setupFixture(home);
    const env = { HOME: home, CAVESTACK_HOME: cavestackHome, CAVESTACK_MEMORY_INGEST_NO_WRITE: "1" };
  ...
  it("--incremental is idempotent — re-run reports 0 changes", () => {
    ...
    // First run
    runBun(INGEST, ["--incremental", "--quiet"], env);
    const stateAfterFirst = readFileSync(join(cavestackHome, ".transcript-ingest-state.json"), "utf-8");
