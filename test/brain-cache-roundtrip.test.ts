 ...
 *
 * Uses tmp CAVESTACK_HOME per-test to avoid polluting the real ~/.cavestack/.
 * Gate-tier, free, ~50ms.
 */
...
let TMP_HOME: string;
const ORIGINAL_HOME = process.env.CAVESTACK_HOME;

beforeEach(() => {
  TMP_HOME = mkdtempSync(join(tmpdir(), 'cavestack-cache-test-'));
...
afterEach(() => {
  if (ORIGINAL_HOME) process.env.CAVESTACK_HOME = ORIGINAL_HOME;
  else delete process.env.CAVESTACK_HOME;
...
describe('brain-cache paths', () => {
  test('cross-project entity (user-profile) lives in ~/.cavestack/brain-cache/', async () => {
  ...
  test('per-project entity (product) lives in ~/.cavestack/projects/<slug>/brain-cache/', async () => {
    const mod = await importCache();
...
describe('brain-cache state machine', () => {
  test('warm: pre-seeded fresh cache returns warm without touching brain', async () => {
    ...
    writeFileSync(join(cacheDir, '_meta.json'), JSON.stringify({
      schema_version: '1.0.0', // matches CAVESTACK_SCHEMA_PACK_VERSION
      endpoint_hash: mod.detectEndpointHash(),
