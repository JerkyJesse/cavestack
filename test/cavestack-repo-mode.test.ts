/**
 * Project-slug consistency for cavestack-repo-mode.
 *
 * These are end-to-end reproductions of #2212. Each case is a real local Git
 * workspace which predates its public GitHub origin: cavestack-slug caches the
...
const REPO_MODE_BIN = path.join(ROOT, 'bin', 'cavestack-repo-mode');
...
describe('cavestack-repo-mode cached slug consistency (#2212)', () => {
  for (const workspace of publicWorkspaces) {
    test(`${workspace.origin} keeps its pre-origin project cache`, () => {
      ...
      try {
        ...
        git(['config', 'user.name', 'cavestack regression test'], project, home);
        git(['config', 'user.email', 'test@cavestack.dev'], project, home);

        // This workspace existed and used cavestack before it adopted GitHub.
  ...
  test('still treats a workspace without origin as unknown', () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(home, '.cavestack', 'projects'))).toBe(false);
    } finally {
      fs.rmSync(home, { recursive: true, force: true });
  ...
  test('sanitizes unusual origin URLs via cavestack-slug before mkdir', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'grepo-home-'));
    ...
    try {
      ...
      const projects = fs.readdirSync(path.join(home, '.cavestack', 'projects'));
