...
// cavestack-learnings-log is the command from the original bug report: bin scripts
...
function extractKiroBlock(): string {
  const startAnchor = 'KIRO_CAVESTACK="$KIRO_SKILLS/cavestack"';
  const endAnchor = '_link_or_copy "$SOURCE_CAVESTACK_DIR/supabase/config.sh" "$KIRO_CAVESTACK/supabase/config.sh"\n  fi';
...
// cavestack-learnings-log from the installed root and check the learning landed.
function buildRootAndRunCommand(
  isWindows: '0' | '1',
  buildScript: (sandbox: string) => { script: string; rootDir: string },
): CommandResult {
  const sandbox = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-runtime-lib-'));
  try {
    ...
    const run = spawnSync('bash', [path.join(rootDir, 'bin', 'cavestack-learnings-log'), PAYLOAD], {
      cwd: project,
      encoding: 'utf-8',
      timeout: 30000,
      env: { ...process.env, HOME: home, CAVESTACK_HOME: path.join(home, '.cavestack') },
    });

    const projectsDir = path.join(home, '.cavestack', 'projects');
    ...
    return {
      ...
      // $CAVESTACK_DIR/supabase/config.sh to resolve CAVESTACK_SUPABASE_URL. The
...
const HOST_ROOTS: Record<string, (sandbox: string) => { script: string; rootDir: string }> = {
  'agents sidecar': (sandbox) => ({
    script: [
      `SOURCE_CAVESTACK_DIR="${ROOT}"`,
      extractFunction('create_agents_sidecar'),
      `mkdir -p "${sandbox}/repo"`,
      `create_agents_sidecar "${sandbox}/repo"`,
    ].join('\n'),
    rootDir: path.join(sandbox, 'repo', '.agents', 'skills', 'cavestack'),
  }),
  codex: (sandbox) => ({
    script: [
      extractFunction('create_codex_runtime_root'),
      `create_codex_runtime_root "${ROOT}" "${sandbox}/home/.codex/skills/cavestack"`,
    ].join('\n'),
    rootDir: path.join(sandbox, 'home', '.codex', 'skills', 'cavestack'),
  }),
  factory: (sandbox) => ({
    script: [
      extractFunction('create_factory_runtime_root'),
      `create_factory_runtime_root "${ROOT}" "${sandbox}/home/.factory/skills/cavestack"`,
    ].join('\n'),
    rootDir: path.join(sandbox, 'home', '.factory', 'skills', 'cavestack'),
  }),
  opencode: (sandbox) => ({
    script: [
      extractFunction('create_opencode_runtime_root'),
      `create_opencode_runtime_root "${ROOT}" "${sandbox}/home/.opencode/skills/cavestack"`,
    ].join('\n'),
    rootDir: path.join(sandbox, 'home', '.opencode', 'skills', 'cavestack'),
  }),
  kiro: (sandbox) => ({
    script: [
      `HOME="${sandbox}/home"`,
    ...
    rootDir: path.join(sandbox, 'home', '.kiro', 'skills', 'cavestack'),
  }),
};
...
describe.skipIf(process.platform === 'win32')('setup: bin commands resolve sibling lib from every host root', () => {
  for (const [host, buildScript] of Object.entries(HOST_ROOTS)) {
    ...
    test(`${host} root (Windows copy install): cavestack-learnings-log imports ../lib and writes the learning`, () => {
