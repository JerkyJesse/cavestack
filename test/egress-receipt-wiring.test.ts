 ...
 * agent-executed instructions, not a cavestack binary, so it is covered by the
...
const POLARITY: Record<string, 'fail-closed' | 'fail-open'> = {
  // fail-closed: cavestack state leaving the machine unrecorded is worse than
...
const MODULE_SINKS = [
  'bin/cavestack-gbrain-sync.ts',
  'bin/cavestack-memory-ingest.ts',
  ...
  'lib/context-bill.ts',
  // supabase-provision engine (bin/cavestack-gbrain-supabase-provision is a thin
...
const SHELL_SINKS = [
  'bin/cavestack-telemetry-sync',
  'bin/cavestack-update-check',
  'bin/cavestack-brain-sync',
  'bin/cavestack-gbrain-mcp-verify',
  'bin/cavestack-security-dashboard',
  'bin/cavestack-community-dashboard',
  'bin/cavestack-artifacts-init',
  'bin/cavestack-brain-restore',
  'bin/cavestack-session-update',
];
...
const SCANNER_EXEMPT: Record<string, string> = {
  'bin/cavestack-team-init':
    'every git clone is inside an echoed instruction string (install docs); the script executes no network ops',
  'bin/cavestack-gbrain-install':
    'user-invoked installer: bodyless HEAD reachability probe to github.com + clone of the public gbrain repo (user-directed install; no cavestack state leaves the machine)',
  'bin/cavestack-next-version':
    'fetches the user\'s own repo\'s base branch for version-claim freshness — a user-repo dev-workflow op, not cavestack-state egress',
  'bin/cavestack-version-bump':
    'git fetch appears only in an error-message string',
  'bin/cavestack-redact-prepush':
  ...
  // not cavestack binaries. Includes the preamble-generated brain-sync block —
  // see the header.
  'scripts/resolvers':
    'skill prose templates — agent-executed instructions rendered into SKILL.md, not cavestack binaries',
};
...
const RECEIPT_MARKER =
  /_receipted_(curl|git|version_fetch)\b|cavestack-egress-receipt["']?\s+write\b|writeReceipt\(|receiptedFetch\(/;
...
describe('egress receipt wiring tripwire', () => {
  ...
  test('every shell sink sources cavestack-egress-lib.sh', () => {
    for (const rel of SHELL_SINKS) {
      const src = read(rel);
      expect(
        src.includes('cavestack-egress-lib.sh'),
        `${rel}: must source bin/cavestack-egress-lib.sh for _receipted_* helpers`,
      ).toBe(true);
    }
  });
  ...
  test('deprecated dead-endpoint brain consumer/reader scripts stay deleted', () => {
    ...
    for (const rel of ['bin/cavestack-brain-consumer', 'bin/cavestack-brain-reader']) {
      let present = true;
      try {
  ...
  test('polarity spot-checks: closed sinks refuse, open sinks warn', () => {
    ...
    expect(read('bin/cavestack-telemetry-sync')).toMatch(/_receipted_curl closed telemetry-sync/);
    // brain-sync (closed): refusal exits before the commit consumes the queue.
    expect(read('bin/cavestack-brain-sync')).toMatch(/cavestack-egress-receipt["']? write/);
    // update-check (open).
    expect(read('bin/cavestack-update-check')).toMatch(/_receipted_curl open update-check/);
    // dashboards (open).
    expect(read('bin/cavestack-security-dashboard')).toMatch(/_receipted_curl open security-dashboard/);
    expect(read('bin/cavestack-community-dashboard')).toMatch(/_receipted_curl open community-dashboard/);
    // mcp-verify (closed).
    expect(read('bin/cavestack-gbrain-mcp-verify')).toMatch(/_receipted_curl closed gbrain-mcp-verify/);
  ...
  test('shebang tripwire: no bin/cavestack-* file carries a node shebang (amendment 2A)', () => {
    const offenders: string[] = [];
    for (const entry of fs.readdirSync(path.join(ROOT, 'bin'))) {
      if (!entry.startsWith('cavestack-')) continue;
