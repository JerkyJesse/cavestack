 ...
 * (~/.cavestack/render/claude) and makes the Claude installers — setup's
 * link_claude_skill_dirs AND bin/cavestack-relink — prefer rendered files when
...
const CONFIG_SRC = fs.readFileSync(path.join(ROOT, 'bin', 'cavestack-config'), 'utf-8');
const RELINK_SRC = fs.readFileSync(path.join(ROOT, 'bin', 'cavestack-relink'), 'utf-8');
const MIGRATION = path.join(ROOT, 'cavestack-upgrade', 'migrations', 'v1.67.0.0.sh');
...
describe(':user render targets the out-dir, never the checkout (#2569)', () => {
  ...
  test('setup renders into a TMP dir, swaps on success, repoints after (never wipes first)', () => {
    ...
    expect(block).toContain('--out-dir "$_CAVESTACK_RENDER_TMP"');
    expect(block).not.toContain('--out-dir "$_CAVESTACK_RENDER_DIR"');
    expect(block).toContain('_swap_in_render "$_CAVESTACK_RENDER_DIR" "$_CAVESTACK_RENDER_TMP"');
    expect(block).toContain('link_claude_skill_dirs "$SOURCE_CAVESTACK_DIR" "$INSTALL_SKILLS_DIR"');
    // Stale-render cleanup on the gbrain-gone path (a deliberate wipe).
    expect(block).toContain('gbrain not detected');
    expect(block).toContain('rm -rf "$_CAVESTACK_RENDER_DIR"');
  });

  test('cavestack-config gbrain-refresh renders to a TMP out-dir, swaps on success', () => {
    ...
    expect(CONFIG_SRC).toContain('cavestack-relink');
  ...
  test('_swap_in_render behavior: success replaces, and the shape means failure never touches the live dir', () => {
    ...
    for (const src of [SETUP_SRC, CONFIG_SRC]) {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-render-swap-'));
      try {
  ...
  test('a FAILED render leaves the previous render dir fully intact (cavestack-config path, end-to-end shape)', () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-render-fail-'));
    try {
      const live = path.join(tmp, 'claude');
      fs.mkdirSync(path.join(live, 'ship'), { recursive: true });
  ...
  test('cavestack-relink prefers the render dir when a rendered SKILL.md exists', () => {
...
describe('link_claude_skill_dirs prefers rendered SKILL.md (behavior)', () => {
  test('a rendered variant is served; skills without one fall back to source', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-render-pref-'));
    try {
      const src = path.join(tmp, 'src');
      const skills = path.join(tmp, 'skills');
      const home = path.join(tmp, 'cavestack-home');
...
describe('migration v1.67.0.0 — legacy in-place render cleanup (F12)', () => {
  ...
  function runMigration(install: string): { status: number | null; stdout: string } {
    const r = spawnSync('bash', [MIGRATION], {
      encoding: 'utf-8',
      env: { ...process.env, CAVESTACK_INSTALL_DIR: install },
      timeout: 15_000,
    });
  ...
  test('restores render-class dirt, leaves user changes alone, idempotent', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-migration-'));
    try {
  ...
  test('clean checkout, missing dir, and symlinked install are all silent no-ops', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-migration-noop-'));
    try {
      const install = makeLegacyInstall(tmp);
