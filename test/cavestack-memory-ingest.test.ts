/**
 * Unit tests for bin/cavestack-memory-ingest.ts (Lane A).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "cavestack-memory-ingest.ts");

// ── Helpers ────────────────────────────────────────────────────────────────

function makeTestHome(): string {
  return mkdtempSync(join(tmpdir(), "cavestack-memory-ingest-"));
}
...
describe("cavestack-memory-ingest CLI", () => {
  it("prints usage on --help and exits 0", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: cavestack-memory-ingest");
  ...
  it("--probe on empty home reports 0 files", () => {
    ...
    const r = runScript(["--probe"], { HOME: home, CAVESTACK_HOME: cavestackHome });
    expect(r.exitCode).toBe(0);
  ...
  it("--probe finds Claude Code sessions", () => {
    ...
    const r = runScript(["--probe", "--include-unattributed"], { HOME: home, CAVESTACK_HOME: cavestackHome });
    expect(r.exitCode).toBe(0);
  ...
  it("--probe finds cavestack artifacts (learnings, eureka, ceo-plan)", () => {
    const home = makeTestHome();
    const cavestackHome = join(home, ".cavestack");
    mkdirSync(join(cavestackHome, "analytics"), { recursive: true });
    mkdirSync(join(cavestackHome, "projects", "foo-bar", "ceo-plans"), { recursive: true });

    writeFileSync(join(cavestackHome, "analytics", "eureka.jsonl"), '{"insight":"lake first"}\n');
    writeFileSync(join(cavestackHome, "projects", "foo-bar", "learnings.jsonl"), '{"key":"a","insight":"b"}\n');
    writeFileSync(join(cavestackHome, "projects", "foo-bar", "ceo-plans", "2026-05-01-test.md"), "# Plan\n");
  ...
  it("--sources filter limits the walk to specific types", () => {
    ...
    const cavestackHome = join(home, ".cavestack");
    mkdirSync(join(cavestackHome, "analytics"), { recursive: true });
    mkdirSync(join(cavestackHome, "projects", "foo", "ceo-plans"), { recursive: true });

    writeFileSync(join(cavestackHome, "analytics", "eureka.jsonl"), '{"insight":"x"}\n');
    writeFileSync(join(cavestackHome, "projects", "foo", "learnings.jsonl"), '{"key":"a"}\n');

    const r = runScript(["--probe", "--sources", "eureka"], { HOME: home, CAVESTACK_HOME: cavestackHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
    expect(r.stdout).toContain("eureka");
...
describe("cavestack-memory-ingest state file", () => {
  it("--incremental on empty home creates state file with schema_version: 1", () => {
    const home = makeTestHome();
    const cavestackHome = join(home, ".cavestack");
    mkdirSync(cavestackHome, { recursive: true });
    const r = runScript(["--incremental", "--quiet"], { HOME: home, CAVESTACK_HOME: cavestackHome });
    expect(r.exitCode).toBe(0);
    const statePath = join(cavestackHome, ".transcript-ingest-state.json");
    expect(existsSync(statePath)).toBe(true);
    const state = JSON.parse(readFileSync(statePath, "utf-8"));
    expect(state.schema_version).toBe(1);
    expect(state.last_writer).toBe("cavestack-memory-ingest");
    rmSync(home, { recursive: true, force: true });
  });
...
describe("cavestack-memory-ingest security: untrusted cwd cannot trigger shell substitution", () => {
  it("does not invoke /bin/sh when a transcript record contains $() in cwd", () => {
    ...
    const markerDir = mkdtempSync(join(tmpdir(), "cavestack-mi-cwd-marker-"));
    ...
    const r = runScript(["--incremental", "--quiet"], {
      ...
      CAVESTACK_MEMORY_INGEST_NO_WRITE: "1",
    });
...
describe("internal: parseTranscriptJsonl + buildTranscriptPage shape", () => {
  it("parses a Claude Code JSONL session", async () => {
    const dir = mkdtempSync(join(tmpdir(), "cavestack-mi-parse-"));
    ...
    const r = runScript(["--probe", "--include-unattributed"], { HOME: home, CAVESTACK_HOME: join(home, ".cavestack") });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
...
describe("cavestack-memory-ingest --limit", () => {
  it("respects --limit by stopping after N writes (mocked via --probe shortcut)", () => {
    ...
    const cavestackHome = join(home, ".cavestack");
    mkdirSync(cavestackHome, { recursive: true });
    const r = runScript(["--probe", "--limit", "1"], { HOME: home, CAVESTACK_HOME: cavestackHome });
...
describe("cavestack-memory-ingest writer (gbrain v0.20+ batch `import` interface)", () => {
  ...
  // (real-world cause: cavestack-artifacts-init writes `.gitignore = "*"` into
  // $CAVESTACK_HOME, and `gbrain import` honours .gitignore, so every file staged
  // under $CAVESTACK_HOME is invisible to it).
  //
...
describe("#2105 codex response_item rollout shape", () => {
  it("extracts messages from response_item records", async () => {
    const { parseTranscriptJsonl } = await import("../bin/cavestack-memory-ingest");
...
describe("#2394: probe applies the same attribution gate as prepare", () => {
  ...
  it("parity: probe post-attribution count equals what prepare actually processes", () => {
    ...
    const probe = runScript(["--probe"], { HOME: home, CAVESTACK_HOME: cavestackHome });
    ...
    const inc = runScript(["--incremental", "--quiet"], { HOME: home, CAVESTACK_HOME: cavestackHome });
  ...
  it("parity: a Codex cwd appearing only on a LATER record is unattributed in probe AND prepare", () => {
    ...
    const inc = runScript(["--incremental"], { HOME: home, CAVESTACK_HOME: cavestackHome });
...
//
// The same store the code-import gate honors (bin/cavestack-gbrain-sync.ts):
...
// The policy store is seeded through the REAL bin/cavestack-gbrain-repo-policy
// script (its `set` verb owns the file schema + URL normalization).

describe("#2392: transcript ingest honors per-remote trust policy", () => {
  const POLICY_BIN = join(import.meta.dir, "..", "bin", "cavestack-gbrain-repo-policy");
  ...
  function setPolicy(cavestackHome: string, url: string, tier: string): void {
    const r = spawnSync(POLICY_BIN, ["set", url, tier], {
      encoding: "utf-8",
      env: { ...process.env, CAVESTACK_HOME: cavestackHome },
    });
    expect(r.status).toBe(0);
  }

  function stateSessions(cavestackHome: string): string[] {
    const statePath = join(cavestackHome, ".transcript-ingest-state.json");
  ...
  it("(a) deny remote's transcript is skipped and counted as skipped_policy_deny", () => {
    ...
    setPolicy(cavestackHome, "https://github.com/denyme/denied.git", "deny");
    ...
    expect(existsSync(logFile)).toBe(true);
    const sessions = stateSessions(cavestackHome);
  ...
  it("(b) read-only remote's transcript is skipped and counted as skipped_policy_readonly", () => {
    ...
    writeSessionForRepo(home, "work-allowed", "oksess1", okCwd);
    setPolicy(cavestackHome, "https://github.com/roorg/rorepo.git", "read-only");
  ...
  it("(c) read-write remote's transcript is ingested (reaches gbrain import)", () => {
    ...
    setPolicy(cavestackHome, "https://github.com/rworg/rwrepo.git", "read-write");
    ...
    const calls = readFileSync(logFile, "utf-8").trim().split("\n").filter(Boolean);
    expect(calls.length).toBe(1);
    expect(stateSessions(cavestackHome).length).toBe(1);
  ...
  it("(d) corrupted store: hard error before any writes, message names recovery", () => {
    ...
    writeFileSync(join(cavestackHome, "gbrain-repo-policy.json"), "not valid json{", "utf-8");
    ...
    expect(r.stderr).toContain("cavestack-gbrain-repo-policy list");
    expect(r.stderr).toContain("/setup-gbrain");
    // Nothing written: no gbrain import call, no state file, store untouched.
    expect(existsSync(logFile)).toBe(false);
    expect(stateSessions(cavestackHome).length).toBe(0);
    expect(readFileSync(join(cavestackHome, "gbrain-repo-policy.json"), "utf-8")).toBe("not valid json{");
  ...
  it("artifacts are never policy-filtered, even when their project's remote is denied", () => {
    const home = makeTestHome();
    const cavestackHome = join(home, ".cavestack");
    mkdirSync(join(cavestackHome, "projects", "denyme-denied"), { recursive: true });
    const { binDir } = installFakeGbrain(home);
    ...
    writeFileSync(join(cavestackHome, "projects", "denyme-denied", "learnings.jsonl"), '{"key":"a","insight":"b"}\n');
    setPolicy(cavestackHome, "https://github.com/denyme/denied.git", "deny");
