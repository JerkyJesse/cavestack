 ...
 * on PATH), runs a cheap cavestack skill through the Agent SDK, intercepts
...
describeE2E('gbrain-sync privacy gate fires once via preamble', () => {
  test('cavestack skill preamble fires the 3-option AskUserQuestion when gbrain is detected', async () => {
    // Stage a fresh CAVESTACK_HOME with artifacts_sync_mode_prompted=false.
    const cavestackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-cavestack-'));
    ...
    fs.writeFileSync(
      path.join(cavestackHome, 'config.yaml'),
    ...
    // break auth). Ambient process.env.CAVESTACK_HOME mutation does NOT work
    // here — hermetic-env scrubs CAVESTACK_* and repoints CAVESTACK_HOME at its
    ...
    try {
      ...
      await runAgentSdkTest({
        ...
        workingDirectory: cavestackHome,
        maxTurns: 10,
  ...
  test('privacy gate does NOT fire when artifacts_sync_mode_prompted is already true', async () => {
    ...
    const cavestackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-off-'));
    ...
    // first test — ambient CAVESTACK_HOME mutation is scrubbed by hermetic-env).
    const childEnv = {
      CAVESTACK_HOME: cavestackHome,
      HOME: tempHome,
      PATH: `${fakeBinDir}:${process.env.PATH ?? '/usr/bin:/bin:/opt/homebrew/bin'}`,
