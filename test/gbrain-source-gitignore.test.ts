 ...
 * `.gbrain-source` to cavestack's own `.gitignore`. Without the consumer-side
...
import { ensureGbrainSourceGitignored } from "../bin/cavestack-gbrain-sync";

describe("ensureGbrainSourceGitignored", () => {
  let root: string;

  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "cavestack-gbrain-gitignore-"));
  });
