...
const CAVESTACK_CONFIG = path.join(ROOT, 'bin', 'cavestack-config');
...
describe('setup: plan-tune hooks are non-interactive-safe', () => {
  ...
  test('resolution falls through env then saved config', () => {
    expect(setupSrc).toContain('CAVESTACK_PLAN_TUNE_HOOKS');
...
describe('setup: PT_EXPLICIT provenance (Conductor auto-opt-in respects explicit decisions)', () => {
  ...
  test('config provenance uses cavestack-config has (env-resolution-safe), never a hardcoded config grep', () => {
    // `cavestack-config get` returns the default "prompt" for absent keys, so
    // key PRESENCE must come from `has`, which resolves CAVESTACK_STATE_ROOT /
    // CAVESTACK_HOME / CAVESTACK_STATE_DIR the same way `get` does. A hardcoded
    // grep of ~/.cavestack/config.yaml misclassifies under env overrides.
    expect(setupSrc).toMatch(/"\$CAVESTACK_CONFIG" has plan_tune_hooks/);
    expect(setupSrc).not.toMatch(/grep -q ["']\^plan_tune_hooks:/);
  });
});

describe('cavestack-config: has subcommand (key-presence provenance)', () => {
  let tmpHome2: string;
  let env2: NodeJS.ProcessEnv;

  beforeAll(() => {
    tmpHome2 = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-cfg-has-'));
    env2 = { ...process.env, CAVESTACK_STATE_ROOT: tmpHome2 };
  });
  ...
  function has(key: string): number {
    try {
      execSync(`${CAVESTACK_CONFIG} has '${key}'`, { encoding: 'utf-8', env: env2 });
      return 0;
    } catch (e: any) {
      return e.status ?? 1;
    }
  ...
  test('absent key exits nonzero even though get returns the default', () => {
    ...
    const got = execSync(`${CAVESTACK_CONFIG} get plan_tune_hooks`, { encoding: 'utf-8', env: env2 }).trim();
  ...
  test('present key exits 0 through the same STATE_DIR resolution as get', () => {
    execSync(`${CAVESTACK_CONFIG} set plan_tune_hooks no`, { encoding: 'utf-8', env: env2 });
    expect(has('plan_tune_hooks')).toBe(0);
    // CAVESTACK_STATE_ROOT was the writer — a hardcoded ~/.cavestack grep would miss it.
  });
...
describe('cavestack-config: plan_tune_hooks key', () => {
  // Isolate state: cavestack-config reads $CAVESTACK_HOME/config.yaml. Point it at a
  // fresh temp dir so `get` returns the built-in default rather than whatever
  // the host machine has in ~/.cavestack/config.yaml (which would make the
  // default-value assertion non-deterministic).
  let tmpHome: string;
  let env: NodeJS.ProcessEnv;

  beforeAll(() => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-cfg-test-'));
  ...
  test('default is "prompt"', () => {
    const out = execSync(`${CAVESTACK_CONFIG} get plan_tune_hooks`, {
      encoding: 'utf-8',
      env,
    }).trim();
  ...
  test('appears in defaults and list output', () => {
    const defaults = execSync(`${CAVESTACK_CONFIG} defaults`, { encoding: 'utf-8', env });
    expect(defaults).toContain('plan_tune_hooks');
    const list = execSync(`${CAVESTACK_CONFIG} list`, { encoding: 'utf-8', env });
    expect(list).toContain('plan_tune_hooks');
  });

  test('accepts valid values (round-trips yes/no/prompt)', () => {
    for (const v of ['yes', 'no', 'prompt']) {
      execSync(`${CAVESTACK_CONFIG} set plan_tune_hooks ${v}`, { encoding: 'utf-8', env });
      const got = execSync(`${CAVESTACK_CONFIG} get plan_tune_hooks`, { encoding: 'utf-8', env }).trim();
      expect(got).toBe(v);
    }
  });

  test('rejects out-of-domain values (warns + falls back to prompt)', () => {
    const res = execSync(`${CAVESTACK_CONFIG} set plan_tune_hooks maybe 2>&1`, { encoding: 'utf-8', env });
