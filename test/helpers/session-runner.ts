...
export async function runSkillTest(options: {
   ...
   *  per-test CAVESTACK_HOME overrides so the test doesn't have to spell out
  ...
  const proc = Bun.spawn(['claude', ...args], {
    ...
    // session context (CONDUCTOR_*, CLAUDECODE, ~/.claude config, ~/.cavestack)
    // never reaches the child; EVALS_HERMETIC=0 restores the legacy env.
    // Default CAVESTACK_HEADLESS=1 so eval/E2E runs classify as headless (BLOCK on an
    ...
    // `env: { CAVESTACK_HEADLESS: '' }` — extraEnv wins because it spreads last.
    env: hermeticChildEnv({ CAVESTACK_HEADLESS: '1', ...extraEnv }),
