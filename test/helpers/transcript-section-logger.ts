...
export const SHIP_ACTIONS = [
  ...
  'bumped_version',    // wrote VERSION / package.json version / ran cavestack-version-bump
...
const BASH_ACTION_PATTERNS: Array<{ action: ShipAction; re: RegExp }> = [
  ...
  { action: 'bumped_version', re: /cavestack-version-bump\b|cavestack-next-version\b|>\s*VERSION\b|npm\s+version\b/ },
...
const DEFAULT_BASELINE_DIR = path.join(os.homedir(), '.cavestack-dev', 'ship-baselines');
