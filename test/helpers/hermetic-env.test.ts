...
const CONTAMINATED: NodeJS.ProcessEnv = {
  ...
  CAVESTACK_HOME: '/Users/op/.cavestack',
  CAVESTACK_HEADLESS_DEFAULT: 'x',
  MCP_TIMEOUT: '5000',
...
const HERMETIC_VARS = { CLAUDE_CONFIG_DIR: '/x/.claude', CAVESTACK_HOME: '/x/cavestack-home' };
...
describe('buildHermeticEnv allowlist', () => {
  ...
  test('drops session-context and operator-credential vars', () => {
    for (const k of [
      ...
      'CLAUDE_CODE_ENTRYPOINT', 'CAVESTACK_HEADLESS_DEFAULT', 'MCP_TIMEOUT',
  ...
  test('redirects CLAUDE_CONFIG_DIR and CAVESTACK_HOME to hermetic values', () => {
    expect(env.CLAUDE_CONFIG_DIR).toBe('/x/.claude');
    expect(env.CAVESTACK_HOME).toBe('/x/cavestack-home');
  });
  ...
  test('overrides merge last — per-test re-contamination is deliberate', () => {
    const e = buildHermeticEnv(CONTAMINATED, HERMETIC_VARS, {
      ...
      CAVESTACK_HOME: '/tmp/test-home',
      CAVESTACK_HEADLESS: '',
    });
    expect(e.CONDUCTOR_WORKSPACE_PATH).toBe('/tmp/test-ws');
    expect(e.CAVESTACK_HOME).toBe('/tmp/test-home');
    expect(e.CAVESTACK_HEADLESS).toBe('');
  });

  test('promotes CAVESTACK_ANTHROPIC_API_KEY when canonical absent (shared shim fn)', () => {
    const base = { ...CONTAMINATED } as NodeJS.ProcessEnv;
    delete base.ANTHROPIC_API_KEY;
    base.CAVESTACK_ANTHROPIC_API_KEY = 'sk-ant-promoted-9876543210';
    const e = buildHermeticEnv(base, HERMETIC_VARS);
    expect(e.ANTHROPIC_API_KEY).toBe('sk-ant-promoted-9876543210');
    expect(e.CAVESTACK_ANTHROPIC_API_KEY).toBeUndefined(); // CAVESTACK_* still dropped
  });
...
describe('EVALS_HERMETIC=0 escape hatch', () => {
  test('returns byte-identical legacy env, overrides still last', () => {
    ...
    const e = buildHermeticEnv(base, HERMETIC_VARS, { CAVESTACK_HEADLESS: '1' });
    ...
    expect(e.CAVESTACK_HOME).toBe('/Users/op/.cavestack');
    expect(e.CAVESTACK_HEADLESS).toBe('1');
    expect(e).toEqual({ ...(base as Record<string, string>), CAVESTACK_HEADLESS: '1' });
  });
...
describe('gcStaleHermeticDirs', () => {
  test('removes dead-pid dirs, keeps live-pid and foreign dirs', () => {
    ...
    const dead = path.join(tmp, `cavestack-hermetic-${deadPid}-abc`);
    const live = path.join(tmp, `cavestack-hermetic-${process.pid}-abc`);
    const foreign = path.join(tmp, 'unrelated-dir');
    const malformed = path.join(tmp, 'cavestack-hermetic-notapid-abc');
  ...
  test('keeps a fresh dead-pid dir (PID-reuse grace window)', () => {
    ...
    const freshDead = path.join(tmp, 'cavestack-hermetic-99999999-xyz');
...
describe('hermeticChildEnv composition', () => {
  test('hermetic by default: redirects config dirs, drops contamination', () => {
    ...
    const e = hermeticChildEnv({ CAVESTACK_HEADLESS: '1' });
    const dirs = getHermeticDirs();
    expect(e.CLAUDE_CONFIG_DIR).toBe(dirs.configDir);
    expect(e.CAVESTACK_HOME).toBe(dirs.cavestackHome);
    expect(e.CAVESTACK_HEADLESS).toBe('1');
