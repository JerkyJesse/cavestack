 ...
 * skills), ~/.cavestack decision logs, and CONDUCTOR_-/CLAUDECODE-style env vars.
 ...
 *   │   AUTH_TOKEN                │
 *   │ CAVESTACK_ANTHROPIC_API_KEY    │── promotedEnv() ─────► ANTHROPIC_API_KEY
 *   │ CONDUCTOR_*, CLAUDECODE,    │
 *   │ CLAUDE_*, CAVESTACK_*, MCP_*,  │── dropped ───────────► ∅
 *   │ GBRAIN_*, GH_TOKEN, ...     │
 *   └─────────────────────────────┘
 *      + per-runner extraAllow (codex: OpenAI vars; gemini: Google vars)
 *      + CLAUDE_CONFIG_DIR=<runRoot>/.claude  CAVESTACK_HOME=<runRoot>/cavestack-home
 ...
 * CAVESTACK_* / MCP_* / GBRAIN_* — session-context contamination; and operator
 ...
 * (CAVESTACK_HOME, CONDUCTOR_WORKSPACE_PATH, CAVESTACK_HEADLESS opt-out) always
...
const DIR_PREFIX = 'cavestack-hermetic-';
...
export function getHermeticDirs(): HermeticDirs {
  ...
  const cavestackHome = path.join(runRoot, 'cavestack-home');
  ...
  try {
    ...
    const seed = buildSeedConfig({
      apiKey: process.env.ANTHROPIC_API_KEY ?? process.env.CAVESTACK_ANTHROPIC_API_KEY,
  ...
  cachedDirs = { configDir, cavestackHome, runRoot };
  return cachedDirs;
}
 ...
 * (dir-name fallback), NO cavestack- prefix; the root SKILL.md router registers
 * as `_cavestack-command`. skillCensus().registryEntries is the authoritative
 ...
 *   `~/.claude/skills/cavestack/...` preamble references inside each SKILL.md
...
export function hermeticSkillsConfigDir(): string {
  ...
  for (const rel of skillCensus(root).physicalSkillFiles) {
    ...
    // symlink to open-cavestack-browser) shares its target's frontmatter name,
...
export function hermeticChildEnv(
  ...
  return buildHermeticEnv(
    process.env,
    { CLAUDE_CONFIG_DIR: dirs.configDir, CAVESTACK_HOME: dirs.cavestackHome },
    overrides,
    opts,
  );
}
