...
export const E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'first-task-scaffold': ['bin/cavestack-first-task-detect', 'scripts/resolvers/preamble/generate-first-run-guidance.ts', 'scripts/resolvers/preamble/generate-preamble-bash.ts', 'test/skill-e2e-first-task-scaffold.test.ts', 'test/helpers/session-runner.ts'],
  ...
  'auto-decide-preserved':        ['scripts/resolvers/question-tuning.ts', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-preamble-bash.ts', 'scripts/resolvers/preamble/generate-completion-status.ts', 'plan-ceo-review/**', 'bin/cavestack-question-preference', 'bin/cavestack-config', 'bin/cavestack-slug', 'hosts/claude/hooks/question-preference-hook.ts', 'lib/is-conductor.ts', 'test/helpers/claude-pty-runner.ts'],
  ...
  'ship-idempotency-pty':        ['ship/**', 'bin/cavestack-next-version', 'bin/cavestack-version-bump', 'scripts/resolvers/sections.ts', 'lib/worktree.ts', 'test/helpers/claude-pty-runner.ts', 'test/skill-e2e-ship-idempotency.test.ts'],
  ...
  // the selector sets CAVESTACK_CARVE_SKILL=<name> to scope cost to the changed
  ...
  'plan-ceo-split-overflow': ['plan-ceo-review/**', 'scripts/resolvers/preamble.ts', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'bin/cavestack-question-preference', 'test/helpers/claude-pty-runner.ts', 'test/fixtures/forcing-finding-seeds.ts', 'test/skill-e2e-plan-ceo-split-overflow.test.ts'],
  'brain-privacy-gate':           ['scripts/resolvers/preamble/generate-brain-sync-block.ts', 'scripts/resolvers/preamble.ts', 'bin/cavestack-brain-sync', 'bin/cavestack-artifacts-init', 'bin/cavestack-config', 'test/helpers/agent-sdk-runner.ts', 'test/skill-e2e-brain-privacy-gate.test.ts'],
  ...
  'setup-gbrain-remote':          ['setup-gbrain/SKILL.md.tmpl', 'bin/cavestack-gbrain-mcp-verify', 'bin/cavestack-artifacts-init', 'bin/cavestack-gbrain-detect', 'test/helpers/agent-sdk-runner.ts', 'test/skill-e2e-setup-gbrain-remote.test.ts'],
  'setup-gbrain-bad-token':       ['setup-gbrain/SKILL.md.tmpl', 'bin/cavestack-gbrain-mcp-verify', 'test/helpers/agent-sdk-runner.ts', 'test/skill-e2e-setup-gbrain-bad-token.test.ts'],
  ...
  'setup-gbrain-path4-local-pglite': ['setup-gbrain/SKILL.md.tmpl', 'bin/cavestack-gbrain-mcp-verify', 'bin/cavestack-gbrain-install', 'bin/cavestack-gbrain-detect', 'lib/gbrain-local-status.ts', 'test/helpers/agent-sdk-runner.ts', 'test/skill-e2e-setup-gbrain-path4-local-pglite.test.ts'],
  ...
  'plan-tune-inspect':         ['plan-tune/**', 'scripts/question-registry.ts', 'scripts/psychographic-signals.ts', 'scripts/one-way-doors.ts', 'bin/cavestack-question-log', 'bin/cavestack-question-preference', 'bin/cavestack-developer-profile'],

  // /plan-tune cathedral (T16 — 5 E2E scenarios, all gate per D12)
  'plan-tune-hook-capture':      ['hosts/claude/hooks/**', 'bin/cavestack-question-log', 'bin/cavestack-developer-profile', 'plan-tune/**'],
  'plan-tune-enforcement':       ['hosts/claude/hooks/**', 'bin/cavestack-question-preference', 'scripts/question-registry.ts'],
  ...
  'plan-tune-codex-import':      ['bin/cavestack-codex-session-import', 'bin/cavestack-question-log', 'docs/spikes/codex-session-format.md'],
  'plan-tune-dream-cycle':       ['bin/cavestack-distill-free-text', 'bin/cavestack-distill-apply', 'hosts/claude/hooks/**', 'plan-tune/**'],
  ...
  // Ship
  'ship-base-branch': ['ship/**', 'bin/cavestack-repo-mode', 'test/skill-e2e-review-attribution.test.ts'],
  ...
  'context-save-writes-file':       ['context-save/**', 'bin/cavestack-slug'],
  'context-restore-loads-latest':   ['context-restore/**', 'bin/cavestack-slug'],
  ...
  'context-save-then-restore-roundtrip':   ['context-save/**', 'context-restore/**', 'bin/cavestack-slug'],
  ...
  'ship-triage': ['ship/**', 'bin/cavestack-repo-mode', 'test/skill-e2e-triage.test.ts'],
  ...
  'autoplan-dual-voice': ['autoplan/**', 'codex/**', 'bin/cavestack-codex-probe', 'scripts/resolvers/review.ts', 'scripts/resolvers/design.ts'],

  // Multi-provider benchmark adapters — live API smoke against real claude/codex/gemini CLIs
  'benchmark-providers-live': ['bin/cavestack-model-benchmark', 'test/helpers/providers/**', 'test/helpers/benchmark-runner.ts', 'test/helpers/pricing.ts', 'test/skill-e2e-benchmark-providers.test.ts'],
  ...
  // path requires CAVESTACK_HAS_IOS_DEVICE=1 and is periodic-tier.
  ...
  // Real-device path — only runs with CAVESTACK_HAS_IOS_DEVICE=1 + a paired
  ...
  'office-hours-brain-writeback': [
    ...
    'bin/cavestack-gbrain-detect',
    'bin/cavestack-config',
  ...
  // T11). Proves the gbrain CLI persistence contract cavestack relies on —
...
export const E2E_TIERS: Record<string, 'gate' | 'periodic'> = {
  ...
  'carve-section-loading':     'periodic',   // ~$1-2/skill, data-driven; CAVESTACK_CARVE_SKILL scopes to one
  ...
  // Privacy gate for cavestack-brain-sync — periodic (non-deterministic LLM call,
  ...
  // dependency, deterministic). with-device path requires CAVESTACK_HAS_IOS_DEVICE.
  'ios-qa-e2e': 'gate',
