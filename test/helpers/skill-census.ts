 ...
 *
 *   1. `connect-chrome/` is a directory SYMLINK to `open-cavestack-browser/`.
 ...
 *      alias name `_cavestack-command`, not as an authored skill.
 *
...
export interface SkillCensus {
   ...
   * plus the `_cavestack-command` root alias when the root router exists.
...
export function skillCensus(root: string): SkillCensus {
  ...
  if (fs.existsSync(path.join(root, 'SKILL.md'))) {
    ...
    registrySet.add('_cavestack-command');
  }
