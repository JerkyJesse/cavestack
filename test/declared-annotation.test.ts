...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-annot-'));
  prevStateRoot = process.env.CAVESTACK_STATE_ROOT;
  prevHome = process.env.CAVESTACK_HOME;
  process.env.CAVESTACK_STATE_ROOT = stateRoot;
  delete process.env.CAVESTACK_HOME;
});

afterEach(() => {
  if (prevStateRoot !== undefined) process.env.CAVESTACK_STATE_ROOT = prevStateRoot;
  else delete process.env.CAVESTACK_STATE_ROOT;
  if (prevHome !== undefined) process.env.CAVESTACK_HOME = prevHome;
