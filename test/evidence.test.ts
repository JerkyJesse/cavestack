...
const EVIDENCE = path.join(ROOT, 'bin', 'cavestack-evidence');
...
function ledgerFile(): string {
  const found = findFilesBySuffix(path.join(cavestackHome, 'projects'), '-evidence.jsonl');
...
beforeEach(() => {
  cavestackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-evidence-home-'));
  repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-evidence-repo-'));
  git('init -q -b main');
...
describe('cavestack-evidence run', () => {
...
describe('cavestack-evidence check', () => {
  ...
  test('check never errors outside a git repo — degrades to STALE', () => {
    expect(run(['run', '--label', 'tests', '--', 'echo green']).status).toBe(0);
    const nonGit = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-evidence-nongit-'));
    try {
