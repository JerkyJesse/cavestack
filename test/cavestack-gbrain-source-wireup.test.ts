/**
 * cavestack-gbrain-source-wireup — unit tests with mocked gbrain CLI.
 *
 * The helper registers the cavestack brain repo as a gbrain federated source
...
const WIREUP_BIN = path.join(BIN_DIR, 'cavestack-gbrain-source-wireup');
...
function run(
  ...
  const env = {
    ...
    CAVESTACK_BRAIN_WORKTREE: worktreeDir,
    CAVESTACK_BRAIN_NO_SYNC: '0',
    ...(opts.env || {}),
  };
...
function setupCavestackRepo(remoteUrl: string) {
  // Real git repo at cavestackHome with at least one commit + an origin remote.
  fs.mkdirSync(cavestackHome, { recursive: true });
  spawnSync('git', ['-C', cavestackHome, 'init', '-q', '-b', 'main'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cavestackHome, 'config', 'user.email', 'test@example.com'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cavestackHome, 'config', 'user.name', 'test'], { stdio: 'pipe' });
  fs.writeFileSync(path.join(cavestackHome, '.brain-allowlist'), '# allowlist\n');
  spawnSync('git', ['-C', cavestackHome, 'add', '.'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cavestackHome, 'commit', '-q', '-m', 'init'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cavestackHome, 'remote', 'add', 'origin', remoteUrl], { stdio: 'pipe' });
}

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-wireup-test-'));
  cavestackHome = path.join(tmpHome, '.cavestack');
  worktreeDir = path.join(tmpHome, '.cavestack-brain-worktree');
...
describe('cavestack-gbrain-source-wireup — wireup mode', () => {
  test('fresh state: registers source + creates worktree + syncs', () => {
    setupCavestackRepo('git@github.com:user/cavestack-brain-user.git');
    makeFakeGbrain({});
    const r = run([], { env: { CAVESTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(fs.existsSync(worktreeDir)).toBe(true);
    const state = readState();
    expect(state.sources).toHaveLength(1);
    expect(state.sources[0].id).toBe('cavestack-brain-user');
  ...
  test('idempotent re-run after success: no new sources add call', () => {
    setupCavestackRepo('git@github.com:user/cavestack-brain-user.git');
    makeFakeGbrain({});
    run([], { env: { CAVESTACK_BRAIN_NO_SYNC: '1' } });
  ...
  test('drift recovery: existing source with different path triggers remove + add', () => {
    ...
    fs.writeFileSync(
      gbrainStateFile,
      JSON.stringify({
        sources: [{ id: 'cavestack-brain-user', local_path: '/old/stale/path', federated: true }],
      })
    );
    const r = run([], { env: { CAVESTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
    expect(calls.some((c) => c.startsWith('gbrain sources remove cavestack-brain-user'))).toBe(true);
    expect(calls.some((c) => c.includes(`gbrain sources add cavestack-brain-user --path ${worktreeDir}`))).toBe(true);
    const state = readState();
    expect(state.sources[0].local_path).toBe(worktreeDir);
  });
  ...
  test('source-id derived from origin URL', () => {
    setupCavestackRepo('git@github.com:user/cavestack-brain-alice.git');
    makeFakeGbrain({});
    const r = run([], { env: { CAVESTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(readState().sources[0].id).toBe('cavestack-brain-alice');
  });

  test('source-id fallback to ~/.cavestack-brain-remote.txt when .git is gone', () => {
    // No git repo at cavestackHome; just the remote-file
    fs.mkdirSync(tmpHome, { recursive: true });
    fs.writeFileSync(
      path.join(tmpHome, '.cavestack-brain-remote.txt'),
      'git@github.com:user/cavestack-brain-bob.git\n'
    );
    makeFakeGbrain({});
    // No --strict: helper should benign-skip because .cavestack/.git is missing
    const r = run([]);
  ...
  test('source-id from --source-id flag overrides everything', () => {
    setupCavestackRepo('git@github.com:user/cavestack-brain-different.git');
    makeFakeGbrain({});
    run(['--source-id', 'custom-id'], { env: { CAVESTACK_BRAIN_NO_SYNC: '1' } });
    const state = readState();
  ...
  test('--probe: read-only, prints state without mutating', () => {
    ...
    const r = run(['--probe']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('source_id=cavestack-brain-user');
...
describe('cavestack-gbrain-source-wireup — --database-url lock (defends against external config rewrites)', () => {
  test('--database-url flag is exported as GBRAIN_DATABASE_URL to child gbrain calls', () => {
    ...
    const r = run(['--database-url', TARGET], { env: { CAVESTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
...
describe('cavestack-gbrain-source-wireup — uninstall mode', () => {
...
describe('cavestack-gbrain-source-wireup — defensive paths', () => {
  test('--no-pull skips HEAD advance on existing worktree', () => {
    ...
    fs.writeFileSync(path.join(cavestackHome, 'newfile.md'), 'new');
    spawnSync('git', ['-C', cavestackHome, 'add', '.'], { stdio: 'pipe' });
    spawnSync('git', ['-C', cavestackHome, 'commit', '-q', '-m', 'second commit'], { stdio: 'pipe' });
    const parentHeadAfter = spawnSync('git', ['-C', cavestackHome, 'rev-parse', 'HEAD'], {
      encoding: 'utf-8',
    }).stdout.trim();
    ...
    const r = run(['--no-pull'], { env: { CAVESTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
