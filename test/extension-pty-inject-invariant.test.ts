/**
 * Static invariant: every cavestackInjectToTerminal call in extension/*.js
 * must be preceded by an await on cavestackScanForPTYInject on the same code
 * path (#1370 / D6).
 *
 ...
 *
 *   Rule 1: every file that calls cavestackInjectToTerminal must also call
 *           cavestackScanForPTYInject.
 *
 *   Rule 2: in any function that calls cavestackInjectToTerminal, an
 *           `await ... cavestackScanForPTYInject` MUST appear before the
...
const INJECT_FN = 'cavestackInjectToTerminal';
const SCAN_FN = 'cavestackScanForPTYInject';
...
function findInjectCallSites(content: string): number[] {
  // Find positions of `cavestackInjectToTerminal(` or `cavestackInjectToTerminal?.(`
  // — but exclude the function DEFINITION (window.cavestackInjectToTerminal = ).
  const sites: number[] = [];
  const callRe = /window\.cavestackInjectToTerminal\s*\??\.?\s*\(/g;
  let match: RegExpExecArray | null;
  while ((match = callRe.exec(content)) !== null) {
    // Look back ~30 chars; if "window.cavestackInjectToTerminal =" appears
    ...
    if (window30.includes('cavestackInjectToTerminal =')) continue;
...
describe('extension/* PTY injection invariant (#1370 / D6)', () => {
  ...
  test('sidepanel-terminal.js defines both cavestackInjectToTerminal and cavestackScanForPTYInject', () => {
    const file = join(EXTENSION_DIR, 'sidepanel-terminal.js');
    const content = readFileSync(file, 'utf-8');
    expect(content).toContain('window.cavestackInjectToTerminal');
    expect(content).toContain('window.cavestackScanForPTYInject');
  });
  ...
  test('inject function stays synchronous (D6 contract preservation)', () => {
    ...
    const match = content.match(/window\.cavestackInjectToTerminal\s*=\s*(async\s+)?function/);
