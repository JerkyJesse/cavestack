 ...
 *   spec-archive            — cavestack-paths eval + atomic tmp/mv + PID suffix
 ...
 *   spec-plan-mode-detection — reads CAVESTACK_PLAN_MODE env
 */
...
describe('/spec fail-closed redaction (shared engine)', () => {
  ...
  test('redaction routes through the shared cavestack-redact bin, not inline regex', () => {
    expect(GEN).toContain('cavestack-redact');
...
describe('/spec archive', () => {
  test('uses eval $(cavestack-paths) not hardcoded ~/.cavestack/', () => {
    expect(TMPL).toMatch(/eval "\$\(.+cavestack-paths\)"/);
    expect(TMPL).toMatch(/\$CAVESTACK_STATE_ROOT\/projects\/\$SLUG\/specs/);
    // No hardcoded ~/.cavestack/projects path:
    expect(TMPL).not.toMatch(/~\/\.cavestack\/projects\/\$SLUG\/specs/);
  });
...
describe('/spec plan-mode-aware Phase 5 (DX7/DX11/F1)', () => {
  test('reads CAVESTACK_PLAN_MODE env at Phase 5 dispatch', () => {
    expect(TMPL).toMatch(/CAVESTACK_PLAN_MODE/);
    expect(TMPL).toMatch(/plan-mode-aware default/i);
  });
  test('plan-mode active → file-only path; inactive → file + spawn', () => {
    expect(TMPL).toMatch(/CAVESTACK_PLAN_MODE=active.*file-only path/);
    expect(TMPL).toMatch(/CAVESTACK_PLAN_MODE=inactive.*file \+ spawn/);
  });
