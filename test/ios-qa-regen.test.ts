...
function copyIntoFakeInstall(workDir: string): { root: string; launcher: string } {
  const root = join(workDir, 'fake cavestack install');
  ...
  const launcher = join(binDir, 'cavestack-ios-qa-regen');
  copyFileSync(join(ROOT, 'bin', 'cavestack-ios-qa-regen'), launcher);
...
describe('cavestack-ios-qa-regen', () => {
  test('repository launcher is executable', () => {
    expect(statSync(join(ROOT, 'bin', 'cavestack-ios-qa-regen')).mode & 0o111).not.toBe(0);
  });
  ...
  test('leaves no completion marker when accessor generation fails', () => {
    ...
    writeFileSync(join(generatedDir, '.cavestack-version'), 'stale-complete-marker\n');
    ...
    expect(existsSync(join(generatedDir, '.cavestack-version'))).toBe(false);
  });
...
`);
    ...
    const env = {
      ...process.env,
      CAVESTACK_IOS_CACHE_ROOT: cacheRoot,
      SWIFT_VERSION: '6.3.3',
    ...
    expect(readFileSync(join(generatedDir, '.cavestack-version'), 'utf8')).toBe(
