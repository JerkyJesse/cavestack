...
beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-egress-'));
});
...
describe('egress receipt library', () => {
  ...
  test('WARN-at-size: >25MB ledger emits one self-explanatory stderr warning per process', () => {
    ...
    expect(warnings[0]).toContain('cavestack-egress list');
...
describe('cavestack-egress-receipt shell bridge', () => {
  const bin = path.join(ROOT, 'bin', 'cavestack-egress-receipt');
  ...
  test('write hashes the exact payload file, prints the receipt id; outcome joins', () => {
    ...
    const write = spawnSync(bin, ['write', '--sink', 'telemetry-sync', '--host', '127.0.0.1:8399',
      ...
      { encoding: 'utf-8', env: { ...process.env, CAVESTACK_HOME: home } });
