...
// Create a mock cavestack install directory with skill subdirs
function setupMockInstall(skills: string[]): void {
  installDir = path.join(tmpDir, 'cavestack-install');
  skillsDir = path.join(tmpDir, 'skills');
  fs.mkdirSync(installDir, { recursive: true });
  fs.mkdirSync(skillsDir, { recursive: true });

  // Copy the real cavestack-config and cavestack-relink to the mock install
  const mockBin = path.join(installDir, 'bin');
  fs.mkdirSync(mockBin, { recursive: true });
  fs.copyFileSync(path.join(BIN, 'cavestack-config'), path.join(mockBin, 'cavestack-config'));
  fs.chmodSync(path.join(mockBin, 'cavestack-config'), 0o755);
  if (fs.existsSync(path.join(BIN, 'cavestack-relink'))) {
    fs.copyFileSync(path.join(BIN, 'cavestack-relink'), path.join(mockBin, 'cavestack-relink'));
    fs.chmodSync(path.join(mockBin, 'cavestack-relink'), 0o755);
  }
  if (fs.existsSync(path.join(BIN, 'cavestack-patch-names'))) {
    fs.copyFileSync(path.join(BIN, 'cavestack-patch-names'), path.join(mockBin, 'cavestack-patch-names'));
    fs.chmodSync(path.join(mockBin, 'cavestack-patch-names'), 0o755);
  }
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-relink-test-'));
});
...
describe('cavestack-relink (#578)', () => {
  ...
  test('creates cavestack-* symlinks when skill_prefix=true', () => {
    ...
    const output = run(`${path.join(installDir, 'bin', 'cavestack-relink')}`, {
      CAVESTACK_INSTALL_DIR: installDir,
      CAVESTACK_SKILLS_DIR: skillsDir,
    });
    // Verify cavestack-* symlinks exist
    expect(fs.existsSync(path.join(skillsDir, 'cavestack-qa'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'cavestack-ship'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'cavestack-review'))).toBe(true);
    expect(output).toContain('cavestack-');
  });
  ...
  // e.g., `qa -> cavestack/qa` gets discovered as "cavestack-qa", not "qa".
  ...
  test('prefixed skills are real directories with SKILL.md symlinks, not dir symlinks', () => {
    ...
    for (const skill of ['cavestack-qa', 'cavestack-ship']) {
      const skillPath = path.join(skillsDir, skill);
      const skillMdPath = path.join(skillPath, 'SKILL.md');
      expect(fs.lstatSync(skillPath).isDirectory()).toBe(true);
      expect(fs.lstatSync(skillPath).isSymbolicLink()).toBe(false);
  ...
  test('first install --no-prefix: only flat names exist, zero cavestack-* entries', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'cavestack-upgrade']);
    ...
    // Expected: qa, ship, review, plan-ceo-review, cavestack-upgrade (its real name)
    expect(entries.sort()).toEqual(['cavestack-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    // No cavestack-qa, cavestack-ship, cavestack-review, cavestack-plan-ceo-review
    const leaked = entries.filter(e => e.startsWith('cavestack-') && e !== 'cavestack-upgrade');
    expect(leaked).toEqual([]);
  });

  // FIRST INSTALL: --prefix must create ONLY cavestack-* names, zero flat-name pollution
  test('first install --prefix: only cavestack-* entries exist, zero flat names', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'cavestack-upgrade']);
    run(`${path.join(installDir, 'bin', 'cavestack-config')} set skill_prefix true`, {
      CAVESTACK_INSTALL_DIR: installDir,
      CAVESTACK_SKILLS_DIR: skillsDir,
    ...
    const entries = fs.readdirSync(skillsDir);
    // Expected: cavestack-qa, cavestack-ship, cavestack-review, cavestack-plan-ceo-review, cavestack-upgrade
    expect(entries.sort()).toEqual([
      'cavestack-plan-ceo-review', 'cavestack-qa', 'cavestack-review', 'cavestack-ship', 'cavestack-upgrade',
    ]);
    // No unprefixed qa, ship, review, plan-ceo-review
    const leaked = entries.filter(e => !e.startsWith('cavestack-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('non-TTY first install defaults to flat names via relink', () => {
    ...
    // cavestack-relink reads config; on fresh install config returns empty → defaults to false
    run(`${path.join(installDir, 'bin', 'cavestack-relink')}`, {
      CAVESTACK_INSTALL_DIR: installDir,
      CAVESTACK_SKILLS_DIR: skillsDir,
    });
  ...
  // SWITCH: prefix → no-prefix must clean up ALL cavestack-* entries
  test('switching prefix to no-prefix removes all cavestack-* entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'cavestack-upgrade']);
    ...
    expect(entries.filter(e => !e.startsWith('cavestack-'))).toEqual([]);
    ...
    // Only flat names + cavestack-upgrade (its real name)
    expect(entries.sort()).toEqual(['cavestack-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    const leaked = entries.filter(e => e.startsWith('cavestack-') && e !== 'cavestack-upgrade');
    expect(leaked).toEqual([]);
  });
  ...
  test('switching no-prefix to prefix removes all flat entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'cavestack-upgrade']);
    ...
    let entries = fs.readdirSync(skillsDir);
    expect(entries.filter(e => e.startsWith('cavestack-') && e !== 'cavestack-upgrade')).toEqual([]);
    ...
    entries = fs.readdirSync(skillsDir);
    // Only cavestack-* names
    expect(entries.sort()).toEqual([
      'cavestack-qa', 'cavestack-review', 'cavestack-ship', 'cavestack-upgrade',
    ]);
    const leaked = entries.filter(e => !e.startsWith('cavestack-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('cleans up stale symlinks from opposite mode', () => {
    ...
    expect(fs.existsSync(path.join(skillsDir, 'qa'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'cavestack-qa'))).toBe(false);
  });

  // Test 14: error when install dir missing
  test('prints error when install dir missing', () => {
    const output = run(`${BIN}/cavestack-relink`, {
      CAVESTACK_INSTALL_DIR: '/nonexistent/path/cavestack',
      CAVESTACK_SKILLS_DIR: '/nonexistent/path/skills',
    }, true);
    expect(output).toContain('setup');
  });

  // Test: cavestack-upgrade does NOT get double-prefixed
  test('does not double-prefix cavestack-upgrade directory', () => {
    setupMockInstall(['qa', 'ship', 'cavestack-upgrade']);
    run(`${path.join(installDir, 'bin', 'cavestack-config')} set skill_prefix true`, {
      CAVESTACK_INSTALL_DIR: installDir,
      CAVESTACK_SKILLS_DIR: skillsDir,
    });
    ...
    // cavestack-upgrade should keep its name, NOT become cavestack-cavestack-upgrade
    expect(fs.existsSync(path.join(skillsDir, 'cavestack-upgrade'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'cavestack-cavestack-upgrade'))).toBe(false);
    // Regular skills still get prefixed
    expect(fs.existsSync(path.join(skillsDir, 'cavestack-qa'))).toBe(true);
  });

  // Test 15: cavestack-config set skill_prefix triggers relink
  test('cavestack-config set skill_prefix triggers relink', () => {
    setupMockInstall(['qa', 'ship']);
    // Run cavestack-config set which should auto-trigger relink
    run(`${path.join(installDir, 'bin', 'cavestack-config')} set skill_prefix true`, {
      CAVESTACK_INSTALL_DIR: installDir,
      CAVESTACK_SKILLS_DIR: skillsDir,
    });
...
describe('upgrade migrations', () => {
  const MIGRATIONS_DIR = path.join(ROOT, 'cavestack-upgrade', 'migrations');
  ...
  test('v0.15.2.0 migration runs cavestack-relink', () => {
    const content = fs.readFileSync(path.join(MIGRATIONS_DIR, 'v0.15.2.0.sh'), 'utf-8');
    expect(content).toContain('cavestack-relink');
  });
  ...
  test('v0.15.2.0 migration fixes stale directory symlinks', () => {
    ...
    run(`${path.join(installDir, 'bin', 'cavestack-config')} set skill_prefix false`, {
      CAVESTACK_SETUP_RUNNING: '1',
    });
    // Verify old state: symlinks
    expect(fs.lstatSync(path.join(skillsDir, 'qa')).isSymbolicLink()).toBe(true);

    // Run the migration (it calls cavestack-relink internally)
...
describe('cavestack-patch-names (#620/#578)', () => {
  ...
  test('prefix=true patches name: field in SKILL.md', () => {
    ...
    // Verify name: field is patched with cavestack- prefix
    expect(readSkillName(path.join(installDir, 'qa'))).toBe('cavestack-qa');
    expect(readSkillName(path.join(installDir, 'ship'))).toBe('cavestack-ship');
    expect(readSkillName(path.join(installDir, 'review'))).toBe('cavestack-review');
  });
  ...
  test('cavestack-upgrade name: not double-prefixed', () => {
    setupMockInstall(['qa', 'cavestack-upgrade']);
    run(`${path.join(installDir, 'bin', 'cavestack-config')} set skill_prefix true`, {
      CAVESTACK_INSTALL_DIR: installDir,
      CAVESTACK_SKILLS_DIR: skillsDir,
    });
    ...
    // cavestack-upgrade should keep its name, NOT become cavestack-cavestack-upgrade
    expect(readSkillName(path.join(installDir, 'cavestack-upgrade'))).toBe('cavestack-upgrade');
