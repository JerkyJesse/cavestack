...
// Static invariants guarding Windows artifact-sync (bin/cavestack-brain-sync).
//
...
const SRC = fs.readFileSync(path.join(ROOT, 'bin', 'cavestack-brain-sync'), 'utf-8');

describe('cavestack-brain-sync — Windows path/exec invariants', () => {
  test('discover-new normalizes relpath separators before fnmatch (bug 1)', () => {
    expect(SRC).toContain('os.path.relpath(full, cavestack_home).replace(os.sep, "/")');
  });
