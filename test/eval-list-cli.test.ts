...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-eval-list-'));
  const evalDir = path.join(tmpHome, '.cavestack-dev', 'evals');
...
function runEvalList(...args: string[]): { stdout: string; stderr: string; status: number } {
  ...
  // cwd-relative .claude/skills/cavestack/bin/cavestack-slug, and on dev machines
  // with the self-symlink that probe succeeds, routing reads to an (empty)
  // project-scoped dir instead of the legacy ~/.cavestack-dev/evals this test
