/**
 * cavestack-upgrade/migrations/v1.40.0.0.sh — migration script unit tests.
 *
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "cavestack-mig-v1400-"));
  cavestackHome = path.join(tmpHome, ".cavestack");
  migrationDir = path.join(cavestackHome, ".migrations");
  donePath = path.join(migrationDir, "v1.40.0.0.done");
  allowlistPath = path.join(cavestackHome, ".brain-allowlist");
  privacyPath = path.join(cavestackHome, ".brain-privacy-map.json");
  gitattrsPath = path.join(cavestackHome, ".gitattributes");
  fs.mkdirSync(cavestackHome, { recursive: true });
});

afterEach(() => {
  try {
    fs.chmodSync(cavestackHome, 0o755);
...
function makeCuratedPath(opts: { jq?: "missing" | "shim-fail" | "real" } = {}): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "cavestack-mig-path-"));
  const utils = [
    "bash",
    "sh",
    "mkdir",
