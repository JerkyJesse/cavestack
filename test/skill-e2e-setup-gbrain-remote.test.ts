...
describeE2E('/setup-gbrain Path 4 (Remote MCP) — happy path', () => {
  test('verifies, registers HTTP MCP, never writes token to CLAUDE.md', async () => {
    ...
    const cavestackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-remote-'));
    const fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-remote-bin-'));
    const callLog = makeFakeClaude(fakeBinDir);

    // The skill writes CLAUDE.md in cwd. Use cavestackHome as cwd so we
    // can inspect it after the run.
    fs.writeFileSync(path.join(cavestackHome, 'CLAUDE.md'), '# Test project\n');
    ...
    // hermetic-env scrubs GBRAIN_*/CAVESTACK_* by allowlist — this test's token
    ...
    try {
      ...
      const claudeMd = fs.readFileSync(path.join(cavestackHome, 'CLAUDE.md'), 'utf-8');
      ...
      // footer contains the literal "CAVESTACK REVIEW REPORT" — a guaranteed
      // false positive on both trees.
      const wroteBefore = /## CAVESTACK REVIEW REPORT|critical_gaps/i.test(result.output);
