 ...
 *
 * 2. cavestack-team-init's required-mode enforcement (the CLAUDE.md
 *    verification snippet and the generated check-cavestack.sh hook) must
 *    resolve the install root across CAVESTACK_ROOT + every host's global
 *    install location, never hardcode ~/.claude/skills/cavestack. The drift
...
describe('team-init resolves CAVESTACK_ROOT across every host (#2500)', () => {
  const teamInit = fs.readFileSync(path.join(ROOT, 'bin', 'cavestack-team-init'), 'utf-8');

  test('probe list covers CAVESTACK_ROOT env + every registered host globalRoot + migrated repo', () => {
    expect(teamInit).toContain('"${CAVESTACK_ROOT:-}"');
    for (const config of ALL_HOST_CONFIGS) {
      expect(teamInit).toContain(`$HOME/${config.globalRoot}`);
    }
    expect(teamInit).toContain('$HOME/.cavestack/repos/cavestack');
  });

  test('enforcement no longer hardcodes the Claude path as the only gate', () => {
    expect(teamInit).not.toContain('test -d ~/.claude/skills/cavestack/bin');
    expect(teamInit).not.toContain('if [ ! -d "$HOME/.claude/skills/cavestack/bin" ]');
  });
  ...
  test('generated hook blocks only when NO install root resolves', () => {
    ...
    expect(teamInit).toContain('if [ -z "$_CAVESTACK_ROOT" ]; then');
  });
});
