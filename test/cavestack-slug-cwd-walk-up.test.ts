/**
 * Regression test — bin/cavestack-slug must resolve to the OUTERMOST project root
 ...
 * `bin/cavestack-slug` derived its slug from the literal `pwd` with no walk-up.
 ...
 * (no manual `rm -rf ~/.cavestack/slug-cache/` required).
 *
...
function runSlug(
  ...
  // CAVESTACK_HOME must be pinned to the temp home too: the cache dir is
  // CAVESTACK_HOME-aware (matching lib/bin-context.ts's native port), and a
  // sibling test file leaking process.env.CAVESTACK_HOME in a shared-process
  ...
  // script's cache writes land in tmpHome, never AJ's real ~/.cavestack.
  const env = { ...process.env, HOME: tmpHome, CAVESTACK_HOME: path.join(tmpHome, '.cavestack'), ...extraEnv };
...
describe('cavestack-slug — outermost project-root resolution', () => {
  let tmpHome: string;
  ...
  beforeEach(() => {
    ...
    tmpHome = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-slug-test-')));
    projectsRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-slug-projects-')));
  });
  ...
  // Edge case: CAVESTACK_PROJECT_SLUG env override wins over walk-up (documented escape hatch).
  test('CAVESTACK_PROJECT_SLUG env override beats every other resolution path', () => {
    const projectRoot = path.join(projectsRoot, 'loadout');
    const siteSubdir = path.join(projectRoot, 'site');
    fs.mkdirSync(path.join(projectRoot, '.git'), { recursive: true });
    fs.mkdirSync(path.join(siteSubdir, '.vercel'), { recursive: true });
    ...
    const result = runSlug(siteSubdir, tmpHome, { CAVESTACK_PROJECT_SLUG: 'custom-override' });
    expect(result.status).toBe(0);
    const { slug } = parseSlug(result.stdout);
...
describe('_outermost_project_root termination (windows-free-tests regression)', () => {
  ...
  const script = fs.readFileSync(path.join(ROOT, 'bin', 'cavestack-slug'), 'utf-8');
