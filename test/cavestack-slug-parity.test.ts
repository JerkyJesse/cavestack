/**
 * bin/cavestack-slug ↔ browse/bin/remote-slug parity.
 *
 * The bug this pins (2026-08-17, observed live in a Conductor worktree of
 * garrytan/cavestack): a stray marker-bearing ancestor above the repo — an empty
 * `~/.git` directory that is not even a valid git repo — captured
 * cavestack-slug's "outermost strong marker" walk-up as the project root. That
 ...
 * `garrytan-cavestack`. Every store keyed on the slug (decisions, timeline,
 * ceo-plans, learnings) filed into ~/.cavestack/projects/garrytan/ — one bucket
 * shared by every repo under $HOME.
 *
 * The fix makes the canonical remote authoritative: cavestack-slug now walks the
 ...
 *    reaches a canonical remote, cavestack-slug's SLUG equals remote-slug's
 ...
 *
 * Test pattern mirrors test/cavestack-slug-cwd-walk-up.test.ts: per-test
...
const SLUG_SCRIPT = path.join(ROOT, 'bin', 'cavestack-slug');
...
function baseEnv(tmpHome: string): Record<string, string | undefined> {
  // Drop any ambient override: a sibling test leaking CAVESTACK_PROJECT_SLUG in
  // a shared-process shard would flip runs into override mode.
  const { CAVESTACK_PROJECT_SLUG: _drop, ...ambient } = process.env;
  return { ...ambient, HOME: tmpHome, CAVESTACK_HOME: path.join(tmpHome, '.cavestack') };
}
...
function expectParity(cwd: string, tmpHome: string, expected: string): void {
  const cavestack = runSlug(cwd, tmpHome);
  ...
  expect(slugOf(cavestack)).toBe(expected);
  expect(remoteOut).toBe(expected);
  expect(slugOf(cavestack)).toBe(remoteOut);
}

describe('cavestack-slug ↔ remote-slug parity', () => {
  let tmpHome: string;
  let fixtures: string;
  ...
  test('git-worktree of a clone (.git FILE, the Conductor shape) — identical slug', () => {
    const main = makeRepo(path.join(fixtures, 'main-clone'), 'https://github.com/JerkyJesse/cavestack');
  ...
  test('LIVE BUG SHAPE: stray empty .git on an ancestor "home" no longer degrades the slug', () => {
    ...
    const main = makeRepo(
      path.join(strayHome, 'conductor', 'workspaces', 'cavestack', 'main-clone'),
      'https://github.com/JerkyJesse/cavestack',
    );
    git(['-C', main, '-c', 'user.email=t@t', '-c', 'user.name=t', 'commit', '-q', '--allow-empty', '-m', 'init']);
    const wt = path.join(strayHome, 'conductor', 'workspaces', 'cavestack', 'beirut-v4');
    git(['-C', main, 'worktree', 'add', '-q', wt, '-b', 'cavestack-fix-wave']);
  ...
  test('walk-up preserved: nested inner repo (no remote) resolves to the OUTER repo slug', () => {
    ...
    const cavestack = runSlug(inner, tmpHome);
    expect(cavestack.status).toBe(0);
    ...
    expect(slugOf(cavestack)).toBe('acme-outer');
  ...
  test('fallback unchanged: no-remote repo resolves to its basename (and remote-slug agrees)', () => {
    ...
    expect(cavestack.status).toBe(0);
    expect(slugOf(cavestack)).toBe('lonely');
  ...
  test('cache self-heal: a pre-fix degraded cache entry is rewritten to the canonical slug', () => {
    ...
    const repo = makeRepo(path.join(strayHome, 'git', 'proj'), 'https://github.com/JerkyJesse/cavestack');
    ...
    const cacheDir = path.join(tmpHome, '.cavestack', 'slug-cache');
  ...
  test('hostile origin `url = ..` cannot become a ".." slug — basename fallback holds', () => {
    ...
    // ABOVE ~/.cavestack/projects/ (confined to ~/.cavestack, but still traversal).
    ...
    const cacheFile = path.join(tmpHome, '.cavestack', 'slug-cache', encodedCacheKey(repo));
  ...
  test('package.json wrapper root (no .git): sticky basename slug is PRESERVED — heal is stray-repo-shape only', () => {
    ...
    // cavestack before an inner dir grew a remote-bearing repo. The degraded-
  ...
  test('sticky identity preserved (#2212): repo that adopted a remote after first use is NOT healed', () => {
    ...
    const cavestack = runSlug(repo, tmpHome);
    expect(cavestack.status).toBe(0);
    expect(slugOf(cavestack)).toBe('stickyproj');
