/**
 * Unit tests for bin/cavestack-brain-context-load.ts (Lane C).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "cavestack-brain-context-load.ts");
...
function prependPath(binDir: string): Record<string, string> {
  ...
  return {
    ...
    CAVESTACK_BRAIN_TIMEOUT_MS: "10000",
  };
}

describe("cavestack-brain-context-load CLI", () => {
  it("--help exits 0 with usage", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: cavestack-brain-context-load");
...
describe("cavestack-brain-context-load — manifest dispatch", () => {
  ...
  it("uses skill manifest when --skill-file points at a valid SKILL.md", () => {
    const dir = mkdtempSync(join(tmpdir(), "cavestack-bcl-"));
...
describe("cavestack-brain-context-load — graceful gbrain absence", () => {
...
`,
  ...
  it("availability survives a 1ms query budget — detection is not subject to CAVESTACK_BRAIN_TIMEOUT_MS", () => {
...
`,
    ...
    try {
      const env = { ...prependPath(binDir), CAVESTACK_BRAIN_TIMEOUT_MS: "1" };
