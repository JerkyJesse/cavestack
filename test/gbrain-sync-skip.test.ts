/**
 * Tests the split-engine SKIP semantics in bin/cavestack-gbrain-sync.ts (plan D12).
 *
...
describe("cavestack-gbrain-sync — split-engine SKIP (plan D12)", () => {
  it("PROCEEDS (with warning) when the engine probe times out — slow is not broken (#1964)", () => {
    ...
    try {
      ...
      expect(out).toContain("CAVESTACK_GBRAIN_PROBE_TIMEOUT_MS");
    } finally {
      env.cleanup();
    }
