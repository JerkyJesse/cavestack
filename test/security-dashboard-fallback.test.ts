...
const SEC_BIN = join(ROOT, "bin", "cavestack-security-dashboard");
const COMM_BIN = join(ROOT, "bin", "cavestack-community-dashboard");
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), "cavestack-dash-test-"));
...
function run(
  ...
  return spawnSync(BASH, opts.json ? [bin, "--json"] : [bin], {
    ...
    env: {
      ...
      PATH: pathEnv,
      CAVESTACK_DIR: ROOT,
      CAVESTACK_SUPABASE_URL: "https://stub.supabase.test",
      CAVESTACK_SUPABASE_ANON_KEY: "stub-key",
...
describe("cavestack-security-dashboard — never reports fake zeros (#1947)", () => {
...
describe("cavestack-community-dashboard — never reports fake zeros (#1947)", () => {
