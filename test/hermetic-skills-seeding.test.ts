 ...
 * 3. connect-chrome (dir symlink) collapses into open-cavestack-browser — no
...
describe('hermeticSkillsConfigDir', () => {
  ...
  test('connect-chrome collapses into a single open-cavestack-browser entry', () => {
    const seeded = fs.readdirSync(skillsDir);
    expect(seeded.filter((n) => n === 'open-cavestack-browser')).toHaveLength(1);
    expect(seeded).not.toContain('connect-chrome');
  });

  test('root router registered as _cavestack-command pointing at the root SKILL.md', () => {
    const link = path.join(skillsDir, '_cavestack-command', 'SKILL.md');
