...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // cavestack-review-read reads from ~/.cavestack/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock cavestack-review-read script that returns our test data
    ...
    // Write a mock cavestack-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'cavestack-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'cavestack-review-read'), 0o755);
...
Instead of running ~/.claude/skills/cavestack/bin/cavestack-review-read, run this mock: ${mockBinDir}/cavestack-review-read
