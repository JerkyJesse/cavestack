/**
 * cavestack-artifacts-init — provider-selection + brain-admin-hookup tests.
 *
 * Mirrors the cavestack-brain-init-gh-mock.test.ts pattern: install fake gh /
 ...
 * ~/.cavestack-artifacts-remote.txt, (c) prints the "Send this to your brain
...
const INIT_BIN = path.join(ROOT, 'bin', 'cavestack-artifacts-init');
...
function makeFakeGh(opts: {
  ...
  const webUrl = opts.webUrl ?? `https://github.com/testuser/cavestack-artifacts-testuser`;
...
function makeFakeGlab(opts: {
  ...
  const authStatus = opts.authStatus ?? 'ok';
  const repoCreate = opts.repoCreate ?? 'success';
  const webUrl = opts.webUrl ?? 'https://gitlab.com/testuser/cavestack-artifacts-testuser';
  const gitProtocol = opts.gitProtocol ?? 'https';
  const script = `#!/bin/bash
...
describe('cavestack-artifacts-init provider selection', () => {
  test('--host github invokes gh repo create with cavestack-artifacts-$USER', () => {
    makeFakeGh({});
    ...
    expect(createCall).toContain('cavestack-artifacts-testuser');
  ...
  test('neither authed → falls through to manual URL paste', () => {
    // No gh, no glab fakes.
    const r = run([], { input: 'https://github.com/testuser/cavestack-artifacts-testuser\n' });
    expect(r.status).toBe(0);
    expect(r.stderr).toContain('Neither gh nor glab');
  });
});

describe('cavestack-artifacts-init canonical URL storage (codex Finding #10)', () => {
  test('stores HTTPS URL canonical in ~/.cavestack-artifacts-remote.txt', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/cavestack-artifacts-testuser' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const remoteFile = path.join(tmpHome, '.cavestack-artifacts-remote.txt');
    ...
    expect(stored).toBe('https://github.com/testuser/cavestack-artifacts-testuser');
  });

  test('strips trailing .git from gh repo view output', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/cavestack-artifacts-testuser.git' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const stored = fs.readFileSync(path.join(tmpHome, '.cavestack-artifacts-remote.txt'), 'utf-8').trim();
    expect(stored).toBe('https://github.com/testuser/cavestack-artifacts-testuser');
  });
  ...
  test('configures git origin with SSH when gh git_protocol is ssh', () => {
    makeFakeGh({
      webUrl: 'https://github.com/testuser/cavestack-artifacts-testuser',
      gitProtocol: 'ssh',
    });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
...
describe('cavestack-artifacts-init brain-admin hookup printout (codex Finding #3)', () => {
  ...
  test('--url-form-supported true prints the one-liner with --url', () => {
    ...
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('Send this to your brain admin');
    expect(r.stdout).toContain('gbrain sources add cavestack-artifacts-testuser --url');
  ...
  test('the gbrain command line uses canonical HTTPS, not SSH', () => {
    ...
    expect(gbrainLine).toContain('https://github.com/testuser/cavestack-artifacts-testuser');
    expect(gbrainLine).not.toContain('git@github.com');
    // The Push line follows the provider CLI preference independently.
  });
});

describe('cavestack-artifacts-init idempotency', () => {
  ...
  test('--push-protocol overrides the inferred protocol', () => {
    ...
    const r = run([
      '--remote',
      'https://github.com/testuser/cavestack-artifacts-testuser',
      '--push-protocol',
      'ssh',
    ]);
    expect(r.status).toBe(0);
  ...
  test('re-run with same --remote is safe (no conflict error)', () => {
    makeFakeGh({});
    const url = 'https://github.com/testuser/cavestack-artifacts-testuser';
    run(['--remote', url]);
  ...
  test('re-run with DIFFERENT --remote exits 1 with conflict message', () => {
    makeFakeGh({});
    run(['--remote', 'https://github.com/testuser/cavestack-artifacts-testuser']);
