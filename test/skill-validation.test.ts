...
describe('SKILL.md command validation', () => {
  // P2 (v1.2.0): the top-level cavestack skill is a pure ROUTER, not the browse
  ...
  test('top-level SKILL.md is a router with no browse body (P2)', () => {
    ...
    expect(md).not.toContain('cavestack browse: QA Testing'); // browse body removed
...
describe('Cross-skill path consistency', () => {
  ...
  test('all greptile-history write references specify both per-project and global paths', () => {
    ...
    for (const file of filesToCheck) {
      ...
      const hasBoth = (content.includes('per-project') && content.includes('global')) ||
        (content.includes('$REMOTE_SLUG/greptile-history') && content.includes('~/.cavestack/greptile-history'));
  ...
  test('greptile-triage.md contains both project and global history paths', () => {
    ...
    expect(content).toContain('~/.cavestack/greptile-history.md');
  });
...
describe('QA skill structure validation', () => {
  ...
  test('output structure references report directory layout', () => {
    ...
    expect(qaContent).toContain('.cavestack/qa-reports/');
  });
});
...
describe('cavestack-slug', () => {
  ...
  test('eval sets variables under bash with set -euo pipefail', () => {
    const result = Bun.spawnSync(
      ['bash', '-c', 'set -euo pipefail; eval "$(./bin/cavestack-slug 2>/dev/null)"; echo "SLUG=$SLUG"; echo "BRANCH=$BRANCH"'],
  ...
  test('no templates or bin scripts use source process substitution for cavestack-slug', () => {
    const result = Bun.spawnSync(
      ['grep', '-r', 'source <(.*cavestack-slug', '--include=*.tmpl', '--include=cavestack-review-*', '.'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe' }
    );
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Report: .cavestack/qa-reports/');
  });
...
describe('Codex skill', () => {
  ...
  test('codex/SKILL.md contains review log persistence', () => {
    ...
    expect(content).toContain('cavestack-review-log');
  });
  ...
  test('codex-host document-release does NOT contain the Codex doc review', () => {
    ...
    const content = fs.readFileSync(
      path.join(ROOT, '.agents', 'skills', 'cavestack-document-release', 'SKILL.md'), 'utf-8');
...
describe('Skill trigger phrases', () => {
  ...
  // Excluded: root cavestack (browser tool), cavestack-upgrade (cavestack-specific),
  // humanizer (text tool)
...
describe('Codex skill validation', () => {
  ...
  // Host skipSkills (codex + caveman*) plus /claude (external-host-only)
  // are omitted — Codex generation never writes those variants.
  const CODEX_SKIPPED = new Set(getHostConfig('codex').generation.skipSkills ?? []);
...
describe('Repo mode preamble validation', () => {
  test('generated SKILL.md preamble contains REPO_MODE output', () => {
    ...
    expect(content).toContain('cavestack-repo-mode');
  });
...
//
// Browser-skills are bundled in <cavestack-root>/browser-skills/<name>/. Each
