...
describe('cavestack-upgrade template: ff-only precedes the gated reset (#2517)', () => {
  const tmpl = read('cavestack-upgrade/SKILL.md.tmpl');
...
describe('brain-uninstall removes the spool queue', () => {
  test('uninstall cleans .brain-queue.d alongside the legacy queue file', () => {
    const src = read('bin/cavestack-brain-uninstall');
