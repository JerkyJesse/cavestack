 ...
 * auto-deciding a specific question via `cavestack-question-preference --write
 ...
 *
 * Set up:
 *   - tmpDir as CAVESTACK_HOME (isolated state, doesn't touch the user's
 *     real ~/.cavestack)
...
describeE2E('AUTO_DECIDE opt-in preserved under Conductor flags (periodic)', () => {
  test('user-opted-in question still auto-decides when AskUserQuestion is --disallowedTools', async () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-auto-decide-'));
    try {
      // 1. Bootstrap the tmp CAVESTACK_HOME with question_tuning=true.
      const configBin = path.join(ROOT, 'bin', 'cavestack-config');
      const setRes = spawnSync(configBin, ['set', 'question_tuning', 'true'], {
        env: { ...process.env, CAVESTACK_HOME: tmpHome },
        encoding: 'utf-8',
      });
      if (setRes.status !== 0) {
        throw new Error(`cavestack-config set failed: ${setRes.stderr || setRes.stdout}`);
      }
      ...
      const slugBin = path.join(ROOT, 'bin', 'cavestack-slug');
      const slugRes = spawnSync(slugBin, [], {
        cwd: ROOT,
        env: { ...process.env, CAVESTACK_HOME: tmpHome },
        encoding: 'utf-8',
      });
      // cavestack-slug emits `eval`-able shell exports like `SLUG=garrytan-cavestack`.
      ...
      if (writeRes.status !== 0) {
        throw new Error(`cavestack-question-preference --write failed: ${writeRes.stderr || writeRes.stdout}`);
      }
      ...
      //    CAVESTACK_HOME=tmpHome is REQUIRED: the preference + question_tuning were
      //    seeded there. Without it the spawned claude reads the real ~/.cavestack,
      ...
      const obs = await runPlanSkillObservation({
        ...
        timeoutMs: 540_000,
        env: { CAVESTACK_HOME: tmpHome, CONDUCTOR_WORKSPACE_PATH: tmpHome },
      });
