/**
 * cavestack-settings-hook schema-aware surface (T3 plan-tune cathedral).
 *
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-shsa-'));
...
function run(args: string[]): { stdout: string; stderr: string; exitCode: number } {
  try {
    const stdout = execSync([SETTINGS_HOOK, ...args].map((s) => `'${s}'`).join(' '), {
      env: { ...process.env, CAVESTACK_SETTINGS_FILE: settingsFile },
      encoding: 'utf-8',
      timeout: 10000,
    });
...
describe('add-event', () => {
  test('registers a PreToolUse hook with matcher + source tag', () => {
    ...
    expect(s.hooks.PreToolUse[0]._cavestack_source).toBe('plan-tune-cathedral');
  ...
  test('dedup includes command: same (event, matcher, command) with different source updates in place', () => {
    ...
    const s = settings();
    expect(s.hooks.PostToolUse).toHaveLength(1);
    expect(s.hooks.PostToolUse[0]._cavestack_source).toBe('source-B');
  });
  ...
  test('dedup includes command: untagged entry with same command is updated not duplicated', () => {
    ...
    const s = settings();
    expect(s.hooks.PostToolUse).toHaveLength(1);
    expect(s.hooks.PostToolUse[0]._cavestack_source).toBe('plan-tune-cathedral');
  });
...
describe('ensure-event collapses duplicate (event, source) entries', () => {
  test('two same-source entries from the old matcher-keyed dedup collapse to ONE updated entry', () => {
    ...
    // (event, _cavestack_source) — the old dedup keyed on the matcher too, so a
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
          { _cavestack_source: 'plan-tune-cathedral', matcher: 'OldMatcherA', hooks: [{ type: 'command', command: '/old-a', timeout: 5 }] },
          { matcher: 'Bash', hooks: [{ type: 'command', command: '/user-own-hook' }] },
          { _cavestack_source: 'plan-tune-cathedral', matcher: 'OldMatcherB', hooks: [{ type: 'command', command: '/old-b', timeout: 5 }] },
        ],
      },
    }, null, 2));
    ...
    const r = spawnSync('bash', [
      ...
      '--source', 'plan-tune-cathedral',
      '--timeout', '5',
    ], { env: { ...process.env, CAVESTACK_SETTINGS_FILE: settingsFile }, encoding: 'utf-8', timeout: 15_000 });

    expect(r.status).toBe(0);
    // The collapse is reported on stderr, never silent.
    expect(r.stderr).toContain('collapsed 1 duplicate');
    const s = settings();
    const mine = s.hooks.PostToolUse.filter((e: any) => e._cavestack_source === 'plan-tune-cathedral');
  ...
  test('no duplicates → no collapse message, single entry updated as before', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
          { _cavestack_source: 'plan-tune-cathedral', matcher: 'OldMatcher', hooks: [{ type: 'command', command: '/old', timeout: 5 }] },
        ],
      },
    }, null, 2));
...
// the fixed table) because Claude Code strips the _cavestack_source key when it
...
/** run() with hermetic cavestack-config state (prune-stale consults plan_tune_hooks). */
...
function mkCanon(base: string, name = 'canon'): string {
  ...
  const su = path.join(canon, 'bin', 'cavestack-session-update');
...
function hookEntry(cmd: string, matcher?: string, src?: string, extraItems: any[] = []) {
  ...
  if (src) e._cavestack_source = src;
  return e;
}
...
describe('add-event: per-item identity re-point', () => {
  ...
  test('foreign path with a cavestack basename is NOT claimed (wrong relpath suffix)', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
  ...
  test('mixed entry: only the cavestack item (index > 0) is replaced; the user item survives', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
...
describe('legacy remove: per-item (regression)', () => {
  test('mixed SessionStart entry: user item survives in place, cavestack item removed', () => {
    // REGRESSION pin: the pre-v1.67.2 legacy `remove` dropped the ENTIRE
    // entry when any item matched cavestack-session-update, destroying a user's
      ...
      hooks: {
        SessionStart: [{
          hooks: [
            ...
            { type: 'command', command: '/old/install/bin/cavestack-session-update' },
          ],
        }],
      },
    }, null, 2));
    const r = runIso(['remove', '/old/install/bin/cavestack-session-update']);
    expect(r.exitCode).toBe(0);
    const s = settings();
...
describe('review-army hardening (specialist findings)', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [
          ...
          { hooks: 'not-an-array' },
          { hooks: [] },
          { hooks: [{ type: 'command', command: '/x/bin/cavestack-session-update' }] },
        ],
      },
    }, null, 2));
    runIso(['remove', '/x/bin/cavestack-session-update']);
    const s = settings();
  ...
  test('add-event never tags a mixed entry (old-version ratchet guard)', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [{
          matcher: AUQ_MATCHER,
          _cavestack_source: 'plan-tune-cathedral',
          hooks: [
            { type: 'command', command: '/Users/me/my-own-hook' },
    ...
    expect(e._cavestack_source).toBeUndefined();
  });
  ...
  test('a 0600 settings.json keeps its mode across mutations (API keys stay private)', () => {
    ...
    runIso(['add-event', '--event', 'Stop', '--command', '/x/hosts/claude/hooks/timeline-stop-hook', '--source', 'cavestack-timeline-stop']);
  ...
  test('a canonical root containing $ is escaped in the registered command', () => {
    ...
    expect(r2.stdout).toMatch(/removed 0 cavestack hook entries \(repointed 0\)/);
    expect(fs.readFileSync(settingsFile, 'utf-8')).toBe(before);
  });

  test('backups rotate: at most 10 .bak files survive repeated mutations', () => {
    for (let i = 0; i < 13; i++) {
      runIso(['add-event', '--event', 'Stop', '--command', `/x/hosts/claude/hooks/timeline-stop-hook-${i}`, '--source', 'cavestack-timeline-stop']);
    }
  ...
  test('tag-stripped verify-gate entry is table-owned: healed by --repoint, swept by --all', () => {
    ...
    const canon = mkCanon(tmpDir);
    const vg = path.join(canon, 'bin', 'cavestack-verify-gate');
    fs.writeFileSync(vg, '#!/bin/sh\n');
    fs.chmodSync(vg, 0o755);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: { Stop: [hookEntry('/dead/install/bin/cavestack-verify-gate')] },   // tag STRIPPED
    }, null, 2));
    runIso(['prune-stale', '--repoint', canon]);
    let s = settings();
    expect(s.hooks.Stop[0].hooks[0].command).toBe(vg);
    expect(s.hooks.Stop[0]._cavestack_source).toBe('verify-gate');
  ...
  test('a foreign entry that STARTED empty survives prune-stale untouched', () => {
    ...
    expect(r.stdout).toMatch(/removed 0 cavestack hook entries \(repointed 0\)/);
    expect(fs.readFileSync(settingsFile, 'utf-8')).toBe(before);
  });
...
describe('remove-source: per-item', () => {
  test('mixed tagged entry: cavestack item removed, user item survives, tag dropped', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [hookEntry(
    ...
    expect(s.hooks.PostToolUse[0]._cavestack_source).toBeUndefined();
  });
});

describe('prune-stale', () => {
  test('prunes dead cavestack items; keeps live cavestack and dead non-cavestack', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
          hookEntry(`${canon}/hosts/claude/hooks/question-log-hook`, AUQ_MATCHER),   // live cavestack
          hookEntry('/dead/wt/hosts/claude/hooks/auq-error-fallback-hook', AUQ_MATCHER), // dead cavestack
          hookEntry('/dead/user/own-hook', AUQ_MATCHER),                              // dead NON-cavestack
        ],
      },
    }, null, 2));
    const r = runIso(['prune-stale']);
    expect(r.stdout).toMatch(/removed 1 cavestack hook entries/);
    const s = settings();
    expect(s.hooks.PostToolUse).toHaveLength(2);
  ...
  test('--repoint re-points dead AND live items, preserves bash prefix, restores tags', () => {
    ...
    expect(s.hooks.Stop[0]._cavestack_source).toBe('cavestack-timeline-stop');
  ...
  test('Windows backslash path is classified as cavestack-owned and pruned when dead', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
  ...
  test('--all removes live untagged cavestack items, spares user hooks and mixed-entry user items', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [hookEntry(`${canon}/hosts/claude/hooks/question-log-hook`, AUQ_MATCHER)],
        Stop: [hookEntry(
          `${canon}/hosts/claude/hooks/timeline-stop-hook`, undefined, 'cavestack-timeline-stop',
    ...
    expect(s.hooks.Stop[0]._cavestack_source).toBeUndefined();
  ...
  test('--all removes tagged single-item legacy strays (no table match)', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: { Stop: [hookEntry('/old/install/bin/cavestack-verify-gate', undefined, 'cavestack-verify-gate')] },
    }, null, 2));
    const r = runIso(['prune-stale', '--all']);
    expect(r.stdout).toMatch(/removed 1/);
    expect(settings().hooks).toBeUndefined();
  ...
  test('explicit plan_tune_hooks:no — dead plan-tune pruned, live plan-tune NOT re-pointed, Stop still re-pointed', () => {
    const canon = mkCanon(tmpDir);
    const live = mkCanon(tmpDir, 'live-worktree');
    execSync(`'${path.join(ROOT, 'bin', 'cavestack-config')}' set plan_tune_hooks no`, {
      env: { ...process.env, CAVESTACK_STATE_ROOT: tmpDir },
    });
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
  ...
  test('incident facsimile: the exact live-damage shape heals to canonical', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [hookEntry(`${canon}/bin/cavestack-session-update`)],
        PostToolUse: [
        ...
        Stop: [
          ...
          hookEntry(dead2('timeline-stop-hook'), undefined, 'cavestack-timeline-stop'),
        ],
        PreCompact: [hookEntry('/Users/me/my-own-hook')],
      },
    }, null, 2));
    ...
    for (const ev of ['SessionStart', 'PostToolUse', 'PreToolUse', 'Stop']) {
      for (const e of s.hooks[ev]) {
        expect(e._cavestack_source).toBeDefined();
        for (const it of e.hooks) expect(it.command.startsWith(canon)).toBe(true);
      }
    }
    const postSources = s.hooks.PostToolUse.map((e: any) => e._cavestack_source).sort();
...
describe('fail-closed parse (pre-existing data-loss fix)', () => {
  const MUTATORS: string[][] = [
    ['add', '/x/bin/cavestack-session-update'],
    ['remove', '/x/bin/cavestack-session-update'],
...
describe('mutation lock', () => {
  test('stale lock (old mtime) is taken over; mutation proceeds', () => {
    ...
    const r = runIso(['add-event', '--event', 'Stop', '--command', '/x/hosts/claude/hooks/timeline-stop-hook', '--source', 'cavestack-timeline-stop']);
    expect(r.exitCode).toBe(0);
  ...
  test('two concurrent add-events both land (lock serializes; file stays valid JSON)', () => {
    ...
    execSync(`sh -c "${a} & ${b} & wait"`, {
      env: { ...process.env, CAVESTACK_SETTINGS_FILE: settingsFile, CAVESTACK_STATE_ROOT: tmpDir },
      encoding: 'utf-8',
      timeout: 20000,
    });
...
describe('cavestack-settings-hook adversarial hardening', () => {
  let tmpDir: string;
  let settingsFile: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-settings-adv-'));
    settingsFile = path.join(tmpDir, 'settings.json');
  });
  ...
  test('foreign hook whose basename collides with Object.prototype survives --all', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        Stop: [
          ...
          { _cavestack_source: 'cavestack-timeline-stop', hooks: [{ type: 'command', command: '/x/hosts/claude/hooks/timeline-stop-hook' }] },
        ],
      },
    }, null, 2) + '\n');
    const r = runAdv(['prune-stale', '--all']);
    expect(r.exitCode).toBe(0);
    const s = advSettings();
    expect(s.hooks.Stop).toHaveLength(1);   // cavestack entry swept...
    expect(s.hooks.Stop[0].hooks[0].command).toBe('/usr/local/bin/toString');  // ...foreign one kept
  });

  test('CAVESTACK_SWEEP_EXCLUDE_SOURCES preserves verify-gate during an --all sweep', () => {
    // `setup --no-team` sweeps team hooks but must not delete the
    // user-registered verify-gate opt-in whose binary still exists.
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        Stop: [
          { _cavestack_source: 'verify-gate', hooks: [{ type: 'command', command: '/x/bin/cavestack-verify-gate' }] },
          { _cavestack_source: 'cavestack-timeline-stop', hooks: [{ type: 'command', command: '/x/hosts/claude/hooks/timeline-stop-hook' }] },
        ],
      },
    }, null, 2) + '\n');
    const r = runAdv(['prune-stale', '--all'], { CAVESTACK_SWEEP_EXCLUDE_SOURCES: 'verify-gate' });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toMatch(/removed 1 /);
    const s = advSettings();
    expect(s.hooks.Stop).toHaveLength(1);
