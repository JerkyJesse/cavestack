 ...
 *
 * cavestack-review-log WRITES `<canonical-branch>-reviews.jsonl` where the
 * canonical form comes from bin/cavestack-slug (tr '/' '-' then
 ...
 *     cavestack-slug eval that opens Context Recovery).
...
describe('branch slug hygiene (#2550, #1851)', () => {
  ...
  test('Context Recovery probes reviews.jsonl with the slug-canonical $BRANCH', () => {
    ...
    // The cavestack-slug eval that defines $BRANCH must render BEFORE the probe.
    const evalIdx = out.indexOf('cavestack-slug');
  ...
  test('plan content-search BRANCH uses the full cavestack-slug canonical pipeline', () => {
  ...
  test('live round-trip: cavestack-review-log writes, Context Recovery probe finds it (slash branch)', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-home-'));
    const repo = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-repo-'));
    try {
      ...
      // Writer: the real cavestack-review-log (canonicalizes via cavestack-slug).
      execSync(
        `"${path.join(ROOT, 'bin', 'cavestack-review-log')}" '{"skill":"ship","status":"ok"}'`,
        { cwd: repo, env, encoding: 'utf-8' },
      );

      // The slug-canonical filename must exist; the raw form must not.
      const slugVars = execSync(`"${path.join(ROOT, 'bin', 'cavestack-slug')}"`, {
        cwd: repo, env, encoding: 'utf-8',
      });
      ...
      // Reader: execute the rendered probe line with $BRANCH from cavestack-slug.
      const ctx: TemplateContext = {
