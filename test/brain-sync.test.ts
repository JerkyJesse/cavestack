 ...
 *   - bin/cavestack-config gbrain keys (validation, isolation)
 *   - bin/cavestack-brain-enqueue (atomicity, skip list, no-op gates)
 *   - bin/cavestack-jsonl-merge (3-way, ts-sort, hash-fallback)
 *   - bin/cavestack-brain-sync --once (drain, commit, push, secret-scan, skip-file)
 *   - bin/cavestack-artifacts-init + --restore round-trip
 *   - bin/cavestack-brain-uninstall preserves user data
 *   - env isolation (CAVESTACK_HOME never bleeds into real ~/.cavestack/config.yaml)
 *
 * Runs each test against a temp CAVESTACK_HOME and a local bare git repo as
...
function run(argv: string[], opts: { env?: Record<string, string>; input?: string } = {}) {
  ...
  const res = spawnSync(full, argv.slice(1), {
    // HOME is overridden too: cavestack-artifacts-init writes
    // $HOME/.cavestack-artifacts-remote.txt (plain $HOME, not CAVESTACK_HOME), so
    ...
    env: { ...process.env, HOME: tmpHome, CAVESTACK_HOME: tmpHome, ...(opts.env || {}) },
    encoding: 'utf-8',
    input: opts.input,
    cwd: ROOT,
  });
...
afterEach(() => {
  ...
  for (const name of ['.cavestack-brain-remote.txt', '.cavestack-artifacts-remote.txt']) {
...
// ---------------------------------------------------------------
describe('cavestack-config gbrain keys', () => {
  test('default artifacts_sync_mode is off', () => {
    const r = run(['cavestack-config', 'get', 'artifacts_sync_mode']);
    expect(r.status).toBe(0);
    expect(r.stdout.trim()).toBe('off');
  });

  test('default artifacts_sync_mode_prompted is false', () => {
    const r = run(['cavestack-config', 'get', 'artifacts_sync_mode_prompted']);
    expect(r.stdout.trim()).toBe('false');
  });

  test('accepts full / artifacts-only / off', () => {
    for (const val of ['full', 'artifacts-only', 'off']) {
      const set = run(['cavestack-config', 'set', 'artifacts_sync_mode', val]);
      expect(set.status).toBe(0);
      const get = run(['cavestack-config', 'get', 'artifacts_sync_mode']);
      expect(get.stdout.trim()).toBe(val);
    }
  });

  test('invalid artifacts_sync_mode value warns + defaults', () => {
    const r = run(['cavestack-config', 'set', 'artifacts_sync_mode', 'bogus']);
    expect(r.stderr).toContain('not recognized');
    const get = run(['cavestack-config', 'get', 'artifacts_sync_mode']);
    expect(get.stdout.trim()).toBe('off');
  });

  test('CAVESTACK_HOME overrides real config dir', () => {
    // Real ~/.cavestack/config.yaml must not change, regardless of what it
    // already contains on the developer's machine.
    const realConfig = path.join(os.homedir(), '.cavestack', 'config.yaml');
    const before = fs.existsSync(realConfig) ? fs.readFileSync(realConfig, 'utf-8') : null;

    run(['cavestack-config', 'set', 'artifacts_sync_mode', 'full']);
    ...
    // Real ~/.cavestack/config.yaml must not be touched.
...
// Enqueue behavior
// ---------------------------------------------------------------
describe('cavestack-brain-enqueue', () => {
  test('no-op when feature not initialized', () => {
    const r = run(['cavestack-brain-enqueue', 'projects/foo/learnings.jsonl']);
    expect(r.status).toBe(0);
  ...
  test('enqueues one spool file when mode is full and .git exists', () => {
    fs.mkdirSync(path.join(tmpHome, '.git'), { recursive: true });
    run(['cavestack-config', 'set', 'artifacts_sync_mode', 'full']);
    run(['cavestack-brain-enqueue', 'projects/foo/learnings.jsonl']);
  ...
  test('skip list honored', () => {
    ...
    run(['cavestack-brain-enqueue', 'projects/foo/secret.jsonl']);
    run(['cavestack-brain-enqueue', 'projects/foo/ok.jsonl']);
  ...
  test('concurrent enqueues all land (one spool file per record)', async () => {
    ...
    for (let i = 0; i < 10; i++) {
      procs.push(new Promise<void>((resolve) => {
        const r = spawnSync(path.join(BIN, 'cavestack-brain-enqueue'), [`file-${i}.jsonl`], {
          env: { ...process.env, CAVESTACK_HOME: tmpHome },
          encoding: 'utf-8',
        });
        resolve();
      }));
  ...
  test('no args does not crash', () => {
    const r = run(['cavestack-brain-enqueue']);
    expect(r.status).toBe(0);
  });
});
...
// JSONL merge driver
// ---------------------------------------------------------------
describe('cavestack-jsonl-merge', () => {
  test('3-way merge dedups + sorts by ts', () => {
    ...
    const r = run([path.join(BIN, 'cavestack-jsonl-merge'), base, ours, theirs]);
    expect(r.status).toBe(0);
  ...
  test('falls back to hash order for lines without ts', () => {
    ...
    run([path.join(BIN, 'cavestack-jsonl-merge'), base, ours, theirs]);
    const lines = fs.readFileSync(ours, 'utf-8').trim().split('\n');
    expect(lines.length).toBe(3);
    // Order is deterministic (sha256 of each line).
    const again = spawnSync(path.join(BIN, 'cavestack-jsonl-merge'), [base, ours, theirs]);
...
describe('init + sync + restore round-trip', () => {
  test('init creates canonical files + registers drivers', () => {
    const r = run(['cavestack-artifacts-init', '--remote', bareRemote]);
    expect(r.status).toBe(0);
    ...
    expect(cfg.stdout).toContain('cavestack-jsonl-merge');
  });

  test('refuses init on different remote', () => {
    run(['cavestack-artifacts-init', '--remote', bareRemote]);
    const otherRemote = fs.mkdtempSync(path.join(os.tmpdir(), 'brain-other-'));
    spawnSync('git', ['init', '--bare', '-q', '-b', 'main', otherRemote]);
    const r = run(['cavestack-artifacts-init', '--remote', otherRemote]);
  ...
  test('full sync: init → enqueue → --once → commit pushed', () => {
    ...
    run(['cavestack-brain-enqueue', 'projects/p/learnings.jsonl']);
    const r = run(['cavestack-brain-sync', '--once']);
    expect(r.status).toBe(0);
  ...
  test('restore round-trip: writes on machine A visible on machine B', () => {
    ...
    run(['cavestack-brain-enqueue', 'projects/myproj/learnings.jsonl']);
    run(['cavestack-brain-sync', '--once']);

    // Machine B (new temp home).
    const machineB = fs.mkdtempSync(path.join(os.tmpdir(), 'brain-machineB-'));
    const r = run(['cavestack-brain-restore', bareRemote], {
      env: { CAVESTACK_HOME: machineB },
    });
    expect(r.status).toBe(0);
...
// ---------------------------------------------------------------
describe('cavestack-brain-sync secret scan', () => {
  ...
  test('--skip-file unblocks specific file', () => {
    ...
    run(['cavestack-brain-enqueue', leakPath]);
    run(['cavestack-brain-sync', '--once']);  // blocked
    run(['cavestack-brain-sync', '--skip-file', leakPath]);
...
// ---------------------------------------------------------------
describe('cavestack-brain-sync egress receipt gate', () => {
  test('refused receipt leaves the queue intact, makes no commit, and next run retries', () => {
    ...
    try {
      const refused = run(['cavestack-brain-sync', '--once']);
    ...
    const retry = run(['cavestack-brain-sync', '--once']);
...
// ---------------------------------------------------------------
describe('cavestack-brain-uninstall', () => {
  test('removes sync config but preserves learnings/project data', () => {
    ...
    const r = run(['cavestack-brain-uninstall', '--yes']);
    expect(r.status).toBe(0);
    ...
    // Config key reset.
    const mode = run(['cavestack-config', 'get', 'artifacts_sync_mode']);
...
// ---------------------------------------------------------------
describe('cavestack-brain-sync --discover-new', () => {
  test('enqueues new allowlisted files as spool records; idempotent on re-run', () => {
    ...
    run(['cavestack-brain-sync', '--discover-new']);
...
describe('enqueue tmp janitor', () => {
  test('an orphaned .tmp-* older than 1h is reaped on --once; a fresh one survives', () => {
    ...
    expect(run(['cavestack-brain-sync', '--once']).status).toBe(0);
...
describe('#2549 queue integrity', () => {
  function initWithMode(mode: string) {
    run(['cavestack-artifacts-init', '--remote', bareRemote]);
    run(['cavestack-config', 'set', 'artifacts_sync_mode', mode]);
  }
  ...
  test('privacy-held entries are RETAINED and classified, not wiped as "no allowlisted changes"', () => {
    ...
    run(['cavestack-brain-enqueue', 'projects/p/timeline.jsonl']);
    const r = run(['cavestack-brain-sync', '--once']);
    expect(r.status).toBe(0);
  ...
  test('push failure retains the commit locally and the run-start detector re-pushes it', () => {
    ...
    run(['cavestack-brain-enqueue', 'projects/p/learnings.jsonl']);
    const fail = run(['cavestack-brain-sync', '--once']);
  ...
  test('an interleaved user commit disables the detector push (exclusive author gate)', () => {
    ...
    // A user manually commits in ~/.cavestack on top of the stranded bot commit.
...
describe('C12 spool queue', () => {
  ...
  test('two rapid enqueues of different paths create two spool files; one drain syncs both', () => {
    ...
    fs.writeFileSync(path.join(tmpHome, 'retros/week-1.md'), '# retro\n');
    run(['cavestack-brain-enqueue', 'projects/p/learnings.jsonl']);
    run(['cavestack-brain-enqueue', 'retros/week-1.md']);
  ...
  test('--status queue_depth counts spool records plus unmigrated legacy lines', () => {
    ...
    const r = run(['cavestack-brain-sync', '--status']);
    expect(r.status).toBe(0);
  ...
  test('G1: finalize is explicit-delete-only and the fast path is .migrating-aware (static pins)', () => {
    const src = fs.readFileSync(path.join(BIN, 'cavestack-brain-sync'), 'utf-8');
  ...
  test('--drop-queue keeps the --yes gate and counts spool + legacy entries', () => {
    ...
    seedSpool('{"file":"projects/p/b.jsonl","ts":"t"}');
    fs.writeFileSync(path.join(tmpHome, '.brain-queue.jsonl'), '{"file":"projects/p/c.jsonl","ts":"t"}\n');
    const refused = run(['cavestack-brain-sync', '--drop-queue']);
    expect(refused.status).toBe(1);
    expect(refused.stderr).toContain('--yes');
    expect(spoolFiles().length).toBe(2);
    const dropped = run(['cavestack-brain-sync', '--drop-queue', '--yes']);
    ...
    const again = run(['cavestack-brain-sync', '--drop-queue', '--yes']);
