 ...
 * project cwd (the file lives only under the cavestack install root). The
 ...
 * $CAVESTACK_ROOT on env-var hosts).
 */
...
describe('question-tuning registry path is absolute (#2489)', () => {
  test('claude host renders the installed registry path', () => {
    ...
    expect(out).toContain('`~/.claude/skills/cavestack/scripts/question-registry.ts`');
  });

  test('env-var host renders $CAVESTACK_ROOT-anchored registry path', () => {
    const out = generateQuestionTuning(makeCtx('codex'));
    expect(out).toContain('`$CAVESTACK_ROOT/scripts/question-registry.ts`');
  });
  ...
  test('rendered SKILL.md carries the absolute path', () => {
    ...
    expect(rendered).toContain('~/.claude/skills/cavestack/scripts/question-registry.ts');
