...
// cavestack-config accepts arbitrary keys (free-form YAML store), so we can't
  ...
  while (stack.length) {
    ...
    for (const ent of fs.readdirSync(cur, { withFileTypes: true })) {
      ...
      lines.forEach((line, idx) => {
        // Match `cavestack-config set <key>` or `cavestack-config get <key>`.
        for (const m of line.matchAll(/cavestack-config\s+(?:set|get)\s+([a-z][a-z0-9_]*)/g)) {
...
function runConfig(args: string[], tmpHome: string) {
  return spawnSync(CONFIG_BIN, args, {
    encoding: 'utf-8',
    env: { ...process.env, HOME: tmpHome, CAVESTACK_HOME: tmpHome },
    timeout: 5000,
  });
}

describe('docs ↔ cavestack-config key drift guard', () => {
  ...
  // cavestack-config is a bash script; Windows can't exec it via spawnSync
  ...
  test.skipIf(process.platform === 'win32')('`explain_level` is exposed as a documented default', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'cavestack-cfg-'));
    try {
  ...
  test.skipIf(process.platform === 'win32')('`cavestack-config get artifacts_sync_mode` returns a value (the rename landed)', () => {
