 ...
 *      Skill written to ~/.cavestack/browser-skills/<name>/ with full
 ...
 *   - A scoped CAVESTACK_HOME under the workdir so on-disk artifacts are
...
function setupSkillifyWorkdir(suffix: string, installSkills: string[] = ['scrape', 'skillify']): Workdir {
  ...
  for (const script of [
    'cavestack-timeline-log', 'cavestack-slug', 'cavestack-config',
    'cavestack-update-check', 'cavestack-repo-mode',
 ...
 * has to live under <workdir>/.cavestack/browser-skills/ for the project-tier
 * lookup to find it (cavestack's bundled tier resolves from the install dir,
...
function installBundledHackernewsSkill(workDir: string) {
  ...
  const dst = path.join(workDir, '.cavestack', 'browser-skills', 'hackernews-frontpage');
  copyDirSync(src, dst);
}
...
describeIfSelected('Browser-skills Phase 2a E2E (/scrape + /skillify)', [
  ...
  testConcurrentIfSelected('scrape-match-path', async () => {
    const { workDir, cavestackHome } = setupSkillifyWorkdir('match', ['scrape']);
...
Do NOT enter the prototype phase. Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('scrape-prototype-path', async () => {
    const { workDir, cavestackHome } = setupSkillifyWorkdir('prototype', ['scrape']);
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('skillify-happy-path', async () => {
    const { workDir, cavestackHome } = setupSkillifyWorkdir('happy', ['scrape', 'skillify']);
    const fixturePath = path.join(workDir, 'fixture.html');
    fs.writeFileSync(fixturePath, PROTOTYPE_FIXTURE_HTML);
    const fileUrl = `file://${fixturePath}`;
...
(translates to ~/.cavestack/browser-skills/<name>/ via $HOME).

Do NOT halt for clarification.`,
      workingDirectory: workDir,
      env: {
        CAVESTACK_HOME: cavestackHome,
        HOME: workDir,  // /skillify writes to $HOME/.cavestack/browser-skills/
      },
      maxTurns: 40,
    ...
    // The skill should land in $HOME/.cavestack/browser-skills/<name>/
    const skillsRoot = path.join(workDir, '.cavestack', 'browser-skills');
  ...
  testConcurrentIfSelected('skillify-provenance-refusal', async () => {
    const { workDir, cavestackHome } = setupSkillifyWorkdir('refusal', ['skillify']);
...
write any files.`,
    ...
    const skillsRoot = path.join(workDir, '.cavestack', 'browser-skills');
    const stagingRoot = path.join(workDir, '.cavestack', '.tmp');
  ...
  testConcurrentIfSelected('skillify-approval-reject', async () => {
    const { workDir, cavestackHome } = setupSkillifyWorkdir('reject', ['scrape', 'skillify']);
    const fixturePath = path.join(workDir, 'fixture.html');
    fs.writeFileSync(fixturePath, PROTOTYPE_FIXTURE_HTML);
    const fileUrl = `file://${fixturePath}`;
