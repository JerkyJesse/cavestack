...
describeE2E('/setup-gbrain Path 4 — bad token STOPs cleanly', () => {
  test('AUTH classifier fires, no MCP registration, no CLAUDE.md mutation', async () => {
    ...
    const cavestackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-bad-'));
    ...
    fs.writeFileSync(path.join(cavestackHome, 'CLAUDE.md'), ORIGINAL_CLAUDE_MD);
    ...
    const orig = {
      cavestackHome: process.env.CAVESTACK_HOME,
      pathEnv: process.env.PATH,
      mcpToken: process.env.GBRAIN_MCP_TOKEN,
    };
    process.env.CAVESTACK_HOME = cavestackHome;
    ...
    try {
      ...
      const finalClaudeMd = fs.readFileSync(path.join(cavestackHome, 'CLAUDE.md'), 'utf-8');
      ...
      if (orig.cavestackHome === undefined) delete process.env.CAVESTACK_HOME; else process.env.CAVESTACK_HOME = orig.cavestackHome;
