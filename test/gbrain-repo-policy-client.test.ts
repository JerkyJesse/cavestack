 ...
 *
 * Covers the `get --batch` verb of bin/cavestack-gbrain-repo-policy (bash level:
 ...
 *
 * Each test uses a temp CAVESTACK_HOME so nothing leaks into the user's real
 * ~/.cavestack. The client is exercised against the REAL bash script — the
...
import { canonicalizeRemote } from "../lib/cavestack-memory-helpers";

const ROOT = path.resolve(import.meta.dir, "..");
const BIN = path.join(ROOT, "bin", "cavestack-gbrain-repo-policy");

let tmpHome: string;

function env(): NodeJS.ProcessEnv {
  return { ...process.env, CAVESTACK_HOME: tmpHome };
}
...
describe("bin/cavestack-gbrain-repo-policy get --batch (bash level)", () => {
...
//
// bin/cavestack-memory-ingest.ts produces page.git_remote via canonicalizeRemote
// (lib/cavestack-memory-helpers) and then looks the policy up through
