 ...
 *
 * Static tripwires over `setup` and `bin/cavestack-settings-hook`. The defect
 * class these pin against: hook commands baked from the SETUP-TIME tree
 * (`$SOURCE_CAVESTACK_DIR` = `pwd -P` of the running tree) into the user's
 ...
 *     install: ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/cavestack) — an
...
const hookBinSrc = fs.readFileSync(path.join(ROOT, 'bin', 'cavestack-settings-hook'), 'utf-8');
const uninstallSrc = fs.readFileSync(path.join(ROOT, 'bin', 'cavestack-uninstall'), 'utf-8');

describe('setup: canonical-only hook paths', () => {
  test('CANONICAL_CAVESTACK_ROOT honors CLAUDE_CONFIG_DIR with ~/.claude fallback', () => {
    expect(setupSrc).toContain(
      'CANONICAL_CAVESTACK_ROOT="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/cavestack"',
    );
  });
  ...
  test('_hook_command_path body never references the running tree', () => {
    ...
    expect(body).not.toContain('SOURCE_CAVESTACK_DIR');
    expect(body).toContain('CANONICAL_CAVESTACK_ROOT');
  });

  test('every hook var routes through _hook_command_path; no raw SOURCE_CAVESTACK_DIR hook assignment remains', () => {
    for (const rel of [
    ...
    // The bug: FOO_HOOK="$SOURCE_CAVESTACK_DIR/hosts/claude/hooks/..."
    expect(setupSrc).not.toMatch(/="\$SOURCE_CAVESTACK_DIR\/hosts\/claude\/hooks\//);
    expect(setupSrc).not.toMatch(/HOOK_CMD="(bash )?\$SOURCE_CAVESTACK_DIR\/bin\/cavestack-session-update"/);
  });

  test('SessionStart registers via schema-aware add-event under its identity source', () => {
    expect(setupSrc).toMatch(
      /add-event --event SessionStart --command "\$HOOK_CMD" --source cavestack-session-update/,
    );
  });
...
describe('setup: heal-first ordering + visibility', () => {
  ...
  test('--no-team teardown includes the auq source and the identity sweep', () => {
    ...
    expect(slice).toContain('remove-source --source cavestack-timeline-stop');
    expect(slice).toContain('prune-stale --all');
  });
});

describe('cavestack-settings-hook: shared prelude (dedupe key == prune predicate)', () => {
  ...
  test('KNOWN_HOOKS table carries all five identities with source+event+relpath', () => {
    for (const [name, source, event] of [
      ...
      ['timeline-stop-hook', 'cavestack-timeline-stop', 'Stop'],
      ['cavestack-session-update', 'cavestack-session-update', 'SessionStart'],
    ]) {
...
describe('cavestack-uninstall: hook cleanup runs before install-root deletion', () => {
  test('the settings cleanup block precedes every install-root rm -rf', () => {
    // Pre-fix bug: SETTINGS_HOOK=$(dirname "$0")/cavestack-settings-hook resolved
    ...
    const cleanup = uninstallSrc.indexOf('Remove cavestack hooks from Claude Code settings');
    const rootDelete = uninstallSrc.indexOf('rm -rf "$CLAUDE_SKILLS/cavestack"');
  ...
  test('uninstall removes all three sources and sweeps untagged strays', () => {
    ...
    expect(uninstallSrc).toContain('remove-source --source cavestack-timeline-stop');
...
describe('the defect-class warning is written down where the next author will see it', () => {
  test('setup carries the never-register-tree-relative-paths warning', () => {
    expect(setupSrc).toMatch(/NEVER register .*SOURCE_CAVESTACK_DIR.*hook paths/);
  });
});
