/**
 * Locale-independent key validation tests for bin/cavestack-config.
 *
...
const CONFIG = path.join(ROOT, "bin", "cavestack-config");
...
function run(args: string[]) {
  const result = spawnSync(CONFIG, args, {
    encoding: "utf8",
    // CAVESTACK_SETUP_RUNNING suppresses `set skill_prefix`'s auto-relink side
    // effect. Without it, this test invokes the REPO's cavestack-config, whose
    // auto-relink resolves the install dir from its own path — i.e. the repo —
    // and cavestack-patch-names rewrites all 52 tracked SKILL.md files to
    // cavestack-prefixed names, poisoning every downstream test that reads the
    ...
    env: { ...process.env, CAVESTACK_STATE_ROOT: stateRoot, CAVESTACK_SETUP_RUNNING: "1" },
  });
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cavestack-config-locale-"));
});
...
describe("cavestack-config key validation is locale-independent", () => {
