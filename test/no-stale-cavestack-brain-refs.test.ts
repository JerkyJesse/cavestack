/**
 * Regression: no stale `cavestack-brain-init`, `gbrain_sync_mode`, or
 * `~/.cavestack-brain-remote.txt` references survive the v1.27.0.0 rename.
 *
 ...
 *
 * Allowlist: the migration script (`cavestack-upgrade/migrations/v1.27.0.0.sh`)
...
const ALLOWLIST = [
  ...
  'cavestack-upgrade/migrations/v1.27.0.0.sh',
  // Older migration scripts — historical references; these document past state.
  'cavestack-upgrade/migrations/v1.17.0.0.sh',
  ...
  'test/cavestack-upgrade-migration-v1_17_0_0.test.ts',
  ...
  // The bin/cavestack-config comment explicitly preserves the rename note.
  ...
  'bin/cavestack-gbrain-source-wireup',
  'bin/cavestack-brain-uninstall',
  ...
  // cavestack-upgrade.test.ts may exercise old migration behavior.
  'test/cavestack-upgrade.test.ts',
  // This test itself references the patterns to grep for.
  'test/no-stale-cavestack-brain-refs.test.ts',
  ...
  'test/cavestack-artifacts-init.test.ts',
...
const SCAN_PATHS = [
  ...
  // a dead command (cavestack-brain-init) survive ~36 releases as a
  // command-not-found instruction. Docs are user-facing surface; scan them.
  'docs/',
  'README.md',
  'USING_GBRAIN_WITH_CAVESTACK.md',
];
...
describe('no stale cavestack-brain refs (v1.27.0.0 rename)', () => {
