...
describe('preamble — QUESTION_TUNING injection', () => {
  test('tier 2+ skills include the Question Tuning section', async () => {
    ...
    const ctx = {
      ...
      paths: {
        skillRoot: '~/.claude/skills/cavestack',
        localSkillRoot: '.claude/skills/cavestack',
        binDir: '~/.claude/skills/cavestack/bin',
        browseDir: '~/.claude/skills/cavestack/browse/dist',
        designDir: '~/.claude/skills/cavestack/design/dist',
      },
      preambleTier: 2,
    };
    ...
    expect(out).toContain('cavestack-question-preference --check');
    expect(out).toContain('cavestack-question-log');
  ...
  test('codex host produces different paths', async () => {
    ...
    expect(out).toContain('$CAVESTACK_BIN/cavestack-question-preference');
    expect(out).toContain('$CAVESTACK_BIN/cavestack-question-log');
  });
});
...
describe('end-to-end pipeline (binaries working together)', () => {
  test('log many expand choices → derive pushes scope_appetite up', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'cavestack-e2e-'));
    try {
      // CAVESTACK_QUESTION_LOG_NO_DERIVE=1 suppresses cavestack-question-log's
      ...
      const env = { ...process.env, CAVESTACK_HOME: tmpHome, CAVESTACK_QUESTION_LOG_NO_DERIVE: '1' };
      const { spawnSync } = require('child_process');
      const logBin = path.join(ROOT, 'bin', 'cavestack-question-log');
      const devBin = path.join(ROOT, 'bin', 'cavestack-developer-profile');
  ...
  test('preference blocks tune: write from inline-tool-output in full pipeline', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'cavestack-e2e-'));
    try {
      const env = { ...process.env, CAVESTACK_HOME: tmpHome };
      const { spawnSync } = require('child_process');
      const prefBin = path.join(ROOT, 'bin', 'cavestack-question-preference');
  ...
  test('migration preserves sessions, builder-profile shim still works', () => {
    ...
    try {
      ...
      const { spawnSync } = require('child_process');
      const devBin = path.join(ROOT, 'bin', 'cavestack-developer-profile');
      const shimBin = path.join(ROOT, 'bin', 'cavestack-builder-profile');
