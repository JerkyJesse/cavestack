...
// via a crafted CAVESTACK_HOME so the test is deterministic regardless of whether
...
//   (c) its section refs point at the out-dir, not ~/.claude/skills/cavestack,
...
describe('gen-skill-docs --out-dir (B2 render isolation)', () => {
  ...
  test('renders :user to out-dir, rewrites section paths, leaves worktree canonical', () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-home-'));
    const outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-out-'));
    ...
    try {
      ...
      const res = spawnSync(
        ...
        { cwd: ROOT, encoding: 'utf-8', timeout: 120_000, env: { ...process.env, CAVESTACK_HOME: tmpHome } },
      );
      ...
      expect(skillContent).not.toContain('~/.claude/skills/cavestack/ship/sections/');

      // (d) bin refs are NOT rewritten — they still resolve to the global install
      expect(skillContent).toContain('~/.claude/skills/cavestack/bin/');
