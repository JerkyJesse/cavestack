 ...
 *
 * Two compounding bugs, both in cavestack:
 *
 ...
 *
 *   2. Trailing slash in TMP_ROOT. `bin/cavestack-paths` emitted TMP_ROOT straight
...
const PATHS_BIN = path.join(ROOT, 'bin', 'cavestack-paths');
...
/** Directories that are not cavestack-authored skill/template sources. */
...
function collectScannedFiles(dir: string, out: string[]): void {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    // Symlinks (e.g. connect-chrome → open-cavestack-browser) would double-count
...
describe('#2091/#2370 bug 1: every mktemp template is BSD-safe (X placeholder at end)', () => {
  ...
  test('the offender-detector itself still detects the original bug shapes', () => {
    ...
    expect(
      offendingTemplatesOnLine('TMPOUT=$(mktemp "$CAVESTACK_HOME/developer-profile.json.XXXXXX.tmp")'),
    ).toEqual(['$CAVESTACK_HOME/developer-profile.json.XXXXXX.tmp']);
    expect(offendingTemplatesOnLine('RESP_FILE=$(mktemp /tmp/cavestack-claude-response-XXXXXX.json)')).toEqual([
      '/tmp/cavestack-claude-response-XXXXXX.json',
    ]);
    // Fixed shapes pass.
...
// ── Bug 2: cavestack-paths normalizes TMP_ROOT (no trailing slash) ──────────────
// Mirrors the invocation contract used by test/cavestack-paths.test.ts: the helper
...
function tmpRoot(env: Record<string, string | undefined>): string {
  ...
  throw new Error('cavestack-paths did not emit TMP_ROOT');
}

describe('#2091 bug 2: cavestack-paths strips the trailing slash from TMP_ROOT', () => {
