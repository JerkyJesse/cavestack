...
function buildShippedFixture(): ShipFixture {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-ship-fixture-'));
    ...
    async () => {
      ...
      try {
        ...
        while (Date.now() - start < budgetMs) {
          ...
          // (T9), Step 12 runs `cavestack-version-bump classify` which emits JSON
          ...
          //   - agent ran the CLI write path (cavestack-version-bump write) — a
          //     re-bump on an already-shipped branch
          if (
            /"state":\s*"FRESH"/.test(visible) ||
            /STATE:\s*FRESH(?![\w-])/i.test(visible) ||
            /cavestack-version-bump\s+write/i.test(visible) ||
