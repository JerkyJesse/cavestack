...
describeIfSelected('Session Intelligence E2E', [
  ...
  beforeAll(() => {
    ...
    // Compute slug (same logic as cavestack-slug without git remote)
  ...
  // Write a timeline event via cavestack-timeline-log, read it back via cavestack-timeline-read.
  ...
  testConcurrentIfSelected('timeline-event-flow', async () => {
    ...
    const logBin = path.join(workDir, 'bin', 'cavestack-timeline-log');
    const readBin = path.join(workDir, 'bin', 'cavestack-timeline-read');
    const env = { ...process.env, CAVESTACK_HOME: cavestackHome };
    ...
    // Read via cavestack-timeline-read
    ...
    // Verify cavestack-timeline-read output includes the events
