/**
 * Regression pin: `cavestack-memory-ingest` must pass `--include-gitignored` to
 * `gbrain import`.
 *
 * cavestack-artifacts-init writes an ignore-everything `.gitignore` (a bare `*`,
 * headed "Do not edit") at the root of `~/.cavestack`. The memory ingest stages
 * pages into `~/.cavestack/.staging-ingest-<pid>-<ts>/`, which is INSIDE that
...
const SOURCE_PATH = join(import.meta.dir, "..", "bin", "cavestack-memory-ingest.ts");
...
describe("cavestack-memory-ingest: gbrain import must not be filtered by .gitignore", () => {
  ...
  it("proves the ceiling stops git discovery from the staging dir — including through a symlink", () => {
    const dir = mkdtempSync(join(tmpdir(), "cavestack-ingest-ceiling-"));
    try {
      ...
      // ~/.cavestack shape: a git repo whose root ignores everything, with the
      // staging dir as a direct child.
      const home = join(dir, "cavestack-home");
  ...
  it("demonstrates the collision: an ignore-everything root hides staged pages", () => {
    const dir = mkdtempSync(join(tmpdir(), "cavestack-ingest-gitignore-"));
    try {
      ...
      // Exactly what cavestack-artifacts-init writes at the root of ~/.cavestack.
