...
// Static tripwires for the C (machine-wide) render in `cavestack-config
...
const SRC = fs.readFileSync(path.join(ROOT, 'bin', 'cavestack-config'), 'utf-8');
...
describe('cavestack-config gbrain-refresh: machine-wide render guards', () => {
  const branch = okBranch();

  test('targets the global install', () => {
    expect(branch).toContain('$HOME/.claude/skills/cavestack');
  });
  ...
  test('verifies it is a real cavestack clone before mutating it', () => {
