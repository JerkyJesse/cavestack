...
const BIN = path.join(ROOT, 'bin', 'cavestack-paths');
...
// helper from inside a bash block (`eval "$(~/.claude/skills/cavestack/bin/cavestack-paths)"`)
...
function run(env: Record<string, string | undefined>): Record<string, string> {
  ...
  if (result.status !== 0) {
    throw new Error(`cavestack-paths failed (status ${result.status}): ${result.stderr}`);
  }
...
describe('cavestack-paths', () => {
  test('CAVESTACK_HOME wins over CLAUDE_PLUGIN_DATA and HOME', () => {
    const got = run({
      CAVESTACK_HOME: '/tmp/explicit-state',
      CLAUDE_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.CAVESTACK_STATE_ROOT).toBe('/tmp/explicit-state');
  });

  test('CLAUDE_PLUGIN_DATA ignored when CLAUDE_PLUGIN_ROOT is absent or non-cavestack', () => {
    // Without CLAUDE_PLUGIN_ROOT, falls through to HOME path.
    const noRoot = run({ CLAUDE_PLUGIN_DATA: '/tmp/plugin-data', HOME: '/tmp/home' });
    expect(noRoot.CAVESTACK_STATE_ROOT).toBe('/tmp/home/.cavestack');

    // With a CLAUDE_PLUGIN_ROOT that doesn't contain "cavestack" (e.g. the codex plugin),
    ...
    expect(wrongRoot.CAVESTACK_STATE_ROOT).toBe('/tmp/home/.cavestack');
  });

  test('CLAUDE_PLUGIN_DATA respected when CLAUDE_PLUGIN_ROOT identifies cavestack', () => {
    const got = run({
      CLAUDE_PLUGIN_DATA: '/tmp/cavestack-plugin-data',
      CLAUDE_PLUGIN_ROOT: '/tmp/cavestack-garrytan',
      HOME: '/tmp/home',
    });
    expect(got.CAVESTACK_STATE_ROOT).toBe('/tmp/cavestack-plugin-data');
  });

  test('HOME-derived state root when CAVESTACK_HOME and CLAUDE_PLUGIN_DATA unset', () => {
    const got = run({ HOME: '/tmp/myhome' });
    expect(got.CAVESTACK_STATE_ROOT).toBe('/tmp/myhome/.cavestack');
  });
  ...
  test('CWD fallback when HOME also unset (container env)', () => {
    ...
    const got = run({ HOME: '' });
    expect(got.CAVESTACK_STATE_ROOT).toBe('.cavestack');
  });

  test('PLAN_ROOT chain: CAVESTACK_PLAN_DIR > CLAUDE_PLANS_DIR > HOME > CWD', () => {
    expect(run({ CAVESTACK_PLAN_DIR: '/tmp/explicit', HOME: '/h' }).PLAN_ROOT).toBe('/tmp/explicit');
  ...
  test('eval round-trip preserves quotes, and leaves plain paths alone (#2374)', () => {
    ...
    expect(evalRoundTrip({ CAVESTACK_HOME: '/tmp/state root' }, 'CAVESTACK_STATE_ROOT')).toBe(
      '/tmp/state root',
    );
