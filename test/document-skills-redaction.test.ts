...
describe("/document-release redaction", () => {
  test("scans the PR-body temp file before gh pr edit", () => {
    const scanIdx = RELEASE.indexOf("cavestack-redact --from-file /tmp/cavestack-pr-body");
    const editIdx = RELEASE.indexOf("gh pr edit --body-file /tmp/cavestack-pr-body");
...
describe("/document-generate redaction", () => {
  test("scans staged doc diff before commit", () => {
    const scanIdx = GENERATE.indexOf("cavestack-redact --repo-visibility");
  ...
  test("scans added lines of the staged diff", () => {
    expect(GENERATE).toMatch(/git diff --cached[\s\S]{0,80}cavestack-redact/);
  });
