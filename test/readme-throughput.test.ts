...
const ANCHOR = '<!-- CAVESTACK-THROUGHPUT-PLACEHOLDER -->';
const PENDING = 'CAVESTACK-THROUGHPUT-PENDING';
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-readme-test-'));
...
describe('update-readme-throughput script', () => {
  test('happy path: JSON present → anchor replaced with number', () => {
    fs.writeFileSync(tmpReadme, `cavestack hero: ${ANCHOR} 2013 pro-rata.\n`);
  ...
  test('anchor already replaced: script is a no-op', () => {
    fs.writeFileSync(tmpReadme, 'cavestack hero: 7.0× already set.\n');
    ...
    const updated = fs.readFileSync(tmpReadme, 'utf-8');
    expect(updated).toBe('cavestack hero: 7.0× already set.\n');
  });
});
...
describe('CI gate: committed README must not contain PENDING marker', () => {
  ...
  test('real README.md does not contain CAVESTACK-THROUGHPUT-PENDING', () => {
