/**
 * Unit tests for lib/cavestack-decision.ts — event-sourced decision memory model.
 */
...
describe("decisionPaths", () => {
  it("derives log/snapshot/archive under the project slug", () => {
    const p = decisionPaths("garrytan-cavestack", "/tmp/gs");
    expect(p.log).toBe("/tmp/gs/projects/garrytan-cavestack/decisions.jsonl");
    expect(p.snapshot).toBe("/tmp/gs/projects/garrytan-cavestack/decisions.active.json");
    expect(p.archive).toBe("/tmp/gs/projects/garrytan-cavestack/decisions.archive.jsonl");
  });
});
