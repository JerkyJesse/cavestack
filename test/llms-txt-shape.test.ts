...
describe('gen-llms-txt — shape', () => {
  test('emits required top-level sections', () => {
    expect(generated.content).toContain('# cavestack');
...
describe('gen-llms-txt — strict mode', () => {
  test('does NOT throw on the live skill set (every cavestack skill has name + description)', async () => {
...
describe('gen-llms-txt — generated file is fresh', () => {
  test('committed cavestack/llms.txt matches what the generator produces now', () => {
    const committed = fs.readFileSync(path.join(ROOT, 'cavestack', 'llms.txt'), 'utf-8');
