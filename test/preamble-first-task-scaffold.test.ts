...
//   1. bin/cavestack-first-task-detect — classifies a repo into ONE enum bucket.
//   2. The unified first-run-guidance preamble wiring (generated into SKILL.md).

const ROOT = path.join(import.meta.dir, '..');
const DETECT = path.join(ROOT, 'bin', 'cavestack-first-task-detect');
...
describe('cavestack-first-task-detect — bucket classification', () => {
...
describe('cavestack-first-task-detect — contract', () => {
...
describe('first-run-guidance preamble wiring (generated)', () => {
  ...
  test('detection is gated to the first-ever run only (ACTIVATED=no, not headless)', () => {
    ...
    expect(md).toContain('cavestack-first-task-detect');
  });
  ...
  test('marks activated + logs the scaffold telemetry only on the shown path', () => {
    ...
    expect(md).toContain('touch ~/.cavestack/.activated');
    expect(md).toContain('touch ~/.cavestack/.first-loop-tip-shown');
  });
});
