...
// #1974: cavestack-update-check runs under set -e, and its "up to date" signal
...
const SCRIPT = path.join(ROOT, 'bin', 'cavestack-update-check');
...
describe('cavestack-update-check crash sentinel (#1974)', () => {
  test('a mid-script crash emits CHECK_FAILED instead of silent up-to-date', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-upd-'));
    try {
      ...
      const r = run({ CAVESTACK_STATE_DIR: asFile, CAVESTACK_REMOTE_URL: 'file:///dev/null' });
  ...
  test('healthy up-to-date path stays silent (no sentinel noise)', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-upd-'));
    try {
      const r = run({ CAVESTACK_STATE_DIR: path.join(dir, 'state'), CAVESTACK_REMOTE_URL: 'file:///dev/null' });
      expect(r.status).toBe(0);
