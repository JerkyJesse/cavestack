/**
 * Tests for the cavestack-version-bump CLI (v2 plan T9 hybrid extraction). Covers
...
import { classifyState, VERSION_RE } from '../bin/cavestack-version-bump';

const BIN = path.join(import.meta.dir, '..', 'bin', 'cavestack-version-bump');
 ...
 * holding plain 3-digit semver — the shape cavestack's native VERSION-file
 ...
 *
 * These cases pass --version-path explicitly; the .cavestack/version-path pin
...
/**
 * #2462: cmdClassify's current-version read resolved the .cavestack/version-path
 ...
 */
describe('.cavestack/version-path pin, no --version-path flag (#2462)', () => {
  const mkPinned = (pinRel: string): string => {
    ...
    fs.mkdirSync(path.join(d, '.cavestack'), { recursive: true });
    fs.writeFileSync(path.join(d, '.cavestack', 'version-path'), pinRel + '\n');
    return d;
  };
...
describe('subdirectory manifest (no root package.json, #2531)', () => {
  ...
  const mk = (): string => {
    ...
    fs.mkdirSync(path.join(d, '.cavestack'));
    fs.writeFileSync(path.join(d, '.cavestack', 'package-json-path'), 'web/package.json\n');
...
describe('path containment: pins and flags cannot escape the repo', () => {
  // .cavestack/version-path and .cavestack/package-json-path are repo-controlled
  ...
  function resetRepo() {
    ...
    fs.mkdirSync(path.join(dir, '.cavestack'), { recursive: true });
    fs.writeFileSync(path.join(dir, 'VERSION'), '1.0.0.0\n');
    fs.writeFileSync(victim, JSON.stringify({ version: '9.9.9' }, null, 2) + '\n');
  }

  test('a ../ escape in .cavestack/version-path fails exit 2 and writes nothing', () => {
    resetRepo();
    fs.writeFileSync(path.join(dir, '.cavestack', 'version-path'), '../victim.json\n');
  ...
  test('an absolute path in .cavestack/package-json-path fails exit 2', () => {
    resetRepo();
    fs.writeFileSync(path.join(dir, '.cavestack', 'package-json-path'), victim + '\n');
    const r = runFail(['write', '--version', '1.1.0.0']);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain('outside the repository');
    expect(JSON.parse(fs.readFileSync(victim, 'utf-8')).version).toBe('9.9.9');
  ...
  test('an in-repo symlink pointing outside fails exit 2 and never follows', () => {
    ...
    fs.writeFileSync(path.join(dir, '.cavestack', 'version-path'), 'link.json\n');
    const r = runFail(['write', '--version', '1.1.0.0']);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain('outside the repository');
    expect(JSON.parse(fs.readFileSync(victim, 'utf-8')).version).toBe('9.9.9');
  ...
  test('legitimate subdirectory pins still work (containment is not over-broad)', () => {
    ...
    fs.writeFileSync(path.join(dir, '.cavestack', 'version-path'), 'frontend/package.json\n');
