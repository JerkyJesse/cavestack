 ...
 * into bin/cavestack-redact and enforces: HIGH fails (exit 1), MEDIUM is an
...
describe("gate-secret-scan.mjs fail-closed legs", () => {
  test("oversize diff (report.oversize) fails the gate", () => {
    // The script pins --max-bytes 16000000; bin/cavestack-redact refuses to scan
  ...
  test("unexpected cavestack-redact exit code fails the gate even when the report is clean", () => {
    // Stub bin/cavestack-redact that emits a CLEAN JSON report but exits 1 —
  ...
  test("missing cavestack-redact (spawn crash, empty stdout) exits nonzero — never fail-open", () => {
    // cwd with no bin/cavestack-redact at all: bun exits module-not-found with
