/**
 * cavestack-gbrain-detect — gbrain_mcp_mode + cavestack_artifacts_remote tests.
 *
...
const DETECT_BIN = path.join(ROOT, 'bin', 'cavestack-gbrain-detect');
...
function runDetect(extraEnv: Record<string, string> = {}): { code: number; json: any; stderr: string } {
  ...
  const r = spawnSync(DETECT_BIN, [], {
    env: {
      ...
      HOME: tmpHome,
      CAVESTACK_HOME: path.join(tmpHome, '.cavestack'),
      ...extraEnv,
    },
    encoding: 'utf-8',
  });
...
describe('cavestack_artifacts_remote', () => {
  test('reads ~/.cavestack-artifacts-remote.txt when present', () => {
    fs.writeFileSync(
      path.join(tmpHome, '.cavestack-artifacts-remote.txt'),
      'https://github.com/garrytan/cavestack-artifacts-garrytan\n'
    );
    expect(runDetect().json.cavestack_artifacts_remote).toBe(
      'https://github.com/garrytan/cavestack-artifacts-garrytan'
    );
  });

  test('migration-window fallback: reads ~/.cavestack-brain-remote.txt if artifacts file is missing', () => {
    fs.writeFileSync(
      path.join(tmpHome, '.cavestack-brain-remote.txt'),
      'git@github.com:garrytan/cavestack-brain-garrytan.git\n'
    );
    expect(runDetect().json.cavestack_artifacts_remote).toBe(
      'git@github.com:garrytan/cavestack-brain-garrytan.git'
    );
  ...
  test('artifacts file wins over brain file when both exist', () => {
    ...
    expect(runDetect().json.cavestack_artifacts_remote).toBe('https://github.com/x/new');
  });
