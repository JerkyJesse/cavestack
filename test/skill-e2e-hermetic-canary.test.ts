 ...
 *    `/.claude` config dir, a temp CAVESTACK_HOME, and none of the planted
    ...
    try {
      const result = await runSkillTest({
        ...
        prompt: 'Run exactly this bash command and then stop: ' +
          'echo "CFG=${CLAUDE_CONFIG_DIR:-}"; echo "GH=${CAVESTACK_HOME:-}"; ' +
      ...
      expect(cfg).toMatch(/cavestack-hermetic-.*\/\.claude$/);
      expect(bashOut).toMatch(/GH=\S*cavestack-home/);
