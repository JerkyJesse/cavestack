/**
 * cavestack-config explain_level round-trip + validation tests.
 *
 * Coverage:
...
const BIN_CONFIG = path.join(ROOT, 'bin', 'cavestack-config');
...
function run(...args: string[]): { stdout: string; stderr: string; status: number } {
  // cavestack-config precedence is `${CAVESTACK_HOME:-${CAVESTACK_STATE_DIR:-$HOME/.cavestack}}`,
  // so CAVESTACK_HOME from the developer's parent env wins over the test's
  // CAVESTACK_STATE_DIR. Override both to isolate from the real ~/.cavestack.
  const res = spawnSync(BIN_CONFIG, args, {
    env: { ...process.env, CAVESTACK_STATE_DIR: tmpHome, CAVESTACK_HOME: tmpHome },
    encoding: 'utf-8',
    cwd: ROOT,
  });
  return {
...
describe('cavestack-config explain_level', () => {
  ...
  test('get with unset explain_level returns the documented default', () => {
    // cavestack-config returns the documented default ("default") when the
    // key is absent from config.yaml — see bin/cavestack-config:103. Earlier
...
describe('cavestack-config values with spaces', () => {
