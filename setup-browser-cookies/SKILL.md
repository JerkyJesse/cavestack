...
description: |
  ...
  "login to the site", or "authenticate the browser". (cavestack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
echo '{"skill":"setup-browser-cookies","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
# Learnings count
