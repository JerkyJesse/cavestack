...
name: careful
version: 0.1.0
description: Safety guardrails for destructive commands. (cavestack)
triggers:
  - be careful
...
hooks:
  PreToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
          command: "bash $HOME/.claude/skills/cavestack/careful/bin/check-careful.sh"
...
echo '{"skill":"careful","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
`~/.cavestack/careful-patterns.txt` (global) or
`~/.cavestack/projects/<slug>/careful-patterns.txt` (per-project). Consulted
