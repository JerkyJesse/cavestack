...
if [ ! -f "$_HOOK_HELPER" ] || ! . "$_HOOK_HELPER" 2>/dev/null; then
  printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"[freeze] Hook helpers unavailable (broken install?) - blocked, fail closed. Reinstall cavestack or run /unfreeze."}}\n'
  exit 0
fi
...
set +e
FILE_PATH=$(cavestack_hook_extract_field "$INPUT" file_path)
EXTRACT_RC=$?
set -e
...
if [ "$EXTRACT_RC" -ne 0 ] && [ -n "$INPUT" ]; then
  cavestack_hook_decision deny "[freeze] Could not parse the tool payload to check the freeze boundary. Blocked (fail closed). Freeze boundary: $FREEZE_DIR"
  exit 0
fi
...
case "$FILE_PATH" in
  ...
  *)
    ...
    # Log hook fire event (shared helper respects CAVESTACK_HOME)
    cavestack_hook_log_fire freeze boundary_deny
    ...
    cavestack_hook_decision deny "[freeze] Blocked: $FILE_PATH is outside the freeze boundary ($FREEZE_DIR). Only edits within the frozen directory are allowed."
    ;;
esac
