...
preamble-tier: 2
version: 1.0.0
description: "Set up gbrain for this coding agent: install the CLI, initialize a local PGLite or Supabase brain, register MCP, capture per-remote trust policy. (cavestack)"
triggers:
  - setup gbrain
  - install gbrain
  - connect gbrain
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"setup-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cavestack/bin/cavestack-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cavestack/bin/cavestack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/cavestack/bin/cavestack-gbrain-detect
```
...
`cavestack_brain_sync_mode`, `cavestack_brain_git`, `cavestack_artifacts_remote`, and
...
>   ❌ Destructive — existing config moved to ~/.gbrain/config.json.cavestack-bak-{ts}
...
**If A (Retry)**: re-run `~/.claude/skills/cavestack/bin/cavestack-gbrain-detect`
with `CAVESTACK_DETECT_NO_CACHE=1` (busts the 60s cache). If the new
...
```bash
BACKUP="$HOME/.gbrain/config.json.cavestack-bak-$(date +%s)"
mv "$HOME/.gbrain/config.json" "$BACKUP"
# cavestack default: voyage-code-3 (1024d) when VOYAGE_API_KEY is set — best for
...
```bash
[ -f ~/.claude/skills/cavestack/bin/cavestack-code-intelligence ] \
  && bun ~/.claude/skills/cavestack/bin/cavestack-code-intelligence select gbrain 2>/dev/null \
  || true
```
...
- `"offer": false` with reason `bin-absent` → the installed cavestack predates
...
- `"offer": true` → present the returned options ONCE via AskUserQuestion:
  **GBrain** (recommended — semantic memory + code, sends repo content to
  ...
  the choice: `cavestack-code-intelligence select <provider|none>` — `none`
  persists the decline so NO skill ever asks again, on any repo
  (re-enable: `cavestack-code-intelligence select <provider>`). Local-compute
  and remote-send providers are separate consents — never bundle them.
- Per-repo send consent (GBrain/Sourcebot) is recorded with
  `cavestack-code-intelligence consent <repo> yes|no` and is ALWAYS vetoed by a
  `deny` tier in cavestack-gbrain-repo-policy — the trust store is the single
  authority for whether code leaves a repo.

If the user picked GBrain (or asked for this skill directly), continue below.
If they picked Sourcebot/Graphify, run `cavestack-code-intelligence index <repo>`
...
```bash
~/.claude/skills/cavestack/bin/cavestack-gbrain-install
```
...
```bash
. ~/.claude/skills/cavestack/bin/cavestack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.claude/skills/cavestack/bin/cavestack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.claude/skills/cavestack/bin/cavestack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
trap 'echo ""; echo "cavestack-gbrain: interrupted. In-flight ref: $INFLIGHT_REF"; \
...
```bash
result=$(~/.claude/skills/cavestack/bin/cavestack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.claude/skills/cavestack/bin/cavestack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.claude/skills/cavestack/bin/cavestack-gbrain-supabase-provision \
...
```bash
# cavestack default: voyage-code-3 (1024d) when VOYAGE_API_KEY is set — code
...
**4c. Verify via cavestack-gbrain-mcp-verify.** Run the helper; capture the
classified JSON output:

```bash
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.claude/skills/cavestack/bin/cavestack-gbrain-mcp-verify "$MCP_URL")
...
- `URL_FORM_SUPPORTED` (`true|false`) — passed to `cavestack-artifacts-init` in
...
```bash
~/.claude/skills/cavestack/bin/cavestack-gbrain-install || exit $?
...
# cavestack default for local code-search PGLite: voyage-code-3 (1024d) when
...
When A was picked, Step 3 already ran (via cavestack-gbrain-install) and Step 4
...
("Migration didn't complete in 3 minutes — another cavestack session may be
...
```bash
current_tier=$(~/.claude/skills/cavestack/bin/cavestack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.claude/skills/cavestack/bin/cavestack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
Separate AskUserQuestion: "Also sync your cavestack artifacts (CEO plans,
...
`cavestack-artifacts-$USER` (private), and writes the canonical HTTPS URL to
`~/.cavestack-artifacts-remote.txt`. Pass `--url-form-supported` from Step 4c's
verify output (Path 4) or `false` (Paths 1/2/3 — local mode doesn't probe):

```bash
URL_FORM=${URL_FORM_SUPPORTED:-false}
~/.claude/skills/cavestack/bin/cavestack-artifacts-init --url-form-supported "$URL_FORM"
~/.claude/skills/cavestack/bin/cavestack-config set artifacts_sync_mode artifacts-only
# or "full" if user picked yes-full
```

`cavestack-artifacts-init` always prints a "Send this to your brain admin" block
...
In remote mode, the local `cavestack-gbrain-source-wireup` helper does NOT run
...
any gbrain client. The helper creates a `git worktree` of `~/.cavestack/`,
...
try:
    ...
    pass
")
~/.claude/skills/cavestack/bin/cavestack-gbrain-source-wireup --strict \
  ${GBRAIN_URL:+--database-url "$GBRAIN_URL"}
```

`--strict` exits non-zero on missing prereqs (gbrain not installed, < 0.18.0,
or no `~/.cavestack/.git` yet) so the user sees the failure rather than silently
...
this machine's transcripts indexed, they pull from your `cavestack-artifacts-$USER`
repo (set up in Step 7) on whatever schedule they prefer. Set
`cavestack-config set transcript_ingest_mode off` and continue to Step 8.
...
curated `~/.cavestack/` artifacts into gbrain so the retrieval surface
(per-skill manifests, salience block) has data to surface.

Run the probe to size the operation:
```bash
bun run ~/.claude/skills/cavestack/bin/cavestack-memory-ingest.ts --probe
```

Read the output. If `Total files in window: 0`, skip — there's nothing
to ingest. Set `cavestack-config set transcript_ingest_mode incremental`
silently and continue to Step 8.

If `New (never ingested)` is < 200 AND total bytes are < 100MB: silent
bulk via `bun run ~/.claude/skills/cavestack/bin/cavestack-memory-ingest.ts --bulk --quiet`. Set
...
>
> What you get after this: every cavestack skill auto-loads recent salience
...
> it in prior commits. Use `cavestack-transcript-prune` to delete in bulk;
...
After answer:
```bash
~/.claude/skills/cavestack/bin/cavestack-config set transcript_ingest_mode <choice>
bun run ~/.claude/skills/cavestack/bin/cavestack-gbrain-sync.ts --full --no-brain-sync
```
...
hook runs `bun run ~/.claude/skills/cavestack/bin/cavestack-gbrain-sync.ts --incremental --quiet` on every skill
...
- Artifacts repo: {cavestack_artifacts_remote URL or "none"}
...
<!-- cavestack-gbrain-search-guidance:start -->

GBrain is set up and synced on this machine. The agent should prefer gbrain
over Grep when the question is semantic or when you don't know the exact
identifier yet. Two indexed corpora available via the `gbrain` CLI:
- This repo's code (registered as `cavestack-code-<repo>` source).
- `~/.cavestack/` curated memory (registered as `cavestack-brain-<user>` source via
...
- "What did we decide last time?" / past plans, retros, learnings:
    `gbrain search "<terms>" --source cavestack-brain-<user>`

Grep is still right for known exact strings, regex, multiline patterns, and
file globs. The brain auto-syncs incrementally on every cavestack skill start.
Run `/sync-gbrain` to force-refresh, `/sync-gbrain --full` for full reindex.

<!-- cavestack-gbrain-search-guidance:end -->
```
...
The brain trust policy controls whether cavestack auto-pushes `~/.cavestack/`
...
```bash
_HASH=$(~/.claude/skills/cavestack/bin/cavestack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.claude/skills/cavestack/bin/cavestack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.claude/skills/cavestack/bin/cavestack-config set brain_trust_policy@$_HASH personal
...
> shared/team brain?
>
> Personal: cavestack auto-pushes ~/.cavestack/ artifacts (CEO plans, design
...
>
> Shared/team: read-only by default. cavestack reads context but prompts
...
```bash
~/.claude/skills/cavestack/bin/cavestack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.claude/skills/cavestack/bin/cavestack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.claude/skills/cavestack/bin/cavestack-config set artifacts_sync_mode full
...
```bash
~/.claude/skills/cavestack/bin/cavestack-gbrain-detect 2>/dev/null || true
~/.claude/skills/cavestack/bin/cavestack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.claude/skills/cavestack/bin/cavestack-config get artifacts_sync_mode 2>/dev/null || echo "off"
[ -f ~/.cavestack/.gbrain-sync-state.json ] && cat ~/.cavestack/.gbrain-sync-state.json || echo "{}"
```
...
gbrain status: GREEN  (mode: remote-http)
  ...
  Artifacts repo .. OK   {cavestack_artifacts_remote URL}
...
- If user picked B (No): `N/A declined at Step 4d` — `cavestack-config set local_code_index_offered true` to silence future migration notices.

The **Transcripts** row changed in v1.34.0.0: in remote-http mode,
cavestack-memory-ingest now persists staged transcripts to
`~/.cavestack/transcripts/run-<pid>-<ts>/` and cavestack-brain-sync pushes them
...
- **Concurrent-run lock.** At skill start, `mkdir ~/.cavestack/.setup-gbrain.lock.d`
  (atomic). If the mkdir fails, abort with: "Another `/setup-gbrain` instance
  is running. Wait for it, or `rm -rf ~/.cavestack/.setup-gbrain.lock.d` if
