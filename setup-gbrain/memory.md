# cavestack memory ingest — what it does, what stays local, what you can do with it
...
| Eureka log | `eureka` | `~/.cavestack/analytics/eureka.jsonl` | Medium — your insights, often non-secret |
| Project learnings | `learning` | `~/.cavestack/projects/<slug>/learnings.jsonl` | Medium |
| Project timeline | `timeline` | `~/.cavestack/projects/<slug>/timeline.jsonl` | Low |
| CEO plans | `ceo-plan` | `~/.cavestack/projects/<slug>/ceo-plans/*.md` | Medium |
| Design docs | `design-doc` | `~/.cavestack/projects/<slug>/*-design-*.md` | Medium |
| Retros | `retro` | `~/.cavestack/projects/<slug>/retros/*.md` | Medium |
| Builder profile | `builder-profile-entry` | `~/.cavestack/builder-profile.jsonl` | Low |

## What stays local

- **State files** (`~/.cavestack/.gbrain-sync-state.json`,
  `~/.cavestack/.transcript-ingest-state.json`,
  `~/.cavestack/.gbrain-engine-cache.json`,
  `~/.cavestack/.gbrain-errors.jsonl`) are local-only per ED1 (state file
...
(`~/.cavestack/gbrain-repo-policy.json`, managed by
`cavestack-gbrain-repo-policy`). Each transcript's git remote is checked
...
- **Corrupted or unreadable store** — ingestion aborts before any writes
  ...
  `cavestack-gbrain-repo-policy list`; re-run `/setup-gbrain` if it's corrupt.
...
The cross-machine secret boundary is `cavestack-brain-sync` (the git push
...
```bash
bun run bin/cavestack-memory-ingest.ts --bulk --scan-secrets
# or
CAVESTACK_MEMORY_INGEST_SCAN_SECRETS=1 bun run bin/cavestack-memory-ingest.ts --bulk
```
...
was skipped via `bun run bin/cavestack-memory-ingest.ts --probe` (which
...
- **Supabase configured:** code + transcripts go to Supabase Storage
  ...
  brain-linked git repo via `cavestack-brain-sync`.
...
- **Read a specific page:**
  ```bash
  gbrain get_page transcripts/claude-code/garrytan-cavestack/2026-05-01-abc123
  ```
...
- **Bulk-delete by criteria** (V1.0.1 follow-up — `cavestack-transcript-prune`
  helper). For V1.0, use `gbrain delete_page <slug>` per-page or write
  a small loop over `gbrain list_pages` output.

- **Disable entirely:**
  ```bash
  cavestack-config set transcript_ingest_mode off
  cavestack-config set gbrain_context_load off  # also disables retrieval
  ```

## How the agent uses it

At every cavestack skill start, the preamble runs
`cavestack-brain-context-load` which:
...
- **Salience block is empty** — your transcripts may not be ingested
  yet. Run `bun run bin/cavestack-gbrain-sync.ts --full` to do a full pass.
...
- **A page has stale or wrong content** — `gbrain delete_page <slug>`,
  then re-run `bun run bin/cavestack-gbrain-sync.ts --incremental` to re-ingest from
  source if the source file is still on disk and unchanged.

## Privacy + audit

- Every `secretScanFile` finding is logged to stderr at ingest time.
- Every gbrain put/delete is logged to `~/.cavestack/.gbrain-errors.jsonl`
  with `{ts, op, duration_ms, outcome}` for forensic tracing.
- `~/.cavestack/.gbrain-engine-cache.json` shows which storage tier is
...
1. Verifies the URL + token via `cavestack-gbrain-mcp-verify`. Three failure
...
3. Skips local install, local doctor, transcript ingest, and federated
   ...
   doesn't install.
4. Optionally provisions a `cavestack-artifacts-$USER` private repo on
...
`cavestack-artifacts-init` always prints the `gbrain sources add` command
...
- Artifacts repo: github.com/garrytan/cavestack-artifacts-garrytan (private)
...
(See `cavestack/setup-gbrain/SKILL.md.tmpl` for the full Path 4 flow plus
the gbrain enhancement requests around scoped tokens that would let
cavestack auto-rotate in V2.)
