...
5. **Voice** — direct, concrete, builder-first tone in all skill output.
...
```bash
jq '.tests[] | select(.exit_reason == "timeout") | .last_tool_call' ~/.cavestack-dev/evals/_partial-e2e.json
```
...
`eval:compare` diffs two eval runs. `eval:summary` aggregates stats across all runs in `~/.cavestack-dev/evals/`.
