...
(adapt to whatever project you're in — these are not cavestack-specific):
...
cp /tmp/cavestack-pr-body-$$.md /tmp/cavestack-pr-body-orig-$$.md
```
...
```bash
~/.claude/skills/cavestack/bin/cavestack-issue-guard --stdin --source pr-body < /tmp/cavestack-pr-body-$$.md
```
...
4. Redaction scan-at-sink, then write the updated body back. The body is already
   in a temp file (`/tmp/cavestack-pr-body-$$.md`); scan THAT file before editing so
   the bytes scanned are the bytes sent:

```bash
REDACT_VIS=$(~/.claude/skills/cavestack/bin/cavestack-config get redact_repo_visibility 2>/dev/null)
[ -z "$REDACT_VIS" ] && REDACT_VIS=$(gh repo view --json visibility -q .visibility 2>/dev/null | tr 'A-Z' 'a-z')
~/.claude/skills/cavestack/bin/cavestack-redact --from-file /tmp/cavestack-pr-body-$$.md --repo-visibility "${REDACT_VIS:-unknown}" --json
...
if [ ! -f /tmp/cavestack-pr-body-orig-$$.md ] || [ ! -f /tmp/cavestack-pr-body-$$.md ]; then
...
_ORIG_BANNERS=$(grep -c "UNTRUSTED TRACKER CONTENT" /tmp/cavestack-pr-body-orig-$$.md 2>/dev/null)
_ORIG_BANNERS=${_ORIG_BANNERS:-0}
_NEW_BANNERS=$(grep -c "UNTRUSTED TRACKER CONTENT" /tmp/cavestack-pr-body-$$.md 2>/dev/null)
...
**If GitLab:**
Read the contents of `/tmp/cavestack-pr-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
...
```bash
rm -f /tmp/cavestack-pr-body-$$.md /tmp/cavestack-pr-body-orig-$$.md
```
...
```bash
NEW_TITLE=$(~/.claude/skills/cavestack/bin/cavestack-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```
...
stays discoverable: "Running the Codex doc review automatically (standard step). Disable: `cavestack-config set codex_reviews disabled`."
...
```bash
TMPERR_DOC=$(mkdir -p "${TMPDIR:-${TMP:-$HOME/.cavestack/tmp}}" && mktemp "${TMPDIR:-${TMP:-$HOME/.cavestack/tmp}}/codex-docreview-XXXXXXXX")
...
**Persist the result:**
```bash
~/.claude/skills/cavestack/bin/cavestack-review-log '{"skill":"codex-doc-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
