...
preamble-tier: 1
version: 0.2.0
description: Launch CaveStack Browser — AI-controlled Chromium with the sidebar extension baked in.
triggers:
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"open-cavestack-browser","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
