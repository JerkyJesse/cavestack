...
//
// Invoked from bin/cavestack-ios-qa-mint.
...
function printHelp() {
  const help = `cavestack-ios-qa-mint — manage the tailnet allowlist for remote iOS QA agents

USAGE
  cavestack-ios-qa-mint grant  --remote <identity> [--capability <tier>] [--ttl <seconds>] [--note <text>]
  cavestack-ios-qa-mint revoke --remote <identity>
...
EXAMPLES
  ...
  cavestack-ios-qa-mint grant --remote 'tag:ci' --capability mutate --ttl 86400 --note 'nightly run'
  cavestack-ios-qa-mint revoke --remote 'alice@example.com'
  cavestack-ios-qa-mint list

The allowlist lives at ~/.cavestack/ios-qa-allowlist.json (mode 0600). The daemon's
...
if (import.meta.main) {
  main().catch((err) => {
    process.stderr.write(`cavestack-ios-qa-mint: ${(err as Error).message}\n`);
    process.exit(1);
  });
}
