...
export function defaultAuditPath(): string {
  return process.env.CAVESTACK_IOS_AUDIT_PATH
    ?? join(homedir(), '.cavestack', 'security', 'ios-qa-audit.jsonl');
}

export function defaultAttemptsPath(): string {
  return process.env.CAVESTACK_IOS_ATTEMPTS_PATH
    ?? join(homedir(), '.cavestack', 'security', 'attempts.jsonl');
}
...
async function loadDeviceSalt(): Promise<string> {
  ...
  const path = join(homedir(), '.cavestack', 'security', 'device-salt');
  try {
