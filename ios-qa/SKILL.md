...
preamble-tier: 3
version: 1.0.0
description: Live-device iOS QA for SwiftUI apps. (cavestack)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ios-qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cavestack/bin/cavestack-timeline-log '{"skill":"ios-qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cavestack/bin/cavestack-question-log '{"skill":"ios-qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```
       ┌──────────────────────┐   USB CoreDevice (IPv6)   ┌──────────────────┐
       │ cavestack-ios-qa daemon │ ────────────────────────▶ │ iOS app          │
...
If `~/.cavestack/ios-qa-session.json` exists and the device is still connected,
...
```bash
SESSION="$HOME/.cavestack/ios-qa-session.json"
...
6. Spawn the Mac-side daemon (on-demand) — `cavestack-ios-qa-daemon`. Daemon
   acquires an exclusive flock on `~/.cavestack/ios-qa-daemon.pid`. If another
...
`~/.cavestack/security/ios-qa-audit.jsonl`.
...
Owner mints via `cavestack-ios-qa-mint --remote <identity> --capability <tier>`
...
| `403 identity_not_allowed` from `/auth/mint` | identity missing from allowlist | Run `cavestack-ios-qa-mint --remote <identity>` on the Mac |
