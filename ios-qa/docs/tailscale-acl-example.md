...
- **Auth:** Tailscale identity validation via the local `tailscaled` socket
  ...
  `~/.cavestack/ios-qa-allowlist.json` is the single source of truth for who can
  do what.
...
cavestack-ios-qa-mint --remote ci@example.com --capability mutate --ttl 1h
...
row to `~/.cavestack/security/ios-qa-audit.jsonl`:
...
allowlisted, rate limit hit) write to `~/.cavestack/security/attempts.jsonl`.
...
| `403 identity_not_allowed` | identity missing from allowlist | Owner mint: `cavestack-ios-qa-mint --remote <identity>` |
