#!/usr/bin/env bun
/**
 * Generate cavestack/llms.txt — a single discoverable index of every cavestack
 ...
 *
 * Output: cavestack/llms.txt at repo root.
 *
...
const OUTPUT = path.join(ROOT, 'cavestack', 'llms.txt');
 ...
 * the cavestack convention; we collapse those into a single paragraph.
 */
...
function parseSkillFrontmatter(filePath: string): SkillEntry | null {
  const content = yamlStart(fs.readFileSync(filePath, 'utf-8'));
 ...
 * in a full cavestack checkout; absent on minimal installs. Returns {} if the
...
export async function generateLlmsTxt(opts: GenerateOptions = {}): Promise<GenerateResult> {
  ...
  const lines: string[] = [];
  lines.push('# cavestack');
  lines.push('');
  lines.push("> cavestack is CaveStack: AI coding skills + a fast headless browser binary + a design CLI. This file indexes every capability so agents can discover and invoke them without crawling individual SKILL.md files.");
  lines.push('');
  ...
  lines.push('## More');
  lines.push('');
  lines.push('- Repository: https://github.com/JerkyJesse/cavestack');
  ...
  void (async () => {
    ...
    if (dryRun) {
      ...
      if (existing !== result.content) {
        console.error('[gen-llms-txt] OUT OF DATE — run `bun run gen:skill-docs` to regenerate cavestack/llms.txt');
        process.exit(1);
      }
