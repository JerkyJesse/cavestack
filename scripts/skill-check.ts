...
/** Skill dirs the Claude host never generates (tmpl exists, SKILL.md does not). */
export function isPrimaryHostSkippedOutput(output: string): boolean {
  const skipped = new Set(getHostConfig('claude').generation.skipSkills ?? []);
  const normalized = output.replace(/\\/g, '/');
  const slash = normalized.indexOf('/');
  if (slash <= 0) return false;
  return skipped.has(normalized.slice(0, slash));
}
...
function main(): void {
  ...
  for (const { tmpl, output } of TEMPLATES) {
    ...
    if (!fs.existsSync(outPath)) {
      if (isPrimaryHostSkippedOutput(output)) {
        console.log(`  \u26a0\ufe0f  ${output.padEnd(30)} — skipped on primary host (OK)`);
        continue;
      }
      hasErrors = true;
