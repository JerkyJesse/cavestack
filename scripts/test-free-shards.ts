 ...
 *
 * Adapted from the McGluut/cavestack fork's test-free-shards.ts (190 LOC). The
...
export const KNOWN_WINDOWS_INCOMPATIBLE: Array<{ file: string; reason: string }> = [
  ...
  {
    file: 'test/cavestack-decision-semantic.test.ts',
...
const KNOWN_WINDOWS_SAFE: Array<{ file: string; reason: string }> = [
  ...
  },
  {
    ...
    // path.join(ROOT, 'bin', 'cavestack-uninstall') constant, but the script is
 ...
 * Deliberately NOT CAVESTACK_HOME: injecting one shared scratch home for a whole
 ...
 * CAVESTACK_HOME isolation mkdtemp their own per test — the repo convention —
...
export async function runFreeShard(
  ...
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cavestack-free-shard-'));
...
function nextDefaultLogPath(): string {
  ...
  logPathSequence += 1;
  return path.join(os.tmpdir(), `cavestack-free-test-${stamp}-${process.pid}-${logPathSequence}.log`);
}
