 ...
 *   - bin/cavestack-brain-cache (drives TTL + write-back invalidation)
...
export interface BrainCacheEntity {
  /** Filename inside ~/.cavestack/{,projects/<slug>/}brain-cache/ */
  file: string;
   ...
   *   - 'skill-run-write'   — any skill that writes a cavestack/skill-run page
 ...
 * `cavestack-config set salience_allowlist '<comma-separated-prefixes>'`.
 */
...
/**
 * Retention policy: cavestack/skill-run pages auto-archive after this many days.
 ...
 */
export const CAVESTACK_SCHEMA_PACK_NAME = 'cavestack-core';
export const CAVESTACK_SCHEMA_PACK_VERSION = '1.0.0';
 ...
 * and persisted via `cavestack-config set user_slug_at_<endpoint-hash> <slug>`.
