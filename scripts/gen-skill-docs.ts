...
// When --respect-detection is passed, read ~/.cavestack/gbrain-detection.json
...
// bin/cavestack-gbrain-detect and persisted by `cavestack-config gbrain-refresh`
// or by ./setup.
//
...
function loadGbrainOverride(): { detected: boolean } {
  ...
  const stateDir = process.env.CAVESTACK_HOME || path.join(process.env.HOME || '', '.cavestack');
  ...
  try {
    ...
    // "ok", matching cavestack-gbrain-detect --is-ok.
...
// section-base path (`~/.claude/skills/cavestack/<skill>/sections/`) inside the
 ...
 * docs/ references keep pointing at `~/.claude/skills/cavestack` (the global
...
function rewriteSectionBase(content: string): string {
  ...
  return content.replace(
    /~\/\.claude\/skills\/cavestack\/([^\s)`"'*]+\/sections\/)/g,
    `${OUT_DIR}/$1`,
  );
}
...
// "(cavestack)" tag. This pile is the always-loaded catalog surface — every
// session pays for the full text. The catalog trim splits the description
// into a one-line catalog entry (lead sentence + "(cavestack)") that stays in
...
export function splitCatalogDescription(description: string): CatalogParts {
  ...
  const hasCavestackTag = /\(cavestack\)/.test(working);
  if (hasCavestackTag) working = working.replace(/\(cavestack\)/, '').trim();
  ...
  return { lead, routingProse, voiceLine, hasCavestackTag };
}
...
export function buildTrimmedDescription(parts: CatalogParts): string {
  ...
  const suffix = parts.hasCavestackTag ? ' (cavestack)' : '';
  return `${lead}${suffix}`;
}
 ...
 *  - shorten frontmatter `description:` to lead + (cavestack)
...
export function applyCatalogTrim(content: string, skillName: string): { content: string; parts: CatalogParts } | null {
  ...
  // If lead + (cavestack) is already most of the text, no trim needed.
...
for (const currentHost of hostsToRun) {
  ...
  try {
    ...
    // Generate the OpenClaw orchestrator-injection docs (cavestack-lite / cavestack-full /
    // cavestack-plan CLAUDE.md snippets). Sources live in openclaw/templates/ —
    ...
    if (currentHost === 'openclaw' && !DRY_RUN) {
      ...
      for (const variant of ['lite', 'full', 'plan'] as const) {
        const fileName = `cavestack-${variant}-CLAUDE.md`;
...
// Regenerate cavestack/llms.txt — single-file capability index for AI agents.
  ...
  void (async () => {
    try {
      ...
      if (result.warnings.length > 0) {
        ...
        console.log(`[gen-llms-txt] cavestack/llms.txt: ${result.skills.length} skills, ${result.browseCommands.length} browse commands`);
      }
    } catch (err) {
