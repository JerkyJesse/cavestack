 ...
 * `~/.claude/skills/cavestack/bin`. Bash only performs tilde expansion when the
 ...
 *
 * Env-var hosts already use `$CAVESTACK_BIN`, which expands correctly when
 * quoted, so they pass through untouched.
 *
 * Use this ONLY where the path lands inside double quotes. Unquoted
 * interpolations (`${ctx.paths.binDir}/cavestack-slug`) expand fine as-is and are
 ...
 * Claude-style dirs are `~`-rooted (e.g. `~/.claude/skills/cavestack/browse/dist`)
 * and expand via `$HOME`; env-var hosts already carry an absolute `$CAVESTACK_*`
