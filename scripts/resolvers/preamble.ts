...
CAVESTACK_ROOT="$HOME/${hostConfig.globalRoot}"
[ -n "$_ROOT" ] && [ -d "$_ROOT/${ctx.paths.localSkillRoot}" ] && CAVESTACK_ROOT="$_ROOT/${ctx.paths.localSkillRoot}"
...
\`\`\`bash
${runtimeRoot}_UPD=$(${ctx.paths.binDir}/cavestack-update-check 2>/dev/null || ${ctx.paths.localSkillRoot}/bin/cavestack-update-check 2>/dev/null || true)
...
_PROACTIVE=$(${ctx.paths.binDir}/cavestack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.cavestack/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(${ctx.paths.binDir}/cavestack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <(${ctx.paths.binDir}/cavestack-repo-mode 2>/dev/null) || true
...
if [ -f "$_LEARN_FILE" ]; then
  ...
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    ${ctx.paths.binDir}/cavestack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
# Session timeline: record skill start (local-only, never sent anywhere)
${ctx.paths.binDir}/cavestack-timeline-log '{"skill":"${ctx.skillName}","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
function generateUpgradeCheck(ctx: TemplateContext): string {
  return `If \`PROACTIVE\` is \`"false"\`, do not proactively suggest cavestack skills AND do not
...
If output shows \`UPGRADE_AVAILABLE <old> <new>\`: read \`${ctx.paths.skillRoot}/cavestack-upgrade/SKILL.md\` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If \`JUST_UPGRADED <from> <to>\`: tell user "Running cavestack v{to} (just updated!)" and continue.`;
}
...
If A: run \`${ctx.paths.binDir}/cavestack-config set proactive true\`
If B: run \`${ctx.paths.binDir}/cavestack-config set proactive false\`
...
If B: run \`${ctx.paths.binDir}/cavestack-config set routing_declined true\`
...
function generateVendoringDeprecation(ctx: TemplateContext): string {
  return `If \`VENDORED_CAVESTACK\` is \`yes\`: This project has a vendored copy of cavestack at
...
\`\`\`bash
${ctx.paths.binDir}/cavestack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
\`\`\`bash
eval "$(${binDir}/cavestack-slug 2>/dev/null)"
...
// T2: T1 + voice(full) + ask + context-recovery
// T3: T2 + repo-mode
...
export function generatePreamble(ctx: TemplateContext): string {
  ...
  const sections = [
    ...
    ...(tier >= 2 ? [generateContextRecovery(ctx), generateAskUserFormat(ctx), generateZeroShortcutsDirective(), generateTryFirstDirective(), generateMuskAlgorithmDirective(tier), generateCaveProtocol(tier), generateZeroTestDrift(tier), generateResumeProtocol(tier), generateBuildPhilosophyInjection(ctx)] : []),
