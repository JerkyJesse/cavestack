 ...
 * under ~/.cavestack/projects/<slug>/, falling back to newest project-scoped
 ...
 * ~/.cavestack, and the committed copy is what teammates see — but a stale old
 ...
 * the prerequisite-skill re-check in review.ts (CaveStack 2 fork-port wave,
 * time-attack/cavestack). Drift between copies meant plan reviews could
 ...
 */
export const DESIGN_DOC_DISCOVERY_BLOCK = `_LOCALDOC=$(ls -t ~/.cavestack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
