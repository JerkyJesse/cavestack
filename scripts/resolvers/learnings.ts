 ...
 *
 * Learnings are stored per-project at ~/.cavestack/projects/{slug}/learnings.jsonl.
 ...
 * by cavestack-learnings-search ("latest winner" per key+type).
 *
...
If A: run \`${ctx.paths.binDir}/cavestack-config set cross_project_learnings true\`
If B: run \`${ctx.paths.binDir}/cavestack-config set cross_project_learnings false\`
...
\`\`\`bash
${binDir}/cavestack-learnings-log '{"skill":"${ctx.skillName}","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
\`\`\`
