 ...
 *   1. If ~/.cavestack-artifacts-remote.txt (or legacy ~/.cavestack-brain-remote.txt
 *      during the v1.27.0.0 migration window) exists AND ~/.cavestack/.git is
 *      missing, surface a restore-available hint (does NOT auto-run restore).
 *   2. If sync is on, run `cavestack-brain-sync --once` (drain + push). The
 ...
 * to `cavestack-brain-sync --discover-new` + `--once`.
 */
...
_BRAIN_SYNC_BIN="${quoteSafePath(ctx.paths.binDir)}/cavestack-brain-sync"
_BRAIN_CONFIG_BIN="${quoteSafePath(ctx.paths.binDir)}/cavestack-config"
...
${isBrainHost ? `If output shows \`ARTIFACTS_SYNC: artifacts repo detected\`, offer \`cavestack-brain-restore\` via AskUserQuestion; otherwise continue.` : ''}
...
\`\`\`bash
"${quoteSafePath(ctx.paths.binDir)}/cavestack-brain-sync" --discover-new 2>/dev/null || true
"${quoteSafePath(ctx.paths.binDir)}/cavestack-brain-sync" --once 2>/dev/null || true
\`\`\`
`;
}
