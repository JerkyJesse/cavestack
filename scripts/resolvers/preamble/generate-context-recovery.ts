...
export function generateContextRecovery(ctx: TemplateContext): string {
  ...
  // the canonical slug form the cavestack-slug eval on the first line sets
  // (tr '/' '-' then tr -cd 'a-zA-Z0-9._-', matching what cavestack-review-log
  // WRITES). The timeline.jsonl greps keep raw $_BRANCH because the timeline
  // writer (preamble's cavestack-timeline-log call) stores the raw branch in the
...
if [ -d "$_PROJ" ]; then
  ...
  if [ -f "$_PROJ/decisions.active.json" ]; then
    ...
    ${binDir}/cavestack-decision-search --recent 5 2>/dev/null
    echo "--- END DECISIONS ---"
  fi
  echo "--- END ARTIFACTS ---"
fi
...
**Cross-session decisions.** If \`ACTIVE DECISIONS\` are listed, treat them as prior settled calls with their rationale — do not silently re-litigate them; if you're about to reverse one, say so explicitly. Reach for \`${binDir}/cavestack-decision-search\` whenever a question touches a past decision ("what did we decide / why / did we try"). When you or the user make a DURABLE decision (architecture, scope, tool/vendor choice, or a reversal) — NOT a turn-level or trivial choice — log it with \`${binDir}/cavestack-decision-log\` (\`--supersede <id>\` for a reversal). Reliable and local; gbrain not required.`;
}
