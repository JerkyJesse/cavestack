...
preamble-tier: 2
version: 1.0.0
description: Post-deploy canary monitoring. (cavestack)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"canary","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cavestack/bin/cavestack-timeline-log '{"skill":"canary","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cavestack/bin/cavestack-question-log '{"skill":"canary","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
eval "$(~/.claude/skills/cavestack/bin/cavestack-slug 2>/dev/null || echo "SLUG=unknown")"
mkdir -p .cavestack/canary-reports
mkdir -p .cavestack/canary-reports/baselines
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".cavestack/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".cavestack/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".cavestack/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
Screenshots:   .cavestack/canary-reports/screenshots/
