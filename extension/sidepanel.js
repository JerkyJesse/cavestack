...
document.getElementById('chat-cookies-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    console.error('[cavestack sidebar] Failed to open cookie picker:', err.message);
  }
});
...
async function ensureSseSessionCookie() {
  ...
  try {
    ...
    console.warn('[cavestack sidebar] Failed to mint SSE session cookie:', err && err.message);
    return false;
  }
}
...
async function connectSSE() {
  ...
  eventSource.addEventListener('activity', (e) => {
    try { addEntry(JSON.parse(e.data)); } catch (err) {
      console.error('[cavestack sidebar] Failed to parse activity event:', err.message);
    }
  });
  ...
  eventSource.addEventListener('gap', (e) => {
    try {
      ...
      console.error('[cavestack sidebar] Failed to parse gap event:', err.message);
    }
  });
}
...
async function pollMemoryOnce() {
  ...
  try {
    ...
    try { evaluateMemToast(snapshot); } catch (err) {
      console.debug('[cavestack sidebar] mem-toast evaluation failed:', err && err.message);
    }
    ...
    if (elapsed > MEM_POLL_SLOW_THRESHOLD_MS) {
      console.debug('[cavestack sidebar] /memory poll slow/failed:', elapsed, 'ms', err && err.message);
    }
...
async function loadSnoozeState() {
  ...
  try {
    ...
    console.debug('[cavestack sidebar] mem-toast snooze load failed:', err && err.message);
  }
}
...
async function saveSnoozeState() {
  ...
  try {
    ...
    console.debug('[cavestack sidebar] mem-toast snooze save failed:', err && err.message);
  }
}
  ...
  closeBtn.onclick = async () => {
    ...
    for (const id of ids) {
      try {
        ...
        console.warn('[cavestack sidebar] mem-toast closetab failed:', id, err && err.message);
      }
    }
  };
}
  ...
  if (snooze) {
    snooze.addEventListener('click', async () => {
      ...
      try {
        ...
        console.debug('[cavestack sidebar] mem-toast snooze fetch failed:', err && err.message);
      }
      dismissMemToast();
    });
  }
...
async function fetchRefs() {
  ...
  try {
    ...
    console.error('[cavestack sidebar] Failed to fetch refs:', err.message);
  }
}
...
inspectorSendBtn.addEventListener('click', async () => {
  ...
  // cavestackScanForPTYInject is async; cavestackInjectToTerminal stays sync.
  const verdict = await window.cavestackScanForPTYInject?.(message + '\n', 'inspector-send');
  if (verdict?.verdict === 'BLOCK') {
    console.warn('[cavestack sidebar] Inspector send BLOCKED by /pty-inject-scan:', verdict.reasons);
    return;
  }
  ...
  const ok = window.cavestackInjectToTerminal?.(message + '\n');
  if (!ok) {
    console.warn('[cavestack sidebar] Inspector send needs an active Terminal session.');
  }
});
...
async function runCleanup(...buttons) {
  ...
  // confirms every call site goes through cavestackScanForPTYInject first.
  const verdict = await window.cavestackScanForPTYInject?.(cleanupPrompt + '\n', 'cleanup-button');
  if (verdict?.verdict === 'BLOCK') {
    console.warn('[cavestack sidebar] Cleanup BLOCKED by /pty-inject-scan:', verdict.reasons);
  ...
  const sent = window.cavestackInjectToTerminal?.(cleanupPrompt + '\n');
  if (!sent) {
    console.warn('[cavestack sidebar] Cleanup needs an active Terminal session.');
  }
...
async function runScreenshot(...buttons) {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn('[cavestack sidebar] Screenshot failed:', text);
    } else {
      console.log('[cavestack sidebar] Screenshot:', text);
    }
  } catch (err) {
    console.error('[cavestack sidebar] Screenshot error:', err.message);
  } finally {
  ...
  try {
    ...
    inspectorSSE.addEventListener('inspectResult', (e) => {
      try {
        ...
        console.error('[cavestack sidebar] Failed to parse inspectResult:', err.message);
      }
    });
    ...
    console.debug('[cavestack sidebar] Inspector SSE not available:', err.message);
  }
}
...
function updateConnection(url, token) {
  ...
  if (url) {
    try { window.cavestackServerPort = parseInt(new URL(url).port, 10); } catch {}
    window.cavestackAuthToken = token || null;
  } else {
    window.cavestackServerPort = null;
    window.cavestackAuthToken = null;
  }
  if (url) {
...
document.getElementById('conn-copy').addEventListener('click', () => {
  navigator.clipboard.writeText('/open-cavestack-browser').then(() => {
    const btn = document.getElementById('conn-copy');
    btn.textContent = 'copied!';
    setTimeout(() => { btn.textContent = '/open-cavestack-browser'; }, 2000);
  });
});
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'cavestack-migration-notice' && msg.message) {
    console.log('[cavestack sidebar]', msg.message);
    try {
      const banner = document.createElement('div');
      ...
      console.debug('[cavestack sidebar] migration banner failed:', err && err.message);
    }
  }
  ...
  if (msg.type === 'browserTabState') {
    document.dispatchEvent(new CustomEvent('cavestack:tab-state', {
...
window.addEventListener('pagehide', () => {
  const sessionId = window.cavestackPtySession;
  const authToken = window.cavestackAuthToken;
  const port = window.cavestackServerPort;
