...
(function () {
  ...
  if (!Terminal) {
    console.error('[cavestack terminal] xterm not loaded');
    return;
  }
   ...
   * /pty-session and surfaced back via window.cavestackPtySession so the
   ...
   * exposes the relevant fields on window.cavestackHealth (set below in init()).
   * If terminalPort is missing, the agent isn't ready yet.
   */
  function getHealth() {
    return window.cavestackHealth || {};
  }

  function getServerPort() {
    return window.cavestackServerPort || null;
  }

  function getAuthToken() {
    return window.cavestackAuthToken || null;
  }
  ...
  function startReattachLoop(prevSessionId) {
    ...
    const tick = async () => {
      ...
      if (resp.status === 401) {
        ...
        setState(STATE.IDLE, {
          message: 'Auth invalid — reload the sidebar or restart your cavestack session.',
        });
        return;
      }
      if (!resp.ok) {
  ...
  function openReattachWebSocket(terminalPort, attachToken, sessionId) {
    ...
    try { window.cavestackPtySession = currentSessionId; } catch {}
    setState(STATE.LIVE);
    ensureXterm();
    nextBinaryIsReplay = false;
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`cavestack-pty.${attachToken}`]);
    ...
    ws.addEventListener('error', (err) => {
      console.error('[cavestack terminal] reattach ws error', err);
    });
  }
   ...
   * derived input MUST be pre-scanned via window.cavestackScanForPTYInject()
   ...
   * sync `const ok = cavestackInjectToTerminal?.(text)` pattern. Making the
   ...
   */
  window.cavestackInjectToTerminal = function (text) {
   ...
   * Closes #1370.
   */
  window.cavestackScanForPTYInject = async function (text, origin) {
  ...
  // sidepanel uses for /pty-session — window.cavestackAuthToken, set by
  ...
  async function connect() {
    ...
    ws.addEventListener('error', (err) => {
      console.error('[cavestack terminal] ws error', err);
    });
  }
  ...
  async function forceRestart() {
    ...
    if (!nextTuple) {
      ...
      try { window.cavestackPtySession = null; } catch {}
      tryAutoConnect();
      return;
    }
    ...
    setState(STATE.LIVE);
    ensureXterm();
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`cavestack-pty.${token}`]);
    ws.binaryType = 'arraybuffer';
  ...
  function init() {
    ...
    document.addEventListener('cavestack:tab-state', (ev) => {
      if (!ws || ws.readyState !== WebSocket.OPEN) return;
      try {
        ws.send(JSON.stringify({
          type: 'tabState',
   ...
   * window.cavestackServerPort + window.cavestackAuthToken (which it does as soon
