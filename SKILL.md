...
preamble-tier: 1
version: 1.2.0
description: Router for the cavestack skill suite. (cavestack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
triggers:
  - cavestack
  - which cavestack skill
  - route this with cavestack
...
Sends any cavestack request to the right skill
(planning, review, QA, shipping, debugging, docs, security, design). For browser/QA
and dogfooding it points you at /browse. Use when you invoke cavestack without a specific
skill, or ask "which cavestack skill fits this?".
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"cavestack","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
This is the cavestack router. Its one job is to send the request to the right skill.
...
```bash
~/.claude/skills/cavestack/bin/cavestack-telemetry-log --event-type route --skill cavestack --outcome ROUTE_OUTCOME --session-id "$_SESSION_ID" 2>/dev/null || true
```

If `PROACTIVE` is `false`: do NOT proactively invoke or suggest other cavestack skills during
this session. Only run skills the user explicitly invokes. This preference persists across
sessions via `cavestack-config`.
...
- User asks to upgrade cavestack → invoke `/cavestack-upgrade`
...
- User asks to launch a real browser for QA, "open the browser" → invoke `/open-cavestack-browser`
...
- User asks what cavestack has learned, "show learnings" → invoke `/learn`
...
If the user opts out of suggestions, run `cavestack-config set proactive false`.
If they opt back in, run `cavestack-config set proactive true`.
