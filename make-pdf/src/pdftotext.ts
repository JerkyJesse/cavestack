 ...
 *   1. $CAVESTACK_PDFTOTEXT_BIN env override (preferred, matches v1.24 CAVESTACK_*_BIN pattern)
 ...
 *      and guessing causes false positives. Set CAVESTACK_PDFTOTEXT_BIN explicitly.
...
export function resolvePdftotext(env: NodeJS.ProcessEnv = process.env): PdftotextInfo {
  // 1 + 2: env overrides (CAVESTACK_PDFTOTEXT_BIN preferred, PDFTOTEXT_BIN back-compat).
  const overrideRaw = env.CAVESTACK_PDFTOTEXT_BIN ?? env.PDFTOTEXT_BIN;
  ...
  // positives. Windows users set CAVESTACK_PDFTOTEXT_BIN explicitly.
  ...
  throw new PdftotextUnavailableError([
    ...
    "",
    "Or set CAVESTACK_PDFTOTEXT_BIN to an explicit path:",
    process.platform === "win32"
      ? '  setx CAVESTACK_PDFTOTEXT_BIN "C:\\path\\to\\pdftotext.exe"'
      : "  export CAVESTACK_PDFTOTEXT_BIN=/path/to/pdftotext",
  ].join("\n"));
}
 ...
 *   1. $CAVESTACK_<TOOL>_BIN env override (e.g. CAVESTACK_PDFFONTS_BIN)
...
export function resolvePopplerTool(
  ...
  const override = resolveOverride(env[`CAVESTACK_${tool.toUpperCase()}_BIN`], env);
