 ...
 *   1. $CAVESTACK_BROWSE_BIN env override (preferred, matches v1.24 CAVESTACK_*_BIN pattern)
 *   2. $BROWSE_BIN env override (back-compat alias)
 *   3. sibling dir: dirname(argv[0])/../browse/dist/browse[.exe]
 *   4. ~/.claude/skills/cavestack/browse/dist/browse[.exe]
...
export function resolveBrowseBin(
  ...
  const globalPath = path.join(home, ".claude/skills/cavestack/browse/dist/browse");
  ...
  throw new BrowseClientError(
    ...
    [
      ...
      "",
      "make-pdf needs browse (the cavestack Chromium daemon) to render PDFs.",
      "Tried:",
      `  - $CAVESTACK_BROWSE_BIN (${env.CAVESTACK_BROWSE_BIN || "unset"})`,
      `  - $BROWSE_BIN (${env.BROWSE_BIN || "unset"})`,
      `  - sibling: ${siblingCandidates.join(", ")}`,
      `  - global: ${globalPath}`,
      "  - PATH: `browse`",
      "",
      "To fix: run cavestack setup from the cavestack repo:",
      "  cd ~/.claude/skills/cavestack && ./setup",
      "",
      "Or set CAVESTACK_BROWSE_BIN explicitly:",
      process.platform === "win32"
        ? '  setx CAVESTACK_BROWSE_BIN "C:\\path\\to\\browse.exe"'
        : "  export CAVESTACK_BROWSE_BIN=/path/to/browse",
    ].join("\n"),
  );
}
