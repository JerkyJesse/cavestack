...
preamble-tier: 1
version: 1.0.0
description: Cross-model benchmark for cavestack skills. (cavestack)
triggers:
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"benchmark-models","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cavestack/bin/cavestack-timeline-log '{"skill":"benchmark-models","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
You are running the `/benchmark-models` workflow. Wraps the `cavestack-model-benchmark` binary with an interactive flow that picks a prompt, confirms providers, previews auth, and runs the benchmark.

Different from `/benchmark` — that skill measures web page performance (Core Web Vitals, load times). This skill measures AI model performance on cavestack skills or arbitrary prompts.
...
```bash
BIN="$HOME/.claude/skills/cavestack/bin/cavestack-model-benchmark"
[ -x "$BIN" ] || BIN=".claude/skills/cavestack/bin/cavestack-model-benchmark"
[ -x "$BIN" ] || { echo "ERROR: cavestack-model-benchmark not found. Run ./setup in the cavestack install dir." >&2; exit 1; }
echo "BIN: $BIN"
```

If not found, stop and tell the user to reinstall cavestack.
...
- **Options:**
  - A) Benchmark one of my cavestack skills (we'll pick which skill next). Completeness: 10/10.
...
If A: list top-level cavestack skills that have SKILL.md files (from `find . -maxdepth 2 -name SKILL.md -not -path './.*'`), ask the user to pick one via a second AskUserQuestion. Use the picked SKILL.md path as the prompt file.
...
- **Options:**
  - A) Save to `~/.cavestack/benchmarks/<date>-<skill-or-prompt-slug>.json`. Completeness: 10/10.
