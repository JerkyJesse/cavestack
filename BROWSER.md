...
cavestack's browser surface in one document. Headless Chromium daemon, ~70+
...
If you've used cavestack in the last release or two, the productivity loop is the
...
B=./browse/dist/browse           # or ~/.claude/skills/cavestack/browse/dist/browse
...
/skillify                        # writes ~/.cavestack/browser-skills/hn-front/...
...
- **Headed via `$B connect`**. Same daemon, but Chromium is visible (rebranded
  as "CaveStack Browser") with the Side Panel extension auto-loaded. You watch
...
The shipped headline of v1.19.0.0. Two cavestack skills wrap the browser-skills
...
- **D3 — Atomic write.** Stages everything to `~/.cavestack/.tmp/skillify-<spawnId>/`,
...
1. **First call.** CLI checks `<project>/.cavestack/browse.json` for a running
   ...
   cavestack install root (bounded ~120s), and retries once — at most once per
...
collisions. State at `<project>/.cavestack/browse.json`.

| Workspace | State file | Port |
|-----------|-----------|------|
| `/code/project-a` | `/code/project-a/.cavestack/browse.json` | random (10000–49151) |
| `/code/project-b` | `/code/project-b/.cavestack/browse.json` | random (10000–49151) |
...
| `connect` | Launch headed CaveStack Browser with Side Panel extension |
...
The daemon's own stdout/stderr persists to `<project>/.cavestack/browse-daemon.log`
...
| Tier | Path | When |
|------|------|------|
| **Project** | `<project>/.cavestack/browser-skills/<name>/` | Project-specific skills (committed or gitignored) |
| **Global** | `~/.cavestack/browser-skills/<name>/` | Per-user skills, all projects |
| **Bundled** | `<cavestack-install>/browser-skills/<name>/` | Ships with cavestack, read-only |
...
| **Process-side env** | `trusted: true` frontmatter passes `process.env` minus `CAVESTACK_TOKEN`. `trusted: false` (default) drops everything except a minimal allowlist (LANG, LC_ALL, TERM, TZ) and pattern-strips secrets (TOKEN/KEY/SECRET/PASSWORD, AWS_*, ANTHROPIC_*, OPENAI_*, GITHUB_*, etc.) | Untrusted (must opt in) |

`CAVESTACK_PORT` and `CAVESTACK_SKILL_TOKEN` are injected last, so a parent process
can't override them.
...
- `stageSkill(opts)` — writes files to `~/.cavestack/.tmp/skillify-<spawnId>/<name>/`
...
Storage:
- Per-project: `<project>/.cavestack/domain-skills/<host>.md`
- Global: `~/.cavestack/domain-skills/<host>.md`
...
`$B connect` launches **CaveStack Browser** — a rebranded Chromium controlled by
...
```bash
$B connect              # launches CaveStack Browser, headed
...
"cavestack" pill in the bottom-right corner so you always know which Chrome
window is being controlled.

### What "CaveStack Browser" means
...
cavestack extension pre-loaded via `launchPersistentContext`. The UA no longer
carries a `CaveStackBrowser` suffix — that branding string was itself a
...
`CAVESTACK_STEALTH=extended` (also accepts `1` or `true`; off by default) layers
...
`CAVESTACK_*` host-profile env (GPU vendor/renderer, UA-CH platform/model,
hardware) emits the Pack 1 `--cavestack-gpu-vendor` / `--cavestack-gpu-renderer` /
`--cavestack-ua-platform` / `--cavestack-ua-model` / `--cavestack-hw-concurrency` /
`--cavestack-device-memory` switches that push the GPU/UA-CH/hardware spoof down
to native code, and `CAVESTACK_CDP_STEALTH=on` (or `1`/`true`) emits the Pack 2
`--cavestack-suppress-prepare-stack-trace` switch (closes the Cloudflare
...
The Chrome extension that ships baked into CaveStack Browser shows a live
...
pipe text into the live Claude PTY via `window.cavestackInjectToTerminal(text)`,
...
that accepts the Bearer token OR the HttpOnly `cavestack_sse` session cookie
...
| Content script | `extension/content.js` | Page overlays, `cavestack` pill |
...
```bash
bin/cavestack-extension    # opens chrome://extensions, copies path to clipboard
```

Or do it manually: `chrome://extensions` → toggle Developer mode → Load
unpacked → navigate to `~/.claude/skills/cavestack/extension` → pin the
...
HttpOnly `cavestack_sse` cookie (never valid against `/command`).
...
`~/.cavestack/security/attempts.jsonl` — append-only, salted SHA-256 of source
+ domain only (no raw IP, no full request body), rotates at 10MB with 5
generations. Per-device salt at `~/.cavestack/security/device-salt` (mode 0600).

### Tunnel egress receipts (v1.63+)

Every tunnel session open writes a hash-chained egress receipt (sink
`browse-tunnel`) to `~/.cavestack/security/egress.jsonl` BEFORE ngrok forwards
...
`bin/cavestack-egress list` and verify chain integrity with
`bin/cavestack-egress verify` (exit 3 on tamper).
...
The root token is written to `<project>/.cavestack/browse.json` with chmod 600.
...
token OR a 30-minute HttpOnly `cavestack_sse` cookie minted via
...
The Terminal pane uses a separate session cookie, `cavestack_pty`, minted via
...
`chrome-extension://<CAVESTACK_EXTENSION_ID>` — the `key` field in
`extension/manifest.json` pins the extension ID (`CAVESTACK_EXTENSION_ID` in
...
- `CAVESTACK_SECURITY_OFF=1` — emergency kill switch. Classifier stays off
  even if warmed. Just the ML scan is skipped.
- Classifier model cache: `~/.cavestack/models/testsavant-small/` (112MB, first
  run only).
- Attack log: `~/.cavestack/security/attempts.jsonl` (salted SHA-256 + domain
  only, rotates at 10MB, 5 generations).
- Per-device salt: `~/.cavestack/security/device-salt` (0600).
...
(override via `CAVESTACK_BROWSE_MAX_HTML_BYTES`).
...
- Console: `.cavestack/browse-console.log`
- Network: `.cavestack/browse-network.log`
- Dialog: `.cavestack/browse-dialog.log`
...
```bash
$B state save my-session         # save cookies + URLs to .cavestack/browse-state-my-session.json
...
in `.cavestack/browser-scout.jsonl`.
...
| **cavestack browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |
| **cavestack browse + codified skill** | **~3s** | **~200ms** | **0 tokens** (single skill invocation) |

In a 20-command browser session, MCP tools burn 30,000–40,000 tokens on
protocol framing alone. cavestack burns zero. The codified-skill path takes a
...
cavestack skips all of this. Compiled binary. Plain text in, plain text out.
...
project-tier skill at `/code/project-a/.cavestack/browser-skills/foo/` shadows
the global `~/.cavestack/browser-skills/foo/` only inside project-a.
...
| `BROWSE_STATE_FILE` | `.cavestack/browse.json` | Path to state file |
...
| `CAVESTACK_BROWSE_MAX_HTML_BYTES` | 52428800 (50MB) | `load-html` size cap |
| `CAVESTACK_SECURITY_OFF` | unset | Emergency kill switch — disable ML classifier |
| `CAVESTACK_STEALTH` | unset | Set to `extended` (also accepts `1`/`true`) to layer six aggressive patches (WebGL spoof, faked plugins, mediaDevices) on top of Layer C. Actively lies; can break sites. |
| `CAVESTACK_CDP_STEALTH` | unset | Set to `on`/`1`/`true` to emit `--cavestack-suppress-prepare-stack-trace` (gbrowser Pack 2 / B11 C++ patch only; no-op on stock Chromium) |
| `CAVESTACK_GPU_VENDOR`, `CAVESTACK_GPU_RENDERER`, `CAVESTACK_GPU_CHIPSET` | unset | Per-install GPU spoof fed to the Pack 1 WebGL/UA-CH C++ patches. Set by gbd from the host profile; emitted as `--cavestack-gpu-vendor` / `--cavestack-gpu-renderer` / `--cavestack-ua-model` cmdline switches only when present. |
| `CAVESTACK_PLATFORM` | unset | Host platform classification (`MacARM`/`MacIntel` → `macOS`, `Win32` → `Windows`, `Linux*` → `Linux`) emitted as `--cavestack-ua-platform` |
| `CAVESTACK_HW_CONCURRENCY`, `CAVESTACK_DEVICE_MEMORY` | host profile (fallback 8) | Per-install `hardwareConcurrency`/`deviceMemory` reported by Layer C and emitted as `--cavestack-hw-concurrency` / `--cavestack-device-memory` for the worker-navigator C++ patch |
...
│   ├── stealth.ts               # Layer C: webdriver mask + window.chrome.* + Notification/Permissions + per-install hardware + toString proxy + automation-global sweep; buildCaveStackLaunchArgs (CAVESTACK_* cmdline switches); CAVESTACK_STEALTH=extended opt-in
...
│   ├── tunnel-denial-log.ts     # ~/.cavestack/security/attempts.jsonl writer (salted)
...
scrape/SKILL.md.tmpl             # /scrape cavestack skill — match-or-prototype entry point
skillify/SKILL.md.tmpl           # /skillify cavestack skill — codify last /scrape into permanent skill
```
...
lands at `~/.cavestack/browser-skills/<name>/` after the test passes.

### Deploying to the active skill

The active skill lives at `~/.claude/skills/cavestack/`. After making changes:
