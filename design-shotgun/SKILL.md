...
preamble-tier: 2
version: 1.0.0
description: "Design shotgun: generate multiple AI design variants, open a comparison board, collect structured feedback, and iterate. (cavestack)"
triggers:
...
gbrain:
  ...
  context_queries:
    - id: prior-approved-variants
      kind: filesystem
      glob: "~/.cavestack/projects/{repo_slug}/designs/*/approved.json"
      sort: mtime_desc
      limit: 5
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-shotgun","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.cavestack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cavestack/bin/cavestack-timeline-log '{"skill":"design-shotgun","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cavestack/bin/cavestack-question-log '{"skill":"design-shotgun","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
_PREV=$(find ~/.cavestack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
**Persistent taste profile (v1 schema at `~/.cavestack/projects/$SLUG/taste-profile.json`):**

Read the persistent taste profile if it exists:

```bash
_TASTE_PROFILE=~/.cavestack/projects/$SLUG/taste-profile.json
...
the legacy approved.json aggregate — `~/.claude/skills/cavestack/bin/cavestack-taste-update`
...
variant, call `~/.claude/skills/cavestack/bin/cavestack-taste-update approved <variant-path>`. When they
explicitly reject a variant, call `~/.claude/skills/cavestack/bin/cavestack-taste-update rejected <variant-path>`.
