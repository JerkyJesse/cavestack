...
`scripts/cavestack2/browser-provider-contract.ts` (the existing provider-contract idiom),
`lib/cavestack-decision-semantic.ts` (degrade-to-null reliability contract)

## Problem

cavestack carries ~17k LOC of home-grown code-intelligence glue: transcript
ingestion (`bin/cavestack-memory-ingest.ts`, ~1.9k), a unified sync verb
(`bin/cavestack-gbrain-sync.ts`, ~1.6k), context loading
(`bin/cavestack-brain-context-load.ts`), a three-tier planning cache
(`bin/cavestack-brain-cache`), source reconciliation, engine-status classification,
destructive-op guards, plus ~15 `bin/cavestack-gbrain-*` and `bin/cavestack-brain-*`
entrypoints and ~40 tests. All of it is bespoke wiring around one external tool
(GBrain) reached by direct CLI shell-out.

We do not want to keep maintaining a home-grown indexer. We want cavestack to
define a **small optional contract** that external providers implement, so the
indexing/search/graph work lives in the provider, not in cavestack.

Hard requirement, non-negotiable: **cavestack must remain fully functional with the
...
decision-store philosophy (`lib/cavestack-decision.ts` has zero gbrain imports;
`lib/cavestack-decision-semantic.ts` degrades to `null`). The contract is an
...
2. **Install consent (Graphify only).** Graphify is never auto-installed. The
   ...
   and nothing in cavestack runs a Graphify installer. Install is a user action
...
- **GBrain** (`garrytan/gbrain`, the cavestack-ecosystem tool): full contract fit,
...
Each provider exposes a runtime-drivable surface, so cavestack drives them with a
...
with `cavestack-code-intelligence select <provider>`, persisted to
`$CAVESTACK_HOME/code-intelligence.json`. `resolveSelectedProvider()` constructs the
...
|-----------------|--------------------|
| `bin/cavestack-gbrain-sync.ts` (`sync`/`reindex-code`/`sources`) | `provider.registerSource` / `provider.refresh` |
| `lib/cavestack-decision-semantic.ts` `semanticRecall` | `provider.search` (scoped) → same degrade-to-null |
| `bin/cavestack-brain-context-load.ts` (`query`/`list_pages`) | `provider.search` / `provider.status` |
| `bin/cavestack-memory-ingest.ts` (`import`, put) | `provider.add` (optional cap; GBrain-only) |
...
| `bin/cavestack-gbrain-detect` / `-install` / `-source-wireup` / `-repo-policy` | provider setup + picker + consent (thinner) |
...
`gen:cavestack2` / parity re-baseline cycle.
...
- **Phase 1 (this slice): the contract, three real adapters, and a usable CLI.**
  ...
  (HTTP + config) adapters + the selection store + the `cavestack-code-intelligence`
  ...
  generated-file changes yet, so no `gen:cavestack2` / parity re-baseline.
- **Phase 2: route internal consumers through the contract.** Point
  `cavestack-decision-semantic` and `cavestack-brain-context-load` at
...
- **Phase 3: surface selection in the skills.** Offer the picker at the moments a
  ...
  just-in-time consent prompt. Regenerate skills (`bun run gen:cavestack2`), re-run
  `bun run test:cavestack2`, re-baseline parity intentionally.
...
Per the CaveStack 2 canonical contract and CLAUDE.md boundaries: no cloud browsers,
...
is absent. The `cavestack-code-intelligence` CLI was smoke-tested end-to-end:
