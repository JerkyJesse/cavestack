...
How the CaveStack Browser sidebar actually works. Read this before touching
...
4. server.ts /sidebar-command handler
   ...
   └── Appends JSON to ~/.cavestack/sidebar-agent-queue.jsonl
...
4. welcome.html script
   └── addEventListener('cavestack-extension-ready', () => arrow.classList.add('hidden'))
```
...
| Agent stale token | Agent runs but no events appear in sidebar | sidebar-agent has old token from .cavestack/browse.json | Agent re-reads token before each event POST |
...
| Queue file | `~/.cavestack/sidebar-agent-queue.jsonl` | Filesystem |
| State file | `.cavestack/browse.json` | Filesystem |
| Chat log | `~/.cavestack/sessions/<id>/chat.jsonl` | Filesystem |
