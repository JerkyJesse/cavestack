...
consumer browser and bolts AI onto it. CaveStack Browser inverts this. It starts
...
for text editors, CaveStack Browser does for the browser.

## What It Is Today (Phase 1a, shipped)

A double-clickable macOS .app that wraps Playwright's Chromium with the cavestack
...
and run any cavestack skill. All without touching a terminal.

```
CaveStack Browser.app (389MB, 189MB DMG)
...
When the extension side panel hits hard API limits, CaveStack Browser ships to
...
Every cavestack skill becomes a browser capability.
...
| **CaveStack Browser** | CC runtime + browser viewport | See bug in browser, fix in code, verify | Currently macOS-only, no consumer features |

CaveStack Browser doesn't compete with consumer browsers. It competes with the
...
| `CAVESTACK_CHROMIUM_PATH` | **SHIPPED** | Custom Chromium binary support |
...
The 12-month ideal: you open CaveStack Browser, it detects your project, starts
