# Adding a New Host to cavestack

cavestack uses a declarative host config system. Each supported AI coding agent
...
- `globalRoot` / `localSkillRoot`: `.myhost/skills/cavestack`, `hostSubdir`: `.myhost`
...
- `runtimeRoot`: the shared asset list (`bin`, `browse/dist`, `browse/bin`,
  `cavestack-upgrade`, `ETHOS.md` + review checklist files)
...
- `pathRewrites`: replaces the derived list entirely. Only for non-mechanical
  cases — codex and factory rewrite the global path to `$CAVESTACK_ROOT` and add
...
ls .myhost/skills/cavestack-*/SKILL.md
