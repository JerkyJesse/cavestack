...
**Downstream consumers:** T9 (cavestack-codex-session-import)

## Question this spike answers

What's the actual on-disk format of Codex sessions, and how do we recover
AskUserQuestion-shaped events from it for `cavestack-codex-session-import`?
...
## Recovery strategy for `cavestack-codex-session-import`

**Two-tier extraction:**

1. **Marker-first (D18 mechanism).** Search `agent_message` text for the
   `<cavestack-qid:foo-bar>` marker. If present, we have an exact question_id
...
Per existing `bin/cavestack-question-log` schema, augmented with:
...
`bin/cavestack-slug` logic on the cwd path. Conductor worktrees have their
