...
**For our use case:**
- cavestack registers exactly one PreToolUse hook and one PostToolUse hook on
...
- Mitigation: document this constraint in `bin/cavestack-settings-hook`
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CLAUDE_PROJECT_DIR/.claude/skills/cavestack/hosts/claude/hooks/question-preference-hook",
            "timeout": 5
          }
        ]
      }
...
2. **Auto-decided event tagging.** When hook returns `updatedInput`, the
   ...
   tracking — write a marker file in `~/.cavestack/sessions/<id>/.auto-decided-<tool_use_id>`
...
- Existing `bin/cavestack-settings-hook` (SessionStart-only impl, to be
