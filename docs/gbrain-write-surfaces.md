...
|---|---|
| Any host + `cavestack-config gbrain-refresh` reports `gbrain_local_status: "ok"` | Compressed brain-aware blocks render. Agent reads this doc on-demand when it actually saves. ~250 token overhead per planning skill. |
...
`bin/cavestack-gbrain-sync.ts` for code-lookup resolvers; cavestack treats the
...
and writeback. Set via `cavestack-config set
...
- **Any other non-zero exit**: treat as transient failure. Do not retry
  ...
  `cavestack-config gbrain-refresh` if they suspect gbrain itself is
  misconfigured.
...
gbrain sources list                    # confirm cavestack-brain-<user> source
...
endpoint depending on the user's `~/.gbrain/config.json`. **cavestack
...
2. `cavestack-config get brain_trust_policy@<endpoint-hash>` must be
   `personal` for auto-write. Run `cavestack-config endpoint-hash` to get
...
3. If trust policy is `personal` and `gbrain doctor` is clean but the
   page still isn't there, file an issue against gbrain — cavestack's
...
- **`.gbrain-source` write semantics**: cavestack treats the documented
