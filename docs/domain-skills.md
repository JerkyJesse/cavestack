...
This is cavestack's borrow from [browser-use/browser-harness](https://github.com/browser-use/browser-harness).
cavestack copies the per-site-notes pattern, NOT the self-modifying-runtime
...
on every cavestack project I work on"), explicitly run
...
- **Per-project**: `~/.cavestack/projects/<slug>/learnings.jsonl` — same JSONL
  file the `/learn` skill uses. Domain skills are `type:"domain"` rows.
- **Global**: `~/.cavestack/global-domain-skills.jsonl` — only `state:"global"`
  rows.
...
following events are written to `~/.cavestack/analytics/browse-telemetry.jsonl`:
...
`cavestack-config set telemetry off` or `CAVESTACK_TELEMETRY_OFF=1`.
